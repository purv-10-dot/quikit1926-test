...
- **Optional Redis / graceful degradation.** Every consumer calls `getRedis()`
  ...
  and multi-instance coordination. ([packages/redis/index.ts:9-12](QuikIT_New/packages/redis/index.ts#L9-L12))
...
hits on every request. ([packages/auth/cache.ts:10-16](QuikIT_New/packages/auth/cache.ts#L10-L16))
...
[`packages/redis/index.ts`](QuikIT_New/packages/redis/index.ts). It is a
...
- **Loud one-time, prod-only logging** (`logLoudlyInProd`,
  [index.ts:30-49](QuikIT_New/packages/redis/index.ts#L30-L49)): the *first*
...
[`packages/auth/cache.ts`](QuikIT_New/packages/auth/cache.ts) and is initialized
...
[cache.ts:39-65](QuikIT_New/packages/auth/cache.ts#L39-L65)), so test
...
| **admin** (3002) | Typed JSON cache wrapper + `CacheKeys` (perms, feature flags, membership, dashboard stats) — [apps/admin/lib/redis.ts](QuikIT_New/apps/admin/lib/redis.ts) |
...
| **quikscale** (3003) | Feature-gate module routing, tenant resolution, rate limiting, React Query CRUD caching ([lib/hooks/createCRUDHook.ts](QuikIT_New/apps/quikscale/lib/hooks/createCRUDHook.ts)) |
...
`@quikit/auth/cache` ([apps/admin/lib/redis.ts:11-17](QuikIT_New/apps/admin/lib/redis.ts#L11-L17)).
...
|---|---|---|---|---|
| `auth:session:{sessionId}` | Redis | 30 days (set by caller) | JWT soft-revocation handle | [packages/auth/session-store.ts:4-8](QuikIT_New/packages/auth/session-store.ts#L4-L8) |
| `disabledModules:{orgId}:{appSlug}` | LRU + Redis | **30s** | Set of disabled module keys (FF-1) | [feature-gate.ts:56](QuikIT_New/packages/auth/feature-gate.ts#L56) |
| `tenantAppBlocked:{orgId}:{appSlug}` | LRU + Redis | **60s** | Hard app-access gate (SA-A.6) | [feature-gate.ts:191](QuikIT_New/packages/auth/feature-gate.ts#L191) |
| `membership:{userId}:{orgId}` | LRU + Redis | **60s** | Active-membership re-validation | [get-tenant-id.ts:33](QuikIT_New/packages/auth/get-tenant-id.ts#L33) |
| `appAccess:{userId}:{orgId}:{appSlug}` | LRU + Redis | **60s** | Per-app access gate | [get-tenant-id.ts:48](QuikIT_New/packages/auth/get-tenant-id.ts#L48) |
| `firstActiveTenant:{userId}` | LRU + Redis | **60s** | Onboarding org fallback | [get-tenant-id.ts:69](QuikIT_New/packages/auth/get-tenant-id.ts#L69) |
| `rl:{routeKey}\|{clientKey}:{windowStart}` | Redis (mem fallback) | `ceil(windowMs/1000)` | Fixed-window rate-limit counter | [rateLimit.ts:156](QuikIT_New/packages/shared/lib/rateLimit.ts#L156) |
| `otp:reset:{userId}` | Redis (mem fallback) | **180s** | sha256(OTP) for password reset | [otp-store.ts:25](QuikIT_New/apps/auth/lib/otp-store.ts#L25) |
| `otp:reset-attempts:{userId}` | Redis (mem fallback) | **180s** | Wrong-attempt counter (max 5) | [otp-store.ts:26](QuikIT_New/apps/auth/lib/otp-store.ts#L26) |
| `otp:reset-token:{token}` | Redis (mem fallback) | **300s** | One-shot reset token → userId | [otp-store.ts:27](QuikIT_New/apps/auth/lib/otp-store.ts#L27) |
| `oauth-prefill:{userId}` | Redis (mem fallback) | ~600s | OAuth first/last-name prefill | [packages/auth/oauth-prefill-store.ts](QuikIT_New/packages/auth/oauth-prefill-store.ts) |
| `admin:perms:{orgId}:{userId}:{appId}` | Redis | 300s default | Admin permission set | [apps/admin/lib/redis.ts:40](QuikIT_New/apps/admin/lib/redis.ts#L40) |
| `feature-flags:admin:{orgId}` | Redis | 300s | Admin feature flags | [apps/admin/lib/redis.ts:46](QuikIT_New/apps/admin/lib/redis.ts#L46) |
| `auth:membership:{userId}:{orgId}` | Redis | 60s | Admin-app membership cache | [apps/admin/lib/redis.ts:47-48](QuikIT_New/apps/admin/lib/redis.ts#L47-L48) |
| `memberships:list:{userId}` | Redis | 60s | All memberships for a user | [apps/admin/lib/redis.ts:49](QuikIT_New/apps/admin/lib/redis.ts#L49) |
| `dashboard:stats:{orgId}` | Redis | 30s | Admin dashboard stats | [apps/admin/lib/redis.ts:50](QuikIT_New/apps/admin/lib/redis.ts#L50) |
| `quikit:cache-invalidate` | Redis Pub/Sub **channel** | — | Cross-process invalidation signal | [cache.ts:34](QuikIT_New/packages/auth/cache.ts#L34) |
...
([cache.ts:147-175](QuikIT_New/packages/auth/cache.ts#L147-L175)):
...
- **LRU mechanics** ([cache.ts:77-105](QuikIT_New/packages/auth/cache.ts#L77-L105)):
...
([packages/shared/lib/redisCache.ts:28-68](QuikIT_New/packages/shared/lib/redisCache.ts#L28-L68))
...
`invalidate(key)` ([cache.ts:180-184](QuikIT_New/packages/auth/cache.ts#L180-L184))
does three things:
...
its local copy on message ([cache.ts:57-61](QuikIT_New/packages/auth/cache.ts#L57-L61)).
...
([feature-gate.ts:92-97](QuikIT_New/packages/auth/feature-gate.ts#L92-L97)).
...
([apps/quikscale/lib/hooks/createCRUDHook.ts](QuikIT_New/apps/quikscale/lib/hooks/createCRUDHook.ts)):
...
> [apps/admin/lib/redis.ts:58-76](QuikIT_New/apps/admin/lib/redis.ts#L58-L76)
...
`main.duplicate()` ([cache.ts:46-65](QuikIT_New/packages/auth/cache.ts#L46-L65))
...
store is [`packages/auth/session-store.ts`](QuikIT_New/packages/auth/session-store.ts);
...
[`packages/auth/middleware.ts`](QuikIT_New/packages/auth/middleware.ts),
[`packages/auth/jwt.ts`](QuikIT_New/packages/auth/jwt.ts), and
[`apps/auth/app/api/verify-token/route.ts`](QuikIT_New/apps/auth/app/api/verify-token/route.ts).
...
([get-tenant-id.ts](QuikIT_New/packages/auth/get-tenant-id.ts)) trusts the JWT's
...
([feature-gate.ts:145-167](QuikIT_New/packages/auth/feature-gate.ts#L145-L167))
...
([rateLimit.ts:132-181](QuikIT_New/packages/shared/lib/rateLimit.ts#L132-L181))
...
([rateLimit.ts:200-204](QuikIT_New/packages/shared/lib/rateLimit.ts#L200-L204)):
...
([redisCache.ts](QuikIT_New/packages/shared/lib/redisCache.ts)) caches expensive
aggregates/analytics in Redis (no local layer).

**5. HTTP cache headers (quikinfra).**
[`apps/quikinfra/src/lib/http/cache.ts`](QuikIT_New/apps/quikinfra/src/lib/http/cache.ts)
...
([src/lib/workflow/idempotency.ts](QuikIT_New/apps/quikinfra/src/lib/workflow/idempotency.ts)).
...
- `REDIS_URL` is declared in the build `env` list in
  [turbo.json](QuikIT_New/turbo.json) so Turbo's cache busts when it changes.
...
|---|---|---|
| Module toggle doesn't take effect for ~30s | Toggle route didn't call `invalidateDisabledModules`; relying on 30s TTL only | Confirm the toggle route invalidates; check pub/sub is connected (`quikit:cache-invalidate`) — [feature-gate.ts:92-97](QuikIT_New/packages/auth/feature-gate.ts#L92-L97) |
| Logout / revoke doesn't lock the user out | `isAuthSessionActive` failing open (Redis down) **or** the JWT carries no `sessionId` on that app | Verify `REDIS_URL` is set & reachable; confirm the app's JWT includes `sessionId` — [session-store.ts:57-66](QuikIT_New/packages/auth/session-store.ts#L57-L66) |
| Rate limit seems bypassable under load | Using sync `rateLimit()` (per-process) or Redis down with default fail-open | Switch to `rateLimitAsync()`; for auth routes pass `failClosed: true` — [rateLimit.ts:104-118](QuikIT_New/packages/shared/lib/rateLimit.ts#L104-L118) |
| Calling `invalidatePermissionCache` clears nothing | It's an intentional **no-op** (no wildcard delete in `@quikit/redis`) | Rely on the short TTL, or delete exact keys via `CacheKeys` — [apps/admin/lib/redis.ts:58-76](QuikIT_New/apps/admin/lib/redis.ts#L58-L76) |
| `[redis] CRITICAL: REDIS_URL is not set in production` in logs | `REDIS_URL` missing in prod env | Set it in the deploy environment; everything degrades to in-memory-per-instance until then — [index.ts:36-40](QuikIT_New/packages/redis/index.ts#L36-L40) |
| Stale value persists across pods but clears on one | Pub/sub subscriber failed to bind (swallowed); peers on per-TTL eviction | Check Redis pub/sub connectivity; the subscriber is best-effort — [cache.ts:51-56](QuikIT_New/packages/auth/cache.ts#L51-L56) |
| Cached read returns `undefined`/recomputes every time | Corrupted/non-JSON Redis entry, or value was never JSON | `redisGet`/`cacheOrCompute` swallow parse errors and recompute — [cache.ts:111-119](QuikIT_New/packages/auth/cache.ts#L111-L119) |
...
- Redis client: [packages/redis/index.ts](QuikIT_New/packages/redis/index.ts)
- Layered cache + pub/sub: [packages/auth/cache.ts](QuikIT_New/packages/auth/cache.ts)
- Feature gates: [packages/auth/feature-gate.ts](QuikIT_New/packages/auth/feature-gate.ts)
- Tenant resolution: [packages/auth/get-tenant-id.ts](QuikIT_New/packages/auth/get-tenant-id.ts)
- Session store: [packages/auth/session-store.ts](QuikIT_New/packages/auth/session-store.ts)
- Rate limiter: [packages/shared/lib/rateLimit.ts](QuikIT_New/packages/shared/lib/rateLimit.ts)
- Redis-only cache: [packages/shared/lib/redisCache.ts](QuikIT_New/packages/shared/lib/redisCache.ts)
- OTP store: [apps/auth/lib/otp-store.ts](QuikIT_New/apps/auth/lib/otp-store.ts)
- Admin cache wrapper: [apps/admin/lib/redis.ts](QuikIT_New/apps/admin/lib/redis.ts)
- Build env: [turbo.json](QuikIT_New/turbo.json)
