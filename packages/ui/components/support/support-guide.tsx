...
export function SupportGuide({ sections }: { sections: GuideSection[] }) {
  return (
    <div className="flex-1 overflow-y-auto px-5 py-4 text-sm leading-relaxed text-[var(--color-text-secondary,#64748B)] space-y-4">
      {sections.map((section, i) => (
        <section key={i}>
          <h4 className="text-sm font-semibold text-[var(--color-text-primary,#0F172A)] mb-1">
            {section.heading}
          </h4>
          ...
          {section.bullets && section.bullets.length > 0 && (
            <ul className="list-disc pl-5 space-y-1 mt-1">
              {section.bullets.map((bullet, j) => (
                <li key={j}>
                  <strong className="text-[var(--color-text-primary,#0F172A)]">{bullet.label}</strong>
                  {" — "}
                  {bullet.text}
                </li>
              ))}
