...
import type { AIClientLike, AIStreamOptions } from '../types';
...
// ═════════════════════════════════════════════════════════════════
// Agent gateway (P3-S167): MockAI parity for invokeAgent /
// getAgentRun / waitForAgentRun / listAgents
// ═════════════════════════════════════════════════════════════════


test('MockAI.listAgents returns a deterministic canned catalog (no network)', async () => {
  const ai = new MockAI();
  const catalog = await ai.listAgents();
...
test('MockAI.listAgents honours agentCatalog override', async () => {
  const ai = new MockAI({
    agentCatalog: [
      {
        name: 'research',
  ...
  expect(catalog[0]!.name).toBe('research');
});


test('MockAI.invokeAgent returns canned {runId, status: queued} shape and records the call', async () => {
  const ai = new MockAI();
  const result = await ai.invokeAgent(
    'document_extract',
    { url: 'https://x/y.pdf' },
    { appId: 'quikhrms' },
  );
  expect(result.status).toBe('queued');
  expect(result.runId.startsWith('mock-run-')).toBe(true);
  // Recorded call shape — apps can assert on the invocation like they
  // can for execute.
  expect(ai.calls.length).toBe(1);
  expect(ai.calls[0]!.useCase).toBe('agent.document_extract');
  expect(ai.calls[0]!.appId).toBe('quikhrms');
});


test('MockAI.getAgentRun returns a canned completed run for unknown ids', async () => {
  const ai = new MockAI();
  const run = await ai.getAgentRun('any-run-id');
  expect(run.status).toBe('completed');
  expect(run.runId).toBe('any-run-id');
  expect(run.output).toBeTruthy();
});


test('MockAI.getAgentRun returns the configured run when the id matches', async () => {
  const canned = {
    runId: 'r_hrms',
    agentName: 'document_extract',
    status: 'completed' as const,
    output: { fields: { name: 'Priya' }, missing_fields: [], indeterminate_fields: [] },
    error: null,
    startedAt: 't',
    completedAt: 't2',
    createdAt: 't',
  };
  const ai = new MockAI({ agentRuns: new Map([['r_hrms', canned]]) });
  const run = await ai.getAgentRun('r_hrms');
  expect(run).toEqual(canned);
});


test('MockAI.waitForAgentRun returns immediately with completed result — no polling', async () => {
  const ai = new MockAI();
  const start = Date.now();
  const result = await ai.waitForAgentRun('any-run', {
    initialPollMs: 60_000,  // if the mock polled, this test would time out
    timeoutMs: 60_000,
  });
  const elapsed = Date.now() - start;
  expect(result.status).toBe('completed');
  // "No polling under the mock" — should be well under initialPollMs.
  expect(elapsed).toBeLessThan(500);
});


test('MockAI.waitForAgentRun under simulateAgentFailure throws AgentRunFailedError with runId + output', async () => {
  const failureOutput = { reason: 'no_extractable_text', next_action: 'use_rag' };
  const ai = new MockAI({
  ...
  await expect(ai.waitForAgentRun('r_scan')).rejects.toThrow(AgentRunFailedError);
  try {
    await ai.waitForAgentRun('r_scan');
  } catch (err) {
    expect(err).toBeInstanceOf(AgentRunFailedError);
    const e = err as AgentRunFailedError;
    expect(e.runId).toBe('r_scan');
    expect(e.runOutput).toEqual(failureOutput);
    expect(e.message).toContain('pdf is scanned');
  }
});


test('MockAI.withFallback.waitForAgentRun PROPAGATES AgentRunFailedError (parity with real client)', async () => {
  // The load-bearing parity: what the real FallbackProxy does with a
  // real failed run, the MockAI FallbackProxy MUST do too — otherwise
  // an app test wrapping in withFallback would pass against the mock
  // and blow up on real traffic.
  const ai = new MockAI({
    simulateAgentFailure: {
      error: 'failed',
      output: { reason: 'x' },
  ...
  await expect(proxy.waitForAgentRun('any')).rejects.toThrow(AgentRunFailedError);
  expect(fallbackFired).toBe(false);
});


test('MockAI parity — all 4 agent methods present on the AIClientLike surface', () => {
  // Compile-time-ish sanity: assigning MockAI to AIClientLike proves
  // the interface is fully implemented. If any method is missing,
  // this test would fail to compile — which is the whole point of
  // the parity check.
  const ai: AIClientLike = new MockAI();
  expect(typeof ai.invokeAgent).toBe('function');
  expect(typeof ai.getAgentRun).toBe('function');
  expect(typeof ai.waitForAgentRun).toBe('function');
  expect(typeof ai.listAgents).toBe('function');
});
