 ...
 * into the Postgres `users` table so existing invited accounts keep
 ...
 * `users` table to exist — run `npx prisma migrate dev --name
 * add_cn_users` first.
 */
  ...
  try {
    ...
    await prisma.cnUser.create({
      data: {
        ...
        roleKey: u.roleKey ?? "site_engineer",
