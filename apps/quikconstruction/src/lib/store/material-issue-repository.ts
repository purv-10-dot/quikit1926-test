 ...
 * `material_issues`. The `materials` column is JSONB; each line in
...
export async function listMaterialIssues(
  ...
  const where = buildMaterialIssuesWhere(tenantId, opts);
  ...
  const rows: any[] = await (db as any).$queryRaw`
    ...
    FROM app_quikconstruction."Material_issues"
    ${where}
...
export async function countMaterialIssues(
  ...
  const rows: Array<{ c: bigint }> = await (db as any).$queryRaw`
    SELECT COUNT(*)::bigint AS c FROM app_quikconstruction."Material_issues"${where}
  `;
...
export async function createMaterialIssue(
  ...
  await (db as any).$executeRaw`
    INSERT INTO app_quikconstruction."Material_issues" (
      id, "tenantId", "orgId", "issueNumber",
      "projectId", "projectName", "locationId", "locationName",
      "issuedToId", "issuedToName", "issuedById", "issueDate",
      "issueType", "contractorId", "contractorName", "teamDepartment",
      ...
      ${id}, ${input.tenantId}, ${input.orgId}, ${input.issueNumber},
  ...
  return (await findMaterialIssueById(input.tenantId, id))!;
}
...
export async function updateMaterialIssue(
  ...
  const existing = await findMaterialIssueById(input.tenantId, id);
  ...
  await (db as any).$executeRaw`
    UPDATE app_quikconstruction."Material_issues"
    SET
  ...
  return findMaterialIssueById(input.tenantId, id);
}
...
export async function patchMaterialIssueStatus(
  ...
  const existing = await findMaterialIssueById(tenantId, id);
  if (!existing) return null;
  ...
  await (db as any).$executeRaw`
    ...
    WHERE "tenantId" = ${tenantId} AND id = ${id}
  `;
  return findMaterialIssueById(tenantId, id);
}
...
export async function deleteMaterialIssue(
  ...
  const res: any = await (db as any).$executeRaw`
    DELETE FROM app_quikconstruction."Material_issues"
    WHERE "tenantId" = ${tenantId} AND id = ${id}
  `;
  return Number(res) > 0;
}
