/**
 * Good Return repository — Postgres-backed CRUD for `good_returns`.
 *
 * Lines live inside the `materials` JSONB column (same pattern as
 * Gate Pass / Material Issue / Material Estimation) so we don't need
 * a parent/child rewrite while the UI still expresses the composition
 ...
 *
 * Response shape is compatible with what the list / detail page read
 * from the legacy `globalThis.__qcGoodReturns` array (`returnNumber`,
 * `projectName`, `vendorName`, `lineCount`, `lines`, etc.).
 */
...
export interface CreateGoodReturnInput {
  ...
  reason?: string | null;
  remarks?: string | null;
  // Vehicle / driver / commercial — mirror Gate Pass so dispatch info
  // entered at create-time is preserved through approval + dispatch.
...
function mapRow(row: any): any {
  ...
  return {
    ...
    // Dispatch paperwork entered at create time — same fields the
    // Gate Pass detail page surfaces.
...
export async function listGoodReturns(
  ...
  const rows: any[] = await (db as any).$queryRaw`
    SELECT *
    FROM app_quikconstruction."Good_returns"
...
export async function createGoodReturn(
  ...
  // Decimal columns want `null` when blank — coerce empty strings so
  // the raw SQL binding doesn't choke on an empty numeric literal.
  ...
  await (db as any).$executeRaw`
    INSERT INTO app_quikconstruction."Good_returns" (
      id, "tenantId", "orgId", "returnNumber",
      ...
      ${id}, ${input.tenantId}, ${input.orgId}, ${input.returnNumber},
  ...
  return (await findGoodReturnById(input.tenantId, id))!;
}
...
export async function updateGoodReturn(
  ...
  const existing = await findGoodReturnById(input.tenantId, id);
  ...
  await (db as any).$executeRaw`
    UPDATE app_quikconstruction."Good_returns"
    SET
  ...
  return findGoodReturnById(input.tenantId, id);
}
...
export async function patchGoodReturnStatus(
  ...
  const existing = await findGoodReturnById(tenantId, id);
  if (!existing) return null;
  ...
  await (db as any).$executeRaw`
    ...
    WHERE "tenantId" = ${tenantId} AND id = ${id}
  `;
  return findGoodReturnById(tenantId, id);
}
...
export async function deleteGoodReturn(
  ...
  const res: any = await (db as any).$executeRaw`
    DELETE FROM app_quikconstruction."Good_returns"
    WHERE "tenantId" = ${tenantId} AND id = ${id}
  `;
  return Number(res) > 0;
}
