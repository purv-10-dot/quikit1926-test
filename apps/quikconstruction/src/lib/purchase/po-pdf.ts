 ...
 * from `src/lib/purchase/assests/aakar-letterhead.pdf`, clones its
 * first page as the template, and draws the Purchase Order body:
...
function drawInfoBlocks(ctx: Ctx, input: PoPdfInput) {
  ...
  const buyerRows: Array<[string, string]> = [
    ...
    ["GST NO.", input.po.buyerGstin || "23AAVFA4286B1ZH"],
    [
      ...
      input.po.buyerBillingAddress ||
        "Aakar Constructions, 403,404, Mangalam Pearl, New Palasia, Indore",
    ],
    [
      "Delivery Address",
  ...
  const buyer = prep(buyerRows);
  const blockH = Math.max(
    supplier.reduce((s, r) => s + r.h, 0),
    buyer.reduce((s, r) => s + r.h, 0),
  );
  const topY = ctx.y;
...
const COLS = [
  ...
  { key: "desc", label: "DESCRIPTION", width: 170, align: "left" as const },
  ...
  { key: "disc", label: "DISC%", width: 45, align: "right" as const },
...
function drawTableRow(
  ...
  const discT = line.discountPct ? `${line.discountPct}%` : "";
