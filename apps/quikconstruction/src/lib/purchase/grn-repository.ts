/**
 * GRN master — Prisma-backed CRUD for `goods_receipt_notes` +
 * `grn_lines`. Response shape is compatible with the legacy
 * demo-store shape so the GRN list/detail pages keep rendering
 * without code changes (`grnNumber`, `vendorName`, `poNumber`,
 * `projectName`, `lineCount`, `lines`, etc. are all present).
 */
...
export interface CreateGRNInput {
  ...
  /** Filename(s) of the uploaded challan scan/photo. Comma-separated
      when multiple files are picked. */
  challanAttachment?: string | null;
  /** Overall quality verdict (Accepted / Partially Accepted /
      Rejected / Pending Inspection). */
  overallQualityStatus?: string | null;
  /** Weighbridge slip number when the goods were weighed at gate. */
...
export async function createGRN(input: CreateGRNInput): Promise<any> {
  // Resolve UOMs up-front so the transaction stays fast.
  const resolvedLines = await Promise.all(
    input.lines.map(async (l) => {
      let uomId = l.uomId ?? null;
      if (!uomId && l.uomCode) {
        uomId = await resolveUomByCode(input.tenantId, input.orgId, l.uomCode);
      }
      // Defensive number coercion: parseFloat("") / parseFloat("NaN")
      // both return NaN, which then flows through `received - rejected`
      // and into the computed `amount`. Prisma's Decimal column
      // rejects "NaN", so wrap every parse in an `isFinite` fallback.
  ...
  // Drift-tolerant: if Prisma client is stale for the recently-added
  // columns (receivedByName / vehicleNo / ewayBillNo / approxInvoiceValue),
  // retry without them and back-fill via raw SQL.
  ...
  if (needsBackfill) {
    await (db as any).$executeRaw`
      UPDATE app_quikconstruction."Goods_receipt_notes"
