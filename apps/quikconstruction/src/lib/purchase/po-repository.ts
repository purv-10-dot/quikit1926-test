/**
 * Purchase Order master — Prisma-backed CRUD for `purchase_orders`
...
async function augmentPOsWithDriftFields(poRows: any[]): Promise<void> {
  ...
  try {
    ...
    const headers: Array<{
      ...
      FROM app_quikconstruction."Purchase_orders"
      WHERE id = ANY(${ids})
    `;
    ...
    const lineExtras: Array<{
      ...
      FROM app_quikconstruction."Purchase_order_lines"
 ...
 * join against `purchase_indents` / `rfqs`.
 */
...
export async function findPOById(tenantId: string, id: string): Promise<any | null> {
...
export async function createPO(input: CreatePOInput): Promise<any> {
  ...
  if (needsRawBackfill) {
    await (db as any).$executeRaw`
      UPDATE app_quikconstruction."Purchase_orders"
    ...
    await Promise.all(
      (created.lines ?? []).map((row: any, idx: number) => {
        ...
        return (db as any).$executeRaw`
          UPDATE app_quikconstruction."Purchase_order_lines"
