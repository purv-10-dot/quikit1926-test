 ...
 */
async function loadPrLineLookups(rows: any[]): Promise<{
  ...
  const [items, uoms, projects, workCategories, locations] = await Promise.all([
    ...
    workCategoryIds.size
      ? (db as any).cnWorkCategory.findMany({
          where: { id: { in: Array.from(workCategoryIds) } },
...
// numbers already persisted in purchase_requisitions (P2002).
//
...
export async function listPRs(opts: ListPRsOptions): Promise<any[]> {
  const { tenantId, orgId, projectIds, status, projectId, search } = opts;
  ...
  const { itemById, uomById, projectById, workCategoryById, locationById } = await loadPrLineLookups(rows);
...
export async function findPRById(tenantId: string, id: string): Promise<any | null> {
  const row = await (db as any).cnPurchaseRequisition.findFirst({
    where: { id, tenantId },
    include: { lines: true },
  });
  if (!row) return null;
  const { itemById, uomById, projectById, workCategoryById, locationById } = await loadPrLineLookups([row]);
 ...
 */
async function ensureProjectExists(projectId: string, _tenantId: string, _orgId: string, _createdBy: string): Promise<void> {
...
export async function createPR(input: CreatePRInput): Promise<any> {
  // Sync the referenced project into projects if it's not there yet.
  // Masters still live in demo-store; this is a bridge until they migrate.
  await ensureProjectExists(input.projectId, input.tenantId, input.orgId, input.createdBy);
...
export async function updatePR(
  ...
  return findPRById(tenantId, id);
}

export async function deletePR(tenantId: string, id: string): Promise<boolean> {
  try {
