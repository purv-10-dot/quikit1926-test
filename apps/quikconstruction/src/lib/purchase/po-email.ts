...
export async function buildPoPreview(
  ...
  const termsBody = await resolveTermsBody(tenantId, po);
  ...
  let master: any = null;
  try {
    const map = await findVendorsByIds(tenantId, [po.vendorId]);
...
export async function sendPoEmailToVendor(
  ...
  let master: any = null;
  try {
    const map = await findVendorsByIds(tenantId, [vendorId]);
  ...
  try {
    const pdf = await generatePoPdf({
      po: {
        ...
        deliveryAddress: po.deliveryAddress ?? null,
        buyerName: "Aakar Constructions",
        // Aakar's registered address + GSTIN are hard-coded on the
        // letterhead template, but also surface them here as a
        // fallback for the right-hand buyer info block.
        buyerBillingAddress:
          "Aakar Constructions, 403,404, Mangalam Pearl, New Palasia, Indore",
        buyerGstin: "23AAVFA4286B1ZH",
      },
