...
async function withSchemaDriftRetry<T>(
  ...
  for (let i = 0; i < 10; i++) {
    try {
      ...
      // "purchase_indent_lines.estimatedRate"). Strip the table
...
async function resolveUomId(
  ...
  // Prefer an explicit uomId if it resolves in Prisma for this tenant.
  if (input.uomId) {
  ...
  // Admin hasn't seeded the UOM master yet — create on the fly so the
  // save doesn't fail. Line count is small; the cost is negligible.
...
async function loadLineLookups(lines: any[]): Promise<{
...
export async function updateIndent(
  ...
  return findIndentById(tenantId, id);
}
