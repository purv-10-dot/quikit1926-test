 ...
 */
async function findTermsBodyById(tenantId: string, id: string): Promise<string | null> {
  try {
 ...
 */
async function resolveRfqTerms(tenantId: string): Promise<string | null> {
  const base = { tenantId, status: "active" as const };
  const tiers = [
...
export async function buildRfqPreview(
  ...
  const vendorMasters = await findVendorsByIds(tenantId, vendorIds);

  const pickedBody = rfq?.termsTemplateId
    ? await findTermsBodyById(tenantId, String(rfq.termsTemplateId))
    : null;
  const termsBody = pickedBody ?? (await resolveRfqTerms(tenantId));
...
export async function buildRfqPreviewPdfForVendor(
  ...
  const vendorMasters = await findVendorsByIds(tenantId, [v.vendorId]);
...
export async function sendRfqEmailsToVendors(
  ...
  // fields aren't stored on `rfq_vendors`.
  const vendorIds = vendors
    .map((v) => v.vendorId)
    .filter((id): id is string => !!id);
  const vendorMasters = await findVendorsByIds(tenantId, vendorIds);
