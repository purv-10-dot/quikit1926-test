 ...
 *
 * Storage:
 *   - rfqs          (header + buyer contact + status + approvalId)
 *   - rfq_lines     (material lines)
 *   - rfq_vendors   (per-vendor fan-out with assignedItemIds)
 *
...
async function resolveUomId(
  ...
  if (input.uomId) {
    const row = await (db as any).cnUOM.findFirst({
      where: { id: input.uomId, tenantId, orgId },
      select: { id: true },
    });
...
async function loadLookups(
  ...
  const [items, uoms, vendorMap, projects] = await Promise.all([
    ...
    uomIds.size
      ...
      : Promise.resolve([]),
    vendorIds.size
      ? findVendorsByIds(tenantId, Array.from(vendorIds))
...
async function augmentRfqsWithDriftFields(rfqRows: any[]): Promise<void> {
  ...
  try {
    ...
    const rows: Array<{
      ...
      FROM app_quikconstruction."Rfqs"
      WHERE id = ANY(${ids})
    `;
...
async function augmentVendorsWithQuoteFields(rfqRows: any[]): Promise<void> {
  ...
  try {
    const rows: Array<{
      ...
      FROM app_quikconstruction."Rfq_vendors"
...
export async function listRfqs(opts: ListRfqsOptions): Promise<any[]> {
  ...
  const { itemById, uomById, vendorById, projectById } = await loadLookups(opts.tenantId, rows);
...
export async function findRfqById(
  ...
  const { itemById, uomById, vendorById, projectById } = await loadLookups(tenantId, [row]);
...
export async function createRfq(input: CreateRfqInput): Promise<any> {
  ...
  if (needsRawBackfill) {
    await (db as any).$executeRaw`
      UPDATE app_quikconstruction."Rfqs"
...
export async function updateRfqStatus(
  ...
  return findRfqById(tenantId, id);
}
 ...
 * `rfq_vendors` row in-place — one row per vendor per RFQ — and
...
export async function saveVendorQuote(
  ...
  try {
    ...
    if (
      ...
      await (db as any).$executeRaw`
        UPDATE app_quikconstruction."Rfq_vendors"
  ...
  try {
    ...
    if (msg.includes("Unknown argument") || msg.includes("quotedRates")) {
      all = await (db as any).$queryRaw`
        SELECT "quotedRates" FROM app_quikconstruction."Rfq_vendors" WHERE "rfqId" = ${input.rfqId}
      `;
    } else {
      throw err;
    }
  ...
  return findRfqById(input.tenantId, input.rfqId);
}
