 ...
 *
 * To enable:
 *   1. `pnpm add @sentry/nextjs` in apps/quikconstruction
...
/** Attach tenant + user context to the current Sentry scope. */
export function setUserContext(ctx: { userId?: string; tenantId?: string; roleKey?: string }): void {
  ensureInit();
  const s = sdk;
  if (!s) return;
  try {
    s.setUser({ id: ctx.userId, tenant: ctx.tenantId, role: ctx.roleKey });
  } catch {
    /* swallow */
  }
}
