 ...
 *
 * Contract:
 *   - Every line has: ts, level, msg, requestId?, tenantId?, userId?, ...
...
export const logger = {
   ...
   * inside a service to avoid repeating `{ tenantId, userId }`.
   *
   *   const log = logger.with({ tenantId, userId, module: "boq" });
