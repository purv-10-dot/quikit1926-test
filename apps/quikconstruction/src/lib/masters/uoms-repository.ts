/**
 * UOM master — Prisma-backed CRUD for `uoms`.
 */
...
function buildUOMsWhere(
  opts: Pick<ListUOMsOptions, "tenantId" | "orgId" | "search">,
...
export async function findUOMById(tenantId: string, id: string): Promise<UOMRecord | null> {
  const row = await (db as any).cnUOM.findFirst({ where: { id, tenantId } });
...
export interface UpdateUOMInput
  extends Partial<Omit<CreateUOMInput, "tenantId" | "orgId" | "createdBy">> {
  updatedBy: string;
}
