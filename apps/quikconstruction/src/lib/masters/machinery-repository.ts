/**
 * Machinery master — Prisma-backed CRUD for `machinery`.
 */
...
export async function findMachineryById(tenantId: string, id: string): Promise<MachineryRecord | null> {
  const row = await (db as any).cnMachinery.findFirst({ where: { id, tenantId } });
...
export interface UpdateMachineryInput
  extends Partial<Omit<CreateMachineryInput, "tenantId" | "orgId" | "createdBy">> {
  updatedBy: string;
}
