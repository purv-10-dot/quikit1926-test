/**
 * Contractors master — Prisma-backed CRUD for `contractors`.
 *
 * Mirrors vendors-repository: tenant-scoped reads, one-time seed on
 * first list call, soft-delete via `status = "inactive"`, auto code
 * generation (`CON-###`).
 */
...
function buildContractorsWhere(
  opts: Pick<ListContractorsOptions, "tenantId" | "orgId" | "search">,
...
export interface UpdateContractorInput
  extends Partial<Omit<CreateContractorInput, "tenantId" | "orgId" | "createdBy">> {
  updatedBy: string;
}
...
export async function deleteContractor(
  ...
  // Soft delete — keeps FKs from WorkOrder / RAB valid while hiding
  // the row from the master UI's default view.
