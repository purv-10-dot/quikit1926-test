/**
 * Customers master — Prisma-backed CRUD for `customers`.
...
export async function findCustomerById(tenantId: string, id: string): Promise<CustomerRecord | null> {
  const row = await (db as any).cnCustomer.findFirst({ where: { id, tenantId } });
...
export interface UpdateCustomerInput
  extends Partial<Omit<CreateCustomerInput, "tenantId" | "orgId" | "createdBy">> {
  updatedBy: string;
}
