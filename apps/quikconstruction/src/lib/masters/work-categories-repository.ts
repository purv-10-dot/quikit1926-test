/**
 * Work Categories master — Prisma-backed CRUD for `work_categories`.
 */
...
function buildWorkCategoriesWhere(
  opts: Pick<ListWorkCategoriesOptions, "tenantId" | "orgId" | "search">,
...
export async function findWorkCategoryById(
  ...
  const row = await (db as any).cnWorkCategory.findFirst({ where: { id, tenantId } });
...
export interface UpdateWorkCategoryInput
  extends Partial<Omit<CreateWorkCategoryInput, "tenantId" | "orgId" | "createdBy">> {
  updatedBy: string;
}
