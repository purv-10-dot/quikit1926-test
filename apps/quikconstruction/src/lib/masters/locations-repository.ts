/**
 * Locations master — Prisma-backed CRUD for `locations`.
 *
...
function buildLocationsWhere(
  opts: Pick<ListOptions, "tenantId" | "orgId" | "search" | "projectId">,
...
export async function listLocations(opts: ListOptions): Promise<LocationRecord[]> {
  const rows = await (db as any).cnLocation.findMany({
    ...
    include: { itemGroup: { select: { id: true, name: true } } },
...
export async function findLocationById(tenantId: string, id: string): Promise<LocationRecord | null> {
...
export interface UpdateLocationInput
  extends Partial<Omit<CreateLocationInput, "tenantId" | "orgId" | "createdBy">> {
  updatedBy: string;
}
