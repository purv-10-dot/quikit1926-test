/**
 * Projects master — Prisma-backed CRUD for `projects`.
 *
 * Tenant-scoped list/create/update with soft-delete via `status =
 * "inactive"`. Response shape preserves the fields the old demo-store
 * API returned (companyName, clientName, startDate, etc.) by joining
 * `CnCompany` and `CnCustomer` on every read.
 */
...
export interface ListProjectsOptions {
  ...
  /**
   * When provided (non-empty array), restricts the result to these project
   * IDs. Used to enforce per-user `projectsAssigned` scoping — site-scoped
   * users (SITE_ADMIN / USER with assignments) see only the projects they
   * were granted. Pass `undefined` for unrestricted callers (super admin,
   * tenant admin, HO user, or any user without explicit assignments).
   */
  projectIds?: string[];
...
function buildProjectsWhere(
  opts: Pick<
    ListProjectsOptions,
    "tenantId" | "orgId" | "search" | "includeInactive" | "projectIds"
  >,
): Record<string, unknown> {
  const q = (opts.search ?? "").trim();
  // `projectIds: []` would match nothing — that's the right behaviour for a
  // user who has been explicitly restricted to zero projects. We only skip
  // the filter when the caller passed `undefined` (unrestricted).
...
export interface UpdateProjectInput
  extends Partial<Omit<CreateProjectInput, "tenantId" | "orgId" | "createdBy">> {
  updatedBy: string;
}
