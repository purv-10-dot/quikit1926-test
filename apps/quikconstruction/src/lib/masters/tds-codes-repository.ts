/**
 * TDS Codes master — Prisma-backed CRUD for `tds_codes`.
 */
...
function buildTDSCodesWhere(
  opts: Pick<ListTDSOptions, "tenantId" | "orgId" | "search">,
...
export async function findTDSCodeById(
  ...
  const row = await (db as any).cnTDSCode.findFirst({ where: { id, tenantId } });
...
export interface UpdateTDSInput
  extends Partial<Omit<CreateTDSInput, "tenantId" | "orgId" | "createdBy">> {
  updatedBy: string;
}
