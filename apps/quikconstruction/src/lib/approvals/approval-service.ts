...
import { getRequiredPermissionForStep } from "@/lib/rbac/roles";
...
class ApprovalService {
  ...
  async execute(req: ApprovalRequest): Promise<ApprovalResult> {
    ...
    return await db.$transaction(async (tx) => {
      ...
      // ─── Actor permission check (step-locked) ────────────────
      const requiredPerm = getRequiredPermissionForStep(instance.entityType, currentStep);
      if (!requiredPerm) {
        throw new ApprovalStateError(
          "NO_STEP_ACTOR_MAPPING",
          `No actor mapping for ${instance.entityType} step ${currentStep}. ` +
            `Add an entry to APPROVAL_ACTOR_MATRIX in src/lib/rbac/roles.ts.`
        );
      }
      if (!hasPermission(ctx, requiredPerm)) {
