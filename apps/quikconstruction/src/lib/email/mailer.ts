 ...
 *                        "QuikConstruction <no-reply@quikconstruction.local>"
 ...
 * `apps/quikconstruction`. Walking up lets the mailer find its `.env`
 * no matter how you started the server.
 */
function resolveEnvPath(): string | null {
  if (_resolvedEnvPath) return _resolvedEnvPath;
  // Look for `.env.local` first (Next.js loads it with higher priority
  // than `.env` and dev users typically keep secrets there), then `.env`.
  const candidates = [
    path.join(process.cwd(), ".env.local"),
    path.join(process.cwd(), ".env"),
    path.join(process.cwd(), "apps", "quikconstruction", ".env.local"),
    path.join(process.cwd(), "apps", "quikconstruction", ".env"),
  ];
  // Walk upward from cwd looking for either file beside a package.json.
  let dir = process.cwd();
  for (let i = 0; i < 6; i++) {
...
function getMailFrom(): string {
  ...
  return (
    liveEnv("MAIL_FROM") ??
    (liveEnv("SMTP_USER")
      ? `QuikConstruction <${liveEnv("SMTP_USER")}>`
      : "QuikConstruction <no-reply@quikconstruction.local>")
  );
}
...
function randomMessageId(): string {
  ...
  return `<${rand}@quikconstruction.local>`;
}
...
const fileOutboxTransport: Mailer = {
  async sendMail(input: SendMailInput): Promise<SendMailResult> {
    ...
    try {
      ...
      const to = Array.isArray(input.to) ? input.to.join(", ") : input.to;
      console.log(
        `[mailer] sent → ${to}  subject="${input.subject}"  file=${filePath}` +
...
const smtpTransport: Mailer = {
  async sendMail(input: SendMailInput): Promise<SendMailResult> {
    ...
    try {
      ...
      console.log(
        `[mailer:smtp] sent → ${to}  subject="${input.subject}"  id=${info.messageId}  ` +
 ...
 */
// Log the chosen transport on every state change — "I was picking
// file-outbox last call, now I'm picking SMTP" is the exact signal the
// admin wants to see when they edit .env.
let _lastLoggedTransport: "smtp" | "file-outbox" | null = null;
function pickMailer(): Mailer {
  const cfg = getSmtpConfig();
  const kind: "smtp" | "file-outbox" = cfg ? "smtp" : "file-outbox";
  if (kind !== _lastLoggedTransport) {
    _lastLoggedTransport = kind;
    if (cfg) {
      console.log(
        `[mailer] using SMTP → ${cfg.host}:${cfg.port} ` +
          `(secure=${cfg.secure}) user=${cfg.user}`
      );
    } else {
      console.warn(
