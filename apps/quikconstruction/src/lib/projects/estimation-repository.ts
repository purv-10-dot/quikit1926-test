 ...
 * `material_estimations`. The `materials` column is JSONB; each line
...
export async function listEstimations(
  ...
  const rows: any[] = projectId
    ? await (db as any).$queryRaw`
        SELECT
          id, "tenantId", "orgId", "projectId", "projectName",
        ...
        FROM app_quikconstruction."Material_estimations"
...
export async function createEstimation(
  ...
  await (db as any).$executeRaw`
    INSERT INTO app_quikconstruction."Material_estimations" (
      id, "tenantId", "orgId", "projectId", "projectName",
      "boqItemId", "boqNo", "boqDescription", "boqQuantity", "boqUnit",
      phase, status, "totalQty", "totalCost", "materialCount", materials,
      "createdAt", "updatedAt", "createdBy", "updatedBy"
    ) VALUES (
      ${id}, ${input.tenantId}, ${input.orgId}, ${input.projectId},
  ...
  return (await findEstimationById(input.tenantId, id))!;
}
...
export async function updateEstimation(
  ...
  const existing = await findEstimationById(input.tenantId, id);
  ...
  await (db as any).$executeRaw`
    UPDATE app_quikconstruction."Material_estimations"
    SET
  ...
  return findEstimationById(input.tenantId, id);
}
...
// `material_estimations` uses a JSONB blob for the composition, so
...
export async function patchEstimationStatus(
  ...
  const existing = await findEstimationById(tenantId, id);
  if (!existing) return null;
  ...
  return findEstimationById(tenantId, id);
}
...
export async function deleteEstimation(
  ...
  const res: any = await (db as any).$executeRaw`
    DELETE FROM app_quikconstruction."Material_estimations"
    WHERE "tenantId" = ${tenantId} AND id = ${id}
  `;
