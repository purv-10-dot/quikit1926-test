...
export async function idempotencyGuard(
  ...
  const key = headerKey ?? `auto:${ctx.tenantId}:${ctx.userId}:${routeKey}:${bodyHash}`;
  ...
  if (existing) {
    // Tenancy check (defense in depth)
    if (existing.tenantId !== ctx.tenantId) {
      return {
        ...
        conflictResponse: NextResponse.json(
          { error: "Idempotency key belongs to a different tenant", code: "IDEMPOTENCY_CONFLICT" },
          { status: 409 }
        ),
        commit: async () => {},
        parsedBody,
