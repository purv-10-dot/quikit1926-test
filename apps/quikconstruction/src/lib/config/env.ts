 ...
 *   - Production mode is strict: `NEXTAUTH_SECRET`, `DATABASE_URL`, and
 *     `AUTH_DEMO_MODE=false` are REQUIRED. `STORAGE_DRIVER=local` is a
 *     warning (LAN-only demo, not for public internet).
 *
...
const schema = z
  .object({
    ...
    AUTH_DEMO_MODE: z.enum(["true", "false"]).optional(),
  ...
  .superRefine((env, ctx) => {
    ...
    const demoMode = env.AUTH_DEMO_MODE !== "false";
    ...
    if (isProd) {
      ...
      if (env.AUTH_DEMO_MODE !== "false") {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["AUTH_DEMO_MODE"],
          message:
            "AUTH_DEMO_MODE must be 'false' in production — demo mode bypasses NextAuth and exposes a super-admin context to every request.",
        });
      }
    ...
    // Touch the demoMode var so lint doesn't complain when production branch is inactive
    void demoMode;
  });
...
export function loadEnv(): Env {
  ...
  if (!parsed.success) {
    ...
    const msg =
      ...
      `See apps/quikconstruction/.env.example for the full list of variables.`;
