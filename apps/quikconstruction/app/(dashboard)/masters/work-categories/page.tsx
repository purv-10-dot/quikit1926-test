...
const emptyForm = { name: "", description: "", sacCode: "", sortOrder: "1", status: "active" };
...
export default function WorkCategoriesPage() {
  ...
  const handleImportRow = async (row: Record<string, string>) => {
    ...
    try {
      await createMutation.mutateAsync({
        ...
        sacCode: row.sacCode?.replace(/\D/g, "") || undefined,
  ...
  return (
    <>
      <MasterListPage title="Work Categories" entityName="Work Category" columns={columns}
      ...
      <FormDrawer open={drawerOpen} onClose={closeDrawer}
        ...
        <FormSection title="Category Details">
          ...
          <FormRow>
            <Field label="SAC Code" error={errors.sacCode} hint="Service Accounting Code, 4–8 digits">
              <TextInput value={form.sacCode} onChange={v => set("sacCode", v.replace(/\D/g, ""))} placeholder="998311" maxLength={8} invalid={!!errors.sacCode} />
            </Field>
