...
interface Contractor { id: string; code: string; name: string; gstin: string | null; specialization: string | null; city: string | null; status: string }

const columns: ColumnConfig<Contractor>[] = [
  { key: "code", label: "Code", className: "font-mono text-xs text-gray-900" },
  { key: "name", label: "Name", render: (r) => <div><div className="font-medium text-gray-900">{r.name}</div>{r.specialization && <div className="text-xs text-gray-500">{r.specialization}</div>}</div> },
...
const fields: FieldConfig[] = [
  ...
  { name: "legalName", label: "Legal Name", type: "text", width: "half" },
  { name: "gstin", label: "GSTIN", type: "text", width: "half", transform: "uppercase", max: 15 },
  { name: "pan", label: "PAN", type: "text", width: "half", transform: "uppercase", max: 10 },
  { name: "licenseNo", label: "License No", type: "text", width: "half" },
...
export default function ContractorsPage() {
  return (
    <MasterListPage<Contractor>
      title="Contractors"
      subtitle="Construction subcontractors. Codes unique per tenant."
      icon={HardHat}
