...
import { useProjects, useItems, useUOMs, useLocations, useWorkCategories } from "@/hooks/use-masters";
...
export function PRCreateDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // Memoized option list for the searchable material picker. Rebuilds only
  // when the item master list itself changes, not on every line edit.
  const itemOptions = useMemo<SearchSelectOption[]>(
    () =>
      items.map((item: any) => ({
        value: item.id,
        label: item.name ?? item.code ?? item.id,
        sublabel: [item.code, item.uomCode, item.groupName]
          .filter(Boolean)
          .join(" · "),
        searchText: `${item.code ?? ""} ${item.hsnCode ?? ""}`,
        badge: item.uomCode ?? undefined,
      })),
    [items]
  );
      ...
      <div className="fixed right-0 top-0 bottom-0 w-full max-w-3xl bg-white shadow-2xl flex flex-col z-50">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {/* BOQ Item — mandatory standalone section. Picking a leaf
              ...
              `relative z-10` so the SearchSelect's absolute-positioned
                        ...
                        <div className="grid grid-cols-4 gap-3">
                          <div className="col-span-3">
                            ...
                            <SearchSelect
                              ...
                              options={itemOptions}
                              placeholder="Search item by name, code, or group…"
                              emptyText={items.length === 0 ? "Loading items…" : "No matching items"}
                            />
                          </div>
                          <div>
