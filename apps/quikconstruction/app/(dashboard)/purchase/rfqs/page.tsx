...
import { Eye, Send, CheckCircle2, GitCompare, Pencil } from "lucide-react";
...
import { useProjects, useItems, useVendors, useTermsConditions } from "@/hooks/use-masters";
...
export default function RFQsPage() {
  ...
  const vendorOptions = allVendors.map((v: any) => ({
    value: v.id,
    // Prefer company name; fall back to the contact name so the dropdown
    // is still readable when a vendor was added without a company.
    ...
    fields: [
      {
        ...
        // material lines into the RFQ grid.
      ...
      fields: [
        {
          ...
          width: "wide" as const,
          // Auto-fill email from the vendor master so the RFQ-to-send
          // list is ready to dispatch without retyping addresses.
          onChange: (value: string) => {
            if (!value) return;
            const vendor = vendorById.get(value);
            if (!vendor) return;
            const patch: Record<string, string> = {};
            if (vendor.email) patch.email = String(vendor.email);
            return patch;
          },
        },
        {
      ...
      fields: [
        {
          ...
          width: "wide",
          // Auto-fill UOM from the item master on pick — same pattern
          // as the Indent drawer. Qty stays blank (transaction field).
          onChange: (value: string) => {
            if (!value) return;
          ...
          <div className="flex items-center justify-end gap-1.5">
            ...
            {isDraft && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  router.push(`/purchase/rfqs/${row.id}?edit=1`);
                }}
