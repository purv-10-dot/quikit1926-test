...
export default function BOQPage() {
  ...
  const handleLock = async () => {
    ...
    if (res.ok) {
      ...
      alert(err.error ?? "Failed to lock BOQ");
    }
  };
  const confirmDelete = async () => {
    ...
    try {
      ...
      alert(err.message ?? "Failed to delete BOQ item");
    } finally {
      setDeleting(false);
    }
  };
  ...
  const handleUnlock = async () => {
    ...
    if (res.ok) {
      ...
      const err = await res.json();
      alert(err.error ?? "Failed to unlock BOQ");
    }
  };
  ...
  function renderRows(nodes: any[], depth: number = 0): React.ReactNode[] {
    ...
    for (const node of nodes) {
      ...
      result.push(
        <tr key={node.id ?? node.boqNo} className={`border-b border-gray-200/60 hover:bg-blue-50/40 ${grpBg} ${neg ? "bg-red-50/40" : ""}`}>
          {/* BOQ No */}
          ...
          <td className="py-1.5 border-r border-gray-100 max-w-[350px]" style={{ paddingLeft: `${6 + depth * 20}px` }}>
            <div className="flex items-center gap-1">
              {isGrp ? (
                ...
                <FileText className="w-3 h-3 text-blue-400 shrink-0 ml-1" />
              )}
          ...
          <td className="px-1 py-1.5 text-center border-r border-gray-100">
            {v.tender > 0 && (
              <span className={`inline-block px-1.5 py-0.5 rounded text-[9px] font-bold ${
                ...
                v.pct > 50 ? "bg-blue-100 text-blue-700" :
      ...
      <div className="sticky top-0 z-20 bg-white border-b border-gray-200 px-4 py-2">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            {/* Add Item */}
            <PrimaryButton
              onClick={() => !selectedProject ? alert("Select a project first") : lockState.isLocked ? alert("BOQ is locked. Unlock first to add items.") : setAddDrawerOpen(true)}
              disabled={lockState.isLocked}>
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Upload Revision */}
            <PrimaryButton onClick={() => selectedProject ? setImportDrawerOpen(true) : alert("Select a project first")}
        ...
        {!selectedProject ? (
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div className="flex items-center gap-5 px-4 py-2 border-b border-gray-100 bg-gradient-to-r from-indigo-50/60 to-white text-xs">
              ...
              <span className="text-blue-700"><b>{summary.leafCount ?? 0}</b> billable</span>
