...
function nextWbsCode(tasks: WbsTask[], parentId: string | null): string {
  ...
  if (parentId === null) return `${siblings.length + 1}`;
...
export default function WbsPlanningPage() {
  ...
  const [tasksByProject, setTasksByProject] = useState<Record<string, WbsTask[]>>({});
  const tasks = tasksByProject[selectedProject] ?? [];
  ...
  const handleAdd = () => {
    ...
    setTasksByProject((prev) => ({
      ...prev,
      [selectedProject]: [...(prev[selectedProject] ?? []), task],
    }));
    resetForm();
  };
            ...
            >
              ...
              {(() => {
                ...
                return links.map(({ from, to }) => {
                  ...
                  const toX = b.left;
                  const toY = b.top;
                  // Elbow: prefer 10px past predecessor, but never past
                  // the dependent's start (clamp so the path doesn't
                  // cross over its own arrowhead).
                  const elbow = Math.min(fromX + 10, Math.max(fromX + 4, toX - 6));
                  const d = `M ${fromX} ${fromY} L ${elbow} ${fromY} L ${elbow} ${toY} L ${toX - 2} ${toY}`;
                  return (
                    <path
                      key={`${from}->${to}`}
                      d={d}
