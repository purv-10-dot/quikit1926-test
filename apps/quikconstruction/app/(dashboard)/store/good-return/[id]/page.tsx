...
export default function GoodReturnDetailPage() {
  ...
  useEffect(() => {
    fetch(`/api/store/good-return/${id}`).then(r => r.json()).then(j => j.success && setDoc(j.data));
  }, [id]);
  ...
  return (
    <DocDetailLayout
      backHref="/store/good-return"
      backLabel="Vendor Returns"
      ...
      meta={[
        { label: "Source GRN", value: doc.grn ? <a href={`/store/grn/${doc.grn.id}`} className="font-mono text-accent-700 hover:underline">{doc.grn.grnNumber}</a> : "—" },
