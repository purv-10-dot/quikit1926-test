...
export async function GET(req: NextRequest) {
  ...
  if (prInstanceIds.length) {
    const prRows = await (db as any).cnPurchaseRequisition.findMany({
      where: { tenantId: ctx.tenantId, id: { in: prInstanceIds } },
  ...
  // Resolve requester display names in a single batch.
  const requesterIds = Array.from(new Set(actionable.map((i: any) => i.requestedById)));
  const requesters = requesterIds.length
    ...
    : [];
  const nameById = new Map<string, string>(requesters.map((u: any) => [u.id, u.fullName]));
