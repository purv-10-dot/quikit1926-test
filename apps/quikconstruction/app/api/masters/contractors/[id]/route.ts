...
export async function GET(
  ...
  const row = await findContractorById(ctx.tenantId, params.id);
...
async function handleUpdate(req: NextRequest, id: string) {
  ...
  try {
    const next = await updateContractor(ctx.tenantId, id, {
      ...safe,
    ...
    return NextResponse.json(
      { error: e?.message ?? "Failed to update contractor" },
      { status: 500 },
    );
  }
}
...
export async function DELETE(
  ...
  const ctx = await getTenantContext();
  if (!ctx) return NextResponse.json({ error: "Unauthenticated" }, { status: 401 });
  const ok = await deleteContractor(ctx.tenantId, params.id, ctx.userId);
