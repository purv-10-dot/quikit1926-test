...
export async function GET(
  ...
  const row = await findTermsConditionById(ctx.tenantId, params.id);
...
async function handleUpdate(req: NextRequest, id: string) {
  ...
  const {
    id: _a, tenantId: _b, orgId: _c,
    createdAt: _d, createdBy: _e, updatedAt: _f, updatedBy: _g,
    ...safe
  } = body ?? {};

  try {
    const next = await updateTermsCondition(ctx.tenantId, id, {
      title: safe.title,
      body: safe.body,
    ...
    return NextResponse.json(
      { error: e?.message ?? "Failed to update T&C template" },
      { status: 500 },
    );
  }
}
...
export async function DELETE(
  ...
  const ctx = await getTenantContext();
  if (!ctx) return NextResponse.json({ error: "Unauthenticated" }, { status: 401 });
  const ok = await deleteTermsCondition(ctx.tenantId, params.id, ctx.userId);
