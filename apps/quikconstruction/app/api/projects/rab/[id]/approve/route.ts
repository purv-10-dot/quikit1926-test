...
export const POST = withTenantAuth<{ id: string }>(async ({ tenantId, userId, session }, _req, { params }) => {
  ...
  const rab = await db.cnRAB.findFirst({ where: { id: params.id, tenantId, deletedAt: null }, select: { id: true, status: true, total: true } });
  ...
  // Approval gate
  const gate = await checkApprovalGate({ tenantId, docType: "rab", docId: rab.id, amount: Number(rab.total) });
  ...
  await logAudit({ tenantId, userId, actionType: "approve", entityType: "cnRAB", entityId: rab.id, oldValues: { status: rab.status }, newValues: { status: "approved" } });
