 ...
 * `rab_lines` rows are the source of truth for billed-per-BOQ-item
 * detail; the BOQ billing service expects a `boqNo` string, so we
 * resolve each `boqItemId` against `boq_items_v2.boqNo` inside the
 * transaction.
 */
export async function POST(
  ...
  try {
    const rab = await (db as any).cnRunningAccountBill.findFirst({
      where: { id: params.id, tenantId: ctx.tenantId, orgId: ctx.orgId },
      select: {
        id: true,
        status: true,
        projectId: true,
    ...
    if (boqItemIds.length) {
      const boqRows = await (db as any).cnBOQItemV2.findMany({
        where: { id: { in: boqItemIds }, tenantId: ctx.tenantId, projectId: rab.projectId },
    ...
    return NextResponse.json(
      { error: e?.message ?? "Failed to approve RAB" },
      { status: 500 },
    );
  }
}
