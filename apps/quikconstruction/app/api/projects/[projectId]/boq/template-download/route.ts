 ...
 *
 * Returns a blank-but-pre-headed .xlsx template the user can fill and re-upload
 * via the BOQ Import flow.
 *
 *   numeric    — QuikConstruction 6-column format
 ...
 *
 * A few sample rows are baked in so the user understands the hierarchy
 * convention before they start filling.
 */
export async function GET(
  ...
  if (format === "alphabetic") {
    ...
    sheetName = "Civil";
    fileName = "BOQ_Template_QuikConstruction.xlsx";
    aoa = [
      ["BOQ No", "SOR No", "Description", "Unit", "Rate", "Op. Undone Qty"],
  ...
  // Wrap in a Uint8Array so the body type lines up with `BodyInit`. Newer
  // @types/node typings declare Buffer as Uint8Array<ArrayBufferLike>, which
  // the Web Fetch `BodyInit` union doesn't accept directly — Uint8Array<ArrayBuffer> does.
