...
export const DELETE = withTenantAuth<{ id: string }>(async ({ tenantId, userId }, _req, { params }) => {
  const note = await db.cnDebitNote.findFirst({ where: { id: params.id, tenantId }, select: { id: true, status: true, noteNumber: true } });
  ...
  await logAudit({ tenantId, userId, actionType: "delete", entityType: "cnDebitNote", entityId: note.id, entityRef: note.noteNumber });
