...
export const PATCH = withTenantAuth(async ({ tenantId, userId, session }, req, ctx: { params: { id: string } }) => {
  ...
  const bill = await db.cnVendorBill.findFirst({ where: { id: ctx.params.id, tenantId }, select: { id: true, status: true, total: true } });
  if (!bill) return NextResponse.json({ success: false, error: "Not found" }, { status: 404 });
  if (body.action === "approve") {
    if (bill.status !== "draft") return NextResponse.json({ success: false, error: "Only draft bills can be approved" }, { status: 400 });
    const gate = await checkApprovalGate({ tenantId, docType: "vendor_bill", docId: bill.id, amount: Number(bill.total) });
    ...
    await logAudit({ tenantId, userId, actionType: "approve", entityType: "cnVendorBill", entityId: bill.id, oldValues: { status: "draft" }, newValues: { status: "approved" } });
