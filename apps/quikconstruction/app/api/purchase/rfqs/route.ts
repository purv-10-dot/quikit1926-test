...
export async function GET(req: NextRequest) {
  try {  
    ...
    console.error("[purchase/rfqs.GET] failed:", err);
    return NextResponse.json(
...
export async function POST(req: NextRequest) {
  try {
    ...
    let sourceIndent: any = null;
    if (body.sourceIndentId) {
      sourceIndent = await findIndentById(ctx.tenantId, body.sourceIndentId);
    }
    ...
    const project = await findProjectById(ctx.tenantId, projectId);
    if (!project) {
    ...
    const userLines: any[] = Array.isArray(body.lines) ? body.lines : [];
    const rawLines =
      userLines.length > 0
        ? userLines
        : (sourceIndent?.lines ?? []).map((l: any) => ({
            itemId: l.itemId,
    ...
    const itemIds = Array.from(
      new Set<string>(rawLines.map((l: any) => l.itemId).filter(Boolean)),
    );
    ...
    const repoLines = rawLines
      ...
      .map((l: any) => {
        ...
        return {
          ...
          quantity: parseFloat(l.quantity ?? l.qtyRequested ?? "0") || 0,
    ...
    const rawVendors: any[] = Array.isArray(body.vendors) ? body.vendors : [];
    const vendorIdsLookup = Array.from(
      new Set<string>(rawVendors.map((v: any) => v?.vendorId).filter(Boolean)),
    );
    const vendorMasterMap = vendorIdsLookup.length
      ? await findVendorsByIds(ctx.tenantId, vendorIdsLookup)
      : new Map();

    const repoVendors = rawVendors
      .filter((v: any) => v && v.vendorId)
      .map((v: any) => {
        const master = vendorMasterMap.get(v.vendorId);
        const assignedItemIds: string[] = Array.isArray(v.assignedItemIds)
          ? v.assignedItemIds.filter((x: any) => typeof x === "string" && x)
          : [];
        return {
          vendorId: v.vendorId,
