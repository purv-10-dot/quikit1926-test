...
export const POST = withTenantAuth<{ id: string }>(async ({ tenantId, userId }, _req, { params }) => {
  ...
  const gate = await checkApprovalGate({ tenantId, docType: "po", docId: po.id, amount: Number(po.totalAmount) });
  ...
  await logAudit({ tenantId, userId, actionType: "status_change", entityType: "cnPurchaseOrder", entityId: po.id, oldValues: { status: "draft" }, newValues: { status: "sent" } });
