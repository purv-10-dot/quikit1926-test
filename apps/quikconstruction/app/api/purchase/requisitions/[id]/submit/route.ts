...
export const POST = withTenantAuth<{ id: string }>(async ({ tenantId, userId }, _req, { params }) => {
  ...
  const gate = await checkApprovalGate({ tenantId, docType: "pr", docId: pr.id, amount });
  ...
  await logAudit({ tenantId, userId, actionType: "status_change", entityType: "cnPurchaseRequisition", entityId: pr.id, oldValues: { status: "draft" }, newValues: { status: "submitted" } });
