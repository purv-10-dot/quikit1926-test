...
export async function GET(req: NextRequest) {
  try {  
    ...
    console.error("[purchase/grn.GET] failed:", err);
    return NextResponse.json(
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (poIdOrNumber) {
      // Try by id first
      try {
        po = await findPOById(ctx.tenantId, String(poIdOrNumber));
      } catch {
        /* noop */
      }
      // By poNumber
      if (!po) {
        try {
          const row = await (db as any).cnPurchaseOrder.findFirst({
            where: { tenantId: ctx.tenantId, poNumber: String(poIdOrNumber) },
            select: { id: true },
          });
          if (row?.id) po = await findPOById(ctx.tenantId, row.id);
        } catch {
          /* noop */
        }
      }
    ...
    // is keyed on (tenantId, orgId, code) so concurrent GRN creates
    // converge to a single row instead of racing.
    if (!locationId) {
