...
export const POST = withTenantAuth<{ id: string }>(async ({ tenantId, userId, session }, _req, { params }) => {
  if (!canFinalize(session.user.membershipRole)) return forbidden("Payroll finalize requires admin or accountant role");
  const p = await db.cnPayroll.findFirst({ where: { id: params.id, tenantId, deletedAt: null }, select: { id: true, status: true, totalNet: true } });
  ...
  const gate = await checkApprovalGate({ tenantId, docType: "payroll", docId: p.id, amount: Number(p.totalNet) });
  ...
  await logAudit({ tenantId, userId, actionType: "finalize", entityType: "cnPayroll", entityId: p.id, oldValues: { status: "draft" }, newValues: { status: "finalized" } });
