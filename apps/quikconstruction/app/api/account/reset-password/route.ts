 ...
 *   - the forced first-login flow (mustChangePassword=true on users)
 ...
 *
 * On success:
 *   - users.passwordHash rewritten with a fresh scrypt hash
 *   - users.mustChangePassword cleared
...
export async function POST(req: NextRequest) {
  try {
    ...
    // accounts) don't live in users, so they hit a 404 here — that's
    ...
    console.error("[account/reset-password.POST] failed:", err);
    return NextResponse.json(
