...
export async function POST(
  ...
  const instance = await (db as any).cnApprovalInstance.findFirst({
    where: { id: issue.approvalId, tenantId: ctx.tenantId },
  });
  if (!instance) {
    return NextResponse.json(
  ...
  if (issueStatusUpdate) {
    await patchMaterialIssueStatus(ctx.tenantId, issue.id, {
      ...(issueStatusUpdate as any),
      updatedBy: ctx.userId,
    });
  }

  const refreshed = await findMaterialIssueById(ctx.tenantId, issue.id);
