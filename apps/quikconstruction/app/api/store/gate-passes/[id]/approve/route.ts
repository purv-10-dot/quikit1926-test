...
export async function POST(
  ...
  const gatePass = await findGatePassById(ctx.tenantId, params.id);
  if (!gatePass) {
  ...
  if (gpStatusUpdate) {
    await patchGatePassStatus(ctx.tenantId, gatePass.id, {
      ...(gpStatusUpdate as any),
      updatedBy: ctx.userId,
    });
  }

  const refreshed = await findGatePassById(ctx.tenantId, gatePass.id);
  return NextResponse.json({
    ok: true,
    action,
    gatePass: refreshed,
