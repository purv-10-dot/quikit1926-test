...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await listGatePasses(ctx.tenantId, {
      status: status || null,
      type: type || null,
    ...
    console.error("[store/gate-passes.GET] failed:", err);
...
export async function POST(req: NextRequest) {
  ...
  const seqNumber = await nextGatePassSequence(ctx.tenantId, year4, direction);
