 ...
 *
 * Persists to `stock_reconciliations` + `stock_reconciliation_lines`.
...
export async function GET(req: NextRequest) {
  try {  
    ...
    // the location label without joining locations on every row.
    ...
    console.error("[store/reconciliations.GET] failed:", err);
    return NextResponse.json(
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: e?.message ?? "Failed to create reconciliation" },
      { status: 500 },
    );
  }
}
