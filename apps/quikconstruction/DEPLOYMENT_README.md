# QuikConstruction — Deployment & Operations Guide
...
| 7 | RBAC sidebar + server enforcement | ✓ Fixed | Sidebar items carry `requiredPermission`, filtered via `usePermissions().can()` in [`QuikConstructionShell.tsx`](src/components/QuikConstructionShell.tsx); server routes already gated via `requirePermission()` from Phase 1+ |
...
```
src/components/QuikConstructionShell.tsx
...
#   DATABASE_URL="postgresql://postgres:postgres@127.0.0.1:5432/quikconstruction?schema=public"
...
| Object bucket | separate bucket (e.g. `quikconstruction-staging`) | `quikconstruction-prod` |
...
```bash
docker run -p 3010:3010 --env-file .env.production quikconstruction:latest
```
...
```bash
# /etc/cron.daily/quikconstruction-backup
...
pg_dump \
  ...
  --no-acl \
  --dbname="$DATABASE_URL" \
  --file="/backups/quikconstruction-${TIMESTAMP}.dump"

# Retention: keep 30 days locally, sync older to S3 Glacier
find /backups -name 'quikconstruction-*.dump' -mtime +30 -delete
...
createdb quikconstruction_restore
pg_restore \
  --dbname=postgresql://postgres:***@localhost:5432/quikconstruction_restore \
  --no-owner \
  --no-acl \
  --clean \
  --if-exists \
  /backups/quikconstruction-YYYYMMDD-HHMMSS.dump

# Sanity checks
psql quikconstruction_restore -c "SELECT COUNT(*) FROM projects"
psql quikconstruction_restore -c "SELECT COUNT(*) FROM boq_items_v2"
psql quikconstruction_restore -c "SELECT COUNT(*) FROM audit_logs WHERE timestamp > NOW() - INTERVAL '1 day'"
```
...
```bash
pnpm install --filter @quikit/quikconstruction
