...
export function AccountTasksTab({
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/tasks?relatedKind=Account&relatedObjectId=${encodeURIComponent(accountId)}&pageSize=100`,
        { credentials: "include" },
      );
  ...
  const seed: TaskFormSeed = {
    subject: `Follow up with ${accountName}`,
    priority: "Medium",
    status: "Open",
    dueDate: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(),
    relatedKind: "Account",
    relatedObjectId: accountId,
    relatedLabel: accountName,
    ...
    <div>
      ...
      {items.length === 0 ? (
        <LeadEmptyState
          icon={CheckSquare}
          title="No tasks"
          description="Create a follow-up task for this account."
