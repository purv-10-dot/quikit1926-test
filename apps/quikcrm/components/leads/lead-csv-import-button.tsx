...
export function LeadCsvImportButton() {
    ...
    try {
      ...
      if (json.status === "completed" || json.status === "completed_with_errors") {
        ...
        if (imported === 0 && failed > 0) {
          ...
          toast.success(`Imported ${imported} lead(s).`);
        }
        router.refresh();
      } else {
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={close} title="Import leads from CSV" width="max-w-lg">
        <div className="space-y-4">
          <p className="text-sm text-crm-muted">
            Upload a CSV file. Rows are processed in the background — you can keep working while
            the import runs.
          </p>
          <div className="rounded-md border border-crm-border bg-gray-50 px-3 py-2 text-xs text-crm-muted">
            ...
            <p className="mt-1">
              name, email, phone, mobile, company, jobTitle, source, externalId, sourceSystem,
              ownerName
            </p>
            <p className="mt-1">
              Re-import with the same <span className="font-mono">externalId</span> to update an
              existing lead (uses source system <span className="font-mono">ui-upload</span> from
              this screen).
            </p>
          </div>
