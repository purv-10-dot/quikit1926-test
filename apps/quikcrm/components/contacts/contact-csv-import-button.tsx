...
import {
  CONTACTS_IMPORT_SAMPLE_CSV,
  CONTACTS_IMPORT_SAMPLE_FILENAME,
} from "@/lib/import/contacts-import-sample";
import { downloadCsvFile } from "@/lib/utils/download-csv";

export function ContactCsvImportButton() {
  ...
  async function upload() {
    ...
    try {
      ...
      const res = await fetch("/api/imports/queue/contacts", {
        method: "POST",
        credentials: "include",
      ...
      if (json.status === "completed" || json.status === "completed_with_errors") {
        ...
        if (imported === 0 && failed > 0) {
          ...
          toast.success(`Imported ${imported} contact(s).`);
        }
        router.refresh();
      } else {
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={close} title="Import contacts from CSV" width="max-w-lg">
        <div className="space-y-4">
          <p className="text-sm text-crm-muted">
            Upload a CSV file to bulk-create or update contacts. Contacts with a matching email
            are updated; new emails (or rows without email) create new records.
          </p>
          <div className="rounded-md border border-crm-border bg-gray-50 px-3 py-2 text-xs text-crm-muted">
            ...
            <p className="mt-1">
              <span className="font-medium text-crm-text">Required:</span> firstName — all other
              columns are optional.
            </p>
            <p className="mt-1">
              firstName, lastName, email, phone, title, ownerName, city, contactStage, source,
              accountName
            </p>
            <p className="mt-1">
              Use <span className="font-mono">accountName</span> to link a contact to an existing
              account by name.
            </p>
          </div>
          ...
          <button
            type="button"
            onClick={() => downloadCsvFile(CONTACTS_IMPORT_SAMPLE_CSV, CONTACTS_IMPORT_SAMPLE_FILENAME)}
