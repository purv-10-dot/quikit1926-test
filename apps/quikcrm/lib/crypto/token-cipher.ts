 ...
 *
 * Mailbox access/refresh tokens must never be persisted in plaintext. This
 * module encrypts them with a symmetric key from MAILBOX_TOKEN_ENCRYPTION_KEY
 * (a base64-encoded 32-byte key). GCM gives us confidentiality + an auth tag,
 * so a tampered ciphertext fails to decrypt rather than silently returning
 * garbage.
 *
 ...
 *
 * Uses only Node's built-in `crypto` — no new dependency (app CLAUDE.md rule).
 */
 ...
 * unconfigured deployment fails only when a mailbox is actually connected.
 */
function getKey(): Buffer {
  const raw = process.env.MAILBOX_TOKEN_ENCRYPTION_KEY;
  if (!raw) {
    throw new TokenCipherError(
      "MAILBOX_TOKEN_ENCRYPTION_KEY is not set — cannot store mailbox tokens. " +
  ...
  try {
    ...
    throw new TokenCipherError("MAILBOX_TOKEN_ENCRYPTION_KEY is not valid base64.");
  }
  if (key.length !== KEY_BYTES) {
    throw new TokenCipherError(
      `MAILBOX_TOKEN_ENCRYPTION_KEY must decode to ${KEY_BYTES} bytes (got ${key.length}).`,
    );
  }
  return key;
}
