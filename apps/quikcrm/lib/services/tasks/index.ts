...
function buildListWhere(user: SessionUser, q: ListTasksQuery): Prisma.CrmTaskWhereInput {
  const where: Prisma.CrmTaskWhereInput = { orgId: user.orgId };
...
export async function getTask(user: SessionUser, id: string) {
  return prisma.crmTask.findFirst({ where: { id, orgId: user.orgId } });
}
...
export async function updateTask(
  ...
  const existing = await prisma.crmTask.findFirst({ where: { id, orgId: user.orgId } });
  if (!existing) {
...
export async function deleteTask(user: SessionUser, id: string) {
  ...
  const res = await prisma.crmTask.deleteMany({ where: { id, orgId: user.orgId } });
  if (res.count === 0) {
    const err = new Error("Task not found") as Error & { statusCode?: number };
    err.statusCode = 404;
    throw err;
