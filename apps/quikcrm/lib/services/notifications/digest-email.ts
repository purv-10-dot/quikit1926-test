...
// ── §1 Activity volume by rep ───────────────────────────────────────────────
// Ranked rep · count, derived from the per-rep activity field aggregates' owner
// set (the digest's per-rep volume signal). When no per-rep data, render an
// honest empty-but-working line (distinct from the §4 not-built placeholder).
  ...
  for (const fa of d.fieldAggregates) {
    for (const agg of fa.aggregates) {
      for (const rep of agg.perRep) {
        ...
        prev.count += 1; // one activity-field contribution per rep per field
