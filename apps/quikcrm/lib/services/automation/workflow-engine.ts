...
export async function runFrom(
  ...
  const wf = await prisma.crmWorkflowDefinition.findFirst({
    where: { id: workflowId, orgId, status: "Active" },
  });
  if (!wf) return;
  ...
  const lead = await prisma.crmLead.findFirst({ where: { id: leadId, orgId } });
  if (!lead) return;
  ...
  while (nodeId && step < MAX_STEPS) {
    ...
    if (!node) break;
    const ctx: RunContext = { orgId, workflowId, leadId, graph, step };
