...
async function fetchUserView(
  ...
  const [permissionTemplates, appRoles, allowedAccounts] = await Promise.all([
    ...
    appId && isCrmRbacClientReady()
      ? prisma.crmUserAppRole.findMany({
...
export async function deleteUser(opts: { actor: SessionUser; id: string }) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const [leadCount, accountCount, oppCount] = await Promise.all([
      tx.crmLead.count({ where: { orgId: actor.orgId, ownerId: id } }),
      tx.crmAccount.count({ where: { orgId: actor.orgId, ownerId: id } }),
      tx.crmOpportunity.count({ where: { orgId: actor.orgId, ownerId: id } }),
    ]);
    ...
    if (isCrmRbacClientReady()) {
      await tx.crmUserAppRole.deleteMany({ where: { userId: id, orgId: actor.orgId } });
      await tx.crmUserPermissionExtra.deleteMany({ where: { userId: id, orgId: actor.orgId } });
    }
