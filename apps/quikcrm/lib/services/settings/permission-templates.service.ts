...
export async function createTemplate(opts: {
  ...
  return prisma.$transaction(async (tx) => {
    const dupe = await tx.crmPermissionTemplate.findFirst({ where: { orgId: actor.orgId, name: data.name } });
...
export async function updateTemplate(opts: {
  ...
  return prisma.$transaction(async (tx) => {
    const before = await tx.crmPermissionTemplate.findFirst({ where: { id, orgId: actor.orgId } });
...
export async function deleteTemplate(opts: { actor: SessionUser; id: string }) {
  const { actor, id } = opts;
  return prisma.$transaction(async (tx) => {
    const target = await tx.crmPermissionTemplate.findFirst({ where: { id, orgId: actor.orgId } });
