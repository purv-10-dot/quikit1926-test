...
export async function listProductFields(orgId: string): Promise<ProductFieldDefinition[]> {
...
export async function deleteCustomProductField(orgId: string, key: string): Promise<void> {
  if (STANDARD_PRODUCT_KEYS.has(key)) {
