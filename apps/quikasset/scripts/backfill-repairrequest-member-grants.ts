/**
 * Backfill: grant `RepairRequest:view` + `RepairRequest:create` to the default
 ...
 * dry-run and an auditable summary. Same shape as
 * scripts/backfill-assetrequest-member-grants.ts.
 *
 * SCOPE
 ...
 *   npx tsx scripts/backfill-repairrequest-member-grants.ts             # dry-run, all orgs
 *   npx tsx scripts/backfill-repairrequest-member-grants.ts --org=<id>  # dry-run, one org
 *   npx tsx scripts/backfill-repairrequest-member-grants.ts --apply     # perform (all orgs)
 *
 * Idempotent — safe to re-run.
 */
import { PrismaClient } from "@prisma/client";
import { selectMissingGrants, REPAIR_REQUEST_MEMBER_GRANTS } from "./repairRequestGrantPlan";
...
async function main() {
  ...
  console.log(
    `${APPLY ? "APPLY" : "DRY-RUN"} — backfill ${REPAIR_REQUEST_MEMBER_GRANTS.map((g) => `${g.resource}:${g.action}`).join(" + ")} ` +
