...
// GET /api/org/roles/[id]/members — list users currently on this role.
...
// PUT /api/org/roles/[id]/members — reconcile membership to the desired set.
// Users without a UserAppAccess row for QuikAsset are skipped + reported.
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Admin lockout guard — refuse to leave the admin role empty.
    if (isAdminRole(role) && desired.length === 0) {
      return NextResponse.json(
        { success: false, error: "Cannot remove every member from the admin role." },
        { status: 409 },
      );
    }
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      const detached = await tx.astUserAppRole.deleteMany({
        where: {
          roleId: role.id,
          orgId,
      ...
      const existing = await tx.astUserAppRole.findMany({
      ...
      for (const userId of toCreate) {
        await tx.astUserAppRole.create({
