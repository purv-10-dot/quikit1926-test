...
// Accept any shape — we filter to catalog-valid pairs ourselves below so that a
// single stale (resource, action) row from an older registry can't fail the
// whole save with "Unknown resource".
const putBodySchema = z.object({
  permissions: z.array(z.object({ resource: z.string().min(1), action: z.string().min(1) })),
});
...
// PUT /api/org/roles/[id]/permissions — atomic replace.
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await db.$transaction([
      db.astRolePermission.deleteMany({ where: { roleId: role.id } }),
      ...(desired.length > 0
        ? [
            db.astRolePermission.createMany({
              data: desired.map((p) => ({
                roleId: role.id,
                resource: p.resource,
                action: p.action,
