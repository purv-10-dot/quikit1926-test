 ...
 *
 * Returns the apps the current user can see in the in-app <AppSwitcher /> grid.
 * Mirrors the launcher's visibility rule (apps/quikit/app/api/apps/launcher)
 * so the grid and the portal never disagree:
 *   1. Org must have OrgAppAccess.enabled = true for the app.
 *   2. requiresOrgAdmin apps gate on ADMIN_TIER_ROLES / super admin.
 *   3. Org/super admins see every provisioned app; others need a UserAppAccess row.
 *   4. `quikit` (the launcher) is excluded.
 */
