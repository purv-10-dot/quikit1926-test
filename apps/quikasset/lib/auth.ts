...
/**
 * QuikAsset auth configuration — SSO via the QuikIT launcher IdP.
 *
 * When QUIKIT_URL + client creds are present, QuikAsset authenticates through
 * QuikIT's OAuth2/OIDC flow (the production model). When unset (fresh local
 * clone before creds are provisioned), it falls back to the direct credentials
 * provider so the app still boots. Mirrors quikscale/lib/auth.ts.
 */
