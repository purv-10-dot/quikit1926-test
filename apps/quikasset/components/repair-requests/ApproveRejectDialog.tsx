...
import type { RepairRequest, RepairRequestDecision } from "@/types/repairRequest"

interface Props {
  request: RepairRequest
  action: RepairRequestDecision
  onClose: () => void
...
/**
 * Approve / Reject confirmation for a single repair request. A note is optional
 * when approving, required when rejecting (so a rejection always carries a
 * reason). Pure presentation — the caller owns the API call.
 */
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-sm font-semibold text-gray-900">
              {isReject ? "Reject repair request" : "Approve repair request"}
            </h2>
          </div>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <dl className="rounded-lg border border-gray-100 bg-gray-50 p-3 text-xs space-y-1.5">
            <div className="flex justify-between gap-2">
              ...
              <dd className="text-gray-800 font-medium text-right">{request.assetName ?? request.assetId}</dd>
            </div>
            <div className="flex justify-between gap-2">
            ...
            <div className="flex justify-between gap-2">
              ...
              <dd className="text-gray-700 text-right">{request.urgency}</dd>
            </div>
            <div className="pt-1.5 border-t border-gray-200/70">
              <dt className="text-gray-400 mb-0.5">{request.issueTitle}</dt>
              <dd className="text-gray-700 whitespace-pre-wrap">{request.issueDescription}</dd>
            </div>
          </dl>
