...
const SYSTEM_FIELDS: { key: keyof Omit<User, "id">; label: string; required: boolean }[] = [
  { key: "employeeId", label: "Employee ID", required: true },
  { key: "name", label: "Employee Name", required: true },
  { key: "email", label: "User Email ID", required: true },
...
export default function ImportModal({ onClose, onImport }: Props) {
  ...
  const [mapping, setMapping] = useState<Partial<Record<keyof Omit<User, "id">, string>>>({})
  ...
  function handleImport() {
    const users: Omit<User, "id">[] = rows.map((row) => ({
      employeeId: mapping.employeeId ? row[mapping.employeeId] ?? "" : "",
      name: mapping.name ? row[mapping.name] ?? "" : "",
      email: mapping.email ? row[mapping.email] ?? "" : "",
      contact: mapping.contact ? row[mapping.contact] ?? "" : "",
      department: mapping.department ? row[mapping.department] ?? "" : "",
      designation: mapping.designation ? row[mapping.designation] ?? "" : "",
      joiningDate: mapping.joiningDate ? row[mapping.joiningDate] ?? "" : "",
      status: ((): "Active" | "Inactive" => {
        const raw = mapping.status ? (row[mapping.status] ?? "").toLowerCase() : "active"
        return raw.includes("inact") || raw === "0" || raw === "false" ? "Inactive" : "Active"
      })(),
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            <h2 className="text-base font-semibold text-gray-900">Import Users</h2>
