...
import { Check, Copy, GitBranch, Loader2, Terminal, X } from "lucide-react";
...
// Deep-link templates for "Open in coding tool". `{repo}` = owner/name.
...
/** Action buttons for the Development section: create branch / commit / open. */
export function DevelopmentActions({
  ...
  const [dialog, setDialog] = useState<null | "branch" | "commit" | "open">(null);
...
function CommitDialog({ issueKey, onClose }: { issueKey: string; onClose: () => void }) {
...
function OpenToolDialog({ repos, onClose }: { repos: LinkedRepo[]; onClose: () => void }) {
