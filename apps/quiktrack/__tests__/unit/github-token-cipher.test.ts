...
describe("github token-cipher", () => {
  ...
  it("round-trips a token", () => {
    const secret = "ghu_16C7e42F292c6912E7710c838347Ae178B4a";
  ...
  it("fails to decrypt with a different key", () => {
    const enc = encryptToken("secret");
    process.env.GITHUB_TOKEN_ENCRYPTION_KEY = randomBytes(32).toString("base64");
  ...
  it("throws a clear error when the key is missing", () => {
    ...
    expect(() => encryptToken("x")).toThrow(/GITHUB_TOKEN_ENCRYPTION_KEY is not set/);
  });

  it("rejects a wrong-length key", () => {
    process.env.GITHUB_TOKEN_ENCRYPTION_KEY = Buffer.from("too-short").toString("base64");
    expect(isTokenCipherConfigured()).toBe(false);
