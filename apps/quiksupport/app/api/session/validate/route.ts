...
/**
 * GET /api/session/validate — live session/access check for the client
 * SessionGuard. Mirrors quiktrack/quikscale: verifies the session, the active
 * org membership, org suspension, and the org→app entitlement (with per-user
 * UserAppAccess for non-admins). Returns { valid, reason? }.
 */
const QUIKSUPPORT_APP_SLUG = "quiksupport";
...
export async function GET(_request: NextRequest) {
  ...
  // Org status + active membership in one query so a super-admin org suspension
  // is caught here too (otherwise a suspended org's users keep using the app).
  ...
  // Org-level app entitlement (mirrors the launcher + getOrgId gate). Org
  // admins / super admins pass on org-level access alone; non-admin members
  // additionally need an explicit UserAppAccess row.
  const app = await db.app.findUnique({
    where: { slug: QUIKSUPPORT_APP_SLUG },
    select: { id: true },
  });
  if (app) {
