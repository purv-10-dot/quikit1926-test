...
/**
 * Per-user additive grants (QspUserPermissionExtra). Effective permissions =
 * role grants ∪ extras. Extras can only ADD — to revoke role grants for a
 * single user, change their role. Mirrors quiktrack's route.
 */
const pairSchema = z.object({ resource: z.string(), action: z.string() });
const putBodySchema = z.object({ extras: z.array(pairSchema) });

// GET — role grants + extras + effective merged set.
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!appId) {
      return NextResponse.json({ success: false, error: "QuikSupport app not registered" }, { status: 500 });
    }
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await db.$transaction([
      db.qspUserPermissionExtra.deleteMany({ where: { userId: params.id, orgId } }),
      ...(desired.length > 0
        ? [
            db.qspUserPermissionExtra.createMany({
              data: desired.map((p) => ({
                userId: params.id,
                orgId,
                resource: p.resource,
    ...
    return NextResponse.json({ success: true, data: { userId: params.id, count: desired.length } });
  } catch (error: unknown) {
