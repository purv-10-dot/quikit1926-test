...
const patchSchema = z.object({
  ...
  /** New password — replaces the bcrypt hash. ≥ 8 chars. */
  password: z.string().min(8).max(128).optional(),
  /** Membership status flip. */
  status: z.enum(["active", "inactive"]).optional(),
});

// GET /api/org/users/[id] — single member's editable state (prefill Edit drawer).
  ...
  try {
    ...
    const membership = await db.orgMember.findUnique({
      ...
      select: {
        status: true,
        user: {
          select: { id: true, firstName: true, lastName: true, email: true, lastSignInAt: true },
        },
      },
    });
    if (!membership) {
    ...
    return NextResponse.json({
      success: true,
      data: { ...membership.user, status: membership.status },
    });
  } catch (error: unknown) {
...
// PATCH /api/org/users/[id] — update platform User fields + OrgMember.status.
// Email is intentionally NOT editable (login identifier / audit anchor).
