...
/**
 * Client-side permission gate hook for QuikSupport — mirrors quiktrack.
 *
 ...
 *
 * This reads the standard QuikIT (Qsp*) permission layer. The helpdesk UI's
 * primary in-app gate remains `HdUser.role`; use this hook for cross-app
 * consistency and for surfaces that should respect the standard RBAC.
 */
...
export function useMyPermissions(): MyPermissionsApi {
  const { data, isLoading } = useQuery({
    queryKey: ["quiksupport", "me-permissions"],
