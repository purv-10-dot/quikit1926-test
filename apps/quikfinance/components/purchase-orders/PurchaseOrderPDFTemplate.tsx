...
import type { PurchaseOrderPdfData } from "@/lib/purchase-order-pdf";
...
export function PurchaseOrderPDFTemplate({ order }: { order: PurchaseOrderPdfData }) {
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          ...
          <View style={{ alignItems: "flex-end", maxWidth: "38%" }}>
            <Text style={{ fontSize: 22, fontWeight: 700 }}>Purchase Order</Text>
            <Text>Purchase Order# {order.purchaseOrderNumber}</Text>
            {order.referenceNumber ? <Text>Reference: {order.referenceNumber}</Text> : null}
            <Text>Order Date: {order.issueDate}</Text>
            {order.expectedDeliveryDate ? <Text>Expected Delivery: {order.expectedDeliveryDate}</Text> : null}
        ...
        <View style={styles.card}>
          ...
          {order.vendorAddress.map((line) => <Text key={line} style={styles.muted}>{line}</Text>)}
          {order.vendorEmail ? <Text style={styles.muted}>Email: {order.vendorEmail}</Text> : null}
        </View>
