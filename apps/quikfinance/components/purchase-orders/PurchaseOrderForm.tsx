...
export function PurchaseOrderForm({ purchaseOrderId }: { purchaseOrderId?: string }) {
  ...
  const { data: vendors = [] } = useQuery({ queryKey: ["po-form-vendors"], staleTime: 0, queryFn: async () => (await fetchList("/api/v1/vendors?per_page=200")).map((r) => ({ id: String(r.id), label: String(r.display_name ?? "Vendor") }) as Option) });
  const { data: warehouses = [] } = useQuery({ queryKey: ["po-form-warehouses"], queryFn: async () => (await fetchList("/api/v1/warehouses?per_page=200")).map((r) => ({ id: String(r.id), label: String(r.name ?? "") }) as Option) });
  const { data: items = [] } = useQuery({ queryKey: ["po-form-items"], staleTime: 0, queryFn: async () => (await fetchList("/api/v1/inventory?per_page=200")).map((r) => ({ id: String(r.id), name: String(r.name ?? ""), purchase_price: Number(r.purchase_price ?? 0) }) as Item) });
  const itemsById = useMemo(() => new Map(items.map((i) => [i.id, i])), [items]);

  // Prefill the next PO number (Zoho-style) for new purchase orders.
  const nextNum = useNextNumber("purchase-order", !isEdit);
  useEffect(() => { if (!isEdit && nextNum.preview) setPoNumber((cur) => cur || nextNum.preview || ""); }, [nextNum.preview, isEdit]);

  const { data: existing } = useQuery({
    queryKey: ["purchase-order", purchaseOrderId],
    queryFn: async () => {
      const r = await fetch(`/api/v1/purchase-orders/${purchaseOrderId}`);
      if (!r.ok) return null;
  ...
  useEffect(() => {
    ...
    setPoNumber(String(existing.purchase_order_number ?? ""));
    setReference(String(existing.reference_number ?? ""));
    setIssueDate(String(existing.issue_date ?? todayISO()).slice(0, 10));
    setDeliveryDate(existing.expected_delivery_date ? String(existing.expected_delivery_date).slice(0, 10) : "");
  ...
  const { clearDraft } = useFormDraft(
    "qf-draft:purchase-order",
    { contactId, warehouseId, poNumber, reference, issueDate, deliveryDate, paymentTerms, deliveryMethod, notes, terms, adjustment, lines },
    (d) => {
  ...
  const save = async (status: "draft" | "issued") => {
    ...
    const payload = {
      ...
      purchase_order_number: nextNum.numberToSubmit(poNumber),
    ...
    setSubmitting(true);
    try {
      const res = await fetch(isEdit ? `/api/v1/purchase-orders/${purchaseOrderId}` : "/api/v1/purchase-orders", {
        method: isEdit ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload)
      });
      if (!res.ok) {
        const body = (await res.json().catch(() => null)) as { error?: { message?: string } } | null;
        toast.error(body?.error?.message ?? "Could not save the purchase order.");
        return;
      }
      const saved = (await res.json()) as { data?: { id?: string } };
      toast.success(isEdit ? "Purchase order updated." : status === "issued" ? "Purchase order saved and issued." : "Purchase order saved as draft.");
      clearDraft();
      qc.invalidateQueries({ queryKey: ["module", "purchase-orders"] });
      const id = saved?.data?.id ?? purchaseOrderId;
  ...
  return (
    <div className="space-y-5 animate-fade-up">
      <div className="flex items-center justify-between">
        ...
        <Button asChild size="sm" variant="ghost"><a href="/purchase-orders" aria-label="Close"><X className="h-4 w-4" /></a></Button>
      </div>

      <Card><CardContent className="grid gap-5 pt-6 md:grid-cols-2">
        <div>
          <Label className="text-destructive">Vendor Name*</Label>
          <Combobox className="mt-1" value={contactId} onChange={setContactId} placeholder="Select or add a vendor" searchPlaceholder="Search vendors…" createHref="/vendors/new" createLabel="New Vendor" options={vendors.map((v) => ({ value: v.id, label: v.label }))} />
        </div>
        <div>
        ...
        <div>
          <Label className="text-destructive">Purchase Order#</Label>
          <Input className="mt-1" value={poNumber} onChange={(e) => setPoNumber(e.target.value)} placeholder={nextNum.autoGenerate ? "Auto-generated (e.g. PO-00001)" : "Enter a purchase order number"} />
        </div>
        <div><Label>Reference#</Label><Input className="mt-1" value={reference} onChange={(e) => setReference(e.target.value)} /></div>
        <div><Label className="text-destructive">Purchase Order Date*</Label><Input type="date" className="mt-1" value={issueDate} onChange={(e) => setIssueDate(e.target.value)} /></div>
        <div><Label>Expected Delivery Date</Label><Input type="date" className="mt-1" value={deliveryDate} onChange={(e) => setDeliveryDate(e.target.value)} /></div>
        <div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
        <Card><CardContent className="space-y-4 pt-6">
          <div><Label>Notes</Label><Textarea className="mt-1" rows={3} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Enter any notes to be displayed in your transaction" /></div>
      ...
      <div className="flex items-center gap-2">
        ...
        <Button type="button" onClick={() => save("issued")} disabled={submitting}>Save and Issue</Button>
        <Button type="button" variant="ghost" onClick={() => router.push("/purchase-orders")}>Cancel</Button>
      </div>
    </div>
  );
}
