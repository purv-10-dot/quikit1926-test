...
import type { InvoiceSettings as Settings } from "@/lib/settings/invoice";
...
export function InvoiceSettings() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const r = await fetch("/api/v1/settings/invoices");
  ...
  const save = async () => {
    setSaving(true);
    try {
      const r = await fetch("/api/v1/settings/invoices", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
  ...
  return (
    <div className="space-y-5 animate-fade-up">
      <PageHeader title="Invoices — Numbering" description="Control how invoice numbers are generated." />
      <Card><CardContent className="space-y-6 pt-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-base font-semibold">Auto-generate invoice numbers</p>
            <p className="text-sm text-muted-foreground">When enabled, a number is assigned automatically if you leave the Invoice# field blank. When disabled, you must enter one for each invoice.</p>
          </div>
        ...
        {form.auto_generate_number ? (
          <div className="grid gap-5 border-t pt-5 sm:grid-cols-2">
            ...
            <p className="text-sm text-muted-foreground sm:col-span-2">Next invoice will be at least <span className="font-medium text-foreground">{nextPreview}</span> (the series never reuses an existing number).</p>
          </div>
        ) : null}
      </CardContent></Card>
