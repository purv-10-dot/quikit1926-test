...
import type { CreditNotePdfData } from "@/lib/credit-note-pdf";
...
export function CreditNotePDFTemplate({ note }: { note: CreditNotePdfData }) {
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          ...
          <View style={{ alignItems: "flex-end", maxWidth: "40%" }}>
            <Text style={{ fontSize: 20, fontWeight: 700 }}>Credit Note</Text>
            <Text>Credit Note# {note.creditNoteNumber}</Text>
            {note.referenceNumber ? <Text>Reference: {note.referenceNumber}</Text> : null}
            <Text>Credit Date: {note.issueDate}</Text>
          </View>
        </View>

        <View style={styles.card}>
          <Text style={styles.sectionTitle}>Bill To</Text>
          <Text>{note.customerName}</Text>
          {note.customerAddress.map((line) => <Text key={line} style={styles.muted}>{line}</Text>)}
        </View>
        ...
        <View style={[styles.row, { justifyContent: "flex-end" }]}>
          <View style={{ width: "45%" }}>
            ...
            {note.adjustment ? <View style={styles.totalRow}><Text style={styles.muted}>Adjustment</Text><Text>{money(note.adjustment)}</Text></View> : null}
