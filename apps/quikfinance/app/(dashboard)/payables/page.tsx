...
export default function PayablesPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["payables", bucket],
    queryFn: async () => {
      const url = bucket === "all" ? "/api/v1/payables?limit=100" : `/api/v1/payables?bucket=${bucket}&limit=100`;
  ...
  const rows = (data?.data ?? []).filter((r: Record<string, unknown>) => {
    ...
    const num = String(r.bill_number ?? "").toLowerCase();
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      <PageHeader
        title={t("nav.items.Payables", "Payables")}
        description="Track vendor balances, scheduled payments, and AP aging."
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent><p className="text-2xl font-bold">{fmt(summary.total_payable ?? 0)}</p></CardContent>
        </Card>
        <Card className="border-red-200 bg-red-50 dark:bg-red-950/10">
          <CardHeader className="pb-2"><CardTitle className="text-sm text-red-600">Overdue Bills</CardTitle></CardHeader>
          <CardContent><p className="text-2xl font-bold text-red-600">{summary.overdue_count ?? 0} bills</p></CardContent>
        </Card>
        <Card className="border-amber-200 bg-amber-50 dark:bg-amber-950/10">
          <CardHeader className="pb-2"><CardTitle className="text-sm text-amber-600">Due This Week</CardTitle></CardHeader>
          <CardContent><p className="text-2xl font-bold text-amber-600">{summary.due_soon_count ?? 0} bills</p></CardContent>
        </Card>
      </div>

      <div className="flex flex-wrap items-center gap-2">
        {(["all", "overdue", "due_soon", "outstanding"] as Bucket[]).map((b) => (
          <Button key={b} size="sm" variant={bucket === b ? "default" : "outline"} onClick={() => setBucket(b)}>
            {b === "all" ? "All" : b === "overdue" ? "Overdue" : b === "due_soon" ? "Due This Week" : "Not Yet Due"}
          </Button>
        ))}
        <Input
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y">
            ...
            {!isLoading && rows.length === 0 && (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-muted-foreground">No outstanding payables.</td></tr>
            )}
              ...
              return (
                <tr key={String(r.id)} className="hover:bg-muted/30 transition-colors">
                  <td className="px-4 py-3">
                    <Link href={`/bills/${r.id}`} className="font-medium text-primary hover:underline">
