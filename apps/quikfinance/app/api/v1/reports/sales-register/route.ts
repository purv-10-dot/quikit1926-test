...
export async function GET(req: NextRequest) {
  ...
  try {
    let query = db
      .from("invoices")
      .select(
        `id, invoice_number, issue_date, due_date, subtotal, tax_total, discount_total, total, balance_due, status, currency, place_of_supply,
    ...
    const summary = {
      ...
      total_invoiced: rows.reduce((s, r) => s + Number(r.total ?? 0), 0),
