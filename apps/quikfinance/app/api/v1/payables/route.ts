...
export async function GET(req: NextRequest) {
  ...
  try {
    let query = db
      .from("bills")
      .select(
        `id, bill_number, issue_date, due_date, total, balance_due, status, currency,
    ...
    const today = new Date().toISOString().split("T")[0];
    const rows = (data ?? []).map((bill: Record<string, unknown>) => {
      const due = bill.due_date as string;
      ...
      const agingBucket =
        ...
        : "outstanding";
      return { ...bill, days_overdue: daysOverdue, aging_bucket: agingBucket };
    });

    const summary = {
      total_payable: rows.reduce((s: number, r) => s + Number((r as Record<string, unknown>).balance_due ?? 0), 0),
