...
import { GET, PUT, DELETE } from "@/app/api/masters/cost-centers/[id]/route";
...
function req(method: string, body?: unknown): NextRequest {
  return new NextRequest(`http://localhost/api/masters/cost-centers/${ID}`, {
    method,
    ...(body !== undefined
...
describe("GET /api/masters/cost-centers/[id]", () => {
  ...
  it("returns 404 when the row is not found in this org", async () => {
    ...
    db.cnCostCenter.findFirst.mockResolvedValue(null);
  ...
  it("returns the row scoped to the org", async () => {
    setContext(makeAdminCtx());
    db.cnCostCenter.findFirst.mockResolvedValue({
      id: ID,
      orgId: TEST_TENANT,
      code: "CC-001",
      name: "Ops",
    ...
    expect(db.cnCostCenter.findFirst.mock.calls[0][0].where).toMatchObject({
      id: ID,
      orgId: TEST_TENANT,
    });
  });
...
describe("PUT /api/masters/cost-centers/[id]", () => {
  ...
  it("updates and stamps updatedBy", async () => {
    setContext(makeAdminCtx());
    db.cnCostCenter.findFirst.mockResolvedValue({ id: ID });
    db.cnCostCenter.update.mockResolvedValue({
      id: ID,
      orgId: TEST_TENANT,
      code: "CC-001",
      name: "Renamed",
    ...
    expect(res.status).toBe(200);
    expect(db.cnCostCenter.update.mock.calls[0][0].data.updatedBy).toBe(TEST_USER);
  });
});
...
describe("DELETE /api/masters/cost-centers/[id]", () => {
  ...
  it("soft-deletes (status=deleted) within the org and returns success", async () => {
    setContext(makeAdminCtx());
    db.cnCostCenter.updateMany.mockResolvedValue({ count: 1 });
    const res = await DELETE(req("DELETE"), params);
    expect(res.status).toBe(200);
    expect((await res.json()).success).toBe(true);
    const call = db.cnCostCenter.updateMany.mock.calls[0][0];
  ...
  it("returns 404 when nothing was deleted", async () => {
    setContext(makeAdminCtx());
    db.cnCostCenter.updateMany.mockResolvedValue({ count: 0 });
