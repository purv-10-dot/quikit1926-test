 ...
 *
 * Run with:  pnpm --filter @quikit/quikinfra prisma db seed
 * or:        npx ts-node prisma/seed.ts  (from apps/quikinfra)
 *
...
const SUPERADMIN_ROLE_KEY = process.env.SUPERADMIN_ROLE_KEY ?? "super_admin";
...
async function main() {
  ...
  // First, delete any system roles that are no longer in the catalog so the
  // table converges to exactly the current ROLE_DEFINITIONS set. Custom
  // roles (isSystem=false) are preserved.
  const currentRoleKeys = RBAC_ROLES.map((r) => r.key);
  const orphaned = await db.cnRole.deleteMany({
    where: { orgId: O, isSystem: true, key: { notIn: currentRoleKeys } },
  });
  if (orphaned.count > 0) {
    console.log(`  · removed ${orphaned.count} orphaned system role(s)`);
  }

  console.log(`  · roles (${RBAC_ROLES.length})`);
  for (const role of RBAC_ROLES) {
    const created = await db.cnRole.upsert({
      where: { orgId_key: { orgId: O, key: role.key } },
      create: {
        orgId: O,
        key: role.key,
        name: role.name,
