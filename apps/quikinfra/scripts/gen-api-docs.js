...
children.push(new Paragraph({
  ...
  children: [new TextRun({ text: "QuikInfra", bold: true, size: 56, color: COLOR.accent, font: "Arial" })],
}));
children.push(new Paragraph({
  alignment: AlignmentType.CENTER,
  spacing: { after: 240 },
...
children.push(p("This document lists every HTTP endpoint exposed by the QuikInfra backend (Next.js App Router under app/api), together with the PostgreSQL connection string used in local development. Every endpoint is prefixed with /api and is called relative to the base URL of the running server."));
...
children.push(new Paragraph({
  ...
  children: [new TextRun({ text: "postgresql://postgres:postgres@127.0.0.1:5432/quikinfra?schema=public", font: "Consolas", size: 22 })],
}));
...
children.push(bullet("Start a local Postgres: docker run -d -p 5432:5432 -e POSTGRES_PASSWORD=postgres --name quikinfra-pg postgres"));
...
const doc = new Document({
  creator: "QuikInfra",
  title: "QuikInfra API & Database Reference",
  styles: {
    ...
    headers: {
      default: new Header({
        children: [new Paragraph({
          ...
          children: [new TextRun({ text: "QuikInfra  ·  API & DB Reference", color: COLOR.muted, size: 18, font: "Arial" })],
        })],
      }),
    },
    footers: {
...
const outPath = path.join(__dirname, "..", "QuikInfra_API_Reference.docx");
