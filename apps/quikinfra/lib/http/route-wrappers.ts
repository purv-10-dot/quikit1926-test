...
import { getTenantContext, hasMatrixAction, hasPermission, type TenantContext } from "@/lib/auth/context";
 ...
 *       const baseOpts = { orgId: ctx.orgId, orgId: ctx.orgId };
 *       return paginateDb(
 *         pagination,
...
export interface MutationRouteContext<TBody = unknown> {
  ctx: TenantContext;
  req: NextRequest;
  body: TBody;
  searchParams: URLSearchParams;
...
export interface MutationRouteOptions<TBody = unknown> {
   ...
   * omitted, `body` is whatever JSON the client sent (typed as `unknown`).
   */
  ...
  /**
   * Optional permission check. The wrapper rejects with 403 before the
   * handler runs if the user lacks this permission.
   */
  requirePermission?: string;
  /**
   * Optional per-user matrix check (closes the UI-only enforcement gap).
   * The wrapper rejects with 403 if the saved permission matrix denies
   * this action on this menu page. Pairs naturally with `requirePermission`.
   */
  requireMatrix?: { menuKey: string; action: MatrixAction };
}
 ...
 *       },
 *     }, async ({ ctx, body }) => {
 *       return createCompany({ ...body, orgId: ctx.orgId, orgId: ctx.orgId, createdBy: ctx.userId });
 *     });
 */
export async function withMutationRoute<TBody = unknown, TResult = unknown>(
  req: NextRequest,
  ...
  return safeRun(async () => {
    ...
    if (opts.requirePermission && !hasPermission(ctx, opts.requirePermission)) {
      return envelopeErr("FORBIDDEN", `Missing permission: ${opts.requirePermission}`, 403);
    }
    if (
      opts.requireMatrix &&
      !hasMatrixAction(ctx, opts.requireMatrix.menuKey, opts.requireMatrix.action)
    ) {
      return envelopeErr(
        "FORBIDDEN",
        `Action "${opts.requireMatrix.action}" not allowed for ${opts.requireMatrix.menuKey}`,
        403,
      );
    }
