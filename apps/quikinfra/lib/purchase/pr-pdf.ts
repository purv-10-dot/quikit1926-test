/**
 * Purchase Requisition PDF generator.
 *
 * Sibling of `grn-pdf.ts` / `po-pdf.ts` — clones the Aakar letterhead
 * and draws the requisition body: PR + project info blocks and the
 * line-items table. Beyond the usual qty/rate/amount, the table carries
 * two procurement-tracking columns — PO STATUS ("have we ordered this?")
 * and GRN STATUS ("has it arrived?") — resolved upstream from the
 * PR → indent → PO → GRN chain.
 */
...
function drawInfoBlocks(ctx: Ctx, input: PrPdfInput) {
  ...
  const leftRows: Array<[string, string]> = [
    ["PR No.", input.pr.prNumber || "—"],
    ["PR Date", fmtDate(input.pr.prDate) || "—"],
    ["Required By", fmtDate(input.pr.requiredDate) || "—"],
    ["Requested By", input.pr.requestedByName || "—"],
  ];
  const rightRows: Array<[string, string]> = [
    ["Project", input.pr.projectName || "—"],
    ["Status", (input.pr.status || "—").replace(/_/g, " ")],
    ["Purpose", input.pr.purpose || "—"],
  ];
...
function drawTableRow(ctx: Ctx, startX: number, index: number, line: PrPdfLine) {
...
export async function generatePrPdf(input: PrPdfInput): Promise<Buffer> {
  ...
  const titleText = "Purchase Requisition";
  const titleSize = 11;
