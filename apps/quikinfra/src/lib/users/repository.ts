/**
 * User repository — Prisma-backed CRUD for `cn_users`.
 *
 * Call sites:
...
const CLIENT_HAS_MOBILE = CN_USER_FIELDS.has("mobile");
const CLIENT_HAS_PERMISSION_MATRIX = CN_USER_FIELDS.has("permissionMatrix");
...
export async function listUsers(orgId: string, search = ""): Promise<UserRecord[]> {
  // Returns every user for the org — including soft-deleted
  // (`status: "inactive"`) rows. The User Management page handles its
  // own Active / Inactive / All switcher client-side, so excluding
  // inactive rows here would leave the admin no way to find and
...
export async function findByTenantAndUsername(
  ...
  const row = await withMobileFallback((select) =>
    (db as any).cnUser.findUnique({
      where: { orgId_username: { orgId, username } },
      select,
    }),
  );
  return row ? toUserRecord(row) : null;
...
export async function findUserById(
  ...
  try {
    const row = await withMobileFallback((select) =>
      (db as any).cnUser.findFirst({ where: { id: userId, orgId }, select }),
    );
    return row ? toUserRecord(row) : null;
  } catch (err: any) {
 ...
 * Immutable fields (id, orgId, email, username, passwordHash,
...
export async function updateUser(
  ...
  try {
    ...
    if (missingMobile || missingMatrix) {
      ...
      if (Object.keys(retryData).length === 0) return findUserById(orgId, userId);
...
/**
 * Mark the last-login timestamp. Called from NextAuth's authorize() on every
 * successful login. Also auto-accepts the invite on first login.
 *
 * Auto-accept rationale: the invite email's CTA points at /login (not at a
 * dedicated accept page), so logging in with the temp password IS the
 * acceptance step in practice. We therefore stamp `acceptedAt` and clear the
 * one-time token on the first login. The conditional `updateMany` skips this
 * on every subsequent login — once `acceptedAt` is non-null we leave it
 * alone, so the timestamp records the actual moment of acceptance.
 */
export async function touchLastLogin(userId: string): Promise<void> {
  ...
  try {
    ...
    await (db as any).cnUser.updateMany({
      where: { id: userId, acceptedAt: null },
      data: {
        acceptedAt: now,
        inviteToken: null,
