...
export async function resolveApproverChain(
  ...
  const { orgId, requesterId, projectId, module } = opts;
  if (!orgId) return [];

  // Run all three lookups in parallel — each is an independent DB round-trip
  // but they share the same orgId predicate so the scheduler can coalesce.
  const [siteAdmins, hoUsers, admins] = await Promise.all([
    ...
    (db as any).cnUser.findMany({
      where: { orgId, userType: USER_TYPES.ADMIN, status: "active" },
      select: APPROVER_SELECT,
      orderBy: { fullName: "asc" },
    }),
  ]);
 ...
 * Returns `null` only when the org has zero admins, which should
 * never happen in a seeded org.
 */
