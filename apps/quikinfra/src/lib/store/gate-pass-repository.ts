/**
 * Gate Pass repository — Postgres-backed CRUD for `cn_gate_passes`.
 *
...
export async function createGatePass(
  ...
  await (db as any).$executeRaw`
    INSERT INTO app_quikinfra."Gate_passes" (
      ...
      ${id}, ${input.orgId}, ${input.gatePassNumber}, ${input.type},
  ...
  return (await findGatePassById(input.orgId, id))!;
}
...
export async function updateGatePass(
  ...
  return findGatePassById(input.orgId, id);
}
...
export async function patchGatePassStatus(
  ...
  await (db as any).$executeRaw`
    ...
    WHERE "orgId" = ${orgId} AND id = ${id}
  `;
  return findGatePassById(orgId, id);
}
