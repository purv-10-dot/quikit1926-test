...
export const PERMISSION_CATALOG: PermissionDescriptor[] = [
  ...
  // WBS
  { key: PERMISSIONS.WBS_READ, name: "View WBS / Planning", module: "project_mgmt" },
  { key: PERMISSIONS.WBS_WRITE, name: "Create/edit WBS / Planning", module: "project_mgmt" },
  // Purchase
...
const PERMISSION_TO_ASSIGNABLE_MODULE: Record<string, string> = (() => {
  const map: Record<string, string> = {};
  for (const p of PERMISSION_CATALOG) {
    const m = p.module;
    if (m === "boq" || m === "dpr" || m === "rab" || m === "wo" || m === "project_mgmt") {
      map[p.key] = "project_mgmt";
    } else if (m === "quality" || m === "safety") {
      map[p.key] = "quality_safety";
    } else if (m === "purchase" || m === "store" || m === "masters") {
      map[p.key] = m;
    }
  }
  return map;
...
export function filterPermissionsByModules(
  ...
  if (permissions.has("*")) return permissions;
  const assigned = new Set(modulesAssigned);
  const out = new Set<string>();
  permissions.forEach((p) => {
    const required = PERMISSION_TO_ASSIGNABLE_MODULE[p];
    // Cross-cutting (no module mapping) → keep.
    // In assigned module → keep.
    // Read-only permission → keep regardless of module (cross-module lookups
    //   like a PR linking to BOQ items need this).
    // Everything else (writes / approvals in unassigned modules) → drop.
    if (!required || assigned.has(required) || p.endsWith(".read")) {
      out.add(p);
    }
  });
  return out;
