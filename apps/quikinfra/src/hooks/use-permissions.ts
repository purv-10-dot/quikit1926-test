...
export interface MeResponse {
  ...
  orgId: string;
  roleKey: string;
  /** PDF-spec user type from cn_users.userType — drives the sidebar chip. */
...
export function usePermissions() {
  ...
  function hasRole(roleKeys: string | string[]): boolean {
    ...
    if (actual === "super_admin" || actual === "admin") return true;
   ...
   *
   * When a matrix IS present, a missing row means DENY: the /api/me
   * response only includes granted rows (see `stripDeniedRows`), so
   * any catalog key absent from the matrix is treated as fully denied.
   */
   ...
   *
   * Semantics: super admin → true; no matrix at all → true; URL not in
   * the menu catalog → true. When a matrix IS present and the row for
   * this page is missing, return false — the server strips denied rows
   * so an absent row is intentional denial, not "not yet configured".
   */
...
/**
 * Ergonomic wrapper around `canMenuAction` for list pages. Pass the page's
 * sidebar URL (e.g. "/purchase/indents") and receive the four action flags
 * derived from the user's permission matrix. Until the `/api/me` query
 * resolves, all flags are `false` — pages should gate buttons on these so
 * they don't briefly flash a "+ New" CTA the user can't actually use.
 *
 * Super admins and users without any saved matrix get `true` for every
 * action (mirrors `canMenuAction`'s fall-through semantics).
 */
export function useMenuActions(url: string | undefined) {
  const { canMenuAction, isLoading } = usePermissions();
  if (isLoading) {
    return { canAdd: false, canEdit: false, canDelete: false, canView: false };
  }
  return {
    canAdd: canMenuAction(url, "add"),
    canEdit: canMenuAction(url, "edit"),
    canDelete: canMenuAction(url, "delete"),
    canView: canMenuAction(url, "view"),
  };
}
