...
async function handleUpdate(req: NextRequest, id: string) {
  ...
  if (!ctx.permissions.has("construction.master_workman.edit") && !ctx.permissions.has("*")) {
    return envelopeErr("FORBIDDEN", "Missing permission: construction.master_workman.edit", 403);
  }
...
export async function DELETE(
  ...
  const ctxOrResp = await requireMastersAction("construction.master_workman", "delete");
  if (ctxOrResp instanceof NextResponse) return ctxOrResp;
  const ctx = ctxOrResp;
