...
import {
  ...
  updateAsset,
  deleteAsset,
} from "@/lib/masters/assets-repository";

export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  const ctxOrResp = await requireMastersAction("construction.master_asset", "view");
  if (ctxOrResp instanceof NextResponse) return ctxOrResp;
  const ctx = ctxOrResp;
  const row = await findAssetById(ctx.orgId, params.id);
  if (!row) return NextResponse.json({ error: "Asset not found" }, { status: 404 });
  return NextResponse.json(row);
}

async function handleUpdate(req: NextRequest, id: string) {
  const ctxOrResp = await requireMastersAction("construction.master_asset", "edit");
  if (ctxOrResp instanceof NextResponse) return ctxOrResp;
  const ctx = ctxOrResp;
  if (!hasMatrixAction(ctx, "master.asset", "edit")) {
    return envelopeErr("FORBIDDEN", `Action "edit" not allowed for master.asset`, 403);
  }
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  const ctxOrResp = await requireMastersAction("construction.master_asset", "delete");
  if (ctxOrResp instanceof NextResponse) return ctxOrResp;
  const ctx = ctxOrResp;
  if (!hasMatrixAction(ctx, "master.asset", "delete")) {
    return envelopeErr("FORBIDDEN", `Action "delete" not allowed for master.asset`, 403);
  }
  const ok = await deleteAsset(ctx.orgId, params.id, ctx.userId);
