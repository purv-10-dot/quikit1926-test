 ...
 *   Create a new user account, persist it in `cn_users`, and send the
...
export async function GET(req: NextRequest) {
  ...
  const data = await listUsers(ctx.orgId, search);
...
export async function POST(req: NextRequest) {
  ...
  const existingByEmail = await findByTenantAndEmail(ctx.orgId, email);
  if (existingByEmail) {
    return NextResponse.json(
  ...
  const existingByUsername = await findByTenantAndUsername(ctx.orgId, body.username);
  ...
  // Build the login URL (prefills email via ?email=) and fire the
  // invite email. Failures here are non-fatal — the user row is already
  // in the DB and the admin can hit "Resend Invite" to retry.
  ...
  try {
    const { subject, html } = buildUserInviteEmail({
      ...
      loginEmail: email,
      tempPassword,
      invitedByName: record.invitedByName ?? "QuikInfra Admin",
