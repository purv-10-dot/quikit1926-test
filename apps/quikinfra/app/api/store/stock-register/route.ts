 ...
 *
 * By default the register lists only items that have appeared on at
 * least one stock-relevant document (PO line / GRN / MI / GR / ST).
 * Pass `includeAllItems=true` (Stock Valuation report) to union the
 * full items catalogue for the tenant/org so every master row appears
 * with computed qty/value (zero when there is no movement).
 *
...
export async function GET(req: NextRequest) {
  ...
  // ─── GRN receipts (INWARD) ─────────────────────────────────────
  // Real `cn_goods_receipt_notes` + `cn_grn_lines`. Only count rows
  // tied to the current org. Status filter keeps out drafts so
  ...
  const grnRows: any[] = await (db as any).$queryRawUnsafe(
    `SELECT l."itemId"                            AS "itemId",
     ...
     FROM app_quikinfra."Grn_lines" l
     JOIN app_quikinfra."Goods_receipt_notes" g ON g.id = l."grnId"
  ...
  const poRows: any[] = await (db as any).$queryRawUnsafe(
    `SELECT l."itemId"                                    AS "itemId",
     ...
     FROM app_quikinfra."Purchase_order_lines" l
     JOIN app_quikinfra."Purchase_orders" po ON po.id = l."poId"
  ...
  // in bulk and aggregate in JS. Filter by org + (optional) project
  // so scoped views match the IN side.
  const miRowsQuery = `
    SELECT mi."projectId", mi.materials
    FROM app_quikinfra."Material_issues" mi
  ...
  const grRowsQuery = `
    ...
    FROM app_quikinfra."Good_returns" gr
  ...
  //
  // Items master lives in Postgres (`cn_items`) — POs reference real
  // CnItem rows, not the demo-store seed. We pull names from
  // Postgres, joined with `cn_item_groups` for the group name and
  // `cn_uoms` for the UOM code, then fall back to the demo-store
  ...
  if (includeAllItems && ctx) {
    ...
    for (const r of catalogRows) seenItemIds.add(r.id);
  }
    ...
    const dbItems: any[] = ctx
      ? await (db as any).$queryRawUnsafe(
          `SELECT i.id, i.code, i.name,
                  ...
                  g.name AS "groupName",
                  u.code AS "uomCode"
           FROM app_quikinfra."Items" i
           LEFT JOIN app_quikinfra."Item_groups" g ON g.id = i."groupId"
           LEFT JOIN app_quikinfra."Uoms" u        ON u.id = i."uomId"
