...
export function BoqFormPanel({ open, onClose, onSaved }: Props) {
  ...
  async function save() {
    ...
    try {
      const body = {
        boqNumber: header.boqNumber,
        projectId: header.projectId,
        boqDate: header.boqDate,
      ...
      const res = await fetch("/api/projects/boq", {
        method: "POST",
  ...
  const headerValid = header.boqNumber && header.projectId && header.boqDate;
      ...
      <div className="space-y-5">
        ...
        <FormSection title="Header">
          <FormRow cols={2}>
            ...
            <Field label="BOQ Date" required><Input value={header.boqDate} onChange={(e) => setHeader({ ...header, boqDate: e.target.value })} placeholder="YYYY-MM-DD" /></Field>
          </FormRow>
          <FormRow cols={2}>
                    ...
                    <tr key={idx} className={`border-b border-gray-100 align-top ${isGroup ? "bg-accent-50/30" : ""}`}>
                      <td className="py-1.5 pr-2">
                        <Select size="compact" value={l.kind} onChange={(e) => updateLine(idx, { kind: e.target.value as "item" | "group" })}
                          options={[{ value: "item", label: "Item" }, { value: "group", label: "Group" }]} />
                      </td>
                      <td className="py-1.5 pr-2">
                        <Input size="compact" value={l.description} onChange={(e) => updateLine(idx, { description: e.target.value })} />
                      </td>
                      <td className="py-1.5 pr-2">
                        {isGroup ? <span className="text-gray-300">—</span> : (
                          <Select size="compact" value={l.parentIdx == null ? "" : String(l.parentIdx)}
                            ...
                            options={groupOptionsForLine(idx)} />
                        )}
                      </td>
                      <td className="py-1.5 pr-2">
                        {isGroup ? <span className="text-gray-300">—</span> : (
                          <Select size="compact" value={l.itemId ?? ""}
                            ...
                            options={[{ value: "", label: "— custom —" }, ...items.map(i => ({ value: i.id, label: `${i.code} — ${i.name}` }))]} />
                        )}
                      </td>
                      <td className="py-1.5 pr-2">
                        {isGroup ? <span className="text-gray-300">—</span> : <NumberInput size="compact" value={l.quantity} min={0} onChange={(v) => updateLine(idx, { quantity: v })} className="text-right" />}
                      </td>
                      <td className="py-1.5 pr-2">
                        {isGroup ? <span className="text-gray-300">—</span> : (
                      ...
                      <td className="py-1.5 pr-2">
                        {isGroup ? <span className="text-gray-300">—</span> : <NumberInput size="compact" value={l.rate} min={0} onChange={(v) => updateLine(idx, { rate: v })} className="text-right" />}
                      </td>
                      <td className="py-1.5 pr-2">
                        {isGroup ? <span className="text-gray-300">—</span> : <NumberInput size="compact" value={l.gstRate} min={0} max={100} onChange={(v) => updateLine(idx, { gstRate: v })} className="text-right" />}
                      </td>
