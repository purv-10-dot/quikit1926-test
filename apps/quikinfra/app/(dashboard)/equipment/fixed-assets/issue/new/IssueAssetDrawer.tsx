...
/** IssueAssetDrawer — slide-over wrapper around `IssueAssetForm`. */
...
export function IssueAssetDrawer({
  open,
  onClose,
  onSaved,
}: {
      ...
      <div className={RIGHT_DRAWER_FRAME}>
        <div className={`${RIGHT_DRAWER_PANEL} max-w-3xl`}>
          <div className="flex shrink-0 items-center justify-between border-b border-gray-200 px-6 py-4 sm:px-8">
            <div>
              <h2 className="text-base font-semibold text-gray-900">Issue Asset</h2>
              <p className="text-xs text-gray-500 mt-0.5">
                Quantity validated against available; gate pass on returnable exit
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="min-h-0 flex-1">
            <IssueAssetForm embedded onSaved={onSaved} onCancel={onClose} />
          </div>
        </div>
      </div>
    </>
