...
type TabKey = "log-book" | "fuel-reconciliation";
...
export default function EquipmentLogBookPage() {
  ...
  const [tab, setTab] = useState<TabKey>("log-book");
  ...
  const { data: reconResult, isLoading: reconLoading, refetch: refetchRecon } =
    useFuelReconciliation(reconParams);

  const rows: LogRow[] = logsResult?.data ?? [];
  const reconRows = reconResult?.data ?? [];
  ...
  const reconColumns: ColDef<(typeof reconRows)[number]>[] = [
    {
      key: "equipment",
      label: "Equipment",
      render: (row) => (
    ...
    },
    {
      key: "runMeter",
      label: "Run (hr/km)",
      render: (row) => <span className="tabular-nums">{row.runMeter}</span>,
    },
    {
      key: "dieselLitres",
      label: "Diesel (L)",
      render: (row) => <span className="tabular-nums">{row.dieselLitres}</span>,
    },
    {
      key: "actualLPerUnit",
      label: "Actual L/unit",
      render: (row) => (
        <span className="tabular-nums">{row.actualLPerUnit ?? "—"}</span>
      ),
    },
    {
      key: "fuelNorm",
      label: "Norm",
      render: (row) => (
        <span className="tabular-nums">{row.fuelNorm ?? "—"}</span>
      ),
    },
    {
      key: "variancePct",
        ...
        row.variancePct != null ? (
          <span
            className={`tabular-nums ${
              row.variancePct < 0 ? "text-emerald-600" : row.variancePct > 10 ? "text-red-600" : ""
            }`}
          >
            {row.variancePct > 0 ? "+" : ""}
            {row.variancePct}%
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          canAdd ? (
            <PrimaryButton onClick={() => router.push("/equipment/log-book/new")}>
      ...
      <PageContainer>
        <div className="flex gap-6 border-b border-gray-200 mb-6">
          {(
            [
              { key: "log-book", label: "Log Book" },
              { key: "fuel-reconciliation", label: "Fuel Reconciliation" },
            ] as const
          ).map((t) => (
            <button
              key={t.key}
          ...
          <>
            <div className="flex flex-wrap items-end gap-3 mb-4">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 h-9 px-4 rounded-lg border border-gray-200 text-sm font-medium text-gray-700 hover:bg-gray-50"
              >
                <RefreshCw className="w-4 h-4" />
                Refresh
              </button>
              <p className="text-xs text-gray-400 ml-2">
                Diesel issued (existing Diesel Log) + metered run vs fuel norm.
                Read-only — never mutate stock.
              </p>
            </div>
            ...
            <DataTable
              ...
              emptyHint="Adjust project or date filters and refresh."
            />
          </>
        )}
      </PageContainer>
