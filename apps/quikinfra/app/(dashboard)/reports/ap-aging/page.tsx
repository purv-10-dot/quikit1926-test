...
interface Row { billId: string; billNumber: string; vendor: string; daysOverdue: number; bucket: string; outstanding: number; }
...
export default function ApAging() {
  ...
  useEffect(() => { fetch("/api/reports/ap-aging").then(r => r.json()).then(j => j.success && setData(j.data)); }, []);
    ...
    <div className="p-6 max-w-6xl">
      <div className="flex items-center justify-between mb-3">
        ...
        <ExportButton
          ...
          rows={data.rows}
          columns={[
            { header: "Bill #", get: (r) => r.billNumber },
      ...
      <h1 className="text-lg font-semibold text-gray-900 mb-4">AP Aging</h1>
      ...
      <div className="rounded-lg border border-gray-200 bg-white overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>{data.rows.map(r => (
            <tr key={r.billId} className="border-t border-gray-100">
              <td className="px-3 py-2 font-mono text-xs">{r.billNumber}</td>
              <td className="px-3 py-2 text-gray-700">{r.vendor}</td>
