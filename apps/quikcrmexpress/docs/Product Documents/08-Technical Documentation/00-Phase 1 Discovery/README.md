...
> **Subject:** `apps/crmexpress` (product slug `quikcrmexpress`, UI brand "QuikCRM") inside the QuikIT monorepo, plus the platform surfaces it depends on.
> **Method:** static source, schema, configuration and CI analysis. **The application was not executed** and no database was inspected. One attempt was made to run the CrmExpress unit-test suite locally; it failed to boot (see [testing-architecture.md](discovery/testing-architecture.md)).
...
| `SQ-4` — do document links cascade? | `QceDocumentLink.refType`/`refId` are bare strings, no FK. Confirmed. See [data-architecture.md §7](discovery/data-architecture.md) |
...
| Document | Contents |
|---|---|
| **[technical-discovery.md](discovery/technical-discovery.md)** | Master document. Executive architecture model, technology stack, system classification, and the mandated **`# Quikit Platform ↔ CrmExpress Architecture`** section. |
...
| [crmexpress-quikcrm-merge-considerations.md](discovery/crmexpress-quikcrm-merge-considerations.md) | Dependency/risk discovery for a possible CrmExpress → QuikCRM consolidation. **Not a migration plan.** |
...
- Does not recommend whether CrmExpress and QuikCRM should be merged.
