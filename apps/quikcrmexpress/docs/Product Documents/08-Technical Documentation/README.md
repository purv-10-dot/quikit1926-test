# CrmExpress — Technical Architecture Documentation

> **Programme:** CrmExpress Technical Documentation · **Phase 2 — Formal Current-State Architecture**
> **Subject:** `apps/crmexpress` — platform slug `quikcrmexpress`, UI brand "QuikCRM", dev port 3017 — inside the QuikIT monorepo, plus the platform surfaces it depends on.
> **Baseline:** branch `feature/crmexpress-standalone-app`, commit `7e372adc3`, re-verified against source on 2026-08-10.
...
This is an engineering-grade description of **the architecture CrmExpress has today**: how it is built, how it interacts with the QuikIT platform, and what its real boundaries, dependencies and constraints are.

It answers one question:

> *What is the technical architecture of CrmExpress today, how does it interact with the Quikit platform, and what are the important architectural boundaries, dependencies and constraints?*

It deliberately does **not** answer "what should it be", "should CrmExpress and QuikCRM merge", or "how do we fix X". Those are later-phase questions. Where this set names a risk, it names the next *investigation*, not a solution.
...
- CrmExpress reads a **fourth platform-owned table** (`public.SessionEvent`) that Phase 1 did not record — and **nothing writes the rows it queries for**, so the dashboard metric built on it is structurally always empty.
- CrmExpress **writes `auth.User`**, not only `quikit.UserAppAccess`. Its write surface outside its own schema is larger than Phase 1 stated.
- `@quikit/redis` **is** imported (Phase 1 said it was not), which means two independent Redis client factories run in one process and the SSO replay guard is governed by the platform package rather than by CrmExpress's own Redis configuration.
...
| [`Architecture/component-architecture.md`](Architecture/component-architecture.md) | Component inventory — platform components, CrmExpress route groups, service domains, cross-cutting modules |
...
| [`Architecture/quikit-platform-architecture.md`](Architecture/quikit-platform-architecture.md) | **Quikit Platform ↔ CrmExpress** — topology, sharing model, communication, platform contracts, gotchas, unknowns |
...
| [`Design Documents/crmexpress-quikcrm-consolidation-considerations.md`](Design%20Documents/crmexpress-quikcrm-consolidation-considerations.md) | **CrmExpress ↔ QuikCRM consolidation considerations** — dependency and risk exposure only. Not a migration plan, no recommendation. |
...
|---|---|
| New to CrmExpress | [`Architecture/system-architecture.md`](Architecture/system-architecture.md) → [`Architecture/application-architecture.md`](Architecture/application-architecture.md) |
...
| Weighing consolidation | [`Design Documents/crmexpress-quikcrm-consolidation-considerations.md`](Design%20Documents/crmexpress-quikcrm-consolidation-considerations.md) |
...
1. **CrmExpress is one Next.js 14 process plus one optional out-of-process BullMQ worker.** There is no internal service mesh, no internal HTTP hop, and no separate backend deployable.
2. **The QuikIT "platform" is not a runtime host.** Capability reaches CrmExpress through compile-time packages, a shared database, and two HTTPS calls. Nothing executes CrmExpress's code on the platform's behalf.
3. **One PostgreSQL database, sixteen Postgres schemas, one Prisma client.** CrmExpress reads and writes platform tables directly, in-process, with no API in between.
...
6. **No migration creates CrmExpress's 133 tables.** The schema is applied outside the migration history, so no safe, reviewable schema change is currently possible.
7. **CrmExpress appears in no CI workflow**, and ships two mutually exclusive deployment artefacts. How it reaches production is UNKNOWN.
...
10. **CrmExpress and QuikCRM share Redis channel names, BullMQ queue names and an S3 key prefix, byte-for-byte.** Whether they share the underlying infrastructure is UNKNOWN and is the highest-value open question in this set.
