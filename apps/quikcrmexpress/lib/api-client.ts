// apps/quikcrmexpress/lib/api-client.ts
/**
...
export interface CallLogPayload {
  ...
  /** Stage 3-D(a): explicit origin — "dialer" (real call) writes a QceCallLog
...
interface CallLogResponse {
  /** Stage 3-D(a): null when no real call occurred (manual save → no QceCallLog). */
  id: string | null;
...
export async function fetchUsersPicker(): Promise<{ items: UserPickerRow[] }> {
  const res = await apiFetch<{
    items?: UserPickerRow[];
    data?: { items?: UserPickerRow[] };
  }>("/api/users/picker");
  return { items: res.data?.items ?? res.items ?? [] };
}
