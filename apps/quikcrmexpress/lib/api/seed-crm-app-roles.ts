/**
 * Default CrmExpress AppRole seeders (mirrors QuikScale seedAdminAppRole pattern).
 * Uses CrmExpress's own app_quikcrmexpress RBAC tables via the qce* delegates.
 */
...
async function seedRole(orgId: string, appId: string, spec: RoleSpec): Promise<string> {
  ...
  const existing = await client.qceAppRole.findFirst({
    where: { orgId, appId, name: spec.name },
    select: { id: true },
  });

  if (!spec.isSystem && spec.isDefault && !existing) {
    await client.qceAppRole.updateMany({
  ...
  const role =
    existing ??
    (await client.qceAppRole.create({
      data: {
        orgId,
        appId,
        name: spec.name,
  ...
  const grantCount = await client.qceRolePermission.count({ where: { roleId: role.id } });
  ...
  await client.qceRolePermission.createMany({
...
async function backfillAdminPermissions(orgId: string, appId: string): Promise<void> {
  ...
  const admin = await client.qceAppRole.findFirst({
...
export async function seedAllDefaultCrmRoles(
    ...
    if (appId && isCrmRbacClientReady()) {
      ...
      if (client) {
        const [admin, def] = await Promise.all([
          client.qceAppRole.findFirst({ where: { orgId, appId, name: "admin" }, select: { id: true } }),
          client.qceAppRole.findFirst({
            where: { orgId, appId, isDefault: true },
            select: { id: true },
          }),
        ]);
  ...
  const appId = await getQuikcrmexpressAppId();
  if (!appId) throw new Error("CrmExpress App not registered in quikit.App");
  ...
  const client = rbacDb();
  const defaultRoleId =
    (await client?.qceAppRole.findFirst({
      where: { orgId, appId, isDefault: true },
      select: { id: true },
    }))?.id ?? ids[1]!;
