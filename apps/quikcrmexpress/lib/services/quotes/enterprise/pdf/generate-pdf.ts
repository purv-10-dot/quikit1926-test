 ...
 * records QceQuotePdfSnapshot. When S3 is unavailable, returns HTML buffer only.
 */
export async function generateQuotePdfSnapshot(args: {
  ...
  const snapshot = await db.$transaction(async (tx) => {
    const row = await tx.qceQuotePdfSnapshot.create({
      data: {
        orgId: args.orgId,
...
export async function listQuotePdfSnapshots(orgId: string, quoteId: string) {
  return db.qceQuotePdfSnapshot.findMany({
    where: { orgId, quoteId },
