...
async function nextInvoiceNumber(orgId: string): Promise<string> {
  ...
  const row = await db.qceSequence.upsert({
...
export async function createInvoiceFromQuote(args: {
  ...
  const existing = await db.qceInvoice.findFirst({
  ...
  const invoice = await db.$transaction(async (tx) => {
    const row = await tx.qceInvoice.create({
      data: {
        orgId: args.orgId,
        invoiceNumber,
        quoteId: quote.id,
...
export async function listInvoices(orgId: string, page = 1, pageSize = 25) {
  ...
  const [items, total] = await Promise.all([
    db.qceInvoice.findMany({
      where: { orgId, deletedAt: null },
      orderBy: { createdAt: "desc" },
      skip,
      take: pageSize,
    }),
    db.qceInvoice.count({ where: { orgId, deletedAt: null } }),
  ]);
  return {
    items: items.map((i) => ({
      ...i,
