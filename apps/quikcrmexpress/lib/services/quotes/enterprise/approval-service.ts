import type { QceQuoteApprovalStatus } from "@quikit/database";
...
export async function requestQuoteApproval(args: {
  ...
  const row = await db.$transaction(async (tx) => {
    const approval = await tx.qceQuoteApproval.create({
      data: {
        orgId: args.orgId,
...
export async function decideQuoteApproval(args: {
  ...
  const approval = await db.qceQuoteApproval.findFirst({
    where: {
      id: args.approvalId,
      quoteId: args.quoteId,
      orgId: args.orgId,
  ...
  const status: QceQuoteApprovalStatus = args.decision;
  await db.$transaction(async (tx) => {
    await tx.qceQuoteApproval.update({
...
export async function listApprovalInbox(orgId: string) {
  return db.qceQuoteApproval.findMany({
