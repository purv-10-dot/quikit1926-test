 ...
 *
 * Responsibilities:
 *   - Create / read / update / soft-delete QceQuote rows
 *   - Manage QceQuoteLine rows under a quote
 ...
 */
import type { QceQuoteStatus, Prisma } from "@quikit/database";
...
} satisfies Prisma.QceQuoteInclude;
...
const LIST_SELECT = {
  ...
  // (QceQuote.priceList) so this is a single query under the hood — no
  // N+1. Same pattern used by `CrmOpportunity` list endpoints.
  // Note: QceQuote → QceAccount isn't currently a typed relation in the
  ...
  updatedAt: true,
  deletedAt: true,
} satisfies Prisma.QceQuoteSelect;

export interface ListQuotesParams {
  orgId: string;
  page: number;
  pageSize: number;
  status?: QceQuoteStatus;
  accountId?: string;
...
export function buildQuoteWhere(p: Omit<ListQuotesParams, "page" | "pageSize">): Prisma.QceQuoteWhereInput {
  return {
    orgId: p.orgId,
...
export async function listQuotes(p: ListQuotesParams) {
  ...
  // QceOpportunity.accountId comment for the cross-schema-FK rationale.
...
async function repriceQuoteLinesFromPriceList(
  ...
  const lines = await tx.qceQuoteLine.findMany({
    where: { quoteId, orgId },
...
async function applyPriceListToQuoteLines(
  ...
  const now = new Date();
  const pl = await tx.qcePriceList.findFirst({
  ...
  const plItems = await tx.qcePriceListItem.findMany({
  ...
  const q = await tx.qceQuote.findFirst({
...
async function recomputeQuoteTotals(tx: DbClient, orgId: string, quoteId: string): Promise<void> {
  const quote = await tx.qceQuote.findFirst({
    where: { id: quoteId, orgId },
    include: QUOTE_INCLUDE,
  });
...
export async function createQuote(args: {
  ...
  return serverlessTransaction(db, async (tx) => {
    ...
    // isolation on QceQuote itself; references to Account / Contact /
    // Opportunity / PriceList are bare strings (no DB-level FK across
    // schemas — see schema.prisma QceOpportunity comment), so a caller
    // could plant arbitrary IDs.
    const account = await tx.qceAccount.findFirst({
    ...
    if (args.input.contactId) {
      // QceContact isn't middleware-protected — reject linking to a trashed contact.
      const contact = await tx.qceContact.findFirst({
    ...
    if (args.input.opportunityId) {
      const opp = await tx.qceOpportunity.findFirst({
    ...
    const created = await tx.qceQuote.create({
      data: {
        orgId: args.orgId,
        quoteNumber,
        versionNumber: 1,
...
function assertDraft(status: QceQuoteStatus): void {
  if (status !== "Draft") {
    throw new QuoteError(
...
export async function updateQuote(args: {
  ...
  return serverlessTransaction(db, async (tx) => {
    const existing = await tx.qceQuote.findFirst({
    ...
    const data: Prisma.QceQuoteUncheckedUpdateInput = {};
    const i = args.input;
    ...
    await tx.qceQuote.update({ where: { id: args.id }, data });
...
export async function permanentDeleteQuote(orgId: string, id: string): Promise<void> {
  await serverlessTransaction(db, async (tx) => {
    ...
    await tx.qceQuote.delete({ where: { id } });
  });
}
...
export async function expireStaleQuotes(
  ...
  const expired = await db.qceQuote.findMany({
    where: {
      orgId,
      status: "Active",
      effectiveTo: { lt: now },
  ...
  await serverlessTransaction(db, async (tx) => {
    ...
    for (const q of expired) {
      await tx.qceQuoteStatusTransition.create({
        data: {
          orgId,
          quoteId: q.id,
          fromStatus: "Active",
 ...
 * `QuoteSent` QceActivity row so the timeline reflects the touch.
 *
...
export async function addQuoteLine(args: {
  ...
  return serverlessTransaction(db, async (tx) => {
    ...
    const line = await tx.qceQuoteLine.create({
      data: {
        orgId: args.orgId,
...
export async function updateQuoteLine(args: {
  ...
  return serverlessTransaction(db, async (tx) => {
    ...
    return tx.qceQuoteLine.findUnique({ where: { id: args.lineId } });
  });
}
...
export async function deleteQuoteLine(args: {
  ...
  await serverlessTransaction(db, async (tx) => {
    ...
    const lineForLog = await tx.qceQuoteLine.findFirst({
      where: { id: args.lineId, quoteId: args.quoteId, orgId: args.orgId },
      select: { productName: true, lineNumber: true },
    });
    const deleted = await tx.qceQuoteLine.deleteMany({
      where: { id: args.lineId, quoteId: args.quoteId, orgId: args.orgId },
    });
...
export async function loadRevisionChain(
  ...
  if (!parentQuoteId) {
    const childCount = await db.qceQuote.count({
  ...
  for (let i = 0; i < 20; i++) {
    const node = await db.qceQuote.findFirst({
  ...
  while (queue.length > 0) {
    ...
    const rows = await db.qceQuote.findMany({
      where: {
        orgId,
