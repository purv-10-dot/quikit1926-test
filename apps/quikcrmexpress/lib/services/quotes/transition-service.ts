 ...
 * Clone it.
 *
 * Each transition writes BOTH a QceQuoteStatusTransition row and a
 * QceActivity row so the audit log is queryable via either path.
 */
...
const ALLOWED: Record<QceQuoteStatus, QceQuoteStatus[]> = {
  Draft: ["Active"],
...
export interface TransitionInput {
  toStatus: QceQuoteStatus;
  reason?: string | null;
  notes?: string | null;
}

export function validateTransition(from: QceQuoteStatus, input: TransitionInput): void {
 ...
 *     ⤷ QceActivity{type:OpportunityProbabilityFromQuote} on the opportunity
 *
 *   Quote: Active → Lost
...
export async function transitionQuote(args: {
  ...
  return serverlessTransaction(db, async (tx) => {
    ...
    const data: Prisma.QceQuoteUncheckedUpdateInput = { status: args.input.toStatus };
    const now = new Date();
    ...
    await tx.qceQuote.update({ where: { id: args.quoteId }, data });
    ...
    if (existing.opportunityId && (args.input.toStatus === "Won" || args.input.toStatus === "Lost")) {
      ...
      const updated = await tx.qceOpportunity.updateMany({
        where: {
