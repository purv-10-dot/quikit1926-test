...
import type { QceLeadImportJob as LeadImportJob } from "@prisma/client";
...
export async function processLeadsImport(job: LeadImportJob): Promise<ProcessorResult> {
  ...
  const ws = await prisma.qceOrgWorkspaceSettings.findUnique({ where: { orgId: job.orgId } });
  ...
  const existingSourceRows = await prisma.qceLead.findMany({
...
export async function processWorkflowsImport(job: LeadImportJob): Promise<ProcessorResult> {
  ...
  for (let i = 0; i < arr.length; i++) {
    const r = arr[i]!;
    try {
      await prisma.qceWorkflowDefinition.create({
        data: {
          orgId: job.orgId,
...
export async function processSlaImport(job: LeadImportJob): Promise<ProcessorResult> {
  ...
  for (let i = 0; i < (payload.rules ?? []).length; i++) {
    ...
    try {
      await prisma.qceSlaRule.create({
        data: {
          orgId: job.orgId,
