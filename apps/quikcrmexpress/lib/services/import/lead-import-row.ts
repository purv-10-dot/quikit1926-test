...
export type LeadImportRowInput = {
   ...
   * passing real QceLead scalar columns — see IMPORTABLE_STANDARD_KEYS.
   */
...
export async function upsertImportedLeadRow(
  ...
  const row: Prisma.QceLeadUncheckedCreateInput = {
    orgId: data.orgId,
    name: data.name,
  ...
  if (row.externalId && row.sourceSystem) {
    const existing = await prisma.qceLead.findUnique({
      where: {
        lead_external_uk: {
          orgId: row.orgId,
    ...
    if (existing) {
      ...
      const lead = await prisma.qceLead.update({
