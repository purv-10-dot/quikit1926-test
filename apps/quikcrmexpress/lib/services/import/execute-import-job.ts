...
import type { QceImportJobStatus } from "@prisma/client";
...
export async function executeImportJob(data: ImportJobData): Promise<ImportJobRunResult> {
  ...
  const dbJob = await prisma.qceLeadImportJob.findUnique({ where: { id: jobId } });
  ...
  const claim = await prisma.qceLeadImportJob.updateMany({
    where: { id: jobId, status: "queued" },
    data: { status: "processing", startedAt: new Date(), attempts: { increment: 1 } },
  });
  if (claim.count === 0) {
    const current = await prisma.qceLeadImportJob.findUnique({ where: { id: jobId } });
  ...
  try {
    ...
    await prisma.qceLeadImportJob.update({
      where: { id: jobId },
      data: {
        status: finalStatus,
    ...
    const updated = await prisma.qceLeadImportJob.findUnique({ where: { id: jobId } });
