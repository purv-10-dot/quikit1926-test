...
import type { QceImportEntityType as ImportEntityType } from "@prisma/client";
...
export async function isBlocked(jobId: string): Promise<boolean> {
  const job = await prisma.qceLeadImportJob.findUnique({ where: { id: jobId } });
  if (!job || !job.batchId) return false;
  const myRank = ENTITY_RANK[job.entityType];
  const blockers = await prisma.qceLeadImportJob.count({
    where: {
      orgId: job.orgId,
      batchId: job.batchId,
