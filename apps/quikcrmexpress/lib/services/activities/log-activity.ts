...
import type { QceActivity, Prisma, PrismaClient } from "@quikit/database";
...
export async function logActivity(input: LogActivityInput): Promise<QceActivity> {
  ...
  } satisfies Prisma.QceActivityUncheckedCreateInput;
  ...
  const activity =
    input.externalId && input.sourceSystem
      ...
      : await tx.qceActivity.create({ data });
  ...
  // stamps its own (it writes QceActivity directly, bypassing this function),
