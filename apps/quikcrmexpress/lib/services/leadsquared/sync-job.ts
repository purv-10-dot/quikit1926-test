 ...
 * request/response path. It re-reads the current QceLead (tenant-scoped),
...
export function resolveLeadSquaredClient(_tenantId: string): LeadSquaredClient {
  ...
  // QceOrgWorkspaceSettings / secret store keyed by orgId) instead of the
...
export async function processLeadSquaredSyncJob(
  ...
  await lock(`${orgId}:${crmLeadId}`, async () => {
    ...
    const lead = await db.qceLead.findFirst({
