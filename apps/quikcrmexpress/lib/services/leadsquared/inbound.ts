 ...
 *
 *   Note the primary structural guard, too: this path writes QceLead directly
...
export function extractInboundLead(
  ...
  // populate only one. Read each from its own key, but fall QceLead.phone back
...
/**
 * Apply a LeadSquared webhook payload to QuikCRM: upsert the QceLead and the
...
export async function processInboundWebhook(
  ...
  const mapping = await db.qceLeadSquaredSyncMap.findFirst({
  ...
  // Determine the target QceLead. Two independent keys can already point at this
  ...
  //   2. the QceLead's own unique triple (tenantId, sourceSystem='leadsquared',
  ...
  const byTriple: Prisma.QceLeadWhereUniqueInput = {
    lead_external_uk: {
      orgId,
  ...
  if (!existingLead) {
    existingLead = await db.qceLead.findUnique({
      where: byTriple,
      select: existingSelect,
    });
  }
  ...
  if (existingLead) {
    ...
    const updated = await db.qceLead.update({
      where: { id: existingLead.id },
      data: updateData as Prisma.QceLeadUpdateInput,
    });
    ...
    const upserted = await db.qceLead.upsert({
      where: byTriple,
      create: {
        orgId,
      ...
      update: updateData as Prisma.QceLeadUpdateInput,
      select: { id: true },
    });
  ...
  try {
    await db.qceLeadSquaredSyncMap.upsert({
      where: { crmLeadId },
      create: {
        orgId,
        crmLeadId,
    ...
    // different lead. The QceLead write already happened; log and continue
