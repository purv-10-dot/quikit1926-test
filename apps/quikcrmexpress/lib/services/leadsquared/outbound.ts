/**
 * Outbound sync: QuikCRM QceLead create/update -> LeadSquared.
 *
...
export interface OutboundSyncInput {
  orgId: string;
  /** The QceLead id — the stable link key in the mapping table. */
  crmLeadId: string;
...
export async function syncLeadOutbound(
  ...
  const existing = await db.qceLeadSquaredSyncMap.findFirst({
