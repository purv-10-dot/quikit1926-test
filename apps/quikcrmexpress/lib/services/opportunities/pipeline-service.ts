 ...
 *
 * Returns one column per QceOpportunityStage in STAGE_ORDER. Each column
...
export async function getPipelineBoard(args: {
  ...
  where?: Prisma.QceOpportunityWhereInput;
}): Promise<PipelineBoard> {
  const { orgId, aclFilter } = args;

  const baseWhere: Prisma.QceOpportunityWhereInput =
    args.where ??
    ({
      orgId,
      deletedAt: null,
      ...(aclFilter ? (aclFilter as Prisma.QceOpportunityWhereInput) : {}),
  ...
  const byStage = new Map<QceOpportunityStage, typeof rows>();
