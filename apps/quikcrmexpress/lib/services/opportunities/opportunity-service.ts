 ...
 *   - Owner change writes a QceActivity audit row
 */
...
const LIST_SELECT = {
  ...
  deletedAt: true,
  createdAt: true,
} satisfies Prisma.QceOpportunitySelect;
...
export function buildOpportunityListWhere(
  ...
  return {
    ...
    ...(p.q
      ...
      : {}),
    ...(p.aclFilter ? (p.aclFilter as Prisma.QceOpportunityWhereInput) : {}),
  };
}
...
export async function createOpportunity(args: {
  ...
  const stage: QceOpportunityStage = input.stage ?? "Prospecting";
  ...
  const data: Prisma.QceOpportunityUncheckedCreateInput = {
    orgId,
    name: input.name,
    accountId: input.accountId,
  ...
  const created = await client.qceOpportunity.create({ data });

  await client.qceActivity.create({
    data: {
      orgId,
...
export async function updateOpportunity(args: {
  ...
  const data: Prisma.QceOpportunityUncheckedUpdateInput = { ...input };
  ...
  const updated = await db.qceOpportunity.update({
    where: { id, orgId },
    data,
  });
...
export async function softDelete(orgId: string, id: string): Promise<void> {
  await db.qceOpportunity.update({
    where: { id, orgId },
...
export async function recalculateFromProducts(
  ...
  const opp = await db.qceOpportunity.findFirst({
    where: { id: opportunityId, orgId },
    select: { probability: true },
  });
  if (!opp) throw new Error("Opportunity not found");

  const products = await db.qceOpportunityProduct.findMany({
