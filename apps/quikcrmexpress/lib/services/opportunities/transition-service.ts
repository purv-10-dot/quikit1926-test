 ...
 *
 * Governs the legal moves between QceOpportunityStage values, enforces that
 ...
 * QceOpportunityStageTransition row and a QceActivity row on every accepted
 * transition.
 */
import type { QceOpportunityStage, Prisma } from "@quikit/database";
...
// (QceOpportunityStageTransition + QceActivity). The only remaining hard
// guards are:
...
// (e.g. via QceOrgWorkspaceSettings.settings.opportunityTransitions). For
// now every stage allows every other stage.
const ALL_STAGES: QceOpportunityStage[] = [
  "Prospecting",
  "Qualification",
  "Proposal",
  "Negotiation",
...
function allOther(self: QceOpportunityStage): QceOpportunityStage[] {
  return ALL_STAGES.filter((s) => s !== self);
}

export const TRANSITIONS: Record<QceOpportunityStage, QceOpportunityStage[]> = {
...
/**
 * Write the audit row + QceActivity entry. Caller is responsible for actually
 * mutating the QceOpportunity (so the transition write happens in the same
...
export async function recordTransition(
  ...
  await tx.qceOpportunityStageTransition.create({
    data: {
      orgId: args.orgId,
  ...
  if (args.toStage === "ClosedLost") {
    const now = new Date();
    const activeQuotes = await tx.qceQuote.findMany({
      where: {
        orgId: args.orgId,
        opportunityId: args.opportunityId,
        status: "Active",
