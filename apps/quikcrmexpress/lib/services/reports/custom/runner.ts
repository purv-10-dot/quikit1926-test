...
async function leadWhere(ctx: ReportRunContext): Promise<Prisma.QceLeadWhereInput> {
  const acl = await accountScopeFilter(ctx.session);
  const base: Prisma.QceLeadWhereInput = {
    orgId: ctx.orgId,
    createdAt: { gte: ctx.from, lte: ctx.to },
    ...(ctx.ownerId ? { ownerId: ctx.ownerId } : {}),
  };
  if (!acl) return base;
  return { AND: [base, acl as Prisma.QceLeadWhereInput] };
}

async function opportunityWhere(
  ctx: ReportRunContext,
): Promise<Prisma.QceOpportunityWhereInput> {
  const acl = await accountScopeFilter(ctx.session);
  const base: Prisma.QceOpportunityWhereInput = {
    orgId: ctx.orgId,
    deletedAt: null,
    createdAt: { gte: ctx.from, lte: ctx.to },
    ...(ctx.ownerId ? { ownerId: ctx.ownerId } : {}),
  ...
  if (!acl) return base;
  return { AND: [base, acl as Prisma.QceOpportunityWhereInput] };
}

function activityWhere(ctx: ReportRunContext): Prisma.QceActivityWhereInput {
  return {
    orgId: ctx.orgId,
...
function callLogWhere(ctx: ReportRunContext): Prisma.QceCallLogWhereInput {
  return {
    orgId: ctx.orgId,
    createdAt: { gte: ctx.from, lte: ctx.to },
...
async function runDateBucket(
  ...
  if (object === "leads") {
    ...
    const rows = await db.qceLead.findMany({
      where: where as Prisma.QceLeadWhereInput,
    ...
    if (metric === "sumAmount") {
      const rows = await db.qceOpportunity.findMany({
        where: where as Prisma.QceOpportunityWhereInput,
    ...
    const rows = await db.qceActivity.findMany({
      where: where as Prisma.QceActivityWhereInput,
    ...
    if (metric === "sumDuration") {
      const rows = await db.qceCallLog.findMany({
        where: where as Prisma.QceCallLogWhereInput,
...
export async function runCustomReport(
  ...
  if (isDateBucket) {
    ...
    if (definition.metric === "avgScore") {
      const grouped = await db.qceLead.groupBy({
    ...
    if (definition.metric === "sumAmount") {
      const grouped = await db.qceOpportunity.groupBy({
    ...
    const grouped = await db.qceActivity.groupBy({
