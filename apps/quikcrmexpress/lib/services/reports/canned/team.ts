 ...
 * QceSalesGroup structures rather than a hard role enum, so the helpers
...
async function directReportUserIds(ctx: ReportRunContext): Promise<string[]> {
  // QceSalesGroupManager rows for this user → groups managed → members.
  const managed = await db.qceSalesGroupManager.findMany({
    where: { userId: ctx.session.userId },
    select: { groupId: true },
  });
  if (managed.length === 0) return [];
  const groupIds = [...new Set(managed.map((m) => m.groupId))];
  const members = await db.qceSalesGroupMember.findMany({
...
const teamConversionRate: CannedReport = {
  ...
  blurb:
    "For each sales user: qualified-leads ÷ total-leads created in the selected window. " +
  ...
  helpText:
    ...
    "Total leads = leads created in the selected date range (defaults to this month) " +
    "and owned by them; Qualified = of those, leads whose stage is anything other " +
    "than 'New'; Conversion % = qualified ÷ total (rounded to 1 decimal). " +
  ...
  async run(ctx) {
    ...
    const totals = await db.qceLead.groupBy({
      by: ["ownerId"],
      where: {
        orgId: ctx.orgId,
        ownerId: { in: userIds },
    ...
    const qualified = await db.qceLead.groupBy({
      by: ["ownerId"],
      where: {
        orgId: ctx.orgId,
        ownerId: { in: userIds },
...
const teamDispositionMix: CannedReport = {
  ...
  blurb:
    ...
    "over the selected window. Empty when the caller manages no sales group.",
  helpText:
    "Resolves your direct reports by walking CrmSalesGroupManager → CrmSalesGroupMember, " +
    "then counts those users' call logs by `dispositionName` over the selected date " +
    "range (defaults to the last 7 days). Returns the top 8 dispositions sorted by " +
    "count, rendered as a pie chart. Returns empty rows when the signed-in user " +
    "manages no sales group at all.",
  defaultDateRange: "7d",
  buildDrillUrl: (row) => {
  ...
  async run(ctx) {
    ...
    const grouped = await db.qceCallLog.groupBy({
      by: ["dispositionName"],
      where: {
        orgId: ctx.orgId,
