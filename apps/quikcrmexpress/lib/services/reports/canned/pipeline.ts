...
async function aclWhere(ctx: ReportRunContext): Promise<Prisma.QceOpportunityWhereInput> {
...
const pipelineByStage: CannedReport = {
  ...
  async run(ctx) {
    ...
    const rows = await db.qceOpportunity.groupBy({
      by: ["stage"],
...
const stuckDeals: CannedReport = {
  ...
  async run(ctx) {
    ...
    const opps = await db.qceOpportunity.findMany({
      where: {
        ...where,
        stage: { notIn: [...CLOSED_STAGES] },
