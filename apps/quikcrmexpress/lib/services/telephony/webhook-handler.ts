// apps/quikcrmexpress/lib/services/telephony/webhook-handler.ts
import { timingSafeEqual } from "crypto";
/**
 * IndiaVoice webhook pipeline.
 *
 *   1. Secret validation (mandatory in prod or when WEBHOOK_REQUIRE_SECRET=true)
 *   2. Resolve orgId from SERVER-SIDE config only — never from the payload
 *   3. Idempotent audit row upsert into QceIndiaVoiceWebhookLog, keyed by
 ...
 *   5. Match a QceCallLog by providerCallSid/callSid first, then by phone-tail
 *      fallback within a 24h window.
 *   6. Update the matched QceCallLog with duration/recording/status/etc.
...
export async function processIndiaVoiceWebhook(
  ...
  const existingAudit = await prisma.qceIndiaVoiceWebhookLog.findUnique({
  ...
  const audit = await prisma.qceIndiaVoiceWebhookLog.create({
    data: {
      orgId,
      callSid: r.callSid,
      campid: r.campid,
  ...
  let callLog =
    idCandidates.length > 0
      ? await prisma.qceCallLog.findFirst({
          where: {
            orgId,
            createdAt: { gte: since },
            OR: [
  ...
  if (!callLog && idCandidates.length > 0) {
    callLog = await prisma.qceCallLog.findFirst({
      where: {
        orgId,
        OR: [
          { providerCallSid: { in: idCandidates } },
  ...
  if (callLog) {
    ...
    const updated = await prisma.qceCallLog.update({
      where: { id: callLog.id },
      data: {
    ...
    await prisma.qceIndiaVoiceWebhookLog.update({
      where: { id: audit.id },
...
export async function handleWebhook(opts: {
  ...
  if (configuredSecret) {
    ...
    // caller who can measure response timing.
    const a = Buffer.from(presented);
    const b = Buffer.from(configuredSecret);
    if (a.length !== b.length || !timingSafeEqual(a, b)) {
      throw statusError("Invalid webhook secret", 400);
    }
  }

  // 2. Org resolution — SERVER-SIDE CONFIG ONLY.
  //
  // This previously fell back to `flat.tenantId` from the request body, and
  // WEBHOOK_TRUST_PAYLOAD_ORG_ID defaulted to "true". Because one shared secret
  // is used for every org, any caller holding it could write call logs and
  // audit rows into an arbitrary org just by setting tenantId in the payload.
  // A second, unconditional `flat.tenantId` fallback sat below the opt-out, so
  // setting the flag to "false" did not actually disable payload trust.
  //
  // The LeadSquared webhook in this same app already does it correctly
  // ("Never derived from the payload" — lib/services/leadsquared/inbound.ts).
