// apps/quikcrmexpress/lib/services/telephony/call-service.ts
/**
 * Orchestrates click-to-call: place provider call → fire-and-forget audit
 * (success or failure) → pre-create the QceCallLog stub the webhook will
...
export async function placeCall(opts: {
  ...
  if (result.callSid) {
    ...
    try {
      await prisma.qceCallLog.upsert({
