// apps/quikcrmexpress/lib/services/telephony/disposition-engine.ts
/**
 ...
 *
 *   1. Load the chosen QceCallDisposition (tenant-scoped).
 *   2. Backfill from the most recent unmatched QceIndiaVoiceWebhookLog within
 ...
 *   4. UPSERT the QceCallLog row (reuse the dialer pre-stub if we have a sid).
...
export interface CreateCallLogDto {
   ...
   * QceCallLog row. "manual" = an agent disposition update with no call ->
   ...
   * fieldKey. Persisted as QceFieldValue and fed to the rule engine. Type is
...
export interface CreateCallLogResult {
  /** Stage 3-D(a): null on a manual save (no real call -> no QceCallLog row). */
  id: string | null;
...
export async function createCallLog(
  ...
  const disposition = requestedId
    ? (await prisma.qceCallDisposition.findFirst({ where: { id: requestedId, orgId } })) ??
  ...
  const orphanOr: Prisma.QceIndiaVoiceWebhookLogWhereInput[] = [];
  ...
  const orphan =
    orphanOr.length > 0
      ? await prisma.qceIndiaVoiceWebhookLog.findFirst({
          where: {
            orgId,
  ...
  if (dto.linkedLeadId && dto.linkedLeadId.trim()) {
    const explicit = await prisma.qceLead.findFirst({
  ...
  if (!leadId && tailTo) {
    const matched = await prisma.qceLead.findFirst({
      where: {
        orgId,
  ...
  // FR-RE Stage 3-D(a): a QceCallLog row means a real call happened. Only the
  ...
  const callLog = !isRealCall
    ? null
    : existingStub
      ? await prisma.qceCallLog.update({
      ...
      : await prisma.qceCallLog.create({
          data: {
            orgId,
            callSid: sid,
  ...
  if (leadId) {
    ...
    const callActivity = await prisma.qceActivity.create({
      data: {
        orgId,
        type: "Call",
        relatedKind: "Lead",
    ...
    if (lead) {
      ...
      const data: Prisma.QceLeadUpdateInput = {};
      ...
      // `last_activity_date` to the call time. CrmExpress filters on this field
      ...
      if (changed.length > 0) {
        await prisma.qceLead.update({ where: { id: leadId }, data });
    ...
    if (leadId) {
      ...
      // rule fires but records ONLY a QceAuditLog — so the change never showed on
