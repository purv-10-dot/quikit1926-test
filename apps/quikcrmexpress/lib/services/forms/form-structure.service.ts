...
import type { QceFormField, QceFormSection, QceFormTab } from "@quikit/database";
...
export async function assertDraft(formSetVersionId: string): Promise<void> {
  const version = await prisma.qceFormSetVersion.findUnique({
...
export async function getFormStructure(formSetVersionId: string) {
  return prisma.qceFormTab.findMany({
...
export async function createFormTab(input: {
  ...
  return prisma.qceFormTab.create({
    data: {
...
export async function createFormSection(input: {
  ...
  name?: string | null;
  sortOrder: number;
}): Promise<QceFormSection> {
  const tab = await prisma.qceFormTab.findUnique({
  ...
  return prisma.qceFormSection.create({
...
}): Promise<QceFormField> {
  return prisma.qceFormField.update({
...
export async function createFormField(input: {
  ...
  await assertDraft(input.formSetVersionId);
  const field = await prisma.qceFormField.create({
    data: {
      formSetVersionId: input.formSetVersionId,
  ...
  if (input.options?.length) {
    await prisma.qceFormFieldOption.createMany({
...
async function assertFieldDraft(fieldId: string): Promise<QceFormField> {
  const field = await prisma.qceFormField.findUnique({ where: { id: fieldId } });
...
export async function deleteFormField(fieldId: string): Promise<void> {
  ...
  await prisma.qceFormFieldOption.deleteMany({ where: { formFieldId: fieldId } });
  await prisma.qceFormField.delete({ where: { id: fieldId } });
}
...
export async function deleteFormTab(tabId: string): Promise<void> {
  ...
  await prisma.qceFormTab.delete({ where: { id: tabId } });
}
...
export async function seedProtectedDispositionTab(
  ...
  const tab = await prisma.qceFormTab.create({
    data: {
      formSetVersionId,
  ...
  for (let i = 0; i < PROTECTED_FIELDS.length; i++) {
    ...
    await prisma.qceFormField.create({
      data: {
        formSetVersionId,
