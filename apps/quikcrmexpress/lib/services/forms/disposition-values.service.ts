 ...
 *
 * The raw values live in QceFieldValue keyed by (activityId, fieldKey), typed by
 * valueType. This service:
 *   1. loads those rows for the activity (tenant-scoped),
 *   2. resolves each fieldKey -> its human label + tab name (from QceFormField),
...
export async function getDispositionValues(
  ...
  const defs = await prisma.qceFormField.findMany({
