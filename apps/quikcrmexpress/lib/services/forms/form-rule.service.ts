...
import type {
  QceFormRule,
  QceFormRuleCondition,
  QceFormRuleMatchType,
  QceFormRuleSubjectKind,
  QceFormRuleOperator,
...
export interface ConditionInput {
  subjectKind: QceFormRuleSubjectKind;
  subjectFieldKey?: string | null;
  operator: QceFormRuleOperator;
  valueKeys?: string[] | null;
}

const SINGLE_VALUE_OPS: QceFormRuleOperator[] = ["is", "is_not"];
const MULTI_VALUE_OPS: QceFormRuleOperator[] = ["is_any_of", "is_none_of"];
const NO_VALUE_OPS: QceFormRuleOperator[] = ["is_empty", "is_not_empty"];
...
export async function assertFieldExists(formSetVersionId: string, fieldKey: string): Promise<void> {
  const field = await prisma.qceFormField.findFirst({
...
export async function getFormRules(formSetVersionId: string) {
  return prisma.qceFormRule.findMany({
...
export async function createFormRule(input: {
  ...
  name: string;
  matchType: QceFormRuleMatchType;
  sortOrder: number;
  isActive?: boolean;
  createdByUserId?: string | null;
}): Promise<QceFormRule> {
  await assertDraft(input.formSetVersionId);
  return prisma.qceFormRule.create({
    data: {
...
export async function assertRuleDraft(formRuleId: string): Promise<string> {
  const rule = await prisma.qceFormRule.findUnique({
    where: { id: formRuleId },
...
export async function updateFormRule(input: {
  ...
  return prisma.qceFormRule.update({
...
export async function deleteFormRule(ruleId: string): Promise<void> {
  ...
  await prisma.qceFormRule.delete({ where: { id: ruleId } }); // conditions/actions cascade
}
...
export async function addRuleCondition(input: {
  ...
  return prisma.qceFormRuleCondition.create({
    data: {
...
export async function updateRuleCondition(input: {
  ...
  const existing = await prisma.qceFormRuleCondition.findUnique({
  ...
  return prisma.qceFormRuleCondition.update({
    where: { id: input.conditionId },
    data: {
...
export async function deleteRuleCondition(conditionId: string): Promise<void> {
  ...
  await prisma.qceFormRuleCondition.delete({ where: { id: conditionId } });
}
