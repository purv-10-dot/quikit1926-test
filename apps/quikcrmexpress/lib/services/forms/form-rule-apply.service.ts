...
/** Map a stored QceFieldValue row to its EvalContext value, by valueType. */
...
export async function loadEvalContext(
  ...
  const lead = await prisma.qceLead.findUnique({
    where: { id: leadId },
  ...
  if (activityId) {
    const rows = await prisma.qceFieldValue.findMany({
      where: { activityId },
      select: {
        fieldKey: true,
        valueType: true,
