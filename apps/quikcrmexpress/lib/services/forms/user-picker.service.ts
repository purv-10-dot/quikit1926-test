 ...
 *      written to QceFieldValue.valueUserIds on save (full save wiring: Unit 6).
 *
 ...
 * (QceSalesGroupMember + QceSalesGroupManager). Missing scope data fails CLOSED
...
async function callerGroupIds(userId: string): Promise<string[]> {
  const [member, manager] = await Promise.all([
    db.qceSalesGroupMember.findMany({ where: { userId }, select: { groupId: true } }),
    db.qceSalesGroupManager.findMany({ where: { userId }, select: { groupId: true } }),
  ]);
 ...
 * QceFieldValue.valueUserIds. Required-ness is enforced separately by the
