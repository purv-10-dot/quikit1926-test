...
import type {
  QceFormRuleAction,
  QceFormRuleActionType,
  QceFormRuleTargetKind,
} from "@quikit/database";

export { FormRuleError };

export interface ActionInput {
  actionType: QceFormRuleActionType;
  targetKind: QceFormRuleTargetKind;
...
const FIELD_ACTIONS: QceFormRuleActionType[] = [
  "show_field",
  "hide_field",
  "make_mandatory",
  "make_optional",
...
async function assertActionTargets(
  ...
  if (input.actionType === "show_tab") {
    const tab = await prisma.qceFormTab.findFirst({
...
export async function getRuleActions(formRuleId: string) {
  return prisma.qceFormRuleAction.findMany({
    where: { formRuleId },
...
export async function updateRuleAction(input: {
  ...
  const existing = await prisma.qceFormRuleAction.findUnique({
  ...
  return prisma.qceFormRuleAction.update({
    where: { id: input.actionId },
    data: {
...
export async function deleteRuleAction(actionId: string): Promise<void> {
  ...
  await prisma.qceFormRuleAction.delete({ where: { id: actionId } });
}
