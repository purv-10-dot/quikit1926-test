 ...
 * existing QceTask columns. Schema-dependent features (description,
...
function smartViewWhere(key: string | undefined): Prisma.QceTaskWhereInput | null {
  if (!key) return null;
...
function duePresetWhere(preset: string | undefined): Prisma.QceTaskWhereInput | null {
...
function buildListWhere(user: SessionUser, q: ListTasksQuery): Prisma.QceTaskWhereInput {
  const where: Prisma.QceTaskWhereInput = { orgId: user.orgId };
  const ands: Prisma.QceTaskWhereInput[] = [];
...
export async function getTask(user: SessionUser, id: string) {
  return prisma.qceTask.findFirst({ where: { id, orgId: user.orgId } });
}
...
export async function createTask(user: SessionUser, input: CreateTaskInput) {
  const data: Prisma.QceTaskUncheckedCreateInput = {
    orgId: user.orgId,
    ...
    // Who assigned/created the task — server-set (never from the request body),
    // so it can't be spoofed. Surfaced as "Assigned by" in the tasks table.
    createdByUserId: user.userId,
  };
  const task = await prisma.qceTask.create({ data });
 ...
 *
 * Writes a QceActivity { type: "TaskStatusChange" } when status changes and
...
export async function updateTask(
  ...
  const existing = await prisma.qceTask.findFirst({ where: { id, orgId: user.orgId } });
  if (!existing) {
  ...
  const data: Prisma.QceTaskUncheckedUpdateInput = {};
  ...
  const updated = await prisma.qceTask.update({ where: { id }, data });
...
export async function deleteTask(user: SessionUser, id: string) {
  ...
  const res = await prisma.qceTask.deleteMany({ where: { id, orgId: user.orgId } });
  if (res.count === 0) {
    const err = new Error("Task not found") as Error & { statusCode?: number };
    err.statusCode = 404;
    throw err;
...
export async function snoozeTask(
  ...
  id: string,
  newDueDate: Date,
): Promise<{ task: Awaited<ReturnType<typeof prisma.qceTask.update>>; previousDueDate: Date | null }> {
  const existing = await prisma.qceTask.findFirst({ where: { id, orgId: user.orgId } });
  if (!existing) {
    const err = new Error("Task not found") as Error & { statusCode?: number };
    err.statusCode = 404;
  ...
  const updated = await prisma.qceTask.update({
    where: { id },
    data: { dueDate: newDueDate },
  });

  // Status-change activity shape, repurposed for snooze. Stores the old/new
  // due dates in detailNotes since QceTask has no snoozedFromDueDate column.
...
export async function advancedFilter(user: SessionUser, input: AdvancedFilterInput) {
  ...
  const finalWhere: Prisma.QceTaskWhereInput = ands.length ? { AND: [where, ...ands] } : where;
  ...
  const [items, total] = await Promise.all([
    ...
    prisma.qceTask.count({ where: finalWhere }),
  ]);
 ...
 *
 * QceTask has no sourceCallLogId column yet, so we approximate by matching
...
export async function findExistingFollowUpTask(opts: {
  ...
  return prisma.qceTask.findFirst({
    where: {
      orgId: opts.orgId,
      leadId: opts.leadId,
      status: "Open",
