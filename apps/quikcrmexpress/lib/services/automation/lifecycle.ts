 ...
 *
 * State machine (status column on QceWorkflowDefinition; enum extended
...
import type { QceWorkflowStatus, QceWorkflowDefinition } from "@quikit/database";
...
export const ADMIT_NEW_STATUS: QceWorkflowStatus = "Active";

/** New leads may ONLY enter an Active automation. */
export function canAdmitNewLead(status: QceWorkflowStatus): boolean {
...
export function canResumeInFlight(status: QceWorkflowStatus): boolean {
...
export function canEditStructure(status: QceWorkflowStatus): boolean {
  return status === "Draft";
}

// ── Transition rules ────────────────────────────────────────────────────────

const PUBLISH_FROM: QceWorkflowStatus[] = ["Draft"];
const UNPUBLISH_FROM: QceWorkflowStatus[] = ["Active"];
// Draining is intentionally excluded — see softDelete(). Deleted excluded (already gone).
const DELETE_FROM: QceWorkflowStatus[] = ["Draft", "Active", "Stopped", "Paused", "Archived"];

function assertFrom(current: QceWorkflowStatus, allowed: QceWorkflowStatus[], op: string): void {
...
): Promise<QceWorkflowDefinition> {
  const def = await prisma.qceWorkflowDefinition.findFirst({
...
export async function publish(orgId: string, id: string): Promise<QceWorkflowDefinition> {
  ...
  return prisma.qceWorkflowDefinition.update({
...
export async function unpublish(
  ...
  const status: QceWorkflowStatus = mode === "immediate" ? "Stopped" : "Draining";
  return prisma.qceWorkflowDefinition.update({ where: { id: def.id }, data: { status } });
}
...
export async function softDelete(orgId: string, id: string): Promise<QceWorkflowDefinition> {
  const def = await loadDef(orgId, id);
...
export async function restore(orgId: string, id: string): Promise<QceWorkflowDefinition> {
...
export async function listAutomations(
  ...
  return prisma.qceWorkflowDefinition.findMany({
...
export async function isDrained(orgId: string, id: string): Promise<boolean> {
  const outstanding = await prisma.qceAutomationPendingStep.count({
