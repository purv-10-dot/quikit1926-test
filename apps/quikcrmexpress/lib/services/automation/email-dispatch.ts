/**
 * [P3.B1] Email dispatch — consume a queued QceOutboundMessageLog row and
...
export async function dispatchOutboundMessage(
  ...
  const log = await prisma.qceOutboundMessageLog.findFirst({
  ...
  try {
    ...
    await prisma.qceOutboundMessageLog.update({
      where: { id: log.id },
      data: {
        status: "sent",
