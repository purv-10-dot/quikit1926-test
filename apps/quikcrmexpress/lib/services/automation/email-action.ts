 ...
 *     QceOutboundMessageLog row with status "skipped" + the reason — it is not an
 ...
 * Do-Not-Email, then unsubscribed, then address validity. Maps to CrmExpress's own
...
export async function executeSendEmail(input: {
  ...
  if (suppression.suppressed) {
    ...
    const row = await prisma.qceOutboundMessageLog.create({
      data: {
        orgId,
        channel: "email",
        to: recipient,
