...
import type { QceLead as Lead } from "@prisma/client";
...
export async function runFrom(
  ...
  // rows (deletedAt set) are excluded here since QceWorkflowDefinition is not in
  // the shared soft-delete middleware set.
  const wf = await prisma.qceWorkflowDefinition.findFirst({
  ...
  const lead = await prisma.qceLead.findFirst({ where: { id: leadId, orgId } });
  if (!lead) return;
  ...
  try {
    switch (node.kind) {
      ...
      case "wait": {
        ...
        const pending = await prisma.qceAutomationPendingStep.create({
          data: {
            orgId: ctx.orgId,
            workflowId: ctx.workflowId,
            leadId: lead.id,
        ...
        await prisma.qceAutomationPendingStep.update({
          where: { id: pending.id },
          data: { bullJobId },
        });
        return { kind: "wait" };
      ...
      case "notify_user": {
        ...
        if (cfg.userId) {
          await prisma.qceNotification.create({
            data: {
              orgId: ctx.orgId,
              userId: cfg.userId,
