 ...
 *   - FR-D5: every rule-made status change is audit-logged via QceAuditLog with
...
export interface RunAfterActivityLoggedCtx {
  ...
  leadId: string;
  activityId: string;
  /** The QceCallDisposition.code that the agent selected. */
   ...
   * time (QceActivity.occurredAt). Used as the due date for create_task actions.
   */
...
interface SetLeadStatusAction {
  ...
  /** Target QceLead.status value. Validated at rule-creation time. */
  status: string;
  /** Optional QceLead.substatus value. */
  sub_status?: string;
}
...
export async function validateRuleAction(
  ...
  const exists = await prisma.qceLeadStatus.findFirst({
  ...
  if (action.sub_status) {
    const subExists = await prisma.qceLeadSubStatus.findFirst({
...
async function _runEngine(ctx: RunAfterActivityLoggedCtx): Promise<void> {
  const rules = await prisma.qceAutomationRule.findMany({
  ...
  const leadRecord = await prisma.qceLead.findUnique({
    where: { id: ctx.leadId },
  ...
  for (const rule of rules) {
    ...
    if (action.type === "set_lead_status") {
      ...
      await prisma.qceAuditLog.create({
        data: {
          orgId: ctx.orgId,
          userId: null,
          module: "leads",
      ...
      // TODO(FR-D3 notify_owner): wire to QceNotification when notification
