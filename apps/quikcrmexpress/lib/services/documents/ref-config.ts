...
export async function assertDocumentParent(
  ...
  switch (refType) {
    ...
    case "account": {
      const acc = await prisma.qceAccount.findFirst({
        where: { id: refId, orgId },
        select: { id: true },
      });
    ...
    case "opportunity": {
      const opp = await prisma.qceOpportunity.findFirst({
        where: { id: refId, orgId },
        select: { accountId: true },
      });
    ...
    case "quote": {
      const quote = await prisma.qceQuote.findFirst({
        where: { id: refId, orgId },
        select: { id: true },
      });
    ...
    case "order": {
      const order = await prisma.qceOrder.findFirst({
        where: { id: refId, orgId },
        select: { id: true },
      });
