...
export async function buildExecutiveOverview(
  ...
  const [
    ...
    prisma.sessionEvent.findMany({
      where: { orgId: orgId, appSlug: "quikcrmexpress" },
  ...
  const activityTrend = await Promise.all(
    dayBuckets.map(async (b) => {
      ...
      const count = await prisma.qceActivity.count({
        where: {
          orgId,
  ...
  const leadTrend = await Promise.all(
    dayBuckets.map(async (b) => {
      const from = new Date(b.iso);
      const to = endOfDayInTz(from, range.tz);
      const count = await prisma.qceLead.count({
        where: {
          orgId,
          deletedAt: null,
  ...
  // Usage / last activity
  const lastActByUser = await prisma.qceActivity.groupBy({
    by: ["ownerId"],
  ...
  const revenueTrend = await Promise.all(
    dayBuckets.slice(-14).map(async (b) => {
      const from = new Date(b.iso);
      const to = endOfDayInTz(from, range.tz);
      const rows = await prisma.qceOpportunity.findMany({
        where: {
          orgId,
          stage: "ClosedWon",
