...
async function countActivitiesInRange(
  ...
  return prisma.qceActivity.count({ where });
}
...
export async function buildSummary(
  ...
  // Soft-delete-aware bases. QceLead, QceOpportunity, and QceAccount all
  // carry `deletedAt`. They are NOT yet registered in the package-level
  // SOFT_DELETE_MODELS middleware (see schema.prisma comment on QceLead),
  ...
  const [
    ...
    prisma.qceTask.count({
      where: {
        ...taskWhereBase,
        status: { not: "Completed" as QceTaskStatus },
      },
    }),
...
async function buildTeamDashboard(
  ...
  const [calls, activities, dispRaw] = await Promise.all([
    prisma.qceCallLog.count({ where: callWhere }),
    prisma.qceActivity.count({ where: activityWhere }),
