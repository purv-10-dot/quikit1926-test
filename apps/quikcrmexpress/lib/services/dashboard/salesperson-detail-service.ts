...
async function buildHeatmap(
  ...
  const rows = await prisma.qceActivity.findMany({
    where: {
      orgId,
...
async function computeAvgResponseTime(
  ...
  const firstActivities = await prisma.qceActivity.findMany({
...
export async function getSalespersonDetail(
  ...
  const [
    ...
    // Total leads created
    prisma.qceLead.count({ where: baseLeadWhere }),
    ...
    prisma.sessionEvent.findFirst({
      where: { orgId: orgId, userId, appSlug: "quikcrmexpress", event: "login" },
      orderBy: { createdAt: "desc" },
      select: { createdAt: true },
    }),

    // Last CRM activity by this user (all time)
    prisma.qceActivity.findFirst({
    ...
    prisma.qceOpportunity.aggregate({
      where: { orgId, deletedAt: null, stage: "ClosedWon", updatedAt: rangeWhere, ownerId: userId },
      _sum: { amount: true },
    }),
