 ...
 */
import type { QceOpportunityStage, QceTaskStatus } from "@prisma/client";
...
function oppWhere(
  ...
  if (stage) w.stage = stage as QceOpportunityStage;
  return w;
}
...
export async function buildAdvancedOverview(input: {
  ...
  const [
    ...
    prisma.qceTask.count({
      where: {
        orgId,
        status: "Completed" as QceTaskStatus,
    ...
    prisma.qceOpportunity.count({
      where: {
        ...oppBase,
        stage: { notIn: ["ClosedWon", "ClosedLost"] as QceOpportunityStage[] },
    ...
    prisma.qceTask.groupBy({
      by: ["assignedToUserId"],
      where: {
        orgId,
        status: { notIn: ["Completed", "Cancelled"] as QceTaskStatus[] },
        assignedToUserId: { in: filteredMemberIds },
      },
      _count: true,
    }),
    prisma.qceActivity.groupBy({
      by: ["ownerId"],
      where: {
        orgId,
        followUpAt: { gte: now },
    ...
    prisma.qceQuoteApproval.findMany({
      where: { orgId },
    ...
    prisma.qceQuoteApproval.count({ where: { orgId, status: "Pending" } }),
    prisma.qceQuoteApproval.count({
  ...
  const renewalSoon = await prisma.qceOpportunity.count({
    where: {
      orgId,
      deletedAt: null,
