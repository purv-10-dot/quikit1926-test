...
export async function listGlobalAggregatedTreeChildren(
  ...
  if (parsed.kind === "module" && parsed.module) {
    if (parsed.module === "global") {
      const rows = await prisma.qceDocumentFolder.findMany({
        where: {
          orgId: user.orgId,
          refType: null,
          refId: null,
    ...
    if (folder.refType && folder.refId) {
      ...
      while (cur) {
        ...
        const row: { id: string; name: string; parentFolderId: string | null } | null =
          await prisma.qceDocumentFolder.findFirst({
...
/** Global module panel: tenant-global folders + every QceDocument (all modules). */
