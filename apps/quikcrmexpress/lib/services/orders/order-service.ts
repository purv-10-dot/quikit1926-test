 ...
 *     QceOrder). Re-converting the same quote returns the existing
 ...
 *   - Order numbering reuses QceSequence with the `order-YYYY` bucket,
 ...
 */
import type { QceOrderStatus, Prisma } from "@quikit/database";
...
export async function createOrderFromQuote(args: {
  ...
  return db.$transaction(async (tx) => {
    ...
    const existingOrder = await tx.qceOrder.findFirst({
    ...
    const created = await tx.qceOrder.create({
      data: {
        orgId: args.orgId,
        orderNumber,
        quoteId: quote.id,
...
const LIST_SELECT = {
  ...
  updatedAt: true,
  deletedAt: true,
} satisfies Prisma.QceOrderSelect;

export interface ListOrdersParams {
  orgId: string;
  page: number;
  pageSize: number;
  status?: QceOrderStatus;
  accountId?: string;
  q?: string;
  trashed: boolean;
}

export function buildOrderWhere(p: Omit<ListOrdersParams, "page" | "pageSize">): Prisma.QceOrderWhereInput {
  return {
    orgId: p.orgId,
...
export async function updateOrder(args: {
  ...
  return db.$transaction(async (tx) => {
    ...
    await tx.qceOrder.update({ where: { id: args.id }, data });
