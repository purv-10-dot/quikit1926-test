...
export async function getFullOrderRecord(opts: {
  ...
  const row = await prisma.qceOrder.findFirst({
  ...
  const [account, contact, opportunity, activities, attachments] = await Promise.all([
    row.accountId
      ? prisma.qceAccount.findFirst({
          where: { id: row.accountId, orgId: user.orgId },
          select: { id: true, name: true },
        })
      : null,
    row.contactId
      ? prisma.qceContact.findFirst({
          // QceContact isn't middleware-protected — don't surface a trashed contact.
          where: { id: row.contactId, orgId: user.orgId, deletedAt: null },
          select: { id: true, firstName: true, lastName: true, email: true },
        })
      : null,
    row.opportunityId
      ? prisma.qceOpportunity.findFirst({
