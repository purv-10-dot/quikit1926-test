...
export async function listGroups(orgId: string) {
  return prisma.qceSalesGroup.findMany({
    where: { orgId },
    orderBy: { name: "asc" },
    include: {
...
export async function getGroup(orgId: string, id: string) {
  ...
  const group = await prisma.qceSalesGroup.findFirst({
    where: { id, orgId },
    include: {
      members: true,
      managers: true,
...
export async function createGroup(opts: { actor: SessionUser; data: { name: string } }) {
  ...
  return prisma.$transaction(async (tx) => {
    const dupe = await tx.qceSalesGroup.findFirst({ where: { orgId: actor.orgId, name: data.name } });
    if (dupe) throw new SettingsConflictError(`Sales group "${data.name}" already exists`);
    const created = await tx.qceSalesGroup.create({ data: { orgId: actor.orgId, name: data.name } });
    await audit(
...
export async function updateGroup(opts: { actor: SessionUser; id: string; patch: { name?: string } }) {
  ...
  return prisma.$transaction(async (tx) => {
    const before = await tx.qceSalesGroup.findFirst({ where: { id, orgId: actor.orgId } });
    if (!before) throw new SettingsConflictError("Sales group not found", 404);
    if (patch.name && patch.name !== before.name) {
      const dupe = await tx.qceSalesGroup.findFirst({
    ...
    const updated = await tx.qceSalesGroup.update({ where: { id }, data: patch });
    await audit(
...
export async function deleteGroup(opts: { actor: SessionUser; id: string }) {
  const { actor, id } = opts;
  return prisma.$transaction(async (tx) => {
    const target = await tx.qceSalesGroup.findFirst({ where: { id, orgId: actor.orgId } });
    ...
    await tx.qceSalesGroup.delete({ where: { id } });
    await audit(
...
export async function addMembers(opts: {
  ...
  return prisma.$transaction(async (tx) => {
    const grp = await tx.qceSalesGroup.findFirst({ where: { id: groupId, orgId: actor.orgId } });
    ...
    if (asManager) {
      await tx.qceSalesGroupManager.createMany({
        data: userIds.map((userId) => ({ groupId, userId })),
        skipDuplicates: true,
      });
    } else {
      await tx.qceSalesGroupMember.createMany({
        data: userIds.map((userId) => ({ groupId, userId })),
        skipDuplicates: true,
      });
    }
...
export async function removeMember(opts: {
  ...
  return prisma.$transaction(async (tx) => {
    if (asManager) {
      await tx.qceSalesGroupManager.deleteMany({ where: { groupId, userId } });
    } else {
      await tx.qceSalesGroupMember.deleteMany({ where: { groupId, userId } });
    }
    await audit(
      {
        orgId: actor.orgId,
...
export async function addAccounts(opts: { actor: SessionUser; groupId: string; accountIds: string[] }) {
  ...
  return prisma.$transaction(async (tx) => {
    const grp = await tx.qceSalesGroup.findFirst({ where: { id: groupId, orgId: actor.orgId } });
    if (!grp) throw new SettingsConflictError("Sales group not found", 404);
    const inOrg = await tx.qceAccount.count({ where: { orgId: actor.orgId, id: { in: accountIds } } });
    if (inOrg !== accountIds.length) {
      throw new SettingsConflictError("One or more accounts are not in this org");
    }
    await tx.qceSalesGroupAccount.createMany({
...
export async function removeAccount(opts: { actor: SessionUser; groupId: string; accountId: string }) {
  ...
  return prisma.$transaction(async (tx) => {
    await tx.qceSalesGroupAccount.deleteMany({ where: { groupId, accountId } });
    await audit(
      {
        orgId: actor.orgId,
        userId: actor.userId,
