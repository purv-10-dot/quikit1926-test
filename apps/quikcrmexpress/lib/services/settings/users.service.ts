 ...
 *
 * Native invites hash a per-invite random password (generateTempPassword) and send
...
import { invalidate } from "@quikit/auth/cache";
...
import { ensureQuikcrmexpressAppAccess, getQuikcrmexpressAppId } from "@/lib/api/quikcrmexpress-app";
...
export async function createUser(opts: {
  ...
  // Per-invite random password. Previously a hardcoded constant shared by every
  // invited user across every org — and it is hashed into the PLATFORM-WIDE
  // auth.User row, so one known value was a credential-spraying vector against
  // the central auth host. generateTempPassword() is the shared 12-char
  // crypto.randomInt generator already used by quikcrm.
  ...
  const tenantTemplates = await prisma.qcePermissionTemplate.findMany({
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (templateIds.length > 0) {
      await tx.qceUserPermissionTemplate.createMany({
    ...
    if (data.allowedAccountIds.length > 0) {
      await tx.qceUserAccountAccess.createMany({
  ...
  // Drop the cached membership lookup for the invitee. readSession() caches
  // {role,status} for 60s under `crmexpress:membership:<user>:<org>`, and a
  // miss caches `null` — so without this an invited user's very first request
  // could read a stale "no membership" and be bounced to /login for up to a
  // minute after being granted access.
  await invalidate(`crmexpress:membership:${newUserId}:${actor.orgId}`);

  await ensureQuikcrmexpressAppAccess({
    userId: newUserId,
    orgId: actor.orgId,
  ...
  if (shouldSendInviteEmail({ linkExistingUserId: data.linkExistingUserId, invitationToken })) {
    try {
      const [org, inviter, appRow] = await Promise.all([
        ...
        getQuikcrmexpressAppId().then((appId) =>
          appId
  ...
  const tempPassword = newUserCreated ? generatedTempPassword : null;
...
export async function updateUser(opts: {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (patch.permissionTemplateIds !== undefined) {
      await tx.qceUserPermissionTemplate.deleteMany({ where: { userId: id } });
    ...
    if (patch.allowedAccountIds !== undefined) {
      await tx.qceUserAccountAccess.deleteMany({ where: { userId: id } });
...
export async function deleteUser(opts: { actor: SessionUser; id: string }) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const [leadCount, accountCount, oppCount] = await Promise.all([
      tx.qceLead.count({ where: { orgId: actor.orgId, ownerId: id } }),
      tx.qceAccount.count({ where: { orgId: actor.orgId, ownerId: id } }),
      tx.qceOpportunity.count({ where: { orgId: actor.orgId, ownerId: id } }),
    ]);
    ...
    if (isCrmRbacClientReady()) {
      ...
      await rbacTx.qceUserAppRole.deleteMany({ where: { userId: id, orgId: actor.orgId } });
      await rbacTx.qceUserPermissionExtra.deleteMany({ where: { userId: id, orgId: actor.orgId } });
    }
