/**
 * CRUD + mapping service for QceLeadStatus / QceLeadSubStatus.
 *
 * Storage: app_quikcrm.QceLeadStatus / QceLeadSubStatus / QceLeadStatusSubStatus
...
export async function getLeadStatuses(): Promise<LeadStatusDto[]> {
  const rows = await prisma.qceLeadStatus.findMany({
...
export async function createLeadStatus(
  ...
  const existing = await prisma.qceLeadStatus.findUnique({ where: { name: trimmed } });
  if (existing) throw new LeadStatusError(`Status "${trimmed}" already exists`, 409);

  const created = await prisma.qceLeadStatus.create({
    data: {
      name: trimmed,
...
export async function updateLeadStatus(
  ...
  const current = await prisma.qceLeadStatus.findUnique({ where: { id } });
  ...
  if (data.name !== undefined) {
    ...
    if (trimmed !== current.name) {
      const conflict = await prisma.qceLeadStatus.findUnique({ where: { name: trimmed } });
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (data.subStatusIds !== undefined) {
      await tx.qceLeadStatusSubStatus.deleteMany({ where: { leadStatusId: id } });
      if (data.subStatusIds.length) {
        await tx.qceLeadStatusSubStatus.createMany({
    ...
    if (data.name !== undefined) {
      return tx.qceLeadStatus.update({
        where: { id },
        data: { name: data.name.trim() },
        include: STATUS_INCLUDE,
      });
    }
    return tx.qceLeadStatus.findUniqueOrThrow({ where: { id }, include: STATUS_INCLUDE });
  });
...
export async function deleteLeadStatus(id: string): Promise<void> {
  ...
  await prisma.qceLeadStatus.delete({ where: { id } });
}

// ─── Lead Sub-Status ──────────────────────────────────────────────────────────

export async function getLeadSubStatuses(): Promise<LeadSubStatusDto[]> {
  const rows = await prisma.qceLeadSubStatus.findMany({
...
export async function createLeadSubStatus(
  ...
  const existing = await prisma.qceLeadSubStatus.findUnique({ where: { name: trimmed } });
  if (existing) throw new LeadStatusError(`Sub-status "${trimmed}" already exists`, 409);

  const created = await prisma.qceLeadSubStatus.create({
    data: {
      name: trimmed,
...
export async function updateLeadSubStatus(
  ...
  const current = await prisma.qceLeadSubStatus.findUnique({ where: { id } });
  ...
  if (data.name !== undefined) {
    ...
    if (!trimmed) throw new LeadStatusError("Name is required");
    if (trimmed !== current.name) {
      const conflict = await prisma.qceLeadSubStatus.findUnique({ where: { name: trimmed } });
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (data.statusIds !== undefined) {
      await tx.qceLeadStatusSubStatus.deleteMany({ where: { leadSubStatusId: id } });
    ...
    if (data.name !== undefined) {
      return tx.qceLeadSubStatus.update({
        where: { id },
        data: { name: data.name.trim() },
        include: SUB_STATUS_INCLUDE,
      });
    }
    return tx.qceLeadSubStatus.findUniqueOrThrow({ where: { id }, include: SUB_STATUS_INCLUDE });
  });
...
export async function deleteLeadSubStatus(id: string): Promise<void> {
  const current = await prisma.qceLeadSubStatus.findUnique({ where: { id } });
  if (!current) throw new LeadStatusError("Sub-status not found", 404);
  await prisma.qceLeadSubStatus.delete({ where: { id } });
}
