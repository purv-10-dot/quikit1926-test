...
function coerce(def: LeadFieldDefinition, raw: unknown): unknown {
  switch (def.fieldType) {
    ...
    case "Date": {
      ...
      if (!d || Number.isNaN(d.getTime())) throw new Error(`${def.label} must be a valid date`);
...
function coercePhone(def: LeadFieldDefinition, raw: unknown): { countryIso2: string; dialCode: string; number: string } {
  ...
  if (raw && typeof raw === "object" && !Array.isArray(raw)) {
    ...
    if (s.startsWith("+")) {
      ...
      // Bare string (no leading +). Strip non-digits, then remove an Indian
      // country-code prefix ONLY when doing so leaves a clean 10-digit number
      // (i.e. 12 digits total = 91 + 10). This handles LSQ/import values like
      // "91 9810628950" or "919810628950" without damaging a genuine 10-digit
      // number that happens to start with 91 (e.g. 9123456789 stays untouched).
...
/**
 * Parse a date from the formats CrmExpress's imports actually receive.
 *
 * LeadSquared (and most Indian tools) export DAY-FIRST: "28-07-2026" or
 * "28-07-2026 11:55". JavaScript's `new Date()` cannot parse day-first strings
 * (it reads month-first and silently mis-parses or rejects), so we parse
 * day-first formats explicitly here and fall back to native parsing for ISO /
 * standard inputs. Storage is unchanged (callers still store the ISO string);
 * only INPUT parsing is broadened.
 *
 * Accepts: Date objects; "DD-MM-YYYY" / "DD/MM/YYYY" with optional
 * "HH:MM[:SS]"; and anything `new Date()` already handles (ISO 8601, etc.).
 * Returns null for unparseable input (caller raises the field error).
 *
 * Day-first is assumed because CrmExpress's data is Indian-sourced. If a
 * month-first source is ever onboarded, revisit this assumption.
 */
function parseFlexibleDate(raw: unknown): Date | null {
  ...
  // Day-first: DD-MM-YYYY or DD/MM/YYYY, optional time HH:MM[:SS].
  const m = s.match(
  ...
  if (m) {
    ...
    // Reject impossible day/month so a genuine month-first string surfaces as an
    // error rather than being silently misread as the wrong calendar date.
    if (month < 1 || month > 12 || day < 1 || day > 31) return null;
    // Construct in UTC so the server's local timezone can't shift the calendar
    // date. LSQ times carry no timezone; UTC-anchoring keeps the date stable.
    return new Date(
      Date.UTC(Number(yyyy), month - 1, day, Number(hh), Number(min), Number(ss)),
    );
  }

  // Fallback: native parsing handles ISO 8601 and other standard formats.
