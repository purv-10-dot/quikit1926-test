...
function toLeadInput(lead: QceLead): LeadScoringLeadInput {
  return {
    name: lead.name,
    email: lead.email,
    phone: lead.phone,
...
export async function loadLeadScoringContext(
  ...
  const [activitiesCount, callsCount, notesCount, tasks, lastActivity, lastCall] =
    await Promise.all([
      ...
      prisma.qceCallLog.count({ where: { orgId, leadId } }),
      prisma.qceNote.count({ where: { orgId, OR: relatedOr } }),
