 ...
 *
 * Why capped DB-distinct: CrmExpress has ZERO Select/MultiSelect fields — every
 ...
 * Chosen from CrmExpress's real data: enum fields cluster at <=41 distinct, the
...
async function readPipeline(
  ...
  const ws = await prisma.qceOrgWorkspaceSettings.findUnique({ where: { orgId } });
...
async function distinctDynamicValues(orgId: string, key: string): Promise<string[]> {
  const rows = await prisma.$queryRaw<{ v: string | null; n: number }[]>`
    ...
    FROM app_quikcrmexpress."CrmLead"
    WHERE "orgId" = ${orgId}
...
/**
 * Real (non-dynamicFields) QceLead columns that are enum-ish enough to offer a
...
export async function resolveFieldValues(
  ...
  //    used yet). CrmExpress has none of these today, but it's the right priority.
