 ...
 * Primary: QceCompanyProfile (Settings → Company).
...
export async function getTenantCompanyBranding(
  ...
  const [profile, org] = await Promise.all([
    db.qceCompanyProfile.findUnique({
      where: { orgId },
...
export async function getCompanyProfileForSettings(
  ...
  const [profile, org] = await Promise.all([
    db.qceCompanyProfile.findUnique({ where: { orgId } }),
...
export async function upsertCompanyProfile(
  ...
  const row = await db.qceCompanyProfile.upsert({
    where: { orgId },
    create: {
      orgId,
