 ...
 *   1. Persists the row to QceNotification (awaited — we want durability).
...
export async function createNotification(
  ...
  const row = await prisma.qceNotification.create({
    data: {
      orgId: payload.orgId,
...
export async function getNotifications(
  ...
  const rows = await prisma.qceNotification.findMany({
    where: {
      orgId,
      userId,
  ...
  const unread = await prisma.qceNotification.count({
...
export async function getUnreadCount(
  ...
  return prisma.qceNotification.count({
    where: { orgId, userId, readAt: null },
  });
}
...
export async function markAllRead(
  ...
  const result = await prisma.qceNotification.updateMany({
    where: { orgId, userId, readAt: null },
