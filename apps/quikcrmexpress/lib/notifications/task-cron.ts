 ...
 * QceNotification.metadata column (NOT raw `@>` SQL) so the physical column
 ...
 * which checks the QceNotification.metadata column with Prisma's typed JSON
...
export async function runEveningTaskNotifications(): Promise<EveningSweepResult> {
  ...
  const overdueTasks = await prisma.qceTask.findMany({
    where: {
      status: { notIn: ["Completed", "Cancelled"] },
...
async function hasNotificationBeenSent(
  ...
  const count = await prisma.qceNotification.count({
    where: {
      orgId,
      userId,
