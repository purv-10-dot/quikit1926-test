...
export async function ensureTableExists(): Promise<void> {
  ...
  await prisma.$executeRawUnsafe(`
    CREATE TABLE IF NOT EXISTS app_quikcrmexpress.crm_notification_rule (
  ...
  await prisma.$executeRawUnsafe(`
    CREATE INDEX IF NOT EXISTS idx_crm_notification_rule_lookup
      ON app_quikcrmexpress.crm_notification_rule(org_id, entity_type, is_active)
  `);
...
export async function listRules(orgId: string): Promise<NotificationRule[]> {
  ...
  const rows = await prisma.$queryRaw<RuleRow[]>`
    SELECT * FROM app_quikcrmexpress.crm_notification_rule
    WHERE org_id = ${orgId}
...
export async function createRule(input: CreateRuleInput): Promise<NotificationRule> {
  await ensureTableExists();
  const rows = await prisma.$queryRaw<RuleRow[]>`
    INSERT INTO app_quikcrmexpress.crm_notification_rule
...
export async function updateRule(
  ...
  const rows = await prisma.$queryRaw<RuleRow[]>`
    UPDATE app_quikcrmexpress.crm_notification_rule
...
export async function deleteRule(orgId: string, id: string): Promise<boolean> {
  ...
  await prisma.$executeRaw`
    DELETE FROM app_quikcrmexpress.crm_notification_rule
    WHERE id = ${id} AND org_id = ${orgId}
  `;
  return true;
}
...
export async function countRules(orgId: string): Promise<number> {
  ...
  const rows = await prisma.$queryRaw<[{ count: bigint }]>`
    SELECT COUNT(*) as count FROM app_quikcrmexpress.crm_notification_rule
    WHERE org_id = ${orgId}
  `;
