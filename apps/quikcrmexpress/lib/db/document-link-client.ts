...
export type DocumentLinkDelegate = NonNullable<(typeof prisma)["qceDocumentLink"]>;
...
function warnDelegateMissing(): void {
  ...
  console.warn(
    "[quikcrmexpress] prisma.qceDocumentLink delegate missing. From repo root: npm run db:generate — then restart the dev server.",
  );
}

/** Undefined when Prisma client was generated before QceDocumentLink or dev server is stale. */
export function getDocumentLinkClient(): DocumentLinkDelegate | undefined {
  const client = (prisma as { qceDocumentLink?: DocumentLinkDelegate }).qceDocumentLink;
...
function warnMissingTable(): void {
  ...
  console.warn(
    "[quikcrmexpress] app_quikcrmexpress.CrmDocumentLink table missing. Apply migration: npm run db:migrate:crm-document-links — or run scripts/apply-crm-document-links.sql",
  );
}
