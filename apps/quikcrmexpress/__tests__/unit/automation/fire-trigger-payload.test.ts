...
import type { QceWorkflowDefinition, QceLead } from "@quikit/database";
...
describe("fireTrigger · trigger-time snapshot payload", () => {
  it("enqueues with triggerEventId + triggerType + triggerSnapshot", async () => {
    ...
    db.qceLead.findFirst.mockResolvedValue(
      { id: "L1", orgId: "t1", stage: "New Lead", substatus: "Negotiation" } as unknown as QceLead,
    );
