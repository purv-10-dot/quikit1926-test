...
beforeEach(() => {
  ...
  db.qceOutboundMessageLog.create.mockReset();
  db.qceOutboundMessageLog.create.mockResolvedValue({ id: "log-1" } as never);
});
...
describe("executeSendEmail · orchestration", () => {
  it("renders merge fields then queues + dispatches a mailable lead", async () => {
    ...
    expect(db.qceOutboundMessageLog.create).toHaveBeenCalledWith({
