/**
 * [P3.B1] Email dispatch — consume a queued QceOutboundMessageLog row → send →
...
beforeEach(() => {
  send.mockReset();
  db.qceOutboundMessageLog.findFirst.mockReset();
  db.qceOutboundMessageLog.update.mockReset();
  db.qceOutboundMessageLog.update.mockResolvedValue({} as never);
});

describe("dispatchOutboundMessage · B1", () => {
  it("dispatches a queued email and transitions the row queued→sent", async () => {
    db.qceOutboundMessageLog.findFirst.mockResolvedValue(queuedRow() as never);
    ...
    expect(db.qceOutboundMessageLog.update).toHaveBeenCalledWith({
      where: { id: "log-1" },
  ...
  it("skips a row that is not queued — never double-sends", async () => {
    db.qceOutboundMessageLog.findFirst.mockResolvedValue(queuedRow({ status: "sent" }) as never);
    const res = await dispatchOutboundMessage("t1", "log-1");
    expect(res.outcome).toBe("skipped");
    expect(send).not.toHaveBeenCalled();
    expect(db.qceOutboundMessageLog.update).not.toHaveBeenCalled();
  });

  it("skips (does not send) when the row is missing for the tenant", async () => {
    db.qceOutboundMessageLog.findFirst.mockResolvedValue(null as never);
  ...
  it("is tenant-scoped — loads the row with a tenantId filter", async () => {
    ...
    expect(db.qceOutboundMessageLog.findFirst).toHaveBeenCalledWith({
