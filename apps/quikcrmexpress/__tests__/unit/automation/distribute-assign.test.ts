...
function lead(overrides: Record<string, unknown> = {}): QceLead {
  return { id: "lead-1", orgId: "t1", stage: "New", status: "Open", ownerId: "u0", ...overrides } as unknown as QceLead;
}
...
describe("workflow-engine · distribute_lead rewire", () => {
  ...
  beforeEach(() => {
    ...
    db.qceAutomationDistributionState.upsert.mockResolvedValue({ lastIndex: 0 } as never);
  });
  ...
  it("routes the owner change through applyAutomatedLeadWrite (field ownerId), never a raw update", async () => {
    ...
    db.qceLead.findFirst.mockResolvedValue(lead({ ownerId: "u0" }) as never);
  ...
  it("does not write when nothing is assignable (no rule matched, no default)", async () => {
    ...
    db.qceLead.findFirst.mockResolvedValue(lead({ stage: "Cold" }) as never);
