...
describe("pipeline-by-stage", () => {
  beforeEach(() => {
    asMock(db.qceOpportunity.groupBy).mockReset();
  });

  it("rows total matches the sum of input groupBy counts", async () => {
    asMock(db.qceOpportunity.groupBy).mockResolvedValueOnce([
...
describe("stuck-deals", () => {
  ...
  it("filters at the 30-day threshold (lastStageChangeAt < cutoff)", async () => {
    db.qceOpportunity.findMany.mockResolvedValueOnce([] as never);

    await reportById("stuck-deals").run(ctx({ tz: "UTC" }));

    expect(db.qceOpportunity.findMany).toHaveBeenCalled();
    const call = db.qceOpportunity.findMany.mock.calls[0]?.[0];
...
describe("team-disposition-mix", () => {
  beforeEach(() => {
    db.qceSalesGroupManager.findMany.mockReset();
    db.qceSalesGroupMember.findMany.mockReset();
    asMock(db.qceCallLog.groupBy).mockReset();
  });

  it("returns empty rows when the caller manages no sales group", async () => {
    db.qceSalesGroupManager.findMany.mockResolvedValueOnce([] as never);

    const result = await reportById("team-disposition-mix").run(ctx());
    expect(result.rows).toEqual([]);
    expect(db.qceCallLog.groupBy).not.toHaveBeenCalled();
  });

  it("only counts dispositions for the manager's direct reports", async () => {
    db.qceSalesGroupManager.findMany.mockResolvedValueOnce([
      { groupId: "g1" },
    ] as never);
    db.qceSalesGroupMember.findMany.mockResolvedValueOnce([
      { userId: "report-1" },
      { userId: "report-2" },
    ] as never);
    asMock(db.qceCallLog.groupBy).mockResolvedValueOnce([
    ...
    const call = asMock(db.qceCallLog.groupBy).mock.calls[0]?.[0];
    const where = call!.where as Record<string, unknown>;
