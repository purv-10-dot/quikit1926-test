...
describe("generateQuotePdfSnapshot", () => {
  it("writes CrmActivity.detailNotes (filename) instead of body", async () => {
    txMock.qceQuotePdfSnapshot.create.mockResolvedValueOnce({ id: "snap1" });
    txMock.qceQuote.update.mockResolvedValueOnce(undefined);
    txMock.qceActivity.create.mockResolvedValueOnce(undefined);
    ...
    expect(txMock.qceActivity.create).toHaveBeenCalledTimes(1);
    const activityArg = txMock.qceActivity.create.mock.calls[0]![0];
