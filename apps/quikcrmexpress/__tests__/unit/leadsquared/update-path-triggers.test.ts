...
import type { QceLead, QceWorkflowDefinition } from "@quikit/database";
...
describe("workflow-engine · distribute_lead (owner update)", () => {
  it("triggers outbound sync after the owner update commits", async () => {
    db.qceWorkflowDefinition.findFirst.mockResolvedValue({
      id: "wf1",
      orgId: "t1",
      status: "Active",
    ...
    db.qceLead.findFirst.mockResolvedValue({ id: "lead-1", orgId: "t1", ownerId: null } as unknown as QceLead);
    db.qceLead.update.mockResolvedValue({} as never);
...
describe("convert route (transaction path)", () => {
  ...
  it("triggers outbound sync AFTER the transaction commits (successful convert)", async () => {
    db.qceLead.findFirst.mockResolvedValue(lead as unknown as QceLead);
...
describe("form-rule-apply · applyFormRules (set_stage → status/substatus)", () => {
  it("triggers outbound sync after the status update", async () => {
    db.qceFormRule.findMany.mockResolvedValue([] as never); // rules unused (evaluator mocked)
    db.qceLead.findUnique.mockResolvedValue({ stage: "New", status: "New", substatus: null } as never);
    db.qceLead.update.mockResolvedValue({} as never);
...
describe("bulk-assign route (updateMany fan-out)", () => {
  it("triggers outbound sync ONCE PER reassigned lead", async () => {
    ...
    ] as unknown as QceLead[]);
    db.$transaction.mockResolvedValue([{ count: 2 }, {}] as never);
    db.qceAuditLog.create.mockResolvedValue({} as never); // recordLeadChange
...
describe("bulk import (upsertImportedLeadRow · dedupe update)", () => {
  it("triggers outbound sync after an externalId-match UPDATE", async () => {
    db.qceLead.findUnique.mockResolvedValue({ id: "imp-1", dynamicFields: null } as never);
    db.qceLead.update.mockResolvedValue({ id: "imp-1", orgId: "t1" } as never);
  ...
  it("triggers outbound sync after an email/phone dedupe-match UPDATE", async () => {
    ...
    db.qceLead.findUnique.mockResolvedValue({ dynamicFields: null } as never); // dupe.id fetch
    db.qceLead.update.mockResolvedValue({ id: "imp-2", orgId: "t1" } as never);
