...
/** Build a QceLead-shaped stub from a partial (narrowed through unknown, no `as any`). */
function leadRow(partial: Partial<QceLead>): QceLead {
  return { id: LEAD_ID, orgId: TENANT, name: "Ada", deletedAt: null, ...partial } as unknown as QceLead;
}
...
describe("processLeadSquaredSyncJob", () => {
  it("re-reads the lead (tenant-scoped) and forwards args to syncLeadOutbound", async () => {
    ...
    db.qceLead.findFirst.mockResolvedValue(lead);

    await processLeadSquaredSyncJob(job(), deps);

    expect(db.qceLead.findFirst).toHaveBeenCalledWith({
  ...
  it("serializes work under a per-lead lock keyed by tenant+lead", async () => {
    ...
    db.qceLead.findFirst.mockResolvedValue(leadRow({ id: LEAD_ID }));
  ...
  it("skips a soft-deleted lead", async () => {
    const { db, sync, deps } = setup();
    db.qceLead.findFirst.mockResolvedValue(leadRow({ deletedAt: new Date() }));
  ...
  it("propagates the job's origin (always 'crm' from this queue)", async () => {
    const { db, sync, deps } = setup();
    db.qceLead.findFirst.mockResolvedValue(leadRow({}));
