...
describe("syncLeadOutbound", () => {
  it("pushes a fresh lead and stores the returned ProspectId", async () => {
    ...
    db.qceLeadSquaredSyncMap.findFirst.mockResolvedValue(null); // no mapping yet
    const input = lead();
    ...
    expect(db.qceLeadSquaredSyncMap.upsert).toHaveBeenCalledOnce();
    const arg = db.qceLeadSquaredSyncMap.upsert.mock.calls[0][0];
  ...
  it("still rethrows a non-P2002 mapping error", async () => {
    const { db, deps } = setup();
    db.qceLeadSquaredSyncMap.findFirst.mockResolvedValue(null);
    db.qceLeadSquaredSyncMap.upsert.mockRejectedValue(new Error("connection reset"));
  ...
  it("UPDATES by id (Lead.Update) when a ProspectId is known — never CreateOrUpdate", async () => {
    ...
    expect(db.qceLeadSquaredSyncMap.upsert.mock.calls[0][0].update.lastPayloadHash).toBe(
      expectedHash(input),
    );
  });
  ...
  it("CREATES via CreateOrUpdate on first-time sync (no mapping) and stores the returned id", async () => {
    ...
    expect(db.qceLeadSquaredSyncMap.upsert.mock.calls[0][0].create.lsqProspectId).toBe("PID-1");
  });
  ...
  it("recovers a first-time duplicate-email: resolve by email -> Lead.Update -> persist the id", async () => {
    ...
    expect(db.qceLeadSquaredSyncMap.upsert.mock.calls[0][0].create.lsqProspectId).toBe("PID-FOUND");
  });
