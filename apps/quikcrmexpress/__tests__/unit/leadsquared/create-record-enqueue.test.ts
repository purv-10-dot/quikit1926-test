...
import type { QceLead, Prisma, PrismaClient } from "@quikit/database";
...
vi.mock("@/lib/db/prisma", () => ({
  prisma: { qceLead: { create: h.create, update: h.update } },
}));
...
const CREATE_DATA = { orgId: TENANT, name: "Ada" } as unknown as Prisma.QceLeadUncheckedCreateInput;
const leadRow = (p: Partial<QceLead> = {}): QceLead =>
  ({ id: "lead-1", orgId: TENANT, name: "Ada", deletedAt: null, ...p }) as unknown as QceLead;
...
describe("inbound webhook — must NOT enqueue (loop guard)", () => {
  it("processInboundWebhook writes the lead but never calls the outbound enqueue", async () => {
    ...
    db.qceLead.upsert.mockResolvedValue(leadRow({ id: "lead-inbound" }));
