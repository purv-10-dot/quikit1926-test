...
describe("triggerOutboundSync (shared helper)", () => {
  it("enqueues an outbound sync with the given tenantId + crmLeadId", () => {
    triggerOutboundSync({ orgId: "t1", crmLeadId: "lead-1" });
    expect(h.enqueue).toHaveBeenCalledWith({ orgId: "t1", crmLeadId: "lead-1" });
  });
  ...
  it("is fire-and-forget: returns void and never blocks the caller", () => {
    ...
    expect(triggerOutboundSync({ orgId: "t1", crmLeadId: "lead-1" })).toBeUndefined();
  });
});
