...
beforeAll(async () => {
  const lead = await integrationPrisma.qceLead.create({
    data: { orgId: TENANT, name: "Stage 3-A Lead" },
  });
  leadId = lead.id;
  ...
  const disp = await integrationPrisma.qceCallDisposition.create({
    data: { orgId: TENANT, code: "interested_s3a", label: "Interested", name: "Interested" },
  });
