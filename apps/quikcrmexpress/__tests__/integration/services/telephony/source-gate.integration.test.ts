/**
 * FR-RE Stage 3-D(a) — a QceCallLog row must mean a real call happened.
 *
 * The save path is gated on an EXPLICIT source signal (not the providerCallSid
 * proxy): source="dialer" => a real call => write the QceCallLog row as today;
 ...
 *
 * RED until Stage 3-D(a): createCallLog writes the QceCallLog row
...
beforeAll(async () => {
  manualLeadId = (await integrationPrisma.qceLead.create({
    data: { orgId: TENANT, name: "Manual Lead", phone: "+919999999998", stage: "Old", status: "Old" },
  })).id;
  dialerLeadId = (await integrationPrisma.qceLead.create({
...
describe("FR-RE Stage 3-D(a) — call-log row gated on explicit source", () => {
  it("manual save writes NO CrmCallLog row, but DOES write the activities + move the lead", async () => {
    ...
    const callLogs = await integrationPrisma.qceCallLog.count({
    ...
    const callAct = await integrationPrisma.qceActivity.findFirst({
    ...
    const lead = await integrationPrisma.qceLead.findUnique({
