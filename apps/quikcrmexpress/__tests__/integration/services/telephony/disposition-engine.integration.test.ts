 ...
 *           QceActivity.occurredAt in the actual database row — not server now.
...
beforeAll(async () => {
  const disposition = await integrationPrisma.qceCallDisposition.create({
...
describe("FR-D2 — Activity DateTime wiring (integration)", () => {
  it("INT-D2-1: CrmActivity.occurredAt in the database equals the supplied activityDateTime", async () => {
    ...
    const activity = await integrationPrisma.qceActivity.findFirst({
  ...
  it("INT-D2-2: a future activityDateTime is rejected before any DB write", async () => {
    const before = await integrationPrisma.qceActivity.count({
      where: { orgId: TENANT },
    });
    ...
    const after = await integrationPrisma.qceActivity.count({
      where: { orgId: TENANT },
    });
