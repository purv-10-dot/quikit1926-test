...
afterAll(async () => {
  ...
  await integrationPrisma.qceFormSection.deleteMany({ where: { tab: { formSetVersionId: versionId } } });
  await integrationPrisma.qceFormTab.deleteMany({ where: { formSetVersionId: versionId } });
  await integrationPrisma.qceFormSetVersion.deleteMany({ where: { id: versionId } });
...
describe("FR-RE-4 — form structure builder (integration)", () => {
  ...
  it("creates a rule_driven tab + section and places a field onto them", async () => {
    ...
    const field = await integrationPrisma.qceFormField.create({
      data: {
        formSetVersionId: versionId,
