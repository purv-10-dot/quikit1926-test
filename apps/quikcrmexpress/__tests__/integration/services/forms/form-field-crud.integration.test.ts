...
afterAll(async () => {
  const tabs = await db.qceFormTab.findMany({ where: { formSetVersionId: versionId }, select: { id: true } });
  const fields = await db.qceFormField.findMany({ where: { formSetVersionId: versionId }, select: { id: true } });
...
describe("field CRUD on a draft", () => {
  it("creates a dropdown field with options", async () => {
    ...
    expect(await db.qceFormFieldOption.count({ where: { formFieldId: field.id } })).toBe(2);
  });
  ...
  it("deletes a non-protected field (and its options)", async () => {
    ...
    expect(await db.qceFormField.findUnique({ where: { id: field.id } })).toBeNull();
  });

  it("refuses to delete a protected field", async () => {
    const prot = await db.qceFormField.findFirst({ where: { formSetVersionId: versionId, isProtected: true } });
