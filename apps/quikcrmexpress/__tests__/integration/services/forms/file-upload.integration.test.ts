 ...
 *
 *   AC-RE-7  — a valid upload records a QceFileAttachment and points the
 *              field value (QceFieldValue.valueFileId) at it.
...
// Restricted agent: scoped (via QceUserAccountAccess) to account B only.
...
beforeAll(async () => {
  const [accA, accB] = await Promise.all([
    integrationPrisma.qceAccount.create({ data: { orgId: TENANT, name: `Acct A ${STAMP}` } }),
    integrationPrisma.qceAccount.create({ data: { orgId: TENANT, name: `Acct B ${STAMP}` } }),
  ]);
  accountAId = accA.id;
  accountBId = accB.id;

  // Lead + activity live on account A.
  const leadA = await integrationPrisma.qceLead.create({
    data: { orgId: TENANT, name: `Lead A ${STAMP}`, accountId: accountAId },
  });
  leadAId = leadA.id;
  const activityA = await integrationPrisma.qceActivity.create({
  ...
  await integrationPrisma.qceUserAccountAccess.create({
...
afterAll(async () => {
  await integrationPrisma.qceFieldValue.deleteMany({ where: { orgId: TENANT } });
  await integrationPrisma.qceFileAttachment.deleteMany({ where: { orgId: TENANT } });
  ...
  await integrationPrisma.qceActivity.deleteMany({ where: { orgId: TENANT } });
  await integrationPrisma.qceLead.deleteMany({ where: { orgId: TENANT } });
  await integrationPrisma.qceUserAccountAccess.deleteMany({ where: { userId: agentB.userId } });
  await integrationPrisma.qceAccount.deleteMany({ where: { orgId: TENANT } });
});
...
describe("recordDispositionAttachment — AC-RE-7 (valid upload recorded)", () => {
  it("writes a CrmFileAttachment and points the field value at it", async () => {
    ...
    const fv = await integrationPrisma.qceFieldValue.findUnique({
...
describe("saveDispositionUpload — AC-RE-8 (no partial save on reject)", () => {
  it("rejects an oversize file and writes NOTHING", async () => {
    const before = await integrationPrisma.qceFileAttachment.count({ where: { orgId: TENANT } });
    ...
    const after = await integrationPrisma.qceFileAttachment.count({ where: { orgId: TENANT } });
