...
beforeAll(async () => {
  ...
  setId = set.id;
  versionId = (await db.qceFormSetVersion.create({
...
describe("user_picker field config — scope + mode persist and reach the runtime", () => {
  it("createFormField persists userPickerScope + userPickerMode", async () => {
    const field = await db.qceFormField.findFirst({ where: { formSetVersionId: versionId, fieldKey: "assignees" } });
  ...
  it("CONTAINMENT: a non-user_picker field leaves scope/mode null", async () => {
    const field = await db.qceFormField.findFirst({ where: { formSetVersionId: versionId, fieldKey: "remark" } });
