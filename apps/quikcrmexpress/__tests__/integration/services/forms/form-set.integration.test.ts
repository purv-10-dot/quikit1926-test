...
afterAll(async () => {
  ...
  if (rules.length) {
    await db.qceFormRuleAction.deleteMany({ where: { formRuleId: { in: rules.map((r) => r.id) } } });
    await db.qceFormRuleCondition.deleteMany({ where: { formRuleId: { in: rules.map((r) => r.id) } } });
    await db.qceFormRule.deleteMany({ where: { id: { in: rules.map((r) => r.id) } } });
  }
  if (fields.length) await db.qceFormFieldOption.deleteMany({ where: { formFieldId: { in: fields.map((f) => f.id) } } });
  await db.qceFormField.deleteMany({ where: { formSetVersionId: { in: vids } } });
  if (tabs.length) await db.qceFormSection.deleteMany({ where: { formTabId: { in: tabs.map((t) => t.id) } } });
...
describe("createFormSet + listFormSets", () => {
  it("creates a default call_disposition set, a v1 draft, and seeds the protected tab + 4 fields", async () => {
    const { set, version } = await createFormSet({ orgId: TENANT, name: `CrmExpress ${STAMP}` });
    setId = set.id;
    ...
    const tabs = await db.qceFormTab.findMany({ where: { formSetVersionId: versionId } });
    expect(tabs).toHaveLength(1);
    expect(tabs[0]!.isProtected).toBe(true);
    const protectedFields = await db.qceFormField.count({ where: { formSetVersionId: versionId, isProtected: true } });
