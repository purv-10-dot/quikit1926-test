...
// Globally-unique status names (QceLeadStatus has no orgId; name is @unique).
...
beforeAll(async () => {
  ...
  const status = await integrationPrisma.qceLeadStatus.create({ data: { name: STATUS } });
  statusId = status.id;
  const subA = await integrationPrisma.qceLeadSubStatus.create({ data: { name: SUB_MAPPED } });
  const subB = await integrationPrisma.qceLeadSubStatus.create({ data: { name: SUB_UNMAPPED } });
  subMappedId = subA.id;
  subUnmappedId = subB.id;
  await integrationPrisma.qceLeadStatusSubStatus.create({
...
afterAll(async () => {
  ...
  if (ids.length) {
    await integrationPrisma.qceFormRuleAction.deleteMany({ where: { formRuleId: { in: ids } } });
    await integrationPrisma.qceFormRule.deleteMany({ where: { id: { in: ids } } });
  }
  await integrationPrisma.qceLeadStatusSubStatus.deleteMany({ where: { leadStatusId: statusId } });
  await integrationPrisma.qceLeadSubStatus.deleteMany({ where: { id: { in: [subMappedId, subUnmappedId] } } });
  await integrationPrisma.qceLeadStatus.deleteMany({ where: { id: statusId } });
  await integrationPrisma.qceFormField.deleteMany({ where: { formSetVersionId: versionId } });
  await integrationPrisma.qceFormTab.deleteMany({ where: { formSetVersionId: versionId } });
