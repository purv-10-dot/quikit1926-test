...
beforeAll(async () => {
  const set = await db.qceFormSet.create({ data: { orgId: TENANT, surface: "call_disposition", name: `Set ${STAMP}` } });
  setId = set.id;
  const version = await db.qceFormSetVersion.create({ data: { formSetId: setId, versionNumber: 1, status: "draft" } });
  v1 = version.id;
  ...
  const field = await db.qceFormField.create({
  ...
  srcFieldId = field.id;
  await db.qceFormFieldOption.createMany({
    data: [
...
afterAll(async () => {
  const versions = await db.qceFormSetVersion.findMany({ where: { formSetId: setId }, select: { id: true } });
  const vids = versions.map((v) => v.id);
  const tabs = await db.qceFormTab.findMany({ where: { formSetVersionId: { in: vids } }, select: { id: true } });
  const fields = await db.qceFormField.findMany({ where: { formSetVersionId: { in: vids } }, select: { id: true } });
  const rules = await db.qceFormRule.findMany({ where: { formSetVersionId: { in: vids } }, select: { id: true } });
  ...
  if (ruleIds.length) {
    await db.qceFormRuleAction.deleteMany({ where: { formRuleId: { in: ruleIds } } });
    await db.qceFormRuleCondition.deleteMany({ where: { formRuleId: { in: ruleIds } } });
    await db.qceFormRule.deleteMany({ where: { id: { in: ruleIds } } });
  }
  if (fieldIds.length) await db.qceFormFieldOption.deleteMany({ where: { formFieldId: { in: fieldIds } } });
  await db.qceFieldValue.deleteMany({ where: { orgId: TENANT } });
  await db.qceFormField.deleteMany({ where: { formSetVersionId: { in: vids } } });
  if (tabIds.length) await db.qceFormSection.deleteMany({ where: { formTabId: { in: tabIds } } });
  await db.qceFormTab.deleteMany({ where: { formSetVersionId: { in: vids } } });
...
describe("cloneVersionToDraft — deep-copy completeness (the centerpiece)", () => {
  it("clones EVERY structural level into a new draft, and does NOT clone field values", async () => {
    ...
    const [tabs, sections, fields, rules] = await Promise.all([
      db.qceFormTab.findMany({ where: { formSetVersionId: cid } }),
      db.qceFormSection.findMany({ where: { tab: { formSetVersionId: cid } } }),
      db.qceFormField.findMany({ where: { formSetVersionId: cid } }),
      db.qceFormRule.findMany({ where: { formSetVersionId: cid }, include: { conditions: true, actions: true } }),
    ]);
    ...
    const options = await db.qceFormFieldOption.findMany({ where: { formFieldId: fields[0]!.id } });
    expect(options).toHaveLength(2);

    // field values are record/history — pinned to v1, NEVER cloned
    const clonedValues = await db.qceFieldValue.count({ where: { formSetVersionId: cid } });
    expect(clonedValues).toBe(0);
    expect(await db.qceFieldValue.count({ where: { formSetVersionId: v1 } })).toBe(1);
  });
  ...
  it("remaps intra-version references: the cloned action.targetTabId points at the NEW tab, not the source's", async () => {
    ...
    const newTab = await db.qceFormTab.findFirst({ where: { formSetVersionId: cid } });
    const action = await db.qceFormRuleAction.findFirst({ where: { rule: { formSetVersionId: cid }, actionType: "show_tab" } });
  ...
  it("the clone is independent: editing it does not affect the source version", async () => {
    ...
    expect(await db.qceFormTab.count({ where: { formSetVersionId: clone.id } })).toBe(2);
    expect(await db.qceFormTab.count({ where: { formSetVersionId: v1 } })).toBe(1); // source untouched
  });
});
...
describe("publishVersion — lifecycle (draft -> published, previous retired, currentVersion set)", () => {
  it("publishing a draft makes it current and retires the previously-published version", async () => {
    ...
    const set = await db.qceFormSet.findUnique({ where: { id: setId }, select: { currentVersionId: true } });
    expect(set!.currentVersionId).toBe(clone.id); // new current

    const prev = await db.qceFormSetVersion.findUnique({ where: { id: v1 }, select: { status: true } });
