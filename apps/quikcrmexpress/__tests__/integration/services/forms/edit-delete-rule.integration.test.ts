...
beforeAll(async () => {
  ...
  const draftSet = await db.qceFormSet.create({
    data: { orgId: TENANT, surface: "call_disposition", name: `Draft Set ${STAMP}`, isDefault: true },
  });
  draftSetId = draftSet.id;
  draftVersionId = (await db.qceFormSetVersion.create({
    data: { formSetId: draftSetId, versionNumber: 1, status: "draft" },
  })).id;
  tabA = (await db.qceFormTab.create({ data: { formSetVersionId: draftVersionId, name: "Tab A", visibility: "rule_driven", sortOrder: 0 } })).id;
  tabB = (await db.qceFormTab.create({ data: { formSetVersionId: draftVersionId, name: "Tab B", visibility: "rule_driven", sortOrder: 1 } })).id;

  // Set 2 — the publish/draft-guard case, isolated so publishing can't disturb set 1.
  const pubSet = await db.qceFormSet.create({
    data: { orgId: TENANT, surface: "call_disposition", name: `Pub Set ${STAMP}`, isDefault: false },
  });
  pubSetId = pubSet.id;
  pubVersionId = (await db.qceFormSetVersion.create({
...
afterAll(async () => {
  for (const sId of [draftSetId, pubSetId]) {
    const versions = await db.qceFormSetVersion.findMany({ where: { formSetId: sId }, select: { id: true } });
    for (const v of versions) {
      const rules = await db.qceFormRule.findMany({ where: { formSetVersionId: v.id }, select: { id: true } });
      ...
      await db.qceFormTab.deleteMany({ where: { formSetVersionId: v.id } });
    }
    await db.qceFormSet.update({ where: { id: sId }, data: { currentVersionId: null } });
    await db.qceFormSetVersion.deleteMany({ where: { formSetId: sId } });
    await db.qceFormSet.deleteMany({ where: { id: sId } });
  }
});
...
describe("Phase 2 — edit ACTION (2-B): set_stage target + show_tab tab", () => {
  it("set_stage target changes ST_A -> ST_B and persists", async () => {
    ...
    const row = await db.qceFormRuleAction.findUnique({ where: { id: action.id } });
...
describe("Phase 2 — delete cascades", () => {
  it("deleteFormRule removes the rule AND its conditions + actions", async () => {
    ...
    expect(await db.qceFormRuleCondition.findUnique({ where: { id: cond.id } })).toBeNull();
    expect(await db.qceFormRuleAction.findUnique({ where: { id: action.id } })).toBeNull();
  });
});
