...
async function newVersion(): Promise<string> {
  const v = await integrationPrisma.qceFormSetVersion.create({
...
async function newLead(status = "Open", stage = "New", substatus: string | null = null): Promise<string> {
  const lead = await integrationPrisma.qceLead.create({
...
afterAll(async () => {
  const versions = await integrationPrisma.qceFormSetVersion.findMany({
  ...
  if (rids.length) {
    await integrationPrisma.qceFormRuleAction.deleteMany({ where: { formRuleId: { in: rids } } });
    await integrationPrisma.qceFormRuleCondition.deleteMany({ where: { formRuleId: { in: rids } } });
    await integrationPrisma.qceFormRule.deleteMany({ where: { id: { in: rids } } });
  }
  await integrationPrisma.qceFieldValue.deleteMany({ where: { orgId: TENANT } });
  await integrationPrisma.qceFormField.deleteMany({ where: { formSetVersionId: { in: vids } } });
...
describe("loadEvalRules — Prisma rows -> EvalRule[] adapter", () => {
  ...
  it("excludes inactive rules from the eval set", async () => {
    ...
    await integrationPrisma.qceFormRule.update({ where: { id: rule.id }, data: { isActive: false } });
...
describe("loadEvalContext — lead state + field values -> EvalContext", () => {
  it("loads lead stage/status/sub_stage and resolves field values by type", async () => {
    ...
    await integrationPrisma.qceFieldValue.createMany({
      data: [
...
describe("applyFormRules — set_stage writes the lead's Contact Stage, one hop, NO cascade", () => {
  it("END-TO-END: applies the first set_stage and does NOT cascade into a rule keyed on the new stage", async () => {
    ...
    const lead = await integrationPrisma.qceLead.findUnique({ where: { id: leadId }, select: { stage: true } });
  ...
  it("a decision with no set_stage leaves the lead untouched and returns the visibility decision", async () => {
    ...
    const lead = await integrationPrisma.qceLead.findUnique({ where: { id: leadId }, select: { status: true } });
