...
beforeAll(async () => {
  // Two global statuses we can assert on (QceLeadStatus is global; getPipelineConfig
  // derives cfg.statuses from it).
  await db.qceLeadStatus.create({ data: { name: ST_A } });
  await db.qceLeadStatus.create({ data: { name: ST_B } });
...
afterAll(async () => {
  await db.qceOrgWorkspaceSettings.deleteMany({ where: { orgId: { in: [TENANT, SEED_TENANT] } } });
  await db.qceLeadStatus.deleteMany({ where: { name: { in: [ST_A, ST_B] } } });
});
