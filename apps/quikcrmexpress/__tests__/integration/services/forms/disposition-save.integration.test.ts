...
afterAll(async () => {
  ...
  await db.qceLead.deleteMany({ where: { orgId: TENANT } });
});
...
describe("saveDispositionFieldValues — persist typed custom field values", () => {
  it("writes each value to the right column for its field type", async () => {
    ...
    const drop = await db.qceFieldValue.findUnique({ where: { activityId_fieldKey: { activityId, fieldKey: "payment_mode" } } });
    expect(drop).toMatchObject({ valueType: "dropdown", valueText: "invoice" });

    const picker = await db.qceFieldValue.findUnique({ where: { activityId_fieldKey: { activityId, fieldKey: "verifiers" } } });
  ...
  it("ignores values for fields not defined in the version (server-authoritative)", async () => {
    ...
    expect(await db.qceFieldValue.count({ where: { activityId } })).toBe(0);
  });
});
...
describe("saveAndApplyDisposition — END-TO-END: entered field drives the rule, Contact Stage moves", () => {
  it("writes the field value, fires the rule, and moves the lead's Contact Stage", async () => {
    const lead = await db.qceLead.create({ data: { orgId: TENANT, name: `Lead ${STAMP}`, status: "Open", stage: "New" } });
    ...
    const after = await db.qceLead.findUnique({ where: { id: lead.id }, select: { stage: true } });
    ...
    const fv = await db.qceFieldValue.findUnique({ where: { activityId_fieldKey: { activityId, fieldKey: "payment_mode" } } });
...
describe("Option A ordering — FR-RE overrides the mapping when fired, legacy stands when not", () => {
  it("FR-RE OVERRIDES the (legacy-mapped) Contact Stage when a rule fires", async () => {
    const lead = await db.qceLead.create({ data: { orgId: TENANT, name: `Lead ${STAMP}-ovr`, status: "Open", stage: "MappedStage" } });
    const result = await saveAndApplyDisposition({
      orgId: TENANT,
      leadId: lead.id,
  ...
  it("the legacy-mapped Contact Stage STANDS when no FR-RE rule fires", async () => {
    const lead = await db.qceLead.create({ data: { orgId: TENANT, name: `Lead ${STAMP}-keep`, status: "Open", stage: "MappedStage" } });
    const result = await saveAndApplyDisposition({
      orgId: TENANT,
      leadId: lead.id,
