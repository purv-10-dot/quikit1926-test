 ...
 * unit-test pass:
 *
 *   INT-D3-1   Engine reads a real QceAutomationRule and writes a real status
 *              change + audit row.
...
beforeAll(async () => {
  // Create the minimal QceLead the engine will update.
  const lead = await integrationPrisma.qceLead.create({
    data: { orgId: TENANT, name: "Integration Test Lead", status: INITIAL_STATUS },
  });
  leadId = lead.id;

  // Create a stub QceActivity (engine receives its ID; doesn't create it).
  const activity = await integrationPrisma.qceActivity.create({
    data: {
      orgId: TENANT,
      type: "Call",
      relatedKind: "Lead",
  ...
  const rule = await integrationPrisma.qceAutomationRule.create({
    data: {
      orgId: TENANT,
...
describe("FR-D3 / FR-D5 — Disposition Rule Engine (integration)", () => {
  ...
  it("INT-D3-1: updates CrmLead.status in the database when disposition matches", async () => {
    ...
    const updated = await integrationPrisma.qceLead.findUnique({
      where: { id: leadId },
      select: { status: true },
    });
  ...
  it("INT-D5-1: CrmAuditLog before.status equals the lead's actual prior status (not a mutated in-memory copy)", async () => {
    ...
    // before-capture bug.
    const before = await integrationPrisma.qceLead.findUnique({
      where: { id: leadId },
      select: { status: true },
    });
    ...
    await integrationPrisma.qceAutomationRule.update({
      where: { id: ruleId },
    ...
    const auditRow = await integrationPrisma.qceAuditLog.findFirst({
      where: {
        orgId: TENANT,
        module: "leads",
