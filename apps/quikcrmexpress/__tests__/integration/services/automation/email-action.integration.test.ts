...
async function mkLead(overrides: Partial<QceLead>): Promise<QceLead> {
  return integrationPrisma.qceLead.create({
...
describe("B2 send_email action · real DB (captured transport)", () => {
  it("renders {merge} fields per-lead and sends a mailable lead", async () => {
    ...
    const row = await integrationPrisma.qceOutboundMessageLog.findUnique({ where: { id: res.logId } });
