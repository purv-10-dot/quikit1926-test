 ...
 * call that leaves the QceWorkflowDefinition row in the expected state, that
...
async function newDraft(orgId = TENANT, name = "Rule") {
  return integrationPrisma.qceWorkflowDefinition.create({
    data: { orgId, name, status: "Draft", triggerType: "trigger_lead_updated", graphNodes: [], graphEdges: [] },
  });
}
const read = (id: string) => integrationPrisma.qceWorkflowDefinition.findUnique({ where: { id } });

afterAll(async () => {
  for (const t of [TENANT, OTHER_TENANT]) {
    await integrationPrisma.qceAutomationPendingStep.deleteMany({ where: { orgId: t } });
    await integrationPrisma.qceWorkflowDefinition.deleteMany({ where: { orgId: t } });
  }
