...
beforeAll(async () => {
  await integrationPrisma.qceWorkflowDefinition.create({
    data: {
      id: WF,
      orgId: TENANT,
      name: "B3 assign",
...
afterAll(async () => {
  ...
  await integrationPrisma.qceAutomationDistributionState.deleteMany({ where: { orgId: TENANT } });
  await integrationPrisma.qceLead.deleteMany({ where: { orgId: TENANT } });
  await integrationPrisma.qceWorkflowDefinition.deleteMany({ where: { orgId: TENANT } });
...
describe("B3 distribute_lead · real DB", () => {
  it("assigns a rule-2-only lead from rule 2, records attribution + loop count, no raw update", async () => {
    ...
    const after = await integrationPrisma.qceLead.findUnique({ where: { id: lead.id } });
