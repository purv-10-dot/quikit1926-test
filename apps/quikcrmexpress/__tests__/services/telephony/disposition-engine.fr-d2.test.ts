 ...
 *
 * FR-D2-1  A supplied activityDateTime propagates to QceActivity.occurredAt.
...
function setupHappyPathMocks() {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  db.qceCallDisposition.findFirst.mockResolvedValueOnce(fakeDisposition as any);
  db.qceIndiaVoiceWebhookLog.findFirst.mockResolvedValueOnce(null);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  db.qceLead.findFirst.mockResolvedValue(fakeLead as any);
  db.qceCallLog.findFirst.mockResolvedValueOnce(null); // no orphan stub
  db.qceCallLog.create.mockResolvedValueOnce(fakeCallLog);
  db.qceActivity.create.mockResolvedValue(fakeActivity);
  // engine: no rules to fire (keeps test focused on FR-D2)
  db.qceAutomationRule.findMany.mockResolvedValue([]);
}

beforeEach(() => {
  db.qceCallDisposition.findFirst.mockReset();
  db.qceIndiaVoiceWebhookLog.findFirst.mockReset();
  db.qceLead.findFirst.mockReset();
  db.qceCallLog.findFirst.mockReset();
  db.qceCallLog.create.mockReset();
  db.qceCallLog.update.mockReset();
...
describe("FR-D2 — Activity DateTime wiring", () => {
  it("FR-D2-1: CrmActivity.occurredAt equals the supplied activityDateTime, not server now", async () => {
    ...
    const activityCreateCalls = db.qceActivity.create.mock.calls;
