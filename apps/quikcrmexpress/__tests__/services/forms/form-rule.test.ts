...
beforeEach(() => {
  ...
  db.qceFormRule.create.mockReset();
  db.qceFormRule.findUnique.mockReset();
  db.qceFormRuleCondition.create.mockReset();
  db.qceFormField.findFirst.mockReset();
});
...
describe("createFormRule / addRuleCondition — draft-only guard", () => {
  it("createFormRule creates an all-match rule on a draft version", async () => {
    ...
    db.qceFormRule.create.mockResolvedValue({ id: RULE_ID } as never);
    ...
    expect(db.qceFormRule.create).toHaveBeenCalledOnce();
    const arg = db.qceFormRule.create.mock.calls[0]![0] as { data: { matchType: string } };
  ...
  it("createFormRule REJECTS a published (frozen) version", async () => {
    ...
    expect(db.qceFormRule.create).not.toHaveBeenCalled();
  });

  it("addRuleCondition validates the condition AND checks the draft guard before writing", async () => {
    db.qceFormRule.findUnique.mockResolvedValue({ id: RULE_ID, formSetVersionId: VERSION_ID } as never);
    db.qceFormSetVersion.findUnique.mockResolvedValue(draftVersion);
    ...
    expect(db.qceFormRuleCondition.create).not.toHaveBeenCalled();
  });

  it("addRuleCondition enforces validate-at-build: a nonexistent field reference is rejected", async () => {
    db.qceFormRule.findUnique.mockResolvedValue({ id: RULE_ID, formSetVersionId: VERSION_ID } as never);
    db.qceFormSetVersion.findUnique.mockResolvedValue(draftVersion);
    db.qceFormField.findFirst.mockResolvedValue(null); // field does not exist
