...
beforeEach(() => {
  ...
  db.qceActivity.findFirst.mockReset();
  ...
  db.qceActivity.findFirst.mockResolvedValue({ id: "act_1" } as never);
});
...
describe("saveDispositionUpload ordering (AC-RE-8 + ACL-before-store)", () => {
  ...
  it("ACL-before-store: an out-of-scope activity throws BEFORE saveCrmUpload is called", async () => {
    ...
    db.qceActivity.findFirst.mockResolvedValue(null as never);
