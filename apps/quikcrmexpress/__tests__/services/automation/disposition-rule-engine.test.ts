 ...
 *   FR-D5-1   Audit log           → QceAuditLog entry with rule as actor
...
// Minimal QceLead shape — only fields the engine selects (status, name, ownerId).
// Cast as any to avoid requiring all 40+ QceLead columns in the mock fixture.
...
describe("FR-D3 — Disposition Automation Rule Engine", () => {
  beforeEach(() => {
    db.qceAutomationRule.findMany.mockReset();
    db.qceLead.findUnique.mockReset();
    db.qceLead.update.mockReset();
    db.qceAuditLog.create.mockReset();
    db.qceTask.create.mockReset();
  ...
  it("FR-D3-1: sets lead status when disposition matches trigger", async () => {
    db.qceAutomationRule.findMany.mockResolvedValueOnce([makeRule()]);
    db.qceLead.findUnique.mockResolvedValueOnce(baseLead);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    db.qceLead.update.mockResolvedValueOnce({ ...baseLead, status: "Disqualified" } as any);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    db.qceAuditLog.create.mockResolvedValueOnce({} as any);

    await runAfterActivityLogged({ ...baseCtx, dispositionCode: "not_interested" });

    expect(db.qceLead.update).toHaveBeenCalledWith(
  ...
  it("FR-D3-3: only the first matching set_lead_status fires (sortOrder 1 wins over 2)", async () => {
    db.qceAutomationRule.findMany.mockResolvedValueOnce([
    ...
    db.qceLead.findUnique.mockResolvedValueOnce(baseLead);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    db.qceLead.update.mockResolvedValueOnce({ ...baseLead, status: "Contacted" } as any);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    db.qceAuditLog.create.mockResolvedValueOnce({} as any);

    await runAfterActivityLogged({ ...baseCtx, dispositionCode: "not_interested" });

    expect(db.qceLead.update).toHaveBeenCalledTimes(1);
    expect(db.qceLead.update).toHaveBeenCalledWith(
  ...
  it("FR-D3-4: engine queries rules exactly once — rule-made change does not re-trigger", async () => {
    ...
    expect(db.qceAutomationRule.findMany).toHaveBeenCalledTimes(1);
  ...
  it("FR-D3-5: creates task due at activityDatetime with expanded title (AC-5)", async () => {
    ...
    db.qceLead.findUnique.mockResolvedValueOnce(baseLead);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    db.qceTask.create.mockResolvedValueOnce({} as any);
    ...
    expect(db.qceTask.create).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("FR-D3-7: inactive rules are not evaluated (DB filters isActive=false)", async () => {
    ...
    expect(db.qceLead.findUnique).not.toHaveBeenCalled();
    expect(db.qceLead.update).not.toHaveBeenCalled();
  });
  ...
  it("FR-D5-1: writes CrmAuditLog with ruleId in metadata for every rule-made status change", async () => {
    ...
    db.qceAutomationRule.findMany.mockResolvedValueOnce([rule]);
    db.qceLead.findUnique.mockResolvedValueOnce(baseLead);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    db.qceLead.update.mockResolvedValueOnce({ ...baseLead, status: "Disqualified" } as any);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    expect(db.qceAuditLog.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          orgId: TENANT,
          userId: null,
  ...
  it("FR-D3-M: rule with both set_lead_status and create_task fires both actions", async () => {
    ...
    expect(db.qceTask.create).toHaveBeenCalledTimes(1);
  });
});
