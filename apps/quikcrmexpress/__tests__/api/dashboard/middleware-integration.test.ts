 ...
 *   - apps/quikcrmexpress/__tests__/unit/soft-delete-middleware.test.ts (the
 *     middleware in isolation)
 *   - apps/quikcrmexpress/__tests__/unit/dashboard-soft-delete.test.ts (the
...
describe("dashboard middleware integration — counts exclude soft-deleted rows", () => {
  it("buildSummary reports active-only lead/opp/account counts", async () => {
    setImpl("qceLead", "count", activeAwareCount());
    setImpl("qceOpportunity", "count", activeAwareCount());
    setImpl("qceAccount", "count", activeAwareCount());
    setImpl("qceTask", "count", () => Promise.resolve(0));
    setImpl("qceActivity", "count", () => Promise.resolve(0));
    setImpl("qceLead", "groupBy", () => Promise.resolve([]));
    setImpl("qceOpportunity", "groupBy", () => Promise.resolve([]));
    setImpl("qceOrgWorkspaceSettings", "findUnique", () =>
