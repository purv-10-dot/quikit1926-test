...
const asLead = (partial: Partial<QceLead>): QceLead => partial as unknown as QceLead;
...
describe("POST /api/leadsquared/webhook", () => {
  it("returns 200 for an empty verification payload and does NOT process", async () => {
    ...
    expect(db.qceLeadSquaredSyncMap.findFirst).not.toHaveBeenCalled();
  ...
  it("processes inline (Redis disabled) with a valid secret and returns 200", async () => {
    ...
    db.qceLead.upsert.mockResolvedValue(asLead({ id: "lead-new", orgId: "shield" }));
    ...
    expect(db.qceLeadSquaredSyncMap.upsert).toHaveBeenCalled();
    expect(db.qceLeadSquaredSyncMap.upsert.mock.calls[0][0].create).toMatchObject({
  ...
  it("allows processing in dev when no secret is configured (not required)", async () => {
    db.qceLeadSquaredSyncMap.findFirst.mockResolvedValue(null);
    db.qceLead.upsert.mockResolvedValue(asLead({ id: "lead-dev", orgId: "shield" }));
