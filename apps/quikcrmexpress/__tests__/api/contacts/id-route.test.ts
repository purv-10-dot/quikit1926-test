...
describe("PATCH /api/contacts/[id]", () => {
  beforeEach(() => {
    db.qceContact.findFirst.mockReset();
    db.qceContact.update.mockReset();
    db.qceAccount.findMany.mockReset();
  ...
  it("rejects unknown fields via Zod (no raw body to Prisma)", async () => {
    adminSession();
    db.qceContact.findFirst.mockResolvedValue(makeContact() as never);
    ...
    expect(db.qceContact.update).not.toHaveBeenCalled();
  });

  it("returns 409 on duplicate email (excluding self)", async () => {
    adminSession();
    db.qceContact.findFirst
  ...
  it("re-resolves ownerName when ownerId changes", async () => {
    ...
    db.qceContact.update.mockResolvedValue(
    ...
    const update = db.qceContact.update.mock.calls[0]![0]!.data as {
      ownerId?: string;
      ownerName?: string;
    };
  ...
  it("normalizes a bare-digit phone to E.164 before write", async () => {
    ...
    expect(res.status).toBe(200);
    const update = db.qceContact.update.mock.calls[0]![0]!.data as { phone?: string };
...
describe("DELETE /api/contacts/[id]", () => {
  beforeEach(() => {
    db.qceContact.findFirst.mockReset();
    db.qceContact.update.mockReset();
    db.qceContact.delete.mockReset();
  ...
  it("soft-deletes by setting deletedAt", async () => {
    adminSession();
    db.qceContact.findFirst.mockResolvedValue(makeContact() as never);
    db.qceContact.update.mockResolvedValue(makeContact({ deletedAt: new Date() }) as never);
    ...
    expect(db.qceContact.update).toHaveBeenCalledWith({
      where: { id: "c1" },
      data: expect.objectContaining({ deletedAt: expect.any(Date) }),
    });
    expect(db.qceContact.delete).not.toHaveBeenCalled();
  });
});
