...
describe("POST /api/quotes/[id]/convert-to-order", () => {
  beforeEach(() => {
    ...
    db.qceOrder.findFirst.mockReset();
    db.qceQuote.findFirst.mockReset();
    db.qceSequence.upsert.mockReset();
    db.qceOrder.create.mockReset();
    db.qceOrderLine.create.mockReset();
  ...
  it("returns 404 when the quote is in another tenant", async () => {
    adminSession();
    db.qceOrder.findFirst.mockResolvedValue(null); // no existing order
    db.qceQuote.findFirst.mockResolvedValue(null); // quote not in tenant
  ...
  it("returns 409 when the quote isn't in Won status", async () => {
    adminSession();
    db.qceOrder.findFirst.mockResolvedValue(null);
    db.qceQuote.findFirst.mockResolvedValue({
      id: "q1",
  ...
  it("happy path: 201, snapshots totals, mints ORD-YYYY-NNNN, emits twin activity rows", async () => {
    ...
    db.qceOrder.create.mockResolvedValue({ id: "ord-new" } as never);
    db.qceOrderLine.create.mockResolvedValue({} as never);
    ...
    const orderCreateCall = db.qceOrder.create.mock.calls[0]![0]! as {
    ...
    expect(db.qceOrderLine.create).toHaveBeenCalledOnce();

    // Twin activity rows â€” one on the order, one on the quote.
    const activityTypes = db.qceActivity.create.mock.calls.map(
