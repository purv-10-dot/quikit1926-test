...
describe("GET /api/documents", () => {
  beforeEach(() => {
    db.qceDocument.findMany.mockReset();
    db.qceDocument.count.mockReset();
  ...
  it("returns tenant-scoped paginated list", async () => {
    adminSession();
    db.qceDocument.findMany.mockResolvedValue([
      {
        id: "d1",
        orgId: "t1",
        refType: "lead",
    ...
    db.qceDocument.count.mockResolvedValue(1);
    db.orgMember.findMany.mockResolvedValue([
      { userId: "u1", user: { firstName: "Alice", lastName: "", email: "a@b.co" } },
    ] as never);
    db.qceLead.findMany.mockResolvedValue([{ id: "l1", name: "Acme Lead" }] as never);
    ...
    expect(db.qceDocument.findMany).toHaveBeenCalledWith(
