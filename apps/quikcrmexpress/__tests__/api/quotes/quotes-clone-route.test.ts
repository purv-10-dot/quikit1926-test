...
describe("POST /api/quotes/[id]/clone", () => {
  ...
  it("returns 404 when the source quote is in another tenant", async () => {
    adminSession();
    // Service wraps the work in a $transaction whose callback uses tx.qceQuote.
    ...
    db.qceQuote.findFirst.mockResolvedValue(null);
  ...
  it("clones a quote, mints a fresh QT-YYYY-NNNN number, returns 201", async () => {
    ...
    db.qceSequence.upsert.mockResolvedValue({ counter: 11 } as never);
    db.qceQuote.create.mockResolvedValue({ id: "q-new" } as never);
    ...
    expect(db.qceActivity.create).toHaveBeenCalled();
  });
});
