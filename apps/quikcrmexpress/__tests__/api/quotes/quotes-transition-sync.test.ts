...
describe("transitionQuote â€” Opportunity probability sync (audit W-1)", () => {
  beforeEach(() => {
    ...
    db.qceQuote.update.mockReset();
    db.qceOpportunity.updateMany.mockReset();
    db.qceQuoteStatusTransition.create.mockReset();
  ...
  it("Active â†’ Won: bumps the linked Opportunity to probability=100", async () => {
    ...
    db.qceQuote.findFirst.mockResolvedValueOnce({
      id: "q1",
    ...
    db.qceOpportunity.updateMany.mockResolvedValue({ count: 1 } as never);
    // Final findFirst returns the updated quote shape (irrelevant detail).
    db.qceQuote.findFirst.mockResolvedValueOnce({ id: "q1", lines: [] } as never);
    ...
    expect(db.qceOpportunity.updateMany).toHaveBeenCalledOnce();
    const call = db.qceOpportunity.updateMany.mock.calls[0]![0]! as {
  ...
  it("skips Opportunity sync when the quote has no opportunityId", async () => {
    ...
    expect(db.qceOpportunity.updateMany).not.toHaveBeenCalled();
  });
  ...
  it("doesn't emit the OpportunityProbabilityFromQuote activity when the opp was already closed (updateMany count=0)", async () => {
    ...
    db.qceOpportunity.updateMany.mockResolvedValue({ count: 0 } as never);
    db.qceQuote.findFirst.mockResolvedValueOnce({ id: "q1", lines: [] } as never);
    ...
    const probActivities = db.qceActivity.create.mock.calls.filter(
      (c) =>
