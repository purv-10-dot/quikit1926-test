...
describe("GET /api/products", () => {
  beforeEach(() => {
    db.qceProduct.findMany.mockReset();
    db.qceProduct.count.mockReset();
    setSession(null);
  });
  ...
  it("returns the {success, data} envelope", async () => {
    adminSession();
    db.qceProduct.findMany.mockResolvedValue([
      {
        id: "p1",
        name: "Dell Laptop",
        sku: "DELL-001",
    ...
    db.qceProduct.count.mockResolvedValue(1);
  ...
  it("scopes the where clause by tenantId", async () => {
    adminSession();
    db.qceProduct.findMany.mockResolvedValue([]);
    db.qceProduct.count.mockResolvedValue(0);
    ...
    const where = db.qceProduct.findMany.mock.calls[0]![0]!.where as {
...
describe("POST /api/products", () => {
  beforeEach(() => {
    db.qceProduct.create.mockReset();
    setSession(null);
  });
  ...
  it("creates a product with happy-path payload", async () => {
    adminSession();
    db.qceProduct.create.mockResolvedValue({
      id: "p1",
      orgId: "t1",
      name: "Dell Laptop",
      sku: "DELL-001",
  ...
  it("returns 409 on duplicate SKU", async () => {
    adminSession();
    db.qceProduct.create.mockRejectedValue({ code: "P2002" } as never);
