...
describe("POST /api/price-lists/[id]/items â€” tenant scoping", () => {
  beforeEach(() => {
    ...
    db.qcePriceList.findFirst.mockReset();
    db.qceProduct.findFirst.mockReset();
    db.qcePriceListItem.create.mockReset();
  });
  ...
  it("rejects with 404 when productId belongs to another tenant", async () => {
    ...
    db.qcePriceList.findFirst.mockResolvedValue({ id: "pl1" } as never);
    // But the product isn't visible to tenant t1 â€” pretend it's in t2.
    db.qceProduct.findFirst.mockResolvedValue(null);

    // getPriceList (called by route before service) also resolves the pl.
    db.qcePriceList.findFirst.mockResolvedValueOnce({ id: "pl1", items: [] } as never);
    ...
    expect(db.qcePriceListItem.create).not.toHaveBeenCalled();
  });
  ...
  it("rejects with 404 when priceListId belongs to another tenant (via service check)", async () => {
    ...
    db.qcePriceList.findFirst.mockResolvedValueOnce({ id: "pl1", items: [] } as never); // route preflight
    db.qcePriceList.findFirst.mockResolvedValueOnce(null); // service guard
    db.qceProduct.findFirst.mockResolvedValue({ id: "p1" } as never);
...
describe("PATCH /api/price-lists/[id]/items/[itemId] â€” tenant scoping", () => {
  beforeEach(() => {
    setSession(null);
    db.$transaction.mockReset();
    db.qcePriceListItem.findFirst.mockReset();
    db.qcePriceListItem.update.mockReset();
  });
  ...
  it("returns 404 when the item belongs to another tenant", async () => {
    ...
    db.qcePriceListItem.findFirst.mockResolvedValue(null);
    ...
    expect(db.qcePriceListItem.update).not.toHaveBeenCalled();
  });
});
