...
describe("POST /api/quotes/[id]/send", () => {
  ...
  it("happy path: 200, console driver, updates sentAt, writes QuoteSent activity", async () => {
    ...
    db.qceQuote.update.mockResolvedValue({} as never);
    ...
    const sentAtUpdate = db.qceQuote.update.mock.calls.find(
    ...
    const quoteSentActivity = db.qceActivity.create.mock.calls.find(
