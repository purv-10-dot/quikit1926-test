...
function mockDef(status: string) {
  db.qceWorkflowDefinition.findFirst.mockResolvedValue({ ...base, status } as never);
  db.qceWorkflowDefinition.update.mockImplementation(
...
describe("POST /api/automations/workflows/[id]/publish", () => {
  ...
  it("404 for a cross-tenant / missing definition", async () => {
    ...
    expect((db.qceWorkflowDefinition.findFirst.mock.calls[0]![0]!.where as { orgId: string }).orgId).toBe("t1");
  });
  ...
  it("publishes a Draft → Active (happy path, stamps lastPublishedOn)", async () => {
    ...
    const data = db.qceWorkflowDefinition.update.mock.calls[0]![0]!.data as Record<string, unknown>;
...
describe("POST /api/automations/workflows/[id]/unpublish", () => {
  ...
  it("delayed unpublish drives Active → Draining", async () => {
    ...
    expect(res.status).toBe(200);
    expect((db.qceWorkflowDefinition.update.mock.calls[0]![0]!.data as { status: string }).status).toBe("Draining");
  });
  ...
  it("immediate unpublish drives Active → Stopped", async () => {
    ...
    expect(res.status).toBe(200);
    expect((db.qceWorkflowDefinition.update.mock.calls[0]![0]!.data as { status: string }).status).toBe("Stopped");
  });
});
