...
describe("GET /api/reports/canned/[id]", () => {
  ...
  it("forwards the caller's tenantId into the run query", async () => {
    ...
    asMock(db.qceOpportunity.groupBy).mockResolvedValueOnce([] as never);
    ...
    expect(asMock(db.qceOpportunity.groupBy)).toHaveBeenCalled();
    const args = asMock(db.qceOpportunity.groupBy).mock.calls[0]?.[0];
