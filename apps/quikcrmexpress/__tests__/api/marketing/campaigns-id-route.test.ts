...
describe("GET /api/marketing/campaigns/[id]", () => {
  beforeEach(() => {
    db.qceCampaign.findFirst.mockReset();
    setSession(null);
  });
  ...
  it("returns 404 for missing campaign", async () => {
    adminSession();
    db.qceCampaign.findFirst.mockResolvedValue(null);
    const { GET } = await import("@/app/api/marketing/campaigns/[id]/route");
  ...
  it("returns serialized campaign for tenant", async () => {
    adminSession();
    db.qceCampaign.findFirst.mockResolvedValue({
      id: "c1",
      orgId: "t1",
      name: "Diwali Offer",
      status: "Draft",
    ...
    const where = db.qceCampaign.findFirst.mock.calls[0]![0]!.where as {
      id: string;
      orgId: string;
    };
