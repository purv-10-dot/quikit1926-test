...
describe("POST /api/activities/smb-outreach", () => {
  ...
  it("rejects an invalid disposition chain (400)", async () => {
    adminSession();
    db.qceLead.findFirst.mockResolvedValue({ id: "L1", accountId: null } as never);
  ...
  it("logs activity AND patches the parent lead's country + followupPriority", async () => {
    ...
    const createMock = db.qceActivity.create;
    const updateMock = db.qceLead.update;
    ...
    db.qceActivity.create.mockResolvedValue({
      id: "act1",
      orgId: "t1",
      type: "SMB Outreach",
      relatedKind: "Lead",
    ...
    db.qceLead.findMany.mockResolvedValue([{ id: "L1", name: "ACME" } as never]);
