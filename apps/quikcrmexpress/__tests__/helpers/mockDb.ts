 ...
 *   const db = mockDb();
 *   db.qceOpportunity.findFirst.mockResolvedValue({ id: "1", ... });
 */
...
// Default the lead-status reads to empty arrays. getPipelineConfig() (a shared
// service reached transitively by dashboard/leads code) iterates the result of
// crmLeadStatus.findMany(); an unconfigured DeepMockProxy method returns
// undefined, which throws "dbStatuses is not iterable". Returning [] lets the
// service fall through to its DEFAULT_STAGES/STATUSES path. Tests that need
...
// implementation.
sharedMock.qceLeadStatus.findMany.mockResolvedValue([]);
sharedMock.qceLeadSubStatus.findMany.mockResolvedValue([]);
