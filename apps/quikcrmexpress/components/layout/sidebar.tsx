...
function QuikcrmexpressMark({ className = "h-8 w-8 rounded-lg object-cover" }: { className?: string }) {
  return (
...
function SidebarBrand({
  ...
  return (
    ...
    >
      {collapsed ? (
        <Link href="/dashboard" aria-label="QuikCRMExpress home" className="shrink-0">
          <QuikcrmexpressMark />
        </Link>
      ) : (
        <Link
          href="/dashboard"
          aria-label="QuikCRMExpress home"
          className="flex min-w-0 items-center gap-2.5"
        >
          <QuikcrmexpressMark className="h-8 w-8 shrink-0 rounded-lg object-cover" />
          <div className="min-w-0 leading-tight">
            <p className="truncate text-sm font-semibold text-crm-text">
              <span>Quik</span>
              <span className="text-crm-blue">CrmExpress</span>
            </p>
