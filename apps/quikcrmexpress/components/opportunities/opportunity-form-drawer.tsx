...
async function fetchUsers(): Promise<PickerUser[]> {
  ...
  const j = (await r.json()) as { items?: PickerItem[]; data?: { items?: PickerItem[] } };
  ...
  return (j.data?.items ?? j.items ?? []).map((u) => {
...
export function OpportunityFormDrawer({
  ...
  const [stage, setStage] = useState<QceOpportunityStage>("Prospecting");
      ...
      <div className="space-y-4 p-4">
        ...
        <Field label="Stage">
          <Select
            value={stage}
            onChange={(e) => {
              const next = (e.target as HTMLSelectElement).value as QceOpportunityStage;
              setStage(next);
