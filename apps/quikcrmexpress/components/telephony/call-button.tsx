...
interface Props {
  ...
  /** When true (button variant), render the number after "Call" e.g. "Call +91…". Default off. */
  showNumber?: boolean;
}
 ...
 */
export function CallButton({ to, leadId, leadName, variant = "icon", className = "", disabledReason, showNumber = false }: Props) {
    ...
    return (
      <>
        <button onClick={trigger} disabled={disabled} title={title} className={"crm-btn-secondary " + className}>
          <Phone size={14} /> {showNumber && to ? `Call ${to}` : "Call"}
        </button>
        <CallModal open={open} onClose={() => setOpen(false)} to={to} leadId={leadId} leadName={leadName} />
      </>
    );
