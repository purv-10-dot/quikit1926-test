// apps/quikcrmexpress/components/leads/call-disposition-modal.tsx
"use client";
 ...
 *     QceCallLog row, links the lead, applies the lead-stage transition, and
 *     creates the QceActivity entry — all in one transaction.
 ...
 *     QceActivity to /api/activities — preserving the legacy no-call path.
 */
export function CallDispositionModal({
  ...
  async function submit() {
    ...
    // QceCallDisposition.name/label, so we don't repeat it here.
    ...
    try {
      ...
      if (callLogContext) {
        ...
        // QceActivity. lead-action-bar's "Add disposition" button uses this.
        if (!leadId) {
...
interface LeadCallDispositionModalProps {
  ...
  /** Stage 3-D(a): "dialer" when opened after a real call (writes a QceCallLog
