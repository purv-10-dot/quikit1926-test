...
interface LeadInput {
  ...
  // Details-form fields forwarded from the shell so the Details tab pre-fills.
...
export function LeadDetailTabs({
  ...
  const [internalTab, setInternalTab] = useState<TabKey>("timeline");
  const [mountedTabs, setMountedTabs] = useState<Set<TabKey>>(() => new Set(["timeline"]));
    ...
    <section className="crm-card overflow-hidden shadow-sm">
      <div className="crm-hscroll sticky top-0 z-10 overflow-x-auto border-b border-crm-border bg-white/95 backdrop-blur dark:bg-slate-900/95">
        <div className="flex min-w-max gap-0.5 px-2" role="tablist">
          {/* Overview is intentionally hidden from the tab bar — Timeline is the
              landing tab. The overview render block + TabKey entry are kept so
              programmatic navigation still type-checks. */}
