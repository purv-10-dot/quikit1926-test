      ...
      <div className="flex items-start justify-between">
        <div className="flex items-start gap-2">
          ...
          <div>
            ...
            {isSystem ? (
              activity.ownerName?.trim() ? (
                <div className="text-xs text-crm-muted">{activity.ownerName.trim()}</div>
              ) : (
            ...
            {/* Custom disposition form values (Payment Form etc.) live in
                QceFieldValue keyed to this activity — offer a read-only view
