...
interface Props {
  ...
  /** Selection (bulk actions). When selectedIds is provided, a checkbox column
   *  renders. Omitted entirely in trash mode. */
  selectedIds?: Set<string>;
  onToggleRow?: (id: string) => void;
  /** Toggle all rows on the current page (header checkbox). */
  onTogglePage?: (ids: string[], allSelected: boolean) => void;
}

const DEFAULT_COLUMNS = ["name", "company", "email", "mobile", "stage", "status", "score", "ownerName"];
...
export function LeadTable({
  ...
  // Selection is enabled only when the parent wired it AND we're not in trash
  // mode (bulk actions don't apply to deleted rows). pageIds = the ids visible
  // on the current page, so the header checkbox can select/clear the page.
  const selectionEnabled = Boolean(selectedIds && onToggleRow && !viewTrash);
      ...
      <div className="crm-hscroll relative max-h-[calc(100dvh-200px)] overflow-auto sm:max-h-[calc(100dvh-220px)] lg:max-h-[calc(100dvh-250px)]">
        <Table className="crm-table-sticky">
          <THead>
            <TR>
              {selectionEnabled && (
                <TH
                  className="w-10 px-3"
                  style={{ position: "sticky", left: 0, zIndex: 6, background: "#ffffff" }}
                >
                  <input
                    type="checkbox"
              ...
              <TH
                className="w-10 px-3"
                style={{ position: "sticky", left: selectionEnabled ? 40 : 0, zIndex: 5, background: "#ffffff" }}
              >
          ...
          <TBody>
            {items.length === 0 ? (
              <TR>
                <TD colSpan={cols.length + 4 + (selectionEnabled ? 1 : 0)} className="py-16">
                  {viewTrash ? (
                    ...
                    {selectionEnabled && (
                      ...
                      >
                        <input
                          ...
                          onChange={() => onToggleRow?.(l.id)}
                        />
                      </TD>
                    )}
                    <TD
                      className="px-3"
                      style={{ position: "sticky", left: selectionEnabled ? 40 : 0, zIndex: 1, background: rowBg }}
                    >
                      <button
...
function renderCell(key: string, lead: LeadRow, def?: LeadFieldDefinition): React.ReactNode {
  ...
  // Phone/mobile data is split inconsistently across leads (some have the number
  // in `phone`, some in `mobile`). Show "the number that exists" so the Phone
  // column is never wrongly blank — mirrors what the detail page and Call buttons
  // do (phone ?? mobile). Data consolidation is a separate backfill.
  if (key === "phone") {
    return formatValue(lead.phone ?? lead.mobile ?? null, def);
  }
