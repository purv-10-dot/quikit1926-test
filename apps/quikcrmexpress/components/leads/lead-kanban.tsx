...
async function fetchBoard(
  ...
  const json = (await res.json()) as { data: { buckets: KanbanBucket[] } };
  let fresh = json.data.buckets;
  ...
  if (expandedLimits.size > 0) {
    const expandedResults = await Promise.all(
      Array.from(expandedLimits.entries()).map(async ([stage, limit]) => {
        ...
        if (!r.ok) return null;
        const j = (await r.json()) as { data: { buckets: KanbanBucket[] } };
        return j.data.buckets[0] ?? null;
      }),
    );
