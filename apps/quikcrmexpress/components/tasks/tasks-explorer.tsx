...
/** User cell — resolves a userId to a name with an initials avatar. */
function AssigneeCell({
  ...
  if (!id) return <span className="text-xs italic text-crm-muted">{emptyLabel}</span>;
  const name = map.get(id);
  if (!name) return <span className="text-xs text-crm-muted">—</span>;
  return (
    <span className="inline-flex items-center gap-2">
      <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-crm-blue/10 text-[10px] font-semibold text-crm-blue">
        {initialsFor(name)}
      </span>
      <span className="truncate text-crm-text">{name}</span>
    </span>
  );
}
...
export function TasksExplorer({ currentUserId, assignees }: Props) {
  ...
  // Deep-links from notification emails: `/tasks?focus=<id>&snooze=<minutes>`.
  // Snooze is login-gated — we reuse the authenticated snooze endpoint rather
  // than a session-less link. Runs once and strips the params so a refresh
  // (or back/forward) doesn't re-fire the mutation. The auto-snooze fires only
  // after this authenticated page has loaded, so email-client link prefetch
  ...
  useEffect(() => {
    ...
    const minutes = Number(sp.get("snooze"));
    const validSnooze = Number.isInteger(minutes) && minutes > 0;
    ...
    if (validSnooze) {
      snooze(focusId, minutes);
      return;
    }
    // Focus-only ("Open task"): load the task and open the editor.
    (async () => {
      try {
        const res = await fetch(`/api/tasks/${focusId}`, { credentials: "include" });
        if (!res.ok) return;
        openEdit((await res.json()) as TaskRow);
      } catch {
        /* deep-link is best-effort — ignore a failed open */
      }
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
            ...
            <Table>
              <THead>
                <TR>
                  ...
                  <TH>Status</TH>
                  <TH hideBelow="md">Assigned to</TH>
                ...
                {loading ? (
                  ...
                  items.map((t) => (
                    <TR key={t.id}>
                      ...
                      <TD hideBelow="md">
                        <AssigneeCell map={assigneeMap} id={t.assignedToUserId} />
                      </TD>
                      <TD hideBelow="lg">
                        <AssigneeCell map={assigneeMap} id={t.createdByUserId} emptyLabel="—" />
                      </TD>
                      <TD>
