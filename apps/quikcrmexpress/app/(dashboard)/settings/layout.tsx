...
type NavItem = {
  ...
  label: string;
  icon: LucideIcon;
  /** Admin-only settings surface. Hidden from non-admins in the nav (the
   * backing APIs also enforce this). "My Profile" is the only personal page. */
  adminOnly?: boolean;
};
...
const NAV: Array<{ group: string; items: NavItem[] }> = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/settings/teams", label: "Teams", icon: Layers3, adminOnly: true },
      { href: "/settings/sales-groups", label: "Sales Groups", icon: Workflow, adminOnly: true },
      { href: "/settings/permissions", label: "Permission Templates", icon: ShieldCheck, adminOnly: true },
    ],
  },
  {
    group: "Leads",
    items: [
      { href: "/settings/lead-scoring",  label: "Lead Scoring",      icon: Gauge, adminOnly: true },
      { href: "/settings/fields",        label: "Lead Fields",        icon: SlidersHorizontal, adminOnly: true },
      { href: "/settings/sources",       label: "Lead Sources",       icon: Layers3, adminOnly: true },
      { href: "/settings/stages",        label: "Lead Stages",        icon: ListChecks, adminOnly: true },
      { href: "/settings/statuses",      label: "Lead Statuses",      icon: ListChecks, adminOnly: true },
      { href: "/settings/sub-statuses",  label: "Lead Sub Stages",    icon: Layers3, adminOnly: true },
    ],
  },
  {
    ...
    items: [
      ...
      { href: "/settings/disposition-forms", label: "Disposition Forms", icon: FileText },
    ],
  },
  {
    group: "Products",
    items: [
      { href: "/settings/product-categories", label: "Categories & Brands", icon: Layers3, adminOnly: true },
      { href: "/settings/product-fields", label: "Product Fields", icon: SlidersHorizontal, adminOnly: true },
      { href: "/settings/quote-templates", label: "Quote Templates", icon: FileText, adminOnly: true },
    ],
  },
  {
    group: "Other",
    items: [
      { href: "/settings/integrations", label: "Integrations", icon: Workflow, adminOnly: true },
      { href: "/settings/audit", label: "Audit Log", icon: ShieldCheck, adminOnly: true },
      { href: "/settings/notifications", label: "Notifications", icon: Bell },
      // Deliberately NOT adminOnly — the only entry in this group that isn't.
      // Support status is per-user: every member sees their own requests, so
      // hiding it behind the admin flag would hide it from exactly the people
      // who raise tickets. Matches quiktrack's settings shell.
      { href: "/settings/support", label: "Support Status", icon: LifeBuoy },
    ],
  },
];
