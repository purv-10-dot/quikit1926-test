...
async function loadOwn(orgId: string, id: string) {
  return prisma.qceContact.findFirst({ where: { id, orgId } });
}

async function loadActive(orgId: string, id: string) {
  return prisma.qceContact.findFirst({ where: { id, orgId, deletedAt: null } });
}
...
export async function PATCH(
  ...
  try {
    ...
    const updated = await prisma.qceContact.update({
      where: { id },
      data: updateData,
    });
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.qceContact.update({
      where: { id },
