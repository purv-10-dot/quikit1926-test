...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const list = await prisma.qceLeadSavedList.findFirst({
      where: { id, orgId: user.orgId, userId: user.userId },
    });
    if (!list) return NextResponse.json({ success: false, error: "Not found" }, { status: 404 });

    const filterParsed = filterPayloadSchema.safeParse(list.filters);
    if (!filterParsed.success) {
      return NextResponse.json({ success: false, error: "Saved list has malformed filters" }, { status: 422 });
    }
