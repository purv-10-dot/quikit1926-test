...
async function loadCampaign(orgId: string, id: string) {
  return prisma.qceCampaign.findFirst({ where: { id, orgId } });
}
...
export async function GET(
  ...
  try {
    ...
    if (!c) return NextResponse.json({ success: false, error: "Campaign not found" }, { status: 404 });
...
export async function PATCH(
  ...
  try {
    ...
    const data: Prisma.QceCampaignUpdateInput = {
      ...(name !== undefined ? { name } : {}),
    ...
    const c = await prisma.qceCampaign.update({
      where: { id },
      data,
    });
