...
/**
 * GET /api/opportunities/picker?q=&limit=
 * Returns: { success, data: { items: [{ id, name, stage }] } }
 *
 * Lightweight dropdown source for forms that need to attach an Opportunity.
...
export async function GET(req: NextRequest) {
  try {
    ...
    const opps = await prisma.qceOpportunity.findMany({
      where,
      select: { id: true, name: true, stage: true },
      orderBy: { createdAt: "desc" },
      take: limit,
    });
    return NextResponse.json({ success: true, data: { items: opps } });
  } catch (error: unknown) {
