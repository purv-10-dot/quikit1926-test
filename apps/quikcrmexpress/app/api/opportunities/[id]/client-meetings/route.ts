...
async function loadOpp(orgId: string, id: string) {
  return db.qceOpportunity.findFirst({
    where: { id, orgId },
...
export async function GET(
  ...
  try {
    ...
    const meetings = await db.qceOpportunityClientMeeting.findMany({
...
export async function POST(
  ...
  try {
    ...
    const created = await db.$transaction(async (tx) => {
      const meeting = await tx.qceOpportunityClientMeeting.create({
        data: {
          orgId: user.orgId,
          opportunityId: id,
      ...
      await tx.qceOpportunity.update({
