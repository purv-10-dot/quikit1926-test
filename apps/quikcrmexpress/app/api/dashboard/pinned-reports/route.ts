/**
 * Server-side dashboard pins (QceDashboardPin) — replaces the old
...
async function loadPins(orgId: string, userId: string): Promise<DashboardPin[]> {
  const rows = await prisma.qceDashboardPin.findMany({
    where: { orgId, userId },
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!VALID_REPORT_IDS.has(parsed.data.reportId)) {
      return NextResponse.json({ success: false, error: "Unknown reportId" }, { status: 400 });
    }

    const max = await prisma.qceDashboardPin.aggregate({
      where: { orgId: user.orgId, userId: user.userId },
      _max: { sortOrder: true },
    });
    const nextOrder = (max._max.sortOrder ?? -1) + 1;

    await prisma.qceDashboardPin.upsert({
      where: {
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await prisma.qceDashboardPin.deleteMany({
  ...
  try {
    ...
    await prisma.$transaction(
      parsed.data.order.map((reportId, i) =>
        prisma.qceDashboardPin.updateMany({
