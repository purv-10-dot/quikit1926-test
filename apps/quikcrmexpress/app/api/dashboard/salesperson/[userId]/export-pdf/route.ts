...
export async function GET(
  ...
  try {
    ...
    if (!isCrmAdminUser(user)) {
      return NextResponse.json({ success: false, error: "Export is available to org administrators only." },
        { status: 403 },
      );
    }
    ...
    const data = await getSalespersonDetail(user.orgId, userId, parsed.range);
