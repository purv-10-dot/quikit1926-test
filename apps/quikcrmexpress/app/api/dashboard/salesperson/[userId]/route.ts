...
export async function GET(
  ...
  try {
    ...
    if (!isCrmAdminUser(user)) {
      return NextResponse.json({ success: false, error: "Access restricted to org administrators." },
        { status: 403 },
      );
    }
    ...
    if (!data) {
      return NextResponse.json({ success: false, error: "Salesperson not found." }, { status: 404 });
    }
