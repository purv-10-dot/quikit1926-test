...
export async function GET(req: NextRequest) {
  try {
    ...
    const leadWhere = {
      ...
      // QceLead has `deletedAt`; filter explicitly (no middleware coverage yet).
      deletedAt: null,
    ...
    const oppWhere = {
      ...tenantOwnerWhere(user, filters.resolvedOwnerId),
      // QceOpportunity has `deletedAt`; filter explicitly (no middleware coverage yet).
      deletedAt: null,
    ...
    const [
      ...
      prisma.qceTask.count({ where: taskWhere }),
      ...
      prisma.qceLead.count({ where: leadWhere }),
      ...
      prisma.qceOpportunity.count({ where: oppWhere }),
