...
export async function PATCH(req: NextRequest, ctx: RouteContext) {
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json({ success: false, error: "Invalid body", issues: parsed.error.flatten() },
        { status: 400 },
      );
    }
    ...
    // on the resulting QceActivity audit row (QceTask has no column for it
    // yet).
    if (patch.status === "Cancelled" && !patch.cancellationReason?.trim()) {
      return NextResponse.json({ success: false, error: "Cancellation reason is required when status is Cancelled" },
        { status: 400 },
      );
    }
    ...
    if (patch.status === "Completed") {
      notifyTaskCompleted({
        ...
        taskCreatorUserId: updated.createdByUserId,
