...
export async function GET(req: NextRequest) {
  try {
    ...
    const items = await prisma.qceLeadSource.findMany({
...
export async function POST(req: NextRequest) {
  try {
    ...
    const existing = await prisma.qceLeadSource.findFirst({
      where: { orgId: user.orgId, name: { equals: parsed.data.name, mode: "insensitive" } },
    });
    if (existing) {
      return NextResponse.json({ success: false, error: "A source with that name already exists." }, { status: 409 });
    }
    const created = await prisma.qceLeadSource.create({
      data: {
        orgId: user.orgId,
