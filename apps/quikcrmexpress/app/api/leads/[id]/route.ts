...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existing = await prisma.qceLead.findUnique({ where: { id } });
    if (!existing || existing.orgId !== user.orgId) {
      return NextResponse.json({ success: false, error: "Not found" }, { status: 404 });
    }
    if (existing.deletedAt) {
      return NextResponse.json({ success: false, error: "Lead is in trash. Restore it before editing." },
        { status: 410 },
      );
    }
    ...
    // QceLead has no `owner` relation declared (ownerId is a plain scalar FK to
    ...
    // Strip fields present in the Zod schema but absent from QceLead.
    const {
      ownerId,
      accountId,
      score:              manualScore,
    ...
    } as Prisma.QceLeadUncheckedUpdateInput;
