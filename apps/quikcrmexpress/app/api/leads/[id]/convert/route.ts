...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const lead = await prisma.qceLead.findFirst({ where: { id, orgId: user.orgId } });
    ...
    if (lead.linkedContactId || lead.status === "Converted") {
      return NextResponse.json({ success: false, error: "Lead is already converted", contactId: lead.linkedContactId },
        { status: 409 },
      );
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const claim = await tx.qceLead.updateMany({
      ...
      if (!accountId) {
        ...
        if (accountName) {
          const existing = await tx.qceAccount.findFirst({
      ...
      if (opts.createContact) {
        ...
        const contact = await tx.qceContact.create({
          data: {
            orgId: user.orgId,
      ...
      if (opts.createOpportunity) {
        const opp = await tx.qceOpportunity.create({
          data: {
            orgId: user.orgId,
      ...
      let callsRelinked = 0;
      if (contactId) {
        const activityRes = await tx.qceActivity.updateMany({
        ...
        const taskRes = await tx.qceTask.updateMany({
          where: { orgId: user.orgId, leadId: lead.id },
          data: { relatedKind: "Contact", relatedObjectId: contactId },
        });
        tasksRelinked = taskRes.count;

        const noteRes = await tx.qceNote.updateMany({
          where: { orgId: user.orgId, leadId: lead.id },
          data: { relatedKind: "Contact", relatedObjectId: contactId },
        });
        notesRelinked = noteRes.count;

        const callRes = await tx.qceCallLog.updateMany({
          where: { orgId: user.orgId, leadId: lead.id },
      ...
      const updatedLead = await tx.qceLead.update({
        where: { id: lead.id },
        data: {
          status: "Converted",
