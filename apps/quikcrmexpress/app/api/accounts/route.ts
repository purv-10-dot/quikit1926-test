...
export async function GET(req: NextRequest) {
  try {
    ...
    const where: Prisma.QceAccountWhereInput = applyAccountListWhere(
      { orgId: user.orgId },
      {
        trashed,
        allowedAccountIds,
    ...
    if (format) {
      ...
      const cursor = createPrismaCursorIterator<AccountCsvRow>({
        delegate: prisma.qceAccount as unknown as PrismaListDelegate<AccountCsvRow>,
        where,
    ...
    const orderBy: Prisma.QceAccountOrderByWithRelationInput[] =
      view === "mine"
