...
export async function GET(req: NextRequest) {
  try {
    ...
    const where: Prisma.QceAccountWhereInput = {
      orgId: user.orgId,
      deletedAt: null,
    };
    ...
    const [
      ...
      prisma.qceAccount.count({ where }),
      prisma.qceAccount.count({ where: { ...where, healthScore: { lt: 40 } } }),
      prisma.qceAccount.count({ where: { ...where, healthScore: { gte: 40, lt: 70 } } }),
      prisma.qceAccount.count({ where: { ...where, healthScore: { gte: 70 } } }),
      prisma.qceAccount.count({ where: { ...where, renewalDate: { gte: now, lte: in30 } } }),
      prisma.qceAccount.count({ where: { ...where, renewalDate: { gte: now, lte: in90 } } }),
      prisma.qceAccount.count({ where: { ...where, status: "Active" } }),
      prisma.qceAccount.count({ where: { ...where, status: "Prospect" } }),
      prisma.qceAccount.count({ where: { ...where, status: "Inactive" } }),
    ]);
