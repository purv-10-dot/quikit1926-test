...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!ownerName) {
      return NextResponse.json({ success: false, error: "Owner user not found" }, { status: 400 });
    }

    const targets = await prisma.qceLead.findMany({
      where: {
        id: { in: leadIds },
        orgId: user.orgId,
        accountId,
    ...
    if (targets.length === 0) {
      return NextResponse.json({ success: false, error: "No matching leads on this account (check selection and try again)." },
        { status: 400 },
      );
    }
