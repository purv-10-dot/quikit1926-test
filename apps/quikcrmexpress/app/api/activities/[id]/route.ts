...
async function loadAndAssertAccess(
  ...
  const item = await prisma.qceActivity.findFirst({ where: { id, orgId } });
  if (!item) {
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updated = await prisma.qceActivity.update({
      where: { id },
      data: data as Parameters<typeof prisma.qceActivity.update>[0]["data"],
    });
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.qceActivity.delete({ where: { id } });
