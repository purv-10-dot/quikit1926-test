...
export async function POST(req: NextRequest) {
  try {
    ...
    if (callLogId) {
      const cl = await prisma.qceCallLog.findFirst({
        where: { id: callLogId, orgId: user.orgId },
        select: { id: true },
      });
      if (!cl) return NextResponse.json({ success: false, error: "Call log not found" }, { status: 404 });
    }

    const row = await prisma.qcePaymentVerification.create({
      data: {
        orgId: user.orgId,
        leadId: leadId ?? null,
