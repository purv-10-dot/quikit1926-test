...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ slug: string[] }> }) {
  try {
    ...
    const ws = await prisma.qceOrgWorkspaceSettings.findUnique({ where: { orgId: user.orgId } });
    const tree = (ws?.settings as JsonValue) ?? {};
    return NextResponse.json({ success: true, data: { value: getAt(tree, slug) } });
  } catch (e) {
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ slug: string[] }> }) {
  try {
    ...
    const existing = await prisma.qceOrgWorkspaceSettings.findUnique({ where: { orgId: user.orgId } });
    const next = setAt((existing?.settings as JsonValue) ?? {}, slug, value);
    await prisma.qceOrgWorkspaceSettings.upsert({
      where: { orgId: user.orgId },
      create: { orgId: user.orgId, settings: next as object },
      update: { settings: next as object },
    });
    return NextResponse.json({ success: true, data: { value: getAt(next, slug) } });
  } catch (e) {
    return errorResponse(e);
  }
}
