...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ key: string }> }) {
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json({ success: false, error: "Invalid patch", errors: parsed.error.flatten().fieldErrors },
        { status: 400 },
      );
    }
    ...
    if (e instanceof FieldDefError) return NextResponse.json({ success: false, error: e.message }, { status: e.statusCode });
    return errorResponse(e);
  }
}
