// apps/quikcrmexpress/app/api/telephony/call-logs/route.ts
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!leadId && !contactId) {
      ...
      if (format) {
        ...
        const cursor = createPrismaCursorIterator<CallLogCsvRow>({
          delegate: prisma.qceCallLog as unknown as PrismaListDelegate<CallLogCsvRow>,
    ...
    if (contactId) {
      const contact = await prisma.qceContact.findFirst({
        // QceContact isn't middleware-protected — don't resolve a trashed contact.
        where: { id: contactId, orgId: user.orgId, deletedAt: null },
        select: { phone: true },
      });
      const tails = [lastTen(contact?.phone)].filter((t) => t.length === 10);
      const orClauses: Prisma.QceCallLogWhereInput[] = [
    ...
    const orClauses: Prisma.QceCallLogWhereInput[] = [{ leadId }];
    for (const t of tails) {
      orClauses.push({ destinationNumber: { endsWith: t } });
      orClauses.push({ sourceNumber: { endsWith: t } });
    }
...
const createSchema = z.object({
  ...
  // Stage 3-D(a): explicit origin — "dialer" (real call -> QceCallLog row) vs
