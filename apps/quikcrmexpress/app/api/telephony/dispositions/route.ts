...
async function seedDefaultsIfEmpty(orgId: string) {
  const count = await prisma.qceCallDisposition.count({ where: { orgId } });
  if (count > 0) return;
  await prisma.qceCallDisposition.createMany({
...
export async function GET() {
  try {
    ...
    await seedDefaultsIfEmpty(user.orgId);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const item = await prisma.qceCallDisposition.create({
      data: { ...parsed.data, orgId: user.orgId } as Prisma.QceCallDispositionUncheckedCreateInput,
    });
