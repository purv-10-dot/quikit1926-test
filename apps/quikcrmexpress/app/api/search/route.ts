  ...
  try {
    ...
    const [leads, accounts, contacts] = await Promise.all([
      ...
      prisma.qceContact.findMany({
        // QceContact is NOT in SOFT_DELETE_MODELS, so the soft-delete middleware
    ...
    return NextResponse.json({ success: true, data: { leads, accounts, contacts } });
  } catch (e) {
