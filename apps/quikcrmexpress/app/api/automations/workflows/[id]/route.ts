...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existing = await prisma.qceWorkflowDefinition.findFirst({
      where: { id, orgId: user.orgId, deletedAt: null },
    });
    ...
    const data: Prisma.QceWorkflowDefinitionUpdateInput = {};
    ...
    const wf = await prisma.qceWorkflowDefinition.update({
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (e instanceof LifecycleError) return NextResponse.json({ success: false, error: e.message }, { status: 400 });
    return errorResponse(e);
  }
}
