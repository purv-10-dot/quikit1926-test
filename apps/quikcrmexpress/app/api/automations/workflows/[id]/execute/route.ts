...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const wf = await prisma.qceWorkflowDefinition.findFirst({ where: { id, orgId: user.orgId } });
    ...
    if (!startNodeId) return NextResponse.json({ success: false, error: "Workflow has no entry node" }, { status: 400 });
