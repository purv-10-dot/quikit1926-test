 ...
 *   1. An automation stage write records a QceAutomationAttribution row that
...
async function main() {
  ...
  // ─── fresh fixtures ───
  await prisma.qceAutomationAttribution.deleteMany({ where: { orgId: TENANT, leadId: { in: [AUTO_LEAD, DISPO_LEAD] } } });
  await prisma.qceAutomationLeadDayCount.deleteMany({ where: { orgId: TENANT, leadId: { in: [AUTO_LEAD, DISPO_LEAD] } } });
  await prisma.qceLead.deleteMany({ where: { id: { in: [AUTO_LEAD, DISPO_LEAD] } } });
  await prisma.qceLead.create({ data: { id: AUTO_LEAD, orgId: TENANT, name: "P22 auto", stage: "New Lead", status: "Open", substatus: "Student Lead", source: "p22" } });
  await prisma.qceLead.create({ data: { id: DISPO_LEAD, orgId: TENANT, name: "P22 dispo", stage: "New Lead", status: "Open", source: "p22" } });

  // ─── 1. automation write → attribution ───
  const wf = await prisma.qceWorkflowDefinition.create({ data: {
  ...
  const attr = await prisma.qceAutomationAttribution.findFirst({ where: { orgId: TENANT, leadId: AUTO_LEAD } });
  ...
  const dispoAttr = await prisma.qceAutomationAttribution.findFirst({ where: { orgId: TENANT, leadId: DISPO_LEAD } });
  ...
  // ─── cleanup ───
  await prisma.qceWorkflowDefinition.deleteMany({ where: { id: wf.id } });
  await prisma.qceAutomationRule.deleteMany({ where: { id: rule.id } });
  await prisma.qceAutomationAttribution.deleteMany({ where: { orgId: TENANT, leadId: { in: [AUTO_LEAD, DISPO_LEAD] } } });
  await prisma.qceAutomationLeadDayCount.deleteMany({ where: { orgId: TENANT, leadId: { in: [AUTO_LEAD, DISPO_LEAD] } } });
  await prisma.qceActivity.deleteMany({ where: { orgId: TENANT, leadId: { in: [AUTO_LEAD, DISPO_LEAD] } } });
  await prisma.qceLead.deleteMany({ where: { id: { in: [AUTO_LEAD, DISPO_LEAD] } } });
