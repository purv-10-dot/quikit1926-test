 ...
 *   - Verifies ALL referenced disposition codes exist in QceCallDisposition
...
async function main() {
  ...
  const dispositions = await prisma.qceCallDisposition.findMany({
  ...
  const disqualifiedStatus = await prisma.qceLeadStatus.findFirst({
  ...
  const contactedStatus = await prisma.qceLeadStatus.findFirst({
  ...
  for (const rule of SEED_RULES) {
    const existing = await prisma.qceAutomationRule.findFirst({
    ...
    await prisma.qceAutomationRule.create({
