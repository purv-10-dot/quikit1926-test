 ...
 * first_db_crm_autotest. Builds real QceWorkflowDefinition rows, drives them via
...
async function main() {
  ...
  await prisma.qceLead.update({ where: { id: "autotest-r19-match" }, data: { stage: "New Lead", substatus: "Negotiation" } });
  await prisma.qceLead.update({ where: { id: "autotest-r1-substatus-only" }, data: { stage: "Onboarding Done", substatus: "Student Lead" } });
  await prisma.qceLead.update({ where: { id: "autotest-r1-stage-only" }, data: { stage: "New Lead", substatus: "Support Issue" } });
  for (const id of R1_MATCH) await prisma.qceLead.update({ where: { id }, data: { stage: "New Lead" } });

  // ---- definitions ----
  const r19 = await prisma.qceWorkflowDefinition.create({
    data: {
  ...
  const r1 = await prisma.qceWorkflowDefinition.create({
    data: {
  ...
  const l19 = await prisma.qceLead.findUnique({ where: { id: "autotest-r19-match" } });
  ...
  for (const id of R1_MATCH) {
    ...
    const l = await prisma.qceLead.findUnique({ where: { id } });
  ...
  const lNeg1 = await prisma.qceLead.findUnique({ where: { id: "autotest-r1-substatus-only" } });
  ...
  const lNeg2 = await prisma.qceLead.findUnique({ where: { id: "autotest-r1-stage-only" } });
  ...
  // ---- cleanup defs ----
  await prisma.qceWorkflowDefinition.deleteMany({ where: { id: { in: [r19.id, r1.id] } } });
