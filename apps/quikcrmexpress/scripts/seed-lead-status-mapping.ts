/**
 * One-time seed: QceLeadStatus, QceLeadSubStatus, QceLeadStatusSubStatus
 *
...
async function main() {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const name of allSubStatusNames) {
      const existing = await tx.qceLeadSubStatus.findUnique({ where: { name } });
      if (existing) {
        subStatusIdMap.set(name, existing.id);
        counts.subStatuses.skipped++;
        console.log(`         ⏭  sub-status skipped :  ${name}`);
      } else {
        const created = await tx.qceLeadSubStatus.create({ data: { name } });
    ...
    for (const [statusName, subStatusNames] of Object.entries(STATUS_MAPPING)) {
      ...
      const existingStatus = await tx.qceLeadStatus.findUnique({ where: { name: statusName } });
      let statusId: string;
      if (existingStatus) {
        ...
        const created = await tx.qceLeadStatus.create({ data: { name: statusName } });
        statusId = created.id;
      ...
      for (const subName of subStatusNames) {
        ...
        const existingMapping = await tx.qceLeadStatusSubStatus.findUnique({
          where: {
        ...
        if (existingMapping) {
          ...
          await tx.qceLeadStatusSubStatus.create({
