 ...
 * middleware.ts's `launcherPublicRoutes`).
 *
 * Invite emails link to this launcher's domain (apps.quikit.ai /
 * uatapps.quikit.ai) via NEXT_PUBLIC_QUIKIT_URL, so this statement — not
 * quikinfra's — is the one Android verifies for the default invite flow.
 * Same package name + fingerprints as apps/quikinfra's copy, since both
 * domains verify the same Android app.
 */
