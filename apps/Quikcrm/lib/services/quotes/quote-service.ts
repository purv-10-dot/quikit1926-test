  ...
  return serverlessTransaction(db, async (tx) => {
    ...
    if (args.input.contactId) {
      const contact = await tx.crmContact.findFirst({
        where: { id: args.input.contactId, orgId: args.orgId },
        select: { id: true },
      });
 ...
 *
 * Tenant scoping: caller passes `orgId`; this function never
 ...
 * (`@@index([orgId, parentQuoteId])`), and chains are shallow in
