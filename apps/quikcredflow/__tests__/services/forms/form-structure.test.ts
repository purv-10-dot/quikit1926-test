...
beforeEach(() => {
  ...
  db.qcfFormTab.create.mockReset();
  db.qcfFormTab.findUnique.mockReset();
  db.qcfFormTab.delete.mockReset();
  db.qcfFormSection.create.mockReset();
  db.qcfFormField.update.mockReset();
  db.qcfFormField.create.mockReset();
});
...
describe("FR-RE-4 — form structure builder", () => {
  it("createFormTab: creates a rule_driven tab on a draft version", async () => {
    ...
    db.qcfFormTab.create.mockResolvedValue({ id: "tab_1" } as never);
    ...
    expect(db.qcfFormTab.create).toHaveBeenCalledWith(
  ...
  it("createFormTab: rejects a mutation against a PUBLISHED version (draft-only; clone is Unit 7)", async () => {
    ...
    expect(db.qcfFormTab.create).not.toHaveBeenCalled();
  });

  it("createFormSection: creates a section under a tab", async () => {
    db.qcfFormTab.findUnique.mockResolvedValue({ id: "tab_1", formSetVersionId: VERSION_ID } as never);
    db.qcfFormSetVersion.findUnique.mockResolvedValue(draftVersion);
    db.qcfFormSection.create.mockResolvedValue({ id: "sec_1" } as never);

    await createFormSection({ formTabId: "tab_1", name: "Basic Details", sortOrder: 0 });

    expect(db.qcfFormSection.create).toHaveBeenCalledWith(
  ...
  it("placeFormField: sets formTabId + formSectionId on the field", async () => {
    db.qcfFormField.update.mockResolvedValue({ id: "fld_1" } as never);

    await placeFormField({ fieldId: "fld_1", formTabId: "tab_1", formSectionId: "sec_1" });

    expect(db.qcfFormField.update).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: "fld_1" },
  ...
  it("deleteFormTab: throws FormStructureError for the protected tab (undeletable)", async () => {
    db.qcfFormTab.findUnique.mockResolvedValue({ id: "tab_cd", isProtected: true } as never);

    await expect(deleteFormTab("tab_cd")).rejects.toBeInstanceOf(FormStructureError);
    expect(db.qcfFormTab.delete).not.toHaveBeenCalled();
  });

  it("deleteFormTab: deletes a non-protected tab", async () => {
    db.qcfFormTab.findUnique.mockResolvedValue({ id: "tab_1", isProtected: false } as never);
    db.qcfFormTab.delete.mockResolvedValue({ id: "tab_1" } as never);

    await deleteFormTab("tab_1");

    expect(db.qcfFormTab.delete).toHaveBeenCalledWith({ where: { id: "tab_1" } });
  });

  it("seedProtectedDispositionTab: creates the protected tab AND seeds its four protected fields (D2: self-contained)", async () => {
    db.qcfFormTab.create.mockResolvedValue({ id: "tab_cd" } as never);
    db.qcfFormField.create.mockResolvedValue({} as never);
    ...
    expect(db.qcfFormField.create).toHaveBeenCalledTimes(4);
    const seeded = db.qcfFormField.create.mock.calls.map(
