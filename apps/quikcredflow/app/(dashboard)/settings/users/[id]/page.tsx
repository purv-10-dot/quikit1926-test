...
import { normalizeUserRole } from "@/lib/validators/settings-users";
...
export default function EditUserPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setEmail(usr.email);
      setPhone(usr.phone ?? "");
      // The DB stores legacy/alias roles ("admin"); the dropdown + validator use
      // canonical enum values ("Administrator"). Canonicalize so the option
      // matches and the PATCH submits a value the validator accepts.
      setRole(normalizeUserRole(usr.role));
      // The API stores status lowercase ("active"); the form + validator use
      // canonical case ("Active"/"Inactive"). Normalize so the dropdown matches
      // an option and submits a value the validator accepts.
      setStatus(usr.status?.toLowerCase() === "inactive" ? "Inactive" : "Active");
