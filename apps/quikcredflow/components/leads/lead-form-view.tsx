...
export interface LeadFormViewProps {
  ...
  substatus: string;
  setSubstatus: (v: string) => void;
  visibleSubstatuses: string[];
  source: string;
  ...
  const sourceField = (
    ...
    >
      {p.sourcesLoading ? (
        ...
        <Input value={p.source} onChange={(e) => p.setSource(e.target.value)} error={!!p.errors.source} required />
      )}
    </Field>
  );
    ...
    <LeadFormSection title="Lead Information" description="Ownership, pipeline, and lead classification.">
        ...
        <LeadFormRow>
          <Field label="Lead Name *" error={p.errors.name}>
            <Input value={p.name} onChange={(e) => p.setName(e.target.value)} error={!!p.errors.name} required />
          </Field>
        ...
        <LeadFormRow>
          ...
          >
            <Select value={p.stage} onChange={(e) => p.setStage(e.target.value)} error={!!p.errors.stage} required>
        ...
        <LeadFormRow>
          ...
          <Field label="Sub Stage">
            <Select
              value={p.substatus}
              onChange={(e) => p.setSubstatus(e.target.value)}
              disabled={p.visibleSubstatuses.length === 0}
            >
              <option value="">
                {p.visibleSubstatuses.length === 0 ? "No sub stages configured" : "— Select sub stage —"}
              </option>
              {p.visibleSubstatuses.map((s) => (
    ...
    <LeadFormSection title="Company Information">
        <LeadFormRow>
          <Field label="Company Name *" error={p.errors.company}>
            <Input value={p.company} onChange={(e) => p.setCompany(e.target.value)} error={!!p.errors.company} required />
          </Field>
    ...
    <LeadFormSection title="Contact Information">
        <LeadFormRow>
          <Field label="First Name *" error={p.errors.firstName}>
            <Input value={p.firstName} onChange={(e) => p.setFirstName(e.target.value)} error={!!p.errors.firstName} required />
          </Field>
          <Field label="Last Name *" error={p.errors.lastName}>
            <Input value={p.lastName} onChange={(e) => p.setLastName(e.target.value)} error={!!p.errors.lastName} required />
          </Field>
        </LeadFormRow>
        <LeadFormRow>
          <Field label="Email *" error={p.errors.email}>
            <Input type="email" value={p.email} onChange={(e) => p.setEmail(e.target.value)} error={!!p.errors.email} required />
          </Field>
          <Field label="Secondary Email" error={p.errors.secondaryEmail}>
            <Input type="email" value={p.secondaryEmail} onChange={(e) => p.setSecondaryEmail(e.target.value)} error={!!p.errors.secondaryEmail} />
          </Field>
        </LeadFormRow>
        <LeadFormRow>
          <Field label="Phone" help="Optional alternate number (e.g. landline)." error={p.errors.phone}>
            <PhoneField value={p.phone} onChange={p.setPhone} invalid={!!p.errors.phone} ariaLabel="Phone" />
          </Field>
          <Field label="Mobile *" help="Primary number used for calls." error={p.errors.mobile}>
        ...
        <LeadFormRow>
          <Field label="LinkedIn Account" error={p.errors.contactLinkedinUrl}>
            <Input
              ...
              error={!!p.errors.contactLinkedinUrl}
