...
export function LeadDashboardShell(props: LeadDashboardShellProps) {
  ...
  // Server data is authoritative. When router.refresh() re-runs the page's
  // server component and hands us fresh props — after a disposition save (an
  // FR-RE rule can move the Contact Stage), a quick edit, a transition, etc. —
  // sync it into local state so the header + pipeline update LIVE instead of
  // only after a full browser reload. Every optimistic setLead below is followed
  // by a persist, so syncing to server truth here never drops unsaved edits.
  // The prop identity only changes when the server actually re-renders (not on
  // this component's own state changes), so this won't clobber optimistic state.
  useEffect(() => {
    setLead(initialLead);
  }, [initialLead]);

  // [P3.F1a] Out-of-band staleness: an automation (or any server-side write with
  // NO client event — not just this user's own save) can change the lead, so the
  // sync above only helps after a router.refresh() we trigger ourselves. Refresh
  // on window focus + pageshow so returning to this tab or restoring from the
  // bfcache/router cache pulls committed server truth into `initialLead` (which
  // the effect above then syncs to local state — header, pipeline, AND the Quick
  // Edit drawer). Mirrors the proven 07-10 (e40e281) mount+pathname+pageshow
  // refetch pattern in lead-form.tsx / leads-explorer.tsx. router.refresh() only
  // pulls COMMITTED server state, and every optimistic setLead below is followed
  // by a persist, so this never clobbers unsaved edits.
  useEffect(() => {
    function refresh() {
      router.refresh();
    }
  ...
  // Number used for calls. NOTE: phone/mobile data is split inconsistently across
  // leads (most have the number in `phone`, some in `mobile`) — this is a data
  // issue logged for cleanup, not a display choice. Keep the original phone-first
  // preference; the ?? fallback covers leads whose number is only in mobile.
  ...
  return (
    <div className="pb-20 xl:pb-0">
        ...
        <div className="flex gap-2">
          {!isTrashed && phone ? (
            <CallButton
              ...
              variant="button"
              showNumber
              className="!bg-accent-600 !text-white hover:!bg-accent-700 text-xs"
            />
          ) : null}
          <button
            type="button"
      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start">
        <main className="min-w-0 flex-1 xl:w-[70%] xl:min-h-[32rem]">
          <LeadDetailTabs
            lead={{
              id: lead.id,
              name: lead.name,
      ...
      <QuickEditDrawer
        ...
        owners={owners}
        canEdit={canEdit}
        onPatch={(serverLead) => setLead((l) => ({ ...l, ...serverLead }))}
      />
