...
export function DispositionRuleBuilder({
  ...
  // CredFlow's trigger lives here) or "field:<fieldKey>" for a custom dropdown.
  ...
  const load = useCallback(async () => {
    try {
      const [ruleList, statusList, stageList] = await Promise.all([
        ...
        apiGet<string[]>("/api/leads/stages", "stages"),
      ]);
      setRules(ruleList);
  ...
  function openAdd() {
    setEditing(null);
    setName("");
    setSubject("status"); // default to the Status/disposition trigger (CredFlow's model)
    setOperator("is");
    setValueKey("");
    setValueKeys([]);
