...
async function main() {
  ...
  const existing = await prisma.qcfFormSet.findFirst({
  ...
  await prisma.$transaction(async (tx) => {
    ...
    // 2. version
    await tx.qcfFormSetVersion.create({
      data: {
    ...
    await tx.qcfFormSet.update({ where: { id: set.id }, data: { currentVersionId: ver.id } });
    // 4. tabs (createdByUserId nulled)
    for (const t of PAYLOAD.tabs) {
      await tx.qcfFormTab.create({
        data: {
