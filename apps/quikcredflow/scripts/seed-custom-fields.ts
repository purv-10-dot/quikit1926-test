 ...
 * QcfOrgWorkspaceSettings.settings.leadFieldDefinitions.
 *
 * WHAT / WHY:
 ...
 *     All date variants -> CredFlow "Date". Media fields (Payment Proof, etc.) ->
 ...
 *
 * USAGE (from apps/quikcredflow) — targets whatever DB your .env.local points at:
 ...
 *
 *   DB-target note: packages/database/.env can OVERRIDE apps/quikcredflow/.env.local
...
async function main() {
  ...
  console.log(`CredFlow custom-field seed  |  org=${orgId}  |  mode=${commit ? "COMMIT" : "DRY-RUN"}`);
  console.log("=".repeat(70));
