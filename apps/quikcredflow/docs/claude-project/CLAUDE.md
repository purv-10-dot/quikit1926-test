# CLAUDE.md — CredFlow (living context)
...
- **This project is `CredFlow` and lives at `C:\Projects\CredFlow`.** Before writing to any file in `docs/claude-project/`, verify the filesystem connector is actually pointed at THIS path. The connector on this machine can reach four checkouts (QuikIT, QuikSocial, QuikSign, CredFlow) and has reached the wrong one before. If you cannot confirm you are in `C:\Projects\CredFlow`, do NOT write — tell Rishabh.
- **THE NAMING TRAP:** the repo is `quikcrm-frontend`, the DB schema is `app_quikcrm`, and the UAT URL is `uatquikcrm.quikit.ai`. **None of these is QuikCRM.** CredFlow is a diverged client CRM that happens to share QuikCRM's schema lineage. MoreYeahs' own QuikCRM product is a SEPARATE codebase (`quikit1926`, `apps/quikcrm`). Never import QuikCRM decisions, specs, or facts into CredFlow work. Surface-level schema similarity is NOT evidence that a fact belongs to CredFlow.

---

## What CredFlow is
CredFlow CRM is a multi-tenant Next.js CRM built for the client **CredFlow** (an Indian fintech SaaS company, a MoreYeahs client). Its defining feature is telephony-integrated lead management with a configurable call-disposition **form-builder + rule engine ("FR-RE")**. Owned and solely developed by Rishabh at MoreYeahs. Client stakeholder: **Dev**.

## Repo & environment [confirmed: CC repo audit 2026-07-06]
- **Repo:** `quikcrm-frontend` (GitHub: akhileshgandhi/quikcrm-frontend). Cloned at `C:\Projects\CredFlow`. Working branch **`new_Merge_code_19June`** — clean, level with origin.
...
- **Dev server:** `next dev -p 3076` → **localhost:3076**.
- **Database:** local PG18, DB **`first_db_crm`** — shared multi-app DB, 12 schemas; CredFlow's schema is **`app_quikcrm`**. Full 12-schema DB kept intact [confirmed by Rishabh: sibling schemas inert to CredFlow, trimming risks unknown breakage].
- **Auth:** shared `auth` schema; working login `credflow@example.com`. Passwords bcrypt-hashed.
...
- **Running client deployment: IIS at `https://uatquikcrm.quikit.ai` (UAT)** [confirmed by Rishabh, firsthand — has seen the client app running there]. Despite the hostname, this is CredFlow, not QuikCRM.
...
- `scripts/seed-stage-status-mapping.ts` — PLACEHOLDER seed; real CredFlow mapping owed from Dev.
...
- Whether CredFlow still follows that governance / syncs back to the monorepo, or is a permanent fork, is **[open]** — never established; confirm with Akhilesh (repo owner) or Pravin.
...
- **Still honor the root file's CODING rules** (they're good discipline and enforced by review): `tenantId` filter on every DB query, `withTenantAuth` on DB-touching routes (except `/api/health`, `/api/auth/*`), no `as any`, `{success,data}`/`{success,error}` response shape, tests for 401 + tenant-isolation + happy-path, canonical file locations. See `CredFlow-Engineering-Practices.md`.

## Commands (PowerShell, Windows)
```powershell
npm run dev            # localhost:3076
...
## Reference docs (in-repo, CredFlow-authored — do not duplicate, cite)
...
- Living state: `docs/claude-project/QuikSign... ` → NO. CredFlow living state is `docs/claude-project/SESSION_LOG.md` and `CredFlow-Product-Status.md`.
