              ...
              <TBody>
                {items.map((u) => (
                  <TR key={u.id}>
                    ...
                    <TD>{u.permissionTemplates.map((p) => p.template.name).join(", ") || "—"}</TD>
  ...
  async function submit() {
    ...
    try {
      const res = await fetch("/api/settings/users", {
        ...
        body: JSON.stringify({ firstName: first, lastName: last, email, role }),
      });
      const j = await res.json();
  ...
  return (
    <Modal open={open} onClose={onClose} title="New user">
      ...
      <p className="mt-3 text-xs text-crm-muted">
        A temporary password will be generated and shown to you once. Share it with the user securely.
      </p>
