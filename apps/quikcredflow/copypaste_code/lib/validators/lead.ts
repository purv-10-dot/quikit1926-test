...
const createLeadObjectSchema = z.object({
  ...
  source: z.string().trim().min(1).default(LEAD_CREATE_DEFAULTS.source),
  ...
  followupPriority: z.enum(["Low", "Medium", "High"]).optional().nullable(),
...
export const updateLeadSchema = createLeadObjectSchema.partial();
