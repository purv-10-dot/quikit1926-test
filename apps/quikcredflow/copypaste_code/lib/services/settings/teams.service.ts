...
export async function listTeams(tenantId: string) {
  return prisma.crmSalesTeam.findMany({ where: { tenantId }, orderBy: { name: "asc" } });
}
...
export async function createTeam(opts: { actor: SessionUser; data: { name: string; managerId?: string | null } }) {
  ...
  return prisma.$transaction(async (tx) => {
    const dupe = await tx.crmSalesTeam.findFirst({ where: { tenantId: actor.tenantId, name: data.name } });
...
export async function updateTeam(opts: { actor: SessionUser; id: string; patch: { name?: string; managerId?: string | null } }) {
  ...
  return prisma.$transaction(async (tx) => {
    const before = await tx.crmSalesTeam.findFirst({ where: { id, tenantId: actor.tenantId } });
    if (!before) throw new SettingsConflictError("Team not found", 404);
    if (patch.name && patch.name !== before.name) {
      const dupe = await tx.crmSalesTeam.findFirst({ where: { tenantId: actor.tenantId, name: patch.name, id: { not: id } } });
...
export async function deleteTeam(opts: { actor: SessionUser; id: string }) {
  const { actor, id } = opts;
  return prisma.$transaction(async (tx) => {
    const target = await tx.crmSalesTeam.findFirst({ where: { id, tenantId: actor.tenantId } });
