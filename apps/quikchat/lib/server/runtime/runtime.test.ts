...
/**
 * ⏳ TEMPORARY — the `RUNTIME_TOKEN_DEBUG` diagnostic (19 Aug 2026). Delete this
 * whole block with the helper and its call site; grep the flag name to find all
 * three.
 *
 * The load-bearing test here is the last one: a JWT in a log file is a credential
 * in a log file, and ours is HS256 with a shared secret.
 */
describe("runtime token debug logging (TEMPORARY — RUNTIME_TOKEN_DEBUG)", () => {
  beforeEach(() => {
    process.env.AGENT_JWT_SECRET = "test-agent-secret";
    delete process.env.RUNTIME_TOKEN_DEBUG;
  });
  afterEach(() => {
    vi.restoreAllMocks();
    delete process.env.AGENT_JWT_SECRET;
  ...
  /** Drive a real assist turn and hand back whatever the logger was called with. */
  async function runAssist() {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => sseResponse(['data: {"type":"done","text":"ok","agentRunId":"r1"}\n\n'])),
    );
    const { HttpRuntimeClient } = await import("./http");
    await collect(new HttpRuntimeClient("https://r").assist(input));
    return { info, warn };
  ...
  it("logs NOTHING when the flag is unset — it is off by default on a hot path", async () => {
    const { info } = await runAssist();
    expect(
      info.mock.calls.filter(([, msg]) => String(msg).includes("runtime token debug")),
    ).toHaveLength(0);
  });

  it("logs the decoded payload and the claim keys when the flag is set", async () => {
    process.env.RUNTIME_TOKEN_DEBUG = "1";
    const { info } = await runAssist();

    const call = info.mock.calls.find(([, msg]) => String(msg).includes("runtime token debug"));
    expect(call).toBeTruthy();
    // `as unknown as` because pino's overloads type the first arg as `string`.
    const fields = call![0] as unknown as {
      botAgentId: string;
      tokenPayload: Record<string, unknown>;
      tokenClaimKeys: string[];
    };
    // The shared fixture mints with the real production identity, so this is
    // the payload the runtime team actually receives — not a test double.
    expect(fields.botAgentId).toBe("quikchat-assistant");
    expect(fields.tokenPayload).toMatchObject({
      iss: "quikchat",
      aud: "quikverse-runtime",
      sub: "quikchat-assistant",
      orgId: "o1",
      userId: "u1",
      actorType: "ai_agent",
    });
    // The key set is the actual question the runtime team asked.
    expect(fields.tokenClaimKeys).toContain("orgId");
    expect(fields.tokenClaimKeys).toContain("exp");
  });

  it("reads the flag at call time, so UAT can flip it without a rebuild", async () => {
    // A module-level const would make the first (unset) run poison every later
    // one in the same process — which is exactly what a redeployless toggle
    // must not do.
    const off = await runAssist();
    expect(
      off.info.mock.calls.filter(([, m]) => String(m).includes("runtime token debug")),
    ).toHaveLength(0);

    vi.restoreAllMocks();
    process.env.RUNTIME_TOKEN_DEBUG = "1";
    const on = await runAssist();
    expect(
      on.info.mock.calls.filter(([, m]) => String(m).includes("runtime token debug")),
    ).toHaveLength(1);
  });

  /**
   * ⚠️ THE ONE THAT MATTERS. The helper reads `parts[1]` and nothing else, so the
   * signature is never even bound to a variable — this asserts that structural
   * guarantee against the actual emitted fields rather than trusting it.
   */
  it("never emits the token or its signature", async () => {
    process.env.RUNTIME_TOKEN_DEBUG = "1";
    const { info } = await runAssist();
    ...
    // The signature segment of the token that was actually sent.
    ...
    const [, , signature] = sentToken.split(".");

    expect(sentToken).toBeTruthy();
    expect(signature).toBeTruthy();
    expect(serialised).not.toContain(sentToken);
    expect(serialised).not.toContain(signature);
    // And no field is named `token` — pino would censor it, hiding the payload.
    expect(Object.keys(call[0] as unknown as object)).not.toContain("token");
  });

  it("warns rather than throwing when the token is unparseable", async () => {
    // A diagnostic must never take down an assist turn.
    process.env.RUNTIME_TOKEN_DEBUG = "1";
    const { logger } = await import("@/lib/shared/logger");
    const warn = vi.spyOn(logger, "warn").mockImplementation(() => undefined as never);
    const { logRuntimeTokenPayload } = await import("./http");

    expect(() => logRuntimeTokenPayload("not-a-jwt", "bot-1")).not.toThrow();
    expect(() => logRuntimeTokenPayload("aaa.!!!not-base64-json!!!.ccc", "bot-1")).not.toThrow();
