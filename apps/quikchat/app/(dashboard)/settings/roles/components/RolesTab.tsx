...
/**
 * Roles & Permissions — two-pane admin surface (ported from QuikScale's
 * RolesTab, adapted for QuikChat):
 *   - Left: role list + create/delete (non-system).
 *   - Right: RolePermissionMatrix for the selected role.
 *
 * Adaptations from the QuikScale original:
 *   - `useResourcePermissions` DROPPED — the whole page is `requireAdmin`-gated
 *     (server-enforced), so create/delete affordances are always shown.
 *   - `accent-*` classes swapped for neutral blue/gray (QuikChat doesn't mount
 *     QuikScale's ThemeApplier that backs the accent CSS vars).
 * Data source `/api/org/roles`, `{ success, data }` envelope.
 */
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <main className="flex-1 min-w-0 bg-white overflow-hidden">
        {selectedId ? (
          <RolePermissionMatrix key={selectedId} roleId={selectedId} onRenamed={() => load(selectedId)} />
        ) : (
            ...
            <p className="text-xs text-gray-600 mb-5">
              {confirmDeleteRole._count && confirmDeleteRole._count.members > 0 ? (
                <>
                  ...
                  {confirmDeleteRole._count.members === 1 ? "" : "s"} currently have this role. They
