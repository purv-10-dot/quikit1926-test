 ...
 * (and the AppAccessDeniedPopup's "other apps" count). MUST match what the
 * QuikIT launcher (`/apps`) shows for the same user + active org, so the
 * switcher and the portal never disagree. Ported from quiktrack's route
 * (identical visibility rule); only the quikchat env override + localhost
 * fallback are added.
 *
 * Visibility rule (mirrors the launcher):
 *   1. org must have OrgAppAccess.enabled = true for the app
 *   2. App.requiresOrgAdmin ⇒ caller must be admin-tier (or super admin)
 *   3. org/super admins see every provisioned app; everyone else needs an
 *      explicit UserAppAccess row
 *   4. `quikit` (the launcher) + HIDDEN_APP_SLUGS are excluded
 */
export async function GET() {
  ...
  const orgAllows = orgId
    ? await db.orgAppAccess.findMany({ where: { orgId, enabled: true }, select: { appId: true } })
    : [];
  const orgAllowedAppIds = new Set(orgAllows.map((a) => a.appId));

  const userAccess = orgId
    ? await db.userAppAccess.findMany({ where: { userId, orgId }, select: { appId: true } })
    : [];
  const userAppIds = new Set(userAccess.map((u) => u.appId));

  // Env-override map: use a per-app URL from env if set (lets local dev run
  // against a Neon DB whose App.baseUrl rows hold prod URLs).
