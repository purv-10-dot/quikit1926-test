...
const factoryMiddleware = createMiddleware({
  loginRoute: "/login",
  // Only these two paths are public. Everything else — including "/" — is
  // SSO-gated. The factory exact-matches "/" (see packages/auth/middleware.ts:
  // `r === "/" ? pathname === "/" : pathname.startsWith(r)`), so omitting it
  // here gates the root without any custom passthrough.
