 ...
 *           routed via the launcher handoff (cross-domain cookie bridge).
 ...
 *   to every dashboard module. Keeping the loginRoute as a unique sentinel
 *   and rewriting the bounce here is the safe pattern until the factory
 *   grows an exact-match option.
 */
...
const factoryMiddleware = createMiddleware({
  ...
  publicRoutes: ["/login", "/invitations", "/auth-handoff", "/s", "/api/health"],
  centralLoginUrl: AUTH_URL ? `${AUTH_URL}/login` : undefined,
  centralSelectOrgUrl: QUIKIT_URL ? `${QUIKIT_URL}/apps` : undefined,
  // Disable cross-app token revalidation: QuikScale sessions are minted
  // by the launcher handoff (not the auth app), so the auth app's
  // /api/verify-token cannot decode them and returns {valid:false},
  // which would log the user out on every request.
  enforceRemoteSessionValidation: false,
});
