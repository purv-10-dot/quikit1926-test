...
export const PUT = withTenantAuth<{ id: string }>(async ({ tenantId, userId }, request, { params }) => {
  ...
  const existing = await db.clientMember.findFirst({ where: { id: params.id, tenantId, deletedAt: null } });
  ...
  if (parsed.data.email && parsed.data.email !== existing.email) {
    const dup = await db.clientMember.findFirst({
      where: { tenantId, email: parsed.data.email, deletedAt: null, id: { not: params.id } },
    });
