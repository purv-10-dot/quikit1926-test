...
export const PUT = withOrgAuth<{ id: string }>(
  async ({ orgId }, request, { params }) => {
    const existing = await db.accountabilityFunction.findFirst({
      where: { id: params.id, orgId, chartType: "face", deletedAt: null },
    });
  ...
  { moduleKey: "face", fallbackErrorMessage: "Failed to update FACe function" },
);

// Soft-delete: stamp deletedAt so the row stays in the DB for audit/restore.
// Cascades to child functions (so deleting a parent hides its sub-functions too).
export const DELETE = withOrgAuth<{ id: string }>(
  ...
  { moduleKey: "face", fallbackErrorMessage: "Failed to delete FACe function" },
);
