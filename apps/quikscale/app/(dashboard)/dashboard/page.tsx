...
function AvgKPICard({ kpis, currentWeek, weekCount = 13 }: { kpis: KPIRow[]; currentWeek: number | null; weekCount?: number }) {
  // `avg` = (Σ QTD Achieved / Σ QTD Goal) × 100 across every entered KPI — an
  // aggregate ratio, not a mean of each card's own percentage. See
  // computeKpiOverviewStats for the worked reasoning; the on-track/at-risk/
  // behind buckets still use each card's own resolveProgressQtd-based color so
  // the pill's bucket counts always agree with what's on screen.
