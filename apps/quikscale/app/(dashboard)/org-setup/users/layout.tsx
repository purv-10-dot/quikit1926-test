/**
 * FF-1 route gate for "orgSetup.users" module. Redirects to /dashboard?feature_disabled=orgSetup.users
...
export default async function FeatureFlagLayout({ children }: { children: React.ReactNode }) {
  await gateModuleRoute("quikscale", "orgSetup.users", authOptions);
  return <>{children}</>;
}
