...
function MemberPills({
  ...
  if (members.length === 0)
    return <span className="text-xs text-gray-300">No members yet</span>;
  ...
  return (
    <div className="flex items-center gap-1 flex-wrap">
      ...
      {extra > 0 && (
        <span className="h-6 px-1.5 rounded-full bg-gray-100 text-gray-500 text-[9px] font-semibold flex items-center">
          +{extra}
        </span>
      )}
    </div>
...
function TeamPanel({
  ...
  const selectedMembers = users.filter((u) =>
    form.memberIds.includes(u.userId)
  );
  ...
  return (
    <RightPanel
      ...
      subtitle={
        editTeam
          ? "Update team details"
          : "Set up a new team for your organisation"
      }
      footer={
        <RightPanelFooter>
    ...
    >
      ...
      <div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <label
            title="Custom colour"
            className="h-8 w-8 rounded-full border-2 border-dashed border-gray-300 flex items-center justify-center cursor-pointer hover:border-gray-400 overflow-hidden relative"
          >
            <input
              type="color"
              value={form.color}
          ...
          >
            {selectedHead ? (
              <div className="flex items-center gap-2">
                <div
                  className={`h-5 w-5 rounded-full flex items-center justify-center text-white text-[9px] font-bold ${avatarBg(`${selectedHead.firstName} ${selectedHead.lastName}`)}`}
                >
                  ...
                  filteredUsers.map((u) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div>
                          <span
                            className={`block text-sm ${form.headId === u.userId ? "text-accent-600 font-medium" : "text-gray-700"}`}
                          >
                            {full}
                          </span>
                ...
                {filteredMemberUsers.length === 0 ? (
                  ...
                  filteredMemberUsers.map((u) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className={`h-4 w-4 rounded border flex items-center justify-center flex-shrink-0 ${selected ? "bg-accent-600 border-accent-600" : "border-gray-300 bg-white"}`}
                        >
                          {selected && (
                            <svg
                        ...
                        <div className="min-w-0 text-left">
                          <span
                            className={`block text-sm truncate ${selected ? "text-accent-600 font-medium" : "text-gray-700"}`}
                          >
                            {full}
                          </span>
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-2xl p-6 w-80 max-w-full">
        ...
        <p className="text-xs text-gray-500 mb-5">
          <span className="font-semibold text-gray-700">{teamName}</span> will
          be permanently deleted. Members assigned to this team will be
          unassigned but not removed from the organisation.
        </p>
          ...
          {available.length === 0 ? (
            ...
            available.map((u) => {
              ...
              return (
                ...
                >
                  <div
                    className={`h-8 w-8 rounded-full flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0 ${avatarBg(full)}`}
                  >
                    {initials(u.firstName, u.lastName)}
                  </div>
...
export default function OrgTeamsPage() {
  ...
  async function handleRemoveMember(team: OrgTeam, userId: string) {
    const res = await fetch(`/api/org/teams/${team.id}/members/${userId}`, {
      method: "DELETE",
    });
    const json = await res.json();
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-2">
                        {team.members.map((m) => {
                          ...
                          return (
                            <div
                              key={m.userId}
                              className="group flex items-center gap-2 py-1 pr-1 rounded hover:bg-white transition-colors"
                            >
                              <div
                                className={`h-6 w-6 rounded-full flex items-center justify-center text-white text-[9px] font-bold flex-shrink-0 ${avatarBg(full)}`}
                              >
                              ...
                              <button
                                onClick={() =>
                                  handleRemoveMember(team, m.userId)
                                }
