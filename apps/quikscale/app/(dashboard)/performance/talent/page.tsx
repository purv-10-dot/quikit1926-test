...
> = {
  "high-high": {
    label: "Star",
    sub: "High performer, high potential",
    bg: "bg-purple-50",
  ...
  "high-medium": {
    ...
    sub: "Delivers results, moderate potential",
    bg: "bg-green-50",
  ...
  "high-low": {
    label: "Specialist",
    sub: "Expert in role, limited growth",
    bg: "bg-teal-50",
  ...
  "medium-high": {
    ...
    sub: "Growth ahead, building performance",
    bg: "bg-accent-50",
  ...
  "medium-medium": {
    label: "Core Player",
    sub: "Solid contributor, steady growth",
    bg: "bg-sky-50",
  ...
  "medium-low": {
    ...
    sub: "Meets expectations, limited upside",
    bg: "bg-gray-50",
  ...
  "low-high": {
    label: "Enigma",
    sub: "High potential, underperforming",
    bg: "bg-amber-50",
  ...
  "low-medium": {
    label: "Inconsistent",
    sub: "Variable performance, some potential",
    bg: "bg-orange-50",
  ...
  "low-low": {
    ...
    sub: "Needs significant improvement",
    bg: "bg-red-50",
...
const POTENTIAL_LABELS: Record<PotentialBand, string> = {
  high: "High",
  medium: "Medium",
  low: "Low",
};
const FLIGHT_LABELS: Record<FlightRisk, string> = {
  low: "Low",
  medium: "Medium",
  high: "High",
};
const SUCCESSION_LABELS: Record<Succession, string> = {
  "not-ready": "Not Ready",
...
function flightBadge(risk: FlightRisk | null) {
  ...
  return (
    <span
      className={`px-1.5 py-0.5 rounded text-xs font-medium ${styles[risk]}`}
    >
      {FLIGHT_LABELS[risk]}
    </span>
  );
...
function successionBadge(s: Succession | null) {
  ...
  const styles = {
    ...
    developing: "bg-accent-100 text-accent-700",
    "ready-now": "bg-green-100 text-green-700",
  };
  return (
    <span className={`px-1.5 py-0.5 rounded text-xs font-medium ${styles[s]}`}>
      {SUCCESSION_LABELS[s]}
    </span>
  );
}
...
function AssessmentDrawer({
  ...
  person: Person;
  onClose: () => void;
  onSaved: (updated: Partial<Person>) => void;
  currentQuarter: string;
  currentYear: number;
}) {
  const [potential, setPotential] = useState<PotentialBand>(
    person.potential || "medium"
  );
  const [flightRisk, setFlightRisk] = useState<FlightRisk>(
    person.flightRisk || "low"
  );
  const [succession, setSuccession] = useState<Succession>(
    person.successionReady || "not-ready"
  );
  ...
  return (
      ...
      footer={
        <RightPanelFooter>
          <p className="text-xs text-gray-400">
            {person.lastAssessed
              ? `Last saved: ${new Date(person.lastAssessed).toLocaleDateString()}`
              : "No assessment yet"}
          </p>
          <button
            onClick={save}
    ...
    >
      {/* 9-box preview — first scrollable item, replaces the previously sticky card */}
      <div className={`p-3 rounded-lg border ${box.bg} ${box.border}`}>
        <p className={`text-xs font-bold ${box.text}`}>{box.label}</p>
        <p className={`text-xs mt-0.5 ${box.text} opacity-80`}>{box.sub}</p>
        <p className="text-xs text-gray-500 mt-1">
          Performance:{" "}
          <span className="font-medium capitalize">{person.perfBand}</span>
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          <div className="flex gap-2">
            {(["not-ready", "developing", "ready-now"] as Succession[]).map(
              (v) => (
                <OptionBtn
                  key={v}
                  value={v}
                  current={succession}
                  label={SUCCESSION_LABELS[v]}
        ...
        <div>
          ...
          <div className="flex gap-2 mb-2">
            ...
            <button
              onClick={addSkill}
              className="p-1.5 bg-accent-50 hover:bg-accent-100 rounded border border-accent-200"
            >
          ...
          <div className="flex flex-wrap gap-1.5">
            {skills.map((s) => (
              <span
                key={s}
                className="flex items-center gap-1 bg-accent-50 text-accent-700 border border-accent-200 text-xs px-2 py-0.5 rounded-full"
              >
                {s}
                <button
...
export default function TalentAssessmentPage() {
  ...
  const readyNow = people.filter(
    (p) => p.successionReady === "ready-now"
  ).length;
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex border border-gray-200 rounded overflow-hidden">
            <button
              ...
              className={`p-1.5 ${view === "grid" ? "bg-accent-600 text-white" : "bg-white text-gray-400 hover:bg-gray-50"}`}
            >
              <LayoutGrid className="w-3.5 h-3.5" />
            </button>
            <button
              onClick={() => setView("list")}
              className={`p-1.5 ${view === "list" ? "bg-accent-600 text-white" : "bg-white text-gray-400 hover:bg-gray-50"}`}
            >
    ...
    <div className="mt-2">
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1">
          {/* X-axis top labels */}
          <div
            className="grid grid-cols-3 gap-1 mb-1 ml-0"
            style={{ gridTemplateColumns: "repeat(3, 1fr)" }}
          >
            {potentials.map((pot) => (
              <div
                key={pot}
                className="text-center text-xs text-gray-400 font-medium py-0.5 capitalize"
              >
                {pot} Potential
              </div>
            ))}
            ...
            >
              {potentials.map((pot) => {
                ...
                return (
                  <div
                    key={key}
                    className={`${cfg.bg} border ${cfg.border} rounded-lg p-3 min-h-[130px]`}
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        ...
                        <p
                          className={`text-xs ${cfg.text} opacity-70 leading-tight mt-0.5`}
                        >
                          {cfg.sub}
                        </p>
                      </div>
                      {cell.length > 0 && (
                        <span
                          className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${cfg.bg} border ${cfg.border} ${cfg.text}`}
                        >
                          {cell.length}
                        </span>
                      )}
          ...
          <thead className="sticky top-0 z-10">
            <tr>
              {[
                ...
                <th
                  key={h}
                  className="bg-accent-50 border-b border-r border-gray-200 px-3 py-2 text-left text-xs font-semibold text-gray-500 whitespace-nowrap last:border-r-0"
                >
                  {h}
                </th>
              ))}
          ...
          <tbody>
            ...
            {filtered.map((p, i) => {
              ...
              return (
                ...
                >
                  <td className="px-3 py-2 border-b border-r border-gray-100 text-gray-400">
                    {i + 1}
                  </td>
                  <td className="px-3 py-2 border-b border-r border-gray-100">
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-6 h-6 rounded-full ${avatarColor(p.firstName)} flex items-center justify-center text-white text-xs font-bold flex-shrink-0`}
                      >
                        {initials(p)}
                      </div>
                      <div>
                  ...
                  <td className="px-3 py-2 border-b border-r border-gray-100 text-gray-600">
                    {p.teamName || "—"}
                  </td>
                  <td className="px-3 py-2 border-b border-r border-gray-100">
                    <span
                  ...
                  <td className="px-3 py-2 border-b border-r border-gray-100">
                    <span
                      className={`px-1.5 py-0.5 rounded text-xs font-medium border ${box.bg} ${box.border} ${box.text}`}
                    >
                      {box.label}
                    </span>
                  </td>
                  <td className="px-3 py-2 border-b border-r border-gray-100">
                    {flightBadge(p.flightRisk)}
                  </td>
                  <td className="px-3 py-2 border-b border-r border-gray-100">
                    {successionBadge(p.successionReady)}
                  </td>
                  <td className="px-3 py-2 border-b border-r border-gray-100">
                    <div className="flex flex-wrap gap-1">
                      {p.skills.slice(0, 3).map((s) => (
                        <span
                          key={s}
                          className="bg-accent-50 text-accent-600 border border-accent-100 text-xs px-1.5 py-0.5 rounded-full"
                        >
                          {s}
                        </span>
                      ))}
