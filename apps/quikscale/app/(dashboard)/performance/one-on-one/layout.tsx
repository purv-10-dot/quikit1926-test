/**
 * FF-1 route gate for "people.oneOnOne" module. Redirects to /dashboard?feature_disabled=people.oneOnOne
...
export default async function FeatureFlagLayout({ children }: { children: React.ReactNode }) {
  await gateModuleRoute("quikscale", "people.oneOnOne", authOptions);
  return <>{children}</>;
}
