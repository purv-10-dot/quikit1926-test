...
function scoreColor(score: number | null) {
  if (score === null)
    return { text: "text-gray-400", bg: "bg-gray-100", label: "—" };
  if (score >= 80)
    return { text: "text-green-700", bg: "bg-green-100", label: `${score}%` };
  if (score >= 60)
    return { text: "text-amber-700", bg: "bg-amber-100", label: `${score}%` };
  ...
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((n) => (
        ...
        >
          <Star
            className={`h-4 w-4 ${n <= (hover || value) ? "text-amber-400 fill-amber-400" : "text-gray-300"}`}
          />
        </button>
      ))}
    </div>
            ...
            <thead className="sticky top-0 z-30 bg-accent-50">
              <tr>
                {[
                  ...
                  <th
                    key={h}
                    className="px-3 py-2.5 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wide border-b border-r border-gray-200 bg-accent-50 whitespace-nowrap"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody className="bg-white">
              {reviews.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2.5 border-b border-r border-gray-100 text-gray-500">
                    {r.reviewer?.firstName} {r.reviewer?.lastName}
                  </td>
                  <td className="px-3 py-2.5 border-b border-r border-gray-100">
                    <ScorePill score={r.kpiScore} />
                  </td>
                  <td className="px-3 py-2.5 border-b border-r border-gray-100">
                    <ScorePill score={r.priorityScore} />
                  </td>
                  <td className="px-3 py-2.5 border-b border-r border-gray-100">
                  ...
                  <td className="px-3 py-2.5 border-b border-gray-100">
                    <span
                      className={`px-1.5 py-0.5 rounded text-[11px] font-medium ${r.status === "submitted" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
                    >
                      {r.status}
                    </span>
                  </td>
            ...
            <RightPanelFooter>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  disabled={saving}
                  className="px-3 py-1.5 text-xs bg-accent-500 hover:bg-accent-600 text-white rounded-md transition-colors disabled:opacity-50"
                >
        ...
        >
          ...
          <div>
            <p className="text-xs font-medium text-gray-700 mb-2">
              Scores (auto-populated, editable)
            </p>
            <div className="grid grid-cols-2 gap-3">
              {(
                [
