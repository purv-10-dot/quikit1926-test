/**
 * One-time data backfill: legacy AuditLog (entityType="Priority") → new
 * AuditEvent / AuditChange.
 *
 * Run MANUALLY, once per environment, AFTER the audit schema migrations
 * (20260609120000_add_audit_event_change_authlog + 20260610120000_add_audit_event_read)
 * have been applied.
 *
 * Usage (from the repo root):
 *   npx tsx apps/quikscale/scripts/backfill-priority-audit.ts --dry-run
 *   npx tsx apps/quikscale/scripts/backfill-priority-audit.ts
 *   npx tsx apps/quikscale/scripts/backfill-priority-audit.ts --before 2026-06-10T00:00:00Z
 *
 * Flags mirror backfill-kpilog-audit.ts:
 *   --dry-run         Parse + map + print counts; write NOTHING.
 *   --before <ISO>    Only migrate AuditLog rows older than this (default: now).
...
import { mapPriorityAuditLog, isAggregateLog, type LegacyAuditLog } from "../lib/audit/priorityBackfill";
...
async function main() {
  ...
  console.log(
    `AuditLog(Priority) → AuditEvent backfill ${args.dryRun ? "(DRY RUN)" : ""}\n` +
      `  before:  ${args.before.toISOString()}\n  batch:   ${args.batch}\n`,
  );

  const userNameCache = new Map<string, string>();

  const where = { entityType: "Priority", createdAt: { lt: args.before } } as const;
  const total = await db.auditLog.count({ where });
  console.log(`Found ${total} AuditLog(Priority) rows to process.\n`);
  ...
  while (true) {
    ...
    for (const log of batch) {
      // Skip aggregate bulk-restore rows (entityId = "id1,id2,...").
      if (isAggregateLog(log)) { skipped++; continue; }

      const { event, changes } = mapPriorityAuditLog(log, {
        actorName: userNameCache.get(log.actorId) ?? "—",
      });
      // Skip the empty-UPDATE noise (shares the panel's read-time predicate).
