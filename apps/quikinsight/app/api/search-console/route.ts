...
export async function GET(req: Request) {
  ...
  // ?days=N. No params at all -> undefined, so getSearchConsoleData falls
  // back to its own 28-day default — preserving this endpoint's pre-existing
  ...
  try {
    const data = await getSearchConsoleData(session.user.id, window ?? 28, workspaceId);
