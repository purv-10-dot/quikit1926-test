...
interface Detail { companyName: string; candidateName: string; title: string; files: FileItem[]; completed: boolean }

export default function ExitAckPortal({ params }: { params: { token: string } }) {
  const { token } = params;
  ...
  const refresh = () =>
    fetch(withBasePath(`/api/v1/hrms/offboarding/policy-ack/${token}`))
      .then((r) => r.json())
  ...
  const acknowledge = async (url: string) => {
    setBusy(url);
    try {
      const r = await fetch(withBasePath(`/api/v1/hrms/offboarding/policy-ack/${token}`), {
  ...
  const rawHref = (f: FileItem) =>
    withBasePath(`/api/v1/hrms/offboarding/policy-ack/${token}?raw=${encodeURIComponent(f.key ?? "")}`);
