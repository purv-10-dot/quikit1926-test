...
export const GET = withAuth(async (req: NextRequest, { orgId, userId }) => {
  try {
    ...
    if (bundle && bundle !== "PreOffer" && bundle !== "PostOffer") {
      return validationError("Invalid bundle");
    }
    await ensureCandidateDocDefaults(orgId, userId);

    const where: Record<string, unknown> = { orgId, deletedAt: null };
    if (!includeInactive) where.isActive = true;
    if (bundle) where.bundle = bundle as DocumentBundle;

    const types = await prisma.candidateDocumentType.findMany({
      where,
      orderBy: [{ bundle: "asc" }, { sortOrder: "asc" }],
    });
...
export const POST = withAuth(async (req: NextRequest, { orgId, userId }) => {
  try {
    ...
    if (bundle !== "PreOffer" && bundle !== "PostOffer") return validationError("Invalid bundle");
    ...
    const dup = await prisma.candidateDocumentType.findUnique({
      where: { orgId_code_bundle: { orgId, code: codeRaw, bundle } },
    }).catch(() => null);
    if (dup) return validationError(`A document with code "${codeRaw}" already exists in ${bundle} bundle`);

    const created = await prisma.candidateDocumentType.create({
      data: {
        orgId, name, code: codeRaw, bundle,
