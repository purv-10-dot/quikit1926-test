...
export const POST = withAuth(async (_req: NextRequest, { orgId, userId }, { id }) => {
  try {
    ...
    if (loan.status === "Closed") return validationError("This loan is already closed.");
    if (loan.status !== "Disbursed") return validationError("Only a disbursed loan can be closed.");
