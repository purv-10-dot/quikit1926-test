...
// HR clicks "Send acknowledgement request" on a Policy / Training step → emails
// the new hire a secure link to read & acknowledge the attached files, and marks
// the task In Progress (Requested). Mirrors the document-upload request flow.
export const POST = withAuth(async (_req: NextRequest, { orgId, userId }, params) => {
  try {
    ...
    if (task.stepType !== "ReadPolicy") return validationError("This task is not a policy / training step.");

    const config = (task.config ?? {}) as Record<string, unknown>;
    const files = Array.isArray(config.files) ? (config.files as PolicyFile[]).filter((f) => f && f.url) : [];
    if (files.length === 0) return validationError("No files attached to this step yet.");
    ...
    console.error("POST /onboarding/tasks/[taskId]/ack-request error:", error);
    return internalError();
  }
}, { requiredPermissions: ["hrms.onboarding.write"], rateLimit: { max: 10, windowSec: 60, scope: "onboarding.ack-request" } });
