...
const typeStyles: Record<string, { bg: string; text: string; ring: string }> = {
  PulseCheck: { bg: "bg-blue-50", text: "text-blue-700", ring: "ring-blue-200" },
  ...
  return (
    <div>
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#0F1F3D] via-[#1E3A8A] to-[#2563EB] mb-6">
        <div className="relative px-8 py-7 flex items-center justify-between gap-6">
          <div>
            ...
            <p className="text-white/75 text-sm mt-1">Take a minute to share feedback. Most close in a few days.</p>
          </div>
          <div className="flex items-center gap-3 shrink-0">
            <Link
              href="/engage/surveys"
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white text-[#0F1F3D] text-sm font-semibold shadow-lg hover:shadow-xl transition-shadow"
            >
              <BarChart3 size={16} /> <span className="hidden sm:inline">Surveys &amp; pulse checks</span><span className="sm:hidden">All surveys</span>
            </Link>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
        <StatCard iconBg="bg-blue-50" iconColor="text-blue-600" Icon={ClipboardList} label="Pending" value={pending.length} caption="Awaiting your input" />
      ...
      {/* Pending */}
      <h2 className="text-sm font-bold text-gray-900 mb-3">Awaiting your response</h2>
      {isLoading ? (
        <SkeletonCards count={2} />
      ) : pending.length === 0 ? (
        <div className="surface-card p-12 text-center mb-8">
          <div className="w-14 h-14 rounded-2xl bg-emerald-50 flex items-center justify-center mx-auto mb-3">
            <CheckCircle2 size={24} className="text-emerald-600" />
          </div>
          <p className="font-serif-display text-lg font-bold text-gray-900">All caught up</p>
          <p className="text-sm text-gray-500 mt-1">No pending surveys right now.</p>
        </div>
      ) : (
      ...
      {completed.length > 0 && (
        <>
          <h2 className="text-sm font-bold text-gray-900 mb-3">Completed</h2>
...
function SurveyCard({ s, idx }: { s: MySurvey; idx?: number }) {
  ...
  return (
    <Link
      ...
      className={`row-stagger surface-card p-5 hover:shadow-md transition-all flex items-start gap-4 ${closingSoon ? "ring-1 ring-amber-200 bg-gradient-to-br from-amber-50/40 to-white" : ""}`}
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <h3 className="font-serif-display text-lg font-bold text-gray-900 mt-1.5 leading-snug">{s.title}</h3>
