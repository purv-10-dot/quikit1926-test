...
const STATUS_COLORS: Record<string, string> = {
  ...
  CompOff: "bg-indigo-50 text-indigo-700 border-indigo-200",
...
export default function AdminAttendancePage() {
  ...
  const exportCsv = () => {
    ...
    const headers = [
      ...
      "Late (min)", "Early Out (min)", "Source", "Regularization",
    ];
  ...
  return (
    <div className="w-full px-6 py-6">
      <Link href="/attendance" className="inline-flex items-center gap-1 text-xs text-gray-500 hover:text-[#3b82f6] mb-4">
        <ChevronLeft size={14} /> Back to attendance
      </Link>

      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div>
          <h1 className="font-serif-display text-3xl md:text-4xl font-bold text-gray-900">Admin Attendance</h1>
          <p className="text-sm text-gray-500 mt-1">View all employees check-in/out, hours, and status.</p>
        </div>
        <button
          onClick={exportCsv}
          disabled={records.length === 0}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg shadow-sm disabled:opacity-40"
        >
          ...
          {balancesLoading ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2.5">
              {balances.map((b) => {
                ...
                const color = b.leaveType.color || "#3b82f6";
                return (
                  <div key={b.id} className="border border-gray-200 rounded-lg p-2.5 bg-gradient-to-br from-white to-gray-50/50">
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className="text-[9px] px-1.5 py-0.5 rounded font-mono" style={{ backgroundColor: `${color}20`, color }}>
                        {b.leaveType.code}
                      </span>
                    </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-3 mb-4 shadow-sm space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div>
            <label className="block text-[11px] font-semibold text-gray-600 mb-1">Search name/code</label>
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search..."
                className="w-full pl-7 pr-2 py-2 border border-[var(--border)] rounded-lg text-sm"
              />
            </div>
          </div>
        </div>
            ...
            {(["week", "month", "custom"] as Period[]).map((p) => (
              <button
                ...
                className={clsx(
                  ...
                  period === p ? "bg-[#3b82f6] text-white" : "bg-white text-gray-600 hover:bg-gray-50",
                )}
              >
                {p}
              </button>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-gray-600 text-[11px] uppercase">
              <tr>
                ...
                <th className="text-left px-3 py-2 font-medium">Eff. Hrs</th>
            ...
            <tbody>
              {records.map((r, i) => (
                <tr key={r.id} className="row-stagger border-t border-gray-100 hover:bg-gray-50" style={{ ["--i" as never]: Math.min(i, 10) }}>
                  ...
                  <td className="px-3 py-2 text-gray-600 text-xs">{r.employee.department?.name ?? "—"}</td>
                  <td className="px-3 py-2">
                    <span className={clsx("inline-flex px-2 py-0.5 rounded text-[11px] font-semibold border", STATUS_COLORS[r.status])}>
                      {r.status}
                    </span>
                  </td>
                  <td className="px-3 py-2 font-mono text-xs text-gray-700">{fmtTime(r.checkIn)}</td>
                  <td className="px-3 py-2 font-mono text-xs text-gray-700">{fmtTime(r.checkOut)}</td>
                  ...
                  <td className="px-3 py-2">
                    <div className="flex flex-wrap gap-1">
                      ...
                      {r.regularizationStatus !== "None" && (
                        <span className="text-[10px] px-1.5 py-0.5 bg-indigo-50 text-indigo-700 border border-indigo-200 rounded">
