...
interface WeekSummary {
  days: DayCell[];
  totals: { payableDays: number; presentDays: number; onDutyDays: number; paidLeaveDays: number; holidayDays: number; weekendDays: number; absentDays: number; totalHours: number; };
  shift: { name: string; start: string; end: string } | null;
}
...
const STATUS_COLORS: Record<DayStatus, string> = {
  ...
  HalfDay: "bg-orange-500", CompOff: "bg-indigo-500", NotMarked: "bg-gray-300",
};
...
const STATUS_CHIP: Record<DayStatus, string> = {
  ...
  CompOff: "bg-indigo-50 text-indigo-700 ring-indigo-200",
...
export default function AttendancePage() {
  ...
  return (
    <div>
      <h1 className="font-serif-display text-3xl md:text-4xl font-bold text-gray-900 mb-5">Attendance</h1>
        ...
        <div className="flex items-center gap-6">
          {([
            ...
            <button key={t.id} onClick={() => setTab(t.id)}
              ...
              className={clsx("tab-underline text-sm py-3 -mb-px",
                tab === t.id ? "text-[#16243A] font-semibold" : "text-gray-600 hover:text-gray-900")}>
              {t.label}
            </button>
          ))}
        </div>
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2 surface-card px-2 py-1">
          <button onClick={() => shiftWeek(cursor, setCursor, -7)} className="p-1 hover:bg-gray-100 rounded-full"><ChevronLeft size={14} /></button>
          <Calendar size={14} className="text-gray-500" />
          <span className="text-sm font-medium">{fmtDate(cursor)} — {fmtDate(weekEnd)}</span>
          <button onClick={() => shiftWeek(cursor, setCursor, 7)} className="p-1 hover:bg-gray-100 rounded-full"><ChevronRight size={14} /></button>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex items-center surface-card overflow-hidden">
            <Tooltip content="List view">
              <button onClick={() => setView("list")}
                className={clsx("p-2 border-r border-[var(--border)]", view === "list" ? "bg-[#dbeafe]" : "bg-white hover:bg-gray-50")}>
                <List size={14} className={view === "list" ? "text-[#16243A]" : "text-gray-500"} />
              </button>
            </Tooltip>
            <Tooltip content="Grid view">
              <button onClick={() => setView("grid")}
                className={clsx("p-2 border-r border-[var(--border)]", view === "grid" ? "bg-[#dbeafe]" : "bg-white hover:bg-gray-50")}>
                <LayoutGrid size={14} className={view === "grid" ? "text-[#16243A]" : "text-gray-500"} />
              </button>
            </Tooltip>
            <Tooltip content="Calendar view">
              <button onClick={() => setView("calendar")}
                className={clsx("p-2", view === "calendar" ? "bg-[#dbeafe]" : "bg-white hover:bg-gray-50")}>
                <CalendarDays size={14} className={view === "calendar" ? "text-[#16243A]" : "text-gray-500"} />
              </button>
            </Tooltip>
          </div>
          <button className="p-2 surface-card hover:bg-gray-50"><Filter size={14} className="text-gray-600" /></button>
          <MoreMenu days={summary?.days ?? []} weekStart={cursor} weekEnd={weekEnd} />
        </div>
      </div>

      <div className="surface-card p-5 mb-4 flex items-center gap-4 bg-gradient-to-br from-white to-slate-50/50">
        <div className="flex items-center gap-2.5">
          ...
          <div className="leading-tight">
            ...
            <div className="text-sm font-semibold text-gray-900">{shiftLabel}</div>
          </div>
        </div>
        <input value={notes} onChange={(e) => setNotes(e.target.value)}
          placeholder="Add notes for check-in"
          className="flex-1 border border-[var(--border)] rounded-full px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#16243A]/20 focus:border-[#16243A]/40" />
        <button
          onClick={() => {
          ...
          className={clsx("px-6 py-2.5 rounded-full text-white font-semibold flex items-center gap-3 min-w-[230px] shadow-md hover:shadow-lg transition-all",
          ...
          <span className="flex items-center gap-1.5 text-sm">
            {today?.checkedIn ? <><Pause size={14} fill="currentColor" /> Check-out</> : <><Play size={14} fill="currentColor" /> Check-in</>}
          </span>
      ...
      <div className="surface-card p-4 mb-2">
        <div className="flex items-center">
          <div className="flex flex-col border border-[var(--border)] rounded-lg overflow-hidden mr-4">
            <button onClick={() => setUnit("Days")}
              className={clsx("px-4 py-1.5 text-xs", unit === "Days" ? "bg-gray-100 text-gray-900 font-medium" : "bg-white text-gray-600")}>Days</button>
            <button onClick={() => setUnit("Hours")}
              className={clsx("px-4 py-1.5 text-xs border-t border-[var(--border)]", unit === "Hours" ? "bg-gray-100 text-gray-900 font-medium" : "bg-white text-gray-600")}>Hours</button>
          </div>

          {summary && (
            <div className="flex items-center gap-6 text-sm">
              <SummaryStat label="Payable Days" value={summary.totals.payableDays} unit={unit} color="border-[#16243A]" />
              <SummaryStat label="Present" value={summary.totals.presentDays} unit={unit} color="border-green-500" />
              <SummaryStat label="On Duty" value={summary.totals.onDutyDays} unit={unit} color="border-emerald-500" />
              <SummaryStat label="Paid leave" value={summary.totals.paidLeaveDays} unit={unit} color="border-sky-500" />
              <SummaryStat label="Holidays" value={summary.totals.holidayDays} unit={unit} color="border-purple-500" />
              <SummaryStat label="Weekend" value={summary.totals.weekendDays} unit={unit} color="border-yellow-400" />
              <button className="text-gray-400"><MoreHorizontal size={14} /></button>
            </div>
          )}

          <div className="ml-auto text-sm text-gray-600">{shiftLabel}</div>
        </div>
      </div>
    </div>
  );
        ...
        <table className="w-full text-sm border-separate border-spacing-0">
          <thead>
            <tr className="bg-gray-50 text-left text-[11px] uppercase tracking-wider text-gray-500 font-semibold">
              <th className="px-4 h-12 w-14 border-b border-gray-200">S. No.</th>
              <th className="px-4 h-12 border-b border-gray-200">Date</th>
              <th className="px-4 h-12 border-b border-gray-200">Clock-In</th>
              <th className="px-4 h-12 border-b border-gray-200">Clock-Out</th>
              <th className="px-4 h-12 border-b border-gray-200">Working Time In Office</th>
              <th className="px-4 h-12 border-b border-gray-200">Break Time</th>
              <th className="px-4 h-12 border-b border-gray-200 text-center">Log Time</th>
              <th className="px-4 h-12 border-b border-gray-200">Regularization Status</th>
              <th className="px-4 h-12 border-b border-gray-200 text-right">Action</th>
            </tr>
          </thead>
          <tbody>
            {days.map((day, idx) => {
              ...
              return (
                <tr key={day.date} className={clsx(
                  "transition-colors hover:bg-slate-50/60 align-middle h-14 [&>td]:border-b [&>td]:border-gray-100",
                  ...
                  <td className="px-4">
                    {offLabel ? (
                      <span className={clsx("inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-semibold ring-1", STATUS_CHIP[day.status])}>{offLabel}</span>
                    ) : day.checkIn ? (
                  ...
                  <td className="px-4">
                    {offLabel ? (
                      ...
                      <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-green-50 text-green-700 text-[11px] font-semibold ring-1 ring-green-200">
                        <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" /> Active
                      </span>
                    ) : isLiveToday && !isCheckedIn ? (
                      <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-amber-50 text-amber-700 text-[11px] font-semibold ring-1 ring-amber-200">
                  ...
                  <td className="px-4 text-right">
                    {!offLabel && (
                      <button
                        ...
                        className="inline-flex items-center px-3 py-1.5 rounded-md bg-[#2563eb] hover:bg-[#1d4ed8] text-white text-xs font-semibold shadow-sm transition">
                        Regularization
                      </button>
                    )}
                  </td>
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-3.5 bg-gray-50 border-b border-gray-200">
          ...
          <button onClick={onClose} className="p-1.5 rounded-md hover:bg-gray-200 text-gray-500"><X size={16} /></button>
        </div>

        {isPending ? (
          <div className="px-5 py-5 space-y-4">
            <div className="text-sm text-amber-800 bg-amber-50 border border-amber-100 rounded-md px-3 py-2.5">
            ...
            {day.regularizationReason && (
              <div>
                ...
                <div className="text-sm text-gray-800 border border-gray-200 rounded-md px-3 py-2.5">{day.regularizationReason}</div>
              </div>
            )}
            ...
            <div className="flex justify-end gap-2 pt-1">
              <button
                onClick={onClose}
                className="px-4 py-2.5 rounded-md border border-gray-200 text-gray-600 text-sm font-semibold hover:bg-gray-50"
              >
                Close
              </button>
              <button
                ...
                className="px-5 py-2.5 rounded-md bg-red-600 hover:bg-red-700 text-white text-sm font-semibold shadow-sm disabled:opacity-60"
              >
          ...
          >
            <div>
              <label className="block text-sm font-bold text-gray-900 mb-1.5">Clock-In Time <span className="text-red-500">*</span></label>
              <input
                type="time"
                required
                value={checkInTime}
                onChange={(e) => setCheckInTime(e.target.value)}
                className="w-full border border-gray-200 rounded-md px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#2563eb]/30 focus:border-[#2563eb]"
              />
            </div>
            <div>
              <label className="block text-sm font-bold text-gray-900 mb-1.5">Clock-Out Time <span className="text-red-500">*</span></label>
              <input
                type="time"
                required
                value={checkOutTime}
              ...
              {isOther && (
                <div className="mt-2">
                  <textarea
                    ...
                    rows={3}
                    className="w-full border border-gray-200 rounded-md px-3 py-2.5 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-[#2563eb]/30 focus:border-[#2563eb]"
                  />
            ...
            <div className="flex justify-center pt-1">
              <button
                ...
                className="px-8 py-2.5 rounded-md bg-[#3b82f6] hover:bg-[#2563eb] text-white text-sm font-semibold shadow-sm disabled:opacity-60"
              >
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div>
            <h3 className="text-base font-bold text-gray-900">Log Time</h3>
        ...
        <div className="px-5 py-4 space-y-3">
          ...
          <Row label="Clock-In" value={day.checkIn ? new Date(day.checkIn).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit", second: "2-digit" }) : "—"} />
          <Row label="Clock-Out" value={day.checkOut ? new Date(day.checkOut).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit", second: "2-digit" }) : day.checkIn ? "Missing" : "—"} />
...
function DayCard({ day }: { day: DayCell }) {
  ...
  return (
    <div className={clsx(
      ...
      isToday ? "border-[#93c5fd] ring-1 ring-[#dbeafe]" : "border-gray-200"
    )}>
        ...
        {days.map((d) => {
          ...
          return (
            <div key={d.date} className={clsx(
              ...
              isToday ? "bg-[#dbeafe] border-[#bfdbfe]" : "border-gray-200 bg-white",
            )}>
              <div className={clsx("text-sm font-semibold", isToday ? "text-[#2563eb]" : "text-gray-900")}>
                {dt.getDate()}
              </div>
...
function SummaryStat({ label, value, unit, color }: { label: string; value: number; unit: "Days" | "Hours"; color: string }) {
  return (
    <div className={clsx("border-l-4 pl-3 py-1", color)}>
      <div className="text-[11px] uppercase tracking-wider text-gray-400 font-semibold">{label}</div>
      <div className="text-sm font-medium mt-0.5">
        <span className="text-gray-900 text-base font-bold tabular-nums">{value}</span>
        <span className="text-gray-500 ml-1 text-[11px]">{unit === "Days" ? (value === 1 ? "Day" : "Days") : "Hrs"}</span>
      </div>
    </div>
  );
}
          ...
          <div className="flex items-center surface-card overflow-hidden">
            {(["Weekly", "Monthly"] as const).map((m) => (
              <button key={m} onClick={() => setMode(m)}
                className={clsx("px-4 py-1.5 text-sm", mode === m ? "bg-white text-[#16243A] font-semibold" : "bg-gray-50 text-gray-600")}>
                {m}
              </button>
            ))}
          </div>
          <button onClick={() => setShowAssign(true)} className="btn btn-primary">
            <CalendarPlus size={14} /> Assign shift
          </button>
        ...
        <form onSubmit={(e) => {
          ...
          <FormField label="Dates" required>
            <div className="grid grid-cols-2 gap-2">
              <FormInput type="date" required value={form.fromDate} onChange={(e) => setForm({ ...form, fromDate: e.target.value })} />
              <FormInput type="date" value={form.toDate} onChange={(e) => setForm({ ...form, toDate: e.target.value })} />
            </div>
          </FormField>
    ...
    <div className="relative" ref={ref}>
      ...
      <Tooltip content="More actions">
        <button onClick={() => setOpen(!open)}
          className={clsx("p-2 border rounded-lg", open ? "bg-[#16243A] text-white border-[#16243A]" : "border-[var(--border)] bg-white text-gray-600 hover:bg-gray-50")}>
...
function MenuItem({ icon, label, onClick }: { icon: React.ReactNode; label: string; onClick?: () => void }) {
  return (
    <button type="button" onClick={onClick} className="w-full flex items-center gap-3 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
