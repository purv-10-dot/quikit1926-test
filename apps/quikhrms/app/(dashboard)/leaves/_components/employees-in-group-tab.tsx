...
export function EmployeesInGroupTab() {
  const api = useApiClient();
  const [search, setSearch] = useState("");
  const [page, setPage] = useState(1);

  const { data, isLoading } = useQuery({
    queryKey: ["leaves", "group-members", search, page],
    queryFn: () =>
      api.get<GroupMemberRow[]>(
        `/api/v1/hrms/leaves/group-members?search=${encodeURIComponent(search)}&page=${page}&limit=${LIMIT}`,
      ),
    staleTime: 30_000,
  });
  ...
  return (
    <div className="surface-card overflow-hidden">
      <div className="flex items-center justify-between gap-3 p-4 border-b border-gray-100">
        <h2 className="text-sm font-semibold text-gray-900">Employees In All Leave Groups</h2>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-accent-50 text-left text-[11px] font-semibold uppercase tracking-wide text-gray-600">
              ...
              <th className="px-4 py-3">Leave Group</th>
            </tr>
          </thead>
            ...
            {isLoading ? (
              ...
              rows.map((r, i) => (
                <tr key={r.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-xs text-gray-700">{r.leaveGroupName || "—"}</td>
                </tr>
              ))
            )}
          </tbody>
