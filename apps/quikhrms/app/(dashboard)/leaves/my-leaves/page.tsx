...
const PALETTE = ["#bfdbfe", "#bbf7d0", "#fde68a", "#fecaca", "#ddd6fe", "#fbcfe8", "#a5f3fc", "#fed7aa", "#c7d2fe", "#d9f99d", "#fbb6ce", "#bae6fd"];
...
export default function MyLeavesPage() {
  ...
  const applyMut = useMutation({
    ...
    onSuccess: () => { invalidateLeaves(); setShowApply(false); },
  });
  ...
  const weekly = useMemo(() => {
    ...
    yearRequests.forEach((r) => {
      const d = new Date(r.startDate).getDay();
      counts[d] += Number(r.duration);
    });
  ...
  return (
    <div className="w-full">
      {/* Header */}
      <div className="surface-card p-5 mb-4 flex items-center justify-between flex-wrap gap-3">
        <h1 className="font-serif-display text-2xl md:text-3xl font-bold text-gray-900">Leave</h1>
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button onClick={() => { setForm({ leaveTypeId: "", startDate: "", endDate: "", duration: 1, reason: "", isPlanned: true }); setShowApply(true); }}
            className="btn btn-primary"><Plus size={14} /> Apply Leave</button>
          <a href="/leaves/policy-documents" className="text-sm text-[#2563eb] hover:underline inline-flex items-center gap-1">
      ...
      {/* Stats */}
      <div className="mb-4">
        <h2 className="text-sm font-bold text-gray-900 mb-2 px-1">Leave Stats</h2>
            ...
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={monthly} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  ...
                  <Area type="monotone" dataKey="value" stroke="#2563eb" strokeWidth={2} fill="url(#leaveArea)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      {/* Balance */}
      <div className="mb-4">
        <h2 className="text-sm font-bold text-gray-900 mb-2 px-1">Leave Balance</h2>
        {balances.length === 0 ? (
          <EmptyState
            variant="calendar"
            title={`No leave balances for ${year}`}
            description="Leave types + balances appear here once HR configures policies."
            action={<a href="/leaves/policies" className="text-sm text-[#3b82f6] hover:underline">Configure Leave Policies →</a>}
          />
        ) : (
      ...
      <div className="surface-card p-0 overflow-hidden">
        <div className="p-4 border-b border-gray-100 space-y-3">
          <h2 className="text-sm font-bold text-gray-900">Leave History</h2>
          <FilterBar>
            <Select
              value={statusFilter}
            ...
            <button
              ...
              className="text-xs text-gray-500 hover:text-gray-900 px-3 py-1.5 border border-gray-200 rounded-md"
            >Reset</button>
          </FilterBar>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="p-6"><EmptyState variant="bot" title="No leave records" description="Try changing filters or apply a leave." className="border-0 shadow-none" /></div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm border-separate border-spacing-0">
              <thead>
                <tr className="bg-gray-50 text-left text-[11px] uppercase tracking-wider text-gray-500 font-semibold">
                  <th className="px-4 h-11 border-b border-gray-200 w-12">S. No.</th>
                  <th className="px-4 h-11 border-b border-gray-200">Leave Dates</th>
                  <th className="px-4 h-11 border-b border-gray-200">Leave Type</th>
                  <th className="px-4 h-11 border-b border-gray-200">Status</th>
                  <th className="px-4 h-11 border-b border-gray-200">Reason</th>
                  <th className="px-4 h-11 border-b border-gray-200">Requested On</th>
                  <th className="px-4 h-11 border-b border-gray-200">Action By</th>
                  <th className="px-4 h-11 border-b border-gray-200">Reject Reason</th>
                  <th className="px-4 h-11 border-b border-gray-200">Pending Approver</th>
                  <th className="px-4 h-11 border-b border-gray-200 text-right">Action</th>
                </tr>
              </thead>
              <tbody>
                {filteredRequests.map((r, idx) => {
                  ...
                  return (
                    <tr key={r.id} className="row-stagger hover:bg-slate-50/60 align-middle h-12 [&>td]:border-b [&>td]:border-gray-100" style={{ ["--i" as never]: Math.min(idx, 10) }}>
                      <td className="px-4 text-gray-500 tabular-nums">{idx + 1}</td>
                      <td className="px-4">
                        <div className="text-gray-800 font-medium">{formatDate(r.startDate)} - {formatDate(r.endDate)}</div>
                        <div className="text-[11px] text-[#2563eb]">({dayLabel})</div>
                      </td>
                      <td className="px-4 text-gray-700">{r.leaveType.name}</td>
                      <td className="px-4"><span className={clsx("text-sm font-semibold", STATUS_CHIP[r.status] ?? "text-gray-700")}>{r.status}</span></td>
                      <td className="px-4 text-gray-600 max-w-[180px] truncate" title={r.reason}>{r.reason || "—"}</td>
                      <td className="px-4 text-gray-700 tabular-nums">{formatDate(r.appliedOn)}</td>
                      <td className="px-4 text-gray-700">{actionByName ?? "NA"}</td>
                      <td className="px-4 text-gray-600 max-w-[180px] truncate">{rejectReason ?? "NA"}</td>
                      <td className="px-4 text-gray-700">{pendingName ?? "NA"}</td>
      ...
      {/* Apply Leave Modal */}
      <Modal open={showApply} onClose={() => { setShowApply(false); setApplyError(null); }} title="Apply Leave">
        <form
          onSubmit={(e) => {
            e.preventDefault();
        ...
        >
          <div>
            ...
            <Select value={form.leaveTypeId} onChange={(v) => setForm({ ...form, leaveTypeId: v })} required placeholder="Select type" searchable
            ...
            (() => {
              ...
              return (
                <div className="rounded-lg border border-blue-100 bg-blue-50/60 px-3 py-2.5">
                  <div className="text-[12px] font-semibold uppercase tracking-wide text-blue-700">
                    Policy rules · {dryRun.leaveType.name}
                  </div>
                  {bullets.length === 0 ? (
                    <p className="mt-1 text-[12.5px] text-gray-500">No specific limits — only your balance applies.</p>
                  ) : (
                    <ul className="mt-1.5 space-y-0.5 text-[12.5px] text-gray-700">
                      {bullets.map((b) => (
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                required
                className="w-full border border-[var(--border)] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#16243A] disabled:bg-gray-50 disabled:text-gray-400"
              />
            </div>
          </div>
          <div>
          ...
          {liveBlockingViolations.length > 0 && (
            <div className="rounded-lg border border-red-200 bg-red-50 px-3 py-2.5">
              <div className="text-[12px] font-semibold uppercase tracking-wide text-red-700">
                Won&apos;t pass policy
              </div>
              <ul className="mt-1.5 space-y-0.5 text-[12.5px] text-red-700">
          ...
          {applyError && (
            <p className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600">{applyError}</p>
          )}
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={() => { setShowApply(false); setApplyError(null); }} className="px-4 py-2 border border-[var(--border)] rounded-lg text-sm text-gray-700 hover:bg-gray-50">Cancel</button>
            <button
              type="submit"
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={() => setViewLeave(null)}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-3.5 bg-gray-50 border-b border-gray-200">
              <h3 className="text-base font-bold text-gray-900">Leave Details</h3>
            ...
            <div className="px-5 py-4 space-y-2 text-sm">
              ...
              <Row label="Status" value={viewLeave.status} highlight={STATUS_CHIP[viewLeave.status]} />
            ...
            <div className="p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="mt-1 text-sm text-gray-500">This withdraws your leave request. Cannot be undone.</p>
                </div>
              </div>
            </div>
            <div className="flex justify-end gap-2 px-6 py-4 bg-gray-50 border-t border-gray-100">
              <button type="button" onClick={() => setConfirmCancelId(null)} disabled={cancelMut.isPending}
                className="px-4 py-2 bg-white border border-[var(--border)] rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50">Keep It</button>
...
function BalanceCard({ name, color, consumed, available, quota }: { name: string; color: string; consumed: number; available: number; quota: number }) {
  ...
  return (
    <div className="surface-card p-4 flex flex-col">
      <p className="text-center text-sm font-semibold text-gray-700 mb-2">{name}</p>
      <div className="h-36 relative">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          ...
          <span className="text-xl font-bold text-gray-900 tabular-nums">{available}</span>
        </div>
      </div>
      <div className="grid grid-cols-3 gap-2 pt-3 border-t border-gray-100 mt-2 text-center">
        <Stat label="Available" value={available} accent="text-[#2563eb]" />
