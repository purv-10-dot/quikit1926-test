    ...
    <div className="w-full">
      ...
      <div className="surface-card p-5 mb-4 flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-[#dbeafe] text-[#2563eb] flex items-center justify-center">
            <Calendar size={18} />
          </div>
          <div>
            <h1 className="font-serif-display text-2xl md:text-3xl font-bold text-gray-900 leading-tight">Leave &amp; Holiday Calendar</h1>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="hidden md:flex items-center gap-3 text-xs">
            ...
            <Stat label="Holidays" value={holidays.filter((h) => new Date(h.date).getMonth() === month).length} />
          </div>
        </div>
      </div>
      ...
      <div className="surface-card p-0 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-gradient-to-r from-slate-50 to-white">
          <div className="flex items-center gap-2">
            <button onClick={prev} className="p-1.5 rounded-md hover:bg-gray-100 text-gray-600"><ChevronLeft size={16} /></button>
            <button onClick={goToday} className="text-xs font-semibold text-[#2563eb] hover:underline px-2">Today</button>
            <button onClick={next} className="p-1.5 rounded-md hover:bg-gray-100 text-gray-600"><ChevronRight size={16} /></button>
          </div>
          <h2 className="font-serif-display text-lg font-bold text-gray-900 tabular-nums">{monthName} {year}</h2>
          <div className="flex items-center gap-2 flex-wrap">
            {legend.length > 0 && legend.slice(0, 4).map(([name, color]) => (
              <span key={name} className="inline-flex items-center gap-1 text-[11px] text-gray-600">
        ...
        <div className="grid grid-cols-7 bg-slate-50 border-b border-gray-100">
          {DAY_LABELS.map((d, i) => (
            <div key={d} className={clsx(
              ...
              (i === 0 || i === 6) ? "text-[#2563eb]" : "text-gray-500",
            )}>
              {d}
            </div>
          ))}
        ...
        {/* Cells */}
        <div className="grid grid-cols-7 [&>*]:border-b [&>*]:border-r [&>*]:border-gray-100">
          {cells.map((day, i) => {
            if (!day) {
              return <div key={`empty-${i}`} className="min-h-[110px] bg-slate-50/40" />;
            }
            ...
            return (
              <div
                ...
                className={clsx(
                  ...
                  isToday && "bg-blue-50 ring-2 ring-[#2563eb] ring-inset",
                )}
              >
                <div className="flex items-center justify-between">
                  <span className={clsx(
                    "inline-flex items-center justify-center min-w-[24px] h-6 px-1.5 text-xs rounded-md font-bold tabular-nums",
                    isToday ? "bg-[#2563eb] text-white" :
                    dayHolidays.length > 0 ? "text-purple-700" :
                    isWeekend ? "text-[#2563eb]" : "text-gray-700",
                  )}>
                    {day}
                  </span>
                  {dayLeaves.length > 0 && (
                ...
                <div className="mt-1 space-y-1">
                  ...
                  {dayLeaves.slice(0, 3).map((l) => {
                    const c = l.leaveType.color ?? "#3b82f6";
                    return (
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div className="border-l-2 border-[#2563eb]/40 pl-3">
