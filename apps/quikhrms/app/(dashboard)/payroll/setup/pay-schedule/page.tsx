...
import { Calendar, Save, CheckCircle2, AlertCircle, CalendarDays, CircleDollarSign } from "lucide-react";
...
export default function PayScheduleSetupPage() {
  return (
    <Suspense fallback={<div className="max-w-4xl mx-auto p-4 space-y-2"><SkeletonLine w="40%" h={16} /><SkeletonLine w="70%" h={12} /><SkeletonLine w="60%" h={12} /></div>}>
  ...
  return (
    <div className="max-w-4xl mx-auto pb-24">
      <div className="rounded-2xl bg-gradient-to-r from-[#166534] to-[#16a34a] text-white px-5 py-4 shadow-sm flex items-start gap-4 mb-5">
        ...
        <div>
          <h1 className="font-serif-display text-base font-semibold leading-tight">Pay Schedule</h1>
      ...
      >
          ...
          <div className="space-y-3">
            ...
            >
              {form.salaryCalcBasis === "ActualDaysInMonth" && (
                <div className="mt-2 inline-flex items-center gap-2 rounded-md border border-green-100 bg-green-50 px-2.5 py-1 text-[11px]">
                  <CalendarDays size={12} className="text-[#16a34a]" />
                  <span className="text-gray-600">{monthLabel}:</span>
                  <span className="font-semibold text-[#166534]">{daysInThisMonth} days</span>
                </div>
              )}
            </ChoiceCard>
        ...
        <div className="sticky bottom-4 z-10 flex items-center justify-between gap-3 bg-white border border-gray-200 rounded-xl shadow-md px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <Link href="/payroll/setup" className="px-3 py-1.5 text-xs font-medium border border-gray-200 rounded-lg text-gray-700 hover:bg-gray-50">
              Cancel
            </Link>
            <button
              type="submit"
  ...
  return (
    <div
      className={clsx(
        ...
        active ? "border-[#22c55e] bg-[#22c55e]/5 ring-1 ring-[#22c55e]/30" : "border-gray-200 hover:border-gray-300 bg-white",
      )}
    >
      <button
        type="button"
        onClick={onClick}
        className="w-full text-left px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#166534]/20 rounded-lg"
      >
        <div className="flex items-start gap-3">
          ...
          >
            {active && <span className="w-2 h-2 rounded-full bg-[#22c55e]" />}
          </span>
          <div className="flex-1">
            <p className={clsx("text-[13px] font-semibold", active ? "text-[#15803d]" : "text-gray-800")}>{title}</p>
