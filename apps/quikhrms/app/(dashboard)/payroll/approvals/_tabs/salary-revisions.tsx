    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="inline-flex items-center gap-1 px-3 py-1.5 bg-[#16243A] hover:bg-[#1E3354] text-white rounded-md text-xs font-semibold shadow-sm"
        >
          <Plus size={12} /> Propose Revision
        </button>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="py-10 text-center text-sm text-gray-500">No {status.toLowerCase()} salary revisions.</div>
      ) : (
              ...
              {rows.map((r, i) => {
                ...
                return (
                  <tr key={r.id} className="row-stagger border-b border-gray-50 hover:bg-gray-50" style={{ ["--i" as never]: Math.min(i, 10) }}>
                    ...
                    <td className="py-3 px-3 text-gray-700">{new Date(r.effectiveFrom).toLocaleDateString("en-IN")}</td>
...
function ActionForm({
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit({ reason }); }} className="p-5 space-y-3">
      <div className="rounded bg-gray-50 border border-gray-200 p-3 text-xs">
