...
export function KPI({ label, value, top, hint, linkLabel, href }: {
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
      ...
      <p className="font-serif-display text-3xl md:text-4xl font-bold text-gray-900 truncate">{value}</p>
      {hint && <p className="text-xs text-gray-500 mt-1">{hint}</p>}
      {linkLabel && href && (
        <Link href={href} className="mt-2 inline-block text-xs font-semibold text-[#3b82f6] hover:underline">
          {linkLabel}
        </Link>
      )}
    </div>
