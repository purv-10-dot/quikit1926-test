...
type StepType =
  ...
  | "ReadPolicy" | "Training" | "ITProvisioning" | "AssetAssignment" | "BGV" | "Clearance";
// BGV checks offered in the Background Verification step editor.
const BGV_CHECKS = ["Education", "Employment", "Criminal", "Address", "Identity", "Reference"];
...
const TYPES: Record<StepType, TypeDef> = {
  ...
  // Credentials to create before Day 1 (email, SSO, VPN…) — multi-select grid.
  ITProvisioning:  { label: "Credentials (IT)", color: "gray",   category: "ItSetup",       fields: [] },
  // Assets to get ready before Day 1 (laptop, ID card…) — list editor.
  AssetAssignment: { label: "Asset Readiness",  color: "gray",   category: "ItSetup",       fields: [] },
  // Background Verification — custom "checks to run" editor (see StepCard).
  BGV:             { label: "Background Verification", color: "violet", category: "Compliance", fields: [] },
  // Admin / Facilities clearance (seat, ID/access card) — plain assignee task.
  Clearance:       { label: "Admin / Facilities", color: "blue",  category: "TaskOther",      fields: [] },
};
...
const HIDDEN_STEP_TYPES: StepType[] = ["CompleteProfile", "Approval", "FillForm", "Training", "ReadPolicy"];
...
export default function PreOnboardingTemplatesPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["pre-onboarding", "templates"],
    queryFn: () => api.get<Template[]>("/api/v1/hrms/onboarding/templates?kind=PreOnboarding&limit=100"),
  });
  const invalidate = () => qc.invalidateQueries({ queryKey: ["pre-onboarding", "templates"] });
  ...
  const submit = () => {
    ...
    const body = { name: name.trim(), description: description || null, isActive: true, kind: "PreOnboarding", tasks };
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3"><ListChecks className="text-[#22c55e]" /><h1 className="text-page-title text-gray-900">Pre-Onboarding Templates</h1></div>
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} size="3xl" headerIcon={<FileText size={18} />}
        title={editingId ? "Edit Pre-Onboarding Template" : "Create Pre-Onboarding Template"} subtitle="Build the pre-joining checklist (before Day 1)" bodyClassName="p-0 overflow-hidden">
        <div>
          <div className="p-5 max-h-[72vh] overflow-y-auto space-y-6">
            ...
            <section className="space-y-3">
              <div className="flex items-center justify-between gap-3">
                <div><h2 className="text-sm font-bold text-gray-900">Pre-Onboarding Steps</h2><p className="text-[11px] text-gray-500">Drag to reorder · click a step to expand.</p></div>
...
function StepCard({ index, step, open, onToggle, onPatch, onPatchConfig, onDup, onDel, canDelete, dragging, over, onDragStart, onDragEnter, onDrop, onDragEnd }: {
  ...
  // BGV — which checks to run.
  const bgvChecks = (Array.isArray(step.config.bgvChecks) ? step.config.bgvChecks : []) as string[];
  const toggleBgv = (c: string) => onPatchConfig("bgvChecks", bgvChecks.includes(c) ? bgvChecks.filter((x) => x !== c) : [...bgvChecks, c]);
      ...
      {open && (
        <div className="px-4 pb-4 border-t border-gray-100">
          ...
          {step.stepType === "AssetAssignment" && (
            <div className="mt-3 p-3.5 border border-dashed border-gray-300 rounded-lg bg-gray-50/70">
              <div className="flex items-center justify-between mb-2.5">
                <div className="text-[10.5px] font-bold uppercase tracking-wide text-gray-400">Assets to prepare</div>
          ...
          {step.stepType === "BGV" && (
            <div className="mt-3 p-3.5 border border-dashed border-gray-300 rounded-lg bg-gray-50/70">
              <div className="text-[10.5px] font-bold uppercase tracking-wide text-gray-400 mb-2.5">Background checks to run</div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {BGV_CHECKS.map((c) => {
                  const on = bgvChecks.includes(c);
                  return (
                    <button type="button" key={c} onClick={() => toggleBgv(c)}
              ...
              <p className="text-[11px] text-gray-500 mt-2.5">HR/vendor sets each check to Clear or Discrepancy on the pre-onboarding page. A discrepancy holds the joining.</p>
            </div>
          )}

          {step.stepType === "ITProvisioning" && (
            <div className="mt-3 p-3.5 border border-dashed border-gray-300 rounded-lg bg-gray-50/70">
              <div className="text-[10.5px] font-bold uppercase tracking-wide text-gray-400 mb-2.5">Credentials / access to create</div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {IT_SYSTEMS.map((s) => {
