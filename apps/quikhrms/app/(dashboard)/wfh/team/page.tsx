...
export default function WfhTeamPage() {
  ...
  return (
    <div className="w-full px-6 py-6">
      <PageHeader
        icon={<Inbox size={28} className="text-[#3b82f6]" />}
        title="WFH approvals"
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50/60 border-b border-slate-200">
                ...
                <th className="text-left px-4 py-3 text-[11px] font-semibold text-slate-500 uppercase tracking-wide">Approval Stage</th>
                ...
                <tr key={i.approvalId} className="row-stagger border-b border-slate-100 hover:bg-slate-50/60" style={{ ["--i" as never]: Math.min(idx, 10) }}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-slate-900">{i.request.employee.firstName} {i.request.employee.lastName}</p>
                  ...
                  <td className="px-4 py-3 text-sm font-semibold text-slate-700">{i.request.days}</td>
                  <td className="px-4 py-3 text-sm text-slate-600 max-w-xs">{i.request.reason}</td>
                  <td className="px-4 py-3">
                    <span className={clsx("inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold ring-1",
                      i.role === "Manager" ? "bg-blue-50 text-blue-700 ring-blue-200" : "bg-purple-50 text-purple-700 ring-purple-200")}>
                      L{i.level} · {i.role}
                    </span>
                  </td>
        ...
        {decision && (
          <form onSubmit={(e) => { e.preventDefault(); decideMut.mutate({ id: decision.item.request.id, kind: decision.kind }); }} className="space-y-4">
            <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-sm">
              <div className="font-semibold">{decision.item.request.employee.firstName} {decision.item.request.employee.lastName}</div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Comment {decision.kind === "reject" && "(recommended)"}</label>
              <textarea
                rows={3}
                value={comment}
