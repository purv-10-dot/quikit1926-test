  ...
  return (
    <div className="w-full">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#0F1F3D] via-[#1E3A8A] to-[#2563EB] mb-6">
        ...
        <div className="relative px-8 py-7 flex flex-col md:flex-row md:items-end md:justify-between gap-6">
      ...
      <div className="flex flex-col-reverse md:flex-row md:items-center justify-between gap-3 mb-6">
        ...
        <div className="relative md:w-72">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-2 text-sm bg-white border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-[#16243A]/20 focus:border-[#16243A] transition"
          />
        </div>
      </div>
  ...
  return (
    <button
      ...
      className={clsx(
        ...
        active
          ? "bg-white text-[#16243A] shadow-sm ring-1 ring-gray-200"
          : "text-gray-600 hover:text-[#16243A] hover:bg-white/60",
      )}
    >
      {label}
      <span className={clsx(
        "px-1.5 py-0.5 rounded-full text-[10px] font-bold tabular-nums",
        active ? "bg-[#16243A] text-white" : "bg-gray-200 text-gray-600",
      )}>
        {count}
      </span>
    </button>
      ...
      <div className="flex items-center justify-between gap-3 mb-4">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className="px-2 py-0.5 rounded-full bg-gray-100 text-[10px] font-bold text-gray-600 tabular-nums">
              {count}
            </span>
          </div>
...
function EmptySearchState({ query, onClear }: { query: string; onClear: () => void }) {
  return (
    <div className="py-16 text-center">
      ...
      <p className="text-base font-bold text-gray-900">No dashboards match &quot;{query}&quot;</p>
      <p className="text-xs text-gray-500 mt-1 mb-4">Try a different name or clear the search.</p>
      <button
        type="button"
        onClick={onClear}
        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg ring-1 ring-gray-200 hover:bg-gray-50 text-sm font-semibold text-gray-700 transition"
      >
  ...
  return (
    <section>
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-base font-bold text-gray-900">Your dashboards</h2>
      </div>
      <div className="rounded-3xl ring-1 ring-dashed ring-gray-300 bg-gradient-to-br from-purple-50/60 to-blue-50/60 p-10 text-center">
        ...
        <h3 className="font-serif-display text-xl font-bold text-gray-900">Build a custom dashboard</h3>
...
function Card({ dashboard, delay }: { dashboard: Dashboard; delay: number }) {
  ...
  const color = dashboard.color ?? "#3b82f6";
  ...
  return (
    ...
    >
      ...
      <Link href={`/dashboards/${dashboard.id}`} className="block">
        ...
        {/* Body */}
        <div className="p-4">
          <h3 className="font-serif-display text-xl font-bold text-gray-900 leading-tight truncate">
            {dashboard.name}
          </h3>
          ...
          {widgetCount > 0 && (
            <div className="mt-3 flex items-center gap-1 flex-wrap">
              {Array.from(new Set(dashboard.widgets.map((w) => w.type))).slice(0, 4).map((t) => (
                <span key={t} className="px-1.5 py-0.5 rounded-md bg-gray-100 text-[10px] font-semibold text-gray-600">
                  {t}
                </span>
              ))}
            </div>
          ...
          <div className="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between">
            <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#16243A] group-hover:text-[#3b82f6] transition">
              Open dashboard <ArrowRight size={12} className="transition-transform group-hover:translate-x-0.5" />
            </span>
            {dashboard.isPrebuilt && (
              <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-amber-50 text-amber-700 ring-1 ring-amber-200 text-[10px] font-bold">
...
function CreateDashboardButton({ variant = "default" }: { variant?: "default" | "hero" | "cta" }) {
  ...
  const triggerClass =
    variant === "hero"
      ? "inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white text-[#16243A] hover:bg-gray-50 font-bold text-sm shadow-lg ring-1 ring-white/40 transition"
      : variant === "cta"
        ? "inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#16243A] hover:bg-[#1E3354] text-white font-bold text-sm shadow-md transition"
        : "btn btn-primary";
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-[#16243A]/10 text-[#16243A] flex items-center justify-center">
                  <LayoutDashboard size={16} />
                </div>
                <div>
                  <h2 className="text-base font-bold text-gray-900 leading-tight">New dashboard</h2>
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <input
                  ...
                  autoFocus
                  className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#16243A]/20 focus:border-[#16243A] transition"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-700 uppercase tracking-wide mb-1.5">
                ...
                <textarea
                  ...
                  className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#16243A]/20 focus:border-[#16243A] transition resize-none"
                />
              </div>
            </div>

            <div className="flex justify-end gap-2 px-5 py-3 bg-gray-50 border-t border-gray-100">
              <button onClick={() => setOpen(false)} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition">
                Cancel
              </button>
              <button
                onClick={() => createMut.mutate()}
                disabled={!name.trim() || createMut.isPending}
                className="inline-flex items-center gap-1.5 px-4 py-2 bg-[#16243A] hover:bg-[#1E3354] text-white text-sm font-semibold rounded-lg disabled:opacity-50 transition"
              >
