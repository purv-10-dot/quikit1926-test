    ...
    <div className="w-full px-5 py-4">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
        <div className="flex items-start gap-3">
          ...
          <h1 className="text-page-title text-gray-900">Document library</h1>
        </div>
          ...
          {docs.map((d) => (
            <div key={d.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 hover:border-[#bbf7d0] hover:shadow flex flex-col">
              ...
              <div className="flex items-center gap-1 pt-3 mt-3 border-t border-gray-100 flex-wrap">
                <button onClick={() => openEdit(d)} title="Edit" className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs text-gray-600 hover:text-[#166534] hover:bg-gray-50 border border-gray-200">
                  <Pencil size={11} /> Edit
                </button>
      ...
      <Modal open={showUpload} onClose={() => setShowUpload(false)} title="Upload Document">
        <form onSubmit={(e) => {
            ...
            <div>
              <label className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-1 cursor-pointer">
                <input type="checkbox" checked={form.hasExpiry}
                  onChange={(e) => setForm({ ...form, hasExpiry: e.target.checked, expiryDate: e.target.checked ? form.expiryDate : "" })}
                  className="rounded border-gray-300" />
                This document expires
              </label>
              {form.hasExpiry && (
                <input type="date" min={todayInput()} value={form.expiryDate} onChange={(e) => setForm({ ...form, expiryDate: e.target.value })}
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button type="submit" disabled={uploadMut.isPending} className="px-4 py-2 bg-green-600 text-white rounded-lg text-sm font-medium hover:bg-green-700 disabled:opacity-50">Upload</button>
          </div>
        </form>
      </Modal>
          ...
          <form onSubmit={(e) => {
            ...
            <div>
              ...
              <DocumentSourcePicker
                ...
                onChange={(meta) => setForm({ ...form, ...meta })}
              />
              <p className="text-[11px] text-gray-500 mt-1">Replace the uploaded file, pasted URL, or Google Drive link, or keep the existing one.</p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Category</label>
