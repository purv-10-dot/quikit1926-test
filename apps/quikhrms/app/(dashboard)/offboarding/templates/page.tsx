...
type StepType =
  | "CustomTask" | "AssetReturn" | "AccessRevoke" | "KnowledgeTransfer"
  | "Clearance" | "ExitInterview" | "ReadPolicy" | "SendEmail";

type FieldKind = "text" | "select" | "search" | "toggle" | "emailTemplate";

// Emails you'd send TO the exiting employee during offboarding. Resignation
// acceptance is a generated PDF; the rest come from the Offboarding email group.
const EMAIL_TEMPLATE_OPTIONS = [
  { value: "offboarding.resignation-acceptance", label: "Resignation Acceptance Letter (PDF attached)" },
  { value: "offboarding.relieving-letter", label: "Relieving Letter (PDF attached)" },
  { value: "offboarding.experience-letter", label: "Experience Letter (PDF attached)" },
  ...[...EMAIL_EVENTS]
    .filter((e) => e.group === "Offboarding")
...
// A file HR attaches on a Policy Re-acknowledge step for the employee to acknowledge.
interface PolicyFile { url: string; key?: string; fileName: string; fileType?: string; fileSize?: number }
// One item on an Asset Return step (custom = free-text name when type is "Custom").
interface AssetRow { type: string; qty: number; custom?: string }

const TYPES: Record<StepType, TypeDef> = {
  CustomTask:        { label: "Custom Task",         color: "gray",   category: "Clearance",          fields: [] },
  // Custom "assets to return" list editor (see StepCard) — laptop, ID card, etc.
  AssetReturn:       { label: "Asset Return",        color: "gray",   category: "AssetReturn",        fields: [] },
  // Custom multi-select grid of systems/access to revoke (email, SSO, VPN…).
  AccessRevoke:      { label: "Access Revocation",   color: "gray",   category: "AccessRevoke",       fields: [] },
  KnowledgeTransfer: { label: "Knowledge Transfer",  color: "green",  category: "KnowledgeTransfer",  fields: [] },
  Clearance:         { label: "Clearance",           color: "violet", category: "Clearance",          fields: [] },
  ExitInterview:     { label: "Exit Interview",      color: "blue",   category: "Clearance",          fields: [] },
  // HR attaches files (NDA/confidentiality) the exiting employee re-acknowledges.
  ReadPolicy:        { label: "Policy Re-acknowledge", color: "blue", category: "Clearance",          fields: [] },
  // Multi-select emails (incl. Resignation Acceptance PDF) sent to the employee.
  SendEmail:         { label: "Send Email",          color: "amber",  category: "Clearance",          fields: [] },
};

// Asset types for the Asset Return list editor ("Custom" reveals a free-text name).
const ASSET_TYPES = [
...
// Systems / access options for the Access Revocation multi-select grid.
const IT_SYSTEMS = [
...
// "Exit Interview" is NOT a template step — it's a fixed section on the
// offboarding page (with the real questionnaire + attrition analytics). Kept in
// TYPES only so any existing task still renders its label.
const HIDDEN_STEP_TYPES: StepType[] = ["ExitInterview"];
const STEP_TYPE_KEYS = (Object.keys(TYPES) as StepType[]).filter((k) => !HIDDEN_STEP_TYPES.includes(k));

// Sentinel value in the "Assign To" dropdown meaning "the exiting employee"
// (rather than an org department). Stored in config.assignDepartmentId.
...
export default function OffboardingTemplatesPage() {
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: ["offboarding", "templates"] });
  const createMut = useMutation({ mutationFn: (b: Record<string, unknown>) => api.post("/api/v1/hrms/offboarding/templates", b), onSuccess: () => { invalidate(); setShowCreate(false); } });
  const updateMut = useMutation({ mutationFn: ({ id, b }: { id: string; b: Record<string, unknown> }) => api.patch(`/api/v1/hrms/offboarding/templates/${id}`, b), onSuccess: () => { invalidate(); setShowCreate(false); } });
  const deleteMut = useMutation({ mutationFn: (id: string) => api.delete(`/api/v1/hrms/offboarding/templates/${id}`), onSuccess: () => invalidate(), onSettled: () => setDeleteTarget(null) });
  ...
  return (
    <div>
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} size="3xl" headerIcon={<FileText size={18} />}
        title={editingId ? "Edit Offboarding Template" : "Create Offboarding Template"} subtitle="Build a structured exit journey for departing employees" bodyClassName="p-0 overflow-hidden">
        <div>
          <div className="p-5 max-h-[72vh] overflow-y-auto space-y-6">
            ...
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-4">
                <div className="sm:col-span-4"><label className="block text-xs font-semibold text-gray-700 mb-1.5">Template Name <span className="text-red-500">*</span></label>
                  <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Standard Exit Clearance" className={INPUT} /></div>
            ...
            <section className="space-y-3">
              <div className="flex items-center justify-between gap-3">
                <div><h2 className="text-sm font-bold text-gray-900">Offboarding Steps</h2><p className="text-[11px] text-gray-500">Drag to reorder · click a step to expand.</p></div>
        ...
        <div className="px-4 pb-4 border-t border-gray-100">
          <div className="grid grid-cols-1 md:grid-cols-[2fr_1.2fr_1.2fr_1.4fr] gap-3 pt-4">
            ...
            <div><label className="block text-[10px] uppercase tracking-wide text-gray-400 font-bold mb-1">Assign To</label>
              <Select size="sm" disabled={isReadPolicy} value={isReadPolicy ? CANDIDATE : assignDeptId}
                ...
                options={[{ value: "", label: "Select…" }, { value: CANDIDATE, label: "The exiting employee" }, ...deptOpts]} />
              {isReadPolicy && <p className="text-[10px] text-gray-400 mt-1">Locked to the exiting employee for this step.</p>}</div>
            {!isCandidate && (
          ...
          {step.stepType === "ReadPolicy" && (
            <div className="mt-3 p-3.5 border border-dashed border-gray-300 rounded-lg bg-gray-50/70">
              ...
              {files.length === 0 && <p className="text-[12px] text-gray-400">No files yet. Upload the documents (e.g. NDA, confidentiality) the exiting employee must re-acknowledge.</p>}
              ...
              <p className="text-[11px] text-gray-500 mt-2.5">The exiting employee gets an email link (sent from the offboarding checklist) to view and re-acknowledge each file. The step completes when all are acknowledged.</p>
            </div>
          )}

          {step.stepType === "AssetReturn" && (
            <div className="mt-3 p-3.5 border border-dashed border-gray-300 rounded-lg bg-gray-50/70">
              <div className="flex items-center justify-between mb-2.5">
                <div className="text-[10.5px] font-bold uppercase tracking-wide text-gray-400">Assets to return</div>
              ...
              {assets.length === 0 && <p className="text-[12px] text-gray-400">No assets yet. Add the equipment to collect back (e.g. Laptop, Monitor, Access Card).</p>}
          ...
          {step.stepType === "AccessRevoke" && (
            <div className="mt-3 p-3.5 border border-dashed border-gray-300 rounded-lg bg-gray-50/70">
              <div className="text-[10.5px] font-bold uppercase tracking-wide text-gray-400 mb-2.5">Systems / access to revoke</div>
