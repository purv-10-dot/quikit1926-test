...
export default function OffboardingDetailPage({ params }: { params: Promise<{ employeeId: string }> }) {
  ...
  return (
    <div className="max-w-6xl">
      <Link href="/offboarding" className="inline-flex items-center gap-1 text-sm text-[#3b82f6] hover:underline mb-4">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-4">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h1 className="font-serif-display text-3xl md:text-4xl font-bold text-gray-900">Offboarding: <span className="font-mono text-lg text-gray-600">{inst.employeeId}</span></h1>
        ...
        <div className="flex items-center gap-2 mb-4">
          <button onClick={() => setShowInterview(true)} className="flex items-center gap-1 border border-[var(--border)] px-3 py-1.5 rounded-lg text-sm hover:bg-gray-50">
      ...
      {cl && cl.groups.length > 0 && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
          <h2 className="font-semibold text-gray-900 mb-3">Department Clearance</h2>
            ...
            {cl.groups.map((g) => (
              <div key={g.department} className={clsx("border rounded-lg p-3", g.clearanceGranted ? "border-green-300 bg-green-50/30" : "border-gray-200")}>
                <div className="flex items-center justify-between mb-1">
                  <span className="font-medium text-sm">{g.department}</span>
                ...
                <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden mt-2">
                  <div className={clsx("h-full", g.clearanceGranted ? "bg-green-500" : "bg-[#dbeafe]0")} style={{ width: `${g.progress}%` }} />
                </div>
              </div>
            ))}
          </div>
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {t.department && <span className="px-2 py-0.5 bg-gray-100 text-gray-700 rounded-full text-xs">{t.department}</span>}
                </div>
                {t.description && <p className="text-xs text-gray-500">{t.description}</p>}
              </div>
              <div className="flex items-center gap-1">
                <Tooltip content="Mark in progress"><button onClick={() => updateMut.mutate({ taskId: t.id, status: "TaskInProgress" })} className="p-2 rounded-lg border border-[var(--border)] hover:bg-gray-50"><Clock size={14} /></button></Tooltip>
                <Tooltip content="Mark complete"><button onClick={() => updateMut.mutate({ taskId: t.id, status: "TaskCompleted" })} className="p-2 rounded-lg border border-[var(--border)] hover:bg-green-50"><CheckCircle size={14} /></button></Tooltip>
                <Tooltip content="Skip task"><button onClick={() => updateMut.mutate({ taskId: t.id, status: "TaskSkipped" })} className="p-2 rounded-lg border border-[var(--border)] hover:bg-gray-50"><SkipForward size={14} /></button></Tooltip>
                <Tooltip content="Block task"><button onClick={() => updateMut.mutate({ taskId: t.id, status: "TaskBlocked" })} className="p-2 rounded-lg border border-[var(--border)] hover:bg-red-50"><PauseCircle size={14} /></button></Tooltip>
              </div>
            </div>
          </div>
        ))}
      ...
      <Modal open={showInterview} onClose={() => setShowInterview(false)} title="Exit Interview">
        <form onSubmit={(e) => { e.preventDefault(); interviewMut.mutate(interview); }} className="space-y-4">
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Overall Experience (1-5)</label>
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Reason for Leaving</label>
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Notes</label>
            <textarea required value={interview.notes} onChange={(e) => setInterview({ ...interview, notes: e.target.value })}
              className="w-full border border-[var(--border)] rounded-lg px-3 py-2 text-sm" rows={4} /></div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={() => setShowInterview(false)} className="px-4 py-2 border border-[var(--border)] rounded-lg text-sm">Cancel</button>
