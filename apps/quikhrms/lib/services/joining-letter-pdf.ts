...
export interface JoiningLetterPdfInput {
  employeeName: string;
  ...
  letterDate: string;
  // Branding assets — storage object keys (shared with the offer letter).
...
function fieldValues(input: JoiningLetterPdfInput): Record<string, string> {
  return {
    ...
    ctc: input.offeredCTC != null ? inr(Number(input.offeredCTC)) : "",
...
async function fetchImage(pdf: PDFDocument, key: string): Promise<PDFImage | null> {
  try {
    ...
    console.error(`[joining-letter-pdf] image fetch failed (${key}):`, err);
    return null;
  }
}
...
/**
 * Renders the joining/appointment letter as an A4 PDF using the org's branding
 * (letterhead, seal, signature) and the editable body template. Substitutes
 * {{fields}}, wraps long lines and paginates as needed. `{{signature}}` on its
 * own line draws the uploaded signature image at that position.
 */
export async function generateJoiningLetterPdf(input: JoiningLetterPdfInput): Promise<Buffer> {
  ...
  const template = input.bodyTemplate?.trim() || DEFAULT_JOINING_LETTER_BODY;
