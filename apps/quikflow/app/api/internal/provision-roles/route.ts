 ...
 *
 * Eagerly seeds this org's default QuikFlow roles (system "admin" with full
 * permissions + the default "Member" role). Called by the launcher's
 * super-admin "grant app access" flow (`apps/quikit/lib/provisionAppRoles.ts`)
 * the moment QuikFlow is enabled for an org, so the admin panel's role
 * dropdown shows "admin" immediately instead of "No roles available" until
 * someone first opens QuikFlow.
 *
 * Optionally accepts `adminUserIds: string[]` — for each user id, an
 * `app_quikflow.UserAppRole` row is upserted linking them to the seeded
 * admin AppRole.
 *
 * The lazy seed in GET /api/me/permissions remains as the fallback — this
 * endpoint just removes the provisioning-order gap. Idempotent.
 *
 * Auth: shared INTERNAL_SECRET via `x-internal-secret` (mirrors
 * apps/quikscale/app/api/internal/provision-roles/route.ts). Not a user
