...
// RBAC v2: gated by the `Role` resource. Admin-lockout guard
...
/**
 * Storage model:
 *   - Roles → app_quikflow.AppRole (WfAppRole)
 *   - Members → app_quikflow.UserAppRole (WfUserAppRole) — join table.
 *
 * Pre-condition for membership: the user must already have a
 * quikit.UserAppAccess row for QuikFlow. Users without one are skipped in
 * PUT and reported back so the UI can prompt to invite them first.
 */
...
// Users currently on this role but NOT in the body are removed. Users in
// the body who don't yet have a UserAppAccess row for QuikFlow are skipped
// and reported back — admin must invite them first.
export const PUT = auth.update<{ id: string }>(async ({ orgId, userId: actorId }, req, { params }) => {
  ...
  const result = await db.$transaction(async (tx) => {
    // 1. Filter desired set to users with QuikFlow UserAppAccess.
    const access =
      desired.length > 0
    ...
    const detached = await tx.wfUserAppRole.deleteMany({
      where: {
        roleId: role.id,
        orgId,
    ...
    const existing = await tx.wfUserAppRole.findMany({
