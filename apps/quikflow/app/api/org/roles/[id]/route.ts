...
// RBAC v2: gated by the `Role` resource. The `isSystem` check on
// PATCH/DELETE is preserved so the admin role can never be renamed or
// deleted regardless of grants.
...
export const PATCH = auth.update<{ id: string }>(async ({ orgId }, req, { params }) => {
  ...
  if (data.name && data.name !== existing.name) {
    const dup = await db.wfAppRole.findUnique({
...
export const DELETE = auth.delete<{ id: string }>(async ({ orgId }, _req, { params }) => {
  ...
  await db.wfAppRole.delete({ where: { id: role.id } });
