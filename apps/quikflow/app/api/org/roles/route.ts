...
// RBAC v2: gated by the `Role` resource.
...
// GET /api/org/roles — list all AppRoles for this tenant + QuikFlow.
export const GET = auth.view(async ({ orgId }) => {
  ...
  const roles = await db.wfAppRole.findMany({
    where: { orgId, appId },
    select: {
      id: true,
      name: true,
...
export const POST = auth.create(async ({ orgId, userId }, request) => {
  ...
  const existing = await db.wfAppRole.findUnique({
  ...
  const role = await db.wfAppRole.create({
    data: {
      orgId,
      appId,
      name,
