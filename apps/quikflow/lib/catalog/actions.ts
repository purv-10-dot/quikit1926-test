...
export const ACTION_CATALOG: CatalogAction[] = [
  ...
  // ── Calendar & meetings ────────────────────────────────────────────────
  { id: "calendar.event.create", label: "Create a Teams calendar meeting", category: "Calendar", doesWhat: "For Client Master: set kind = daily OR weekly and leave the rest blank — subject, times, attendees, recurrence days & until all auto-fill from the client. (Advanced: override any of them, or use start_time/end_time + date for a non-Client-Master event.)", requiredInputs: ["subject"], optionalInputs: ["kind", "start_time", "end_time", "start", "end", "date", "attendees", "optional_attendees", "recurrence", "recurrence_days", "recurrence_until", "body", "location", "timezone", "online_meeting", "ref_id", "from_connection"], permission: "integration:teams", output: "{ event_id, web_link, join_url, updated }", real: true },
