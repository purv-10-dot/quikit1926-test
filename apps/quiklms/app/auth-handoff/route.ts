 ...
 * `NEXTAUTH_SECRET`, set it as this app's session cookie, then redirect to the
 * path the user originally wanted (carried in the `to` claim).
 */
export async function GET(request: NextRequest) {
  const token = request.nextUrl.searchParams.get("token");
  // This app's own public origin — never `request.url` (resolves to the pod
  // bind address when the ingress doesn't preserve the Host header). The
  // session cookie below is host-only, so we must redirect back to the same
  // host that served this request. See @quikit/auth/public-url.
