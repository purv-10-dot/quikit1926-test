...
export const POST = route(async (req) => {
  ...
  const result = await uploadCertificateAsset(
    ...
    'Storage permission denied. The AWS IAM user does not have s3:PutObject permission on the certificates/templates path. Please update the IAM policy for the S3 bucket.',
  );
