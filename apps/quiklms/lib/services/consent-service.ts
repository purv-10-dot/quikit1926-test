...
export async function grantConsent(
  ...
  const existing = await prisma.consentRecord.findFirst({
    where: { tenantId, studentId: dto.studentId, parentId, consentType: dto.consentType },
  });
  if (existing) {
  ...
  return prisma.consentRecord.create({
    data: {
      tenantId, studentId: dto.studentId, parentId, consentType: dto.consentType,
...
export async function checkConsent(tenantId: string, studentId: string, consentType: string) {
  const consent = await prisma.consentRecord.findFirst({
    where: { tenantId, studentId, consentType: consentType as ConsentType, granted: true },
  });
  return { hasConsent: !!consent, consent };
}

export async function getConsentHistory(tenantId: string, studentId: string) {
  const records = await prisma.consentRecord.findMany({ where: { tenantId, studentId }, orderBy: { createdAt: 'desc' } });
...
export async function getPendingConsents(tenantId: string, parentId: string) {
  const records = await prisma.consentRecord.findMany({ where: { tenantId, parentId, granted: false }, orderBy: { createdAt: 'desc' } });
