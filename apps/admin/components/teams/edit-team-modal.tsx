...
/** Normalize a stored role value to match an AppRoleOption name (case-insensitive). */
function normalizeRoleName(value: string, roles: AppRoleOption[]): string {
  if (!value) return getDefaultRoleName(roles);
  const match = roles.find((r) => r.name.toLowerCase() === value.toLowerCase());
  return match?.name ?? getDefaultRoleName(roles);
}
...
export default function EditTeamModal({ open, teamId, onClose, onSuccess }: EditTeamModalProps) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/teams/${teamId}`, { signal })
      ...
      .then((res) => {
        ...
        setSelectedApps(team.apps.map((a) => a.slug));
        const ids = team.members.map((m) => m.membershipId);
        setSelectedIds(ids);
        const roles: Record<string, Record<string, string>> = {};
        team.members.forEach((m) => {
          roles[m.membershipId] = Object.fromEntries(m.apps.map((a) => [a.slug, a.role]));
        });
    ...
    // Load provisioned apps, then roles for each
    fetch("/api/apps/provisioned", { signal })
      ...
      .then((results) => {
        ...
        // Normalize pre-loaded member roles against real role names
        setMemberRoles((prev) => {
          const normalized: typeof prev = {};
          for (const [memberId, appRoles] of Object.entries(prev)) {
            normalized[memberId] = {};
            for (const [slug, role] of Object.entries(appRoles)) {
              normalized[memberId][slug] = normalizeRoleName(role, map[slug] ?? []);
            }
          }
          return normalized;
        });
  ...
  function toggleMember(id: string) {
    setSelectedIds((prev) => {
      ...
      selectedApps.forEach((slug) => {
        const existing = member?.apps.find((a) => a.slug === slug)?.role;
        defaults[slug] = existing
          ? normalizeRoleName(existing, rolesPerApp[slug] ?? [])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      else setError(json.error ?? "Failed to update team");
    } catch {
  ...
  const isLoading       = loadingTeam || loadingApps;
      ...
      <div className="relative w-full max-w-4xl rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-primary)] shadow-2xl mx-4">
        ...
        <div className="flex items-center justify-between border-b border-[var(--color-border)] px-6 py-4">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-base font-semibold text-[var(--color-text-primary)]">Edit Team</h2>
          </div>
          <button
            onClick={onClose}
        ...
        {isLoading ? (
          <div className="px-6 py-12 flex items-center justify-center">
            <span className="h-6 w-6 animate-spin rounded-full border-2 border-[var(--color-border)] border-t-[var(--color-secondary)]" />
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="px-6 py-5 space-y-6">
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-[var(--color-text-tertiary)]">Toggle apps for new members</span>
              </div>
                            ...
                            >
                              ...
                              <span className="ml-auto flex gap-1 shrink-0">
                                {member.apps.filter((a) => selectedApps.includes(a.slug)).slice(0, 3).map((a) => {
                                  ...
                                  const Icon = meta?.icon ?? LayoutGrid;
                                  return (
                                    <span key={a.slug} className={`flex h-5 w-5 items-center justify-center rounded-full ${meta?.bg ?? "bg-[var(--color-neutral-100)]"}`}>
