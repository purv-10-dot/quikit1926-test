...
const callsByDay: CannedReport = {
  ...
  category: "Telephony",
  title: "Calls by day",
  blurb: "Daily call volume over the last 14 days.",
  helpText:
    "Daily call count over the last 14 days. Each day is bucketed in your timezone " +
    "(midnight to midnight). Empty days are pre-filled with zero so the line chart " +
    "is continuous. Click a row to drill into that day's call log.",
  defaultDateRange: "30d",
  ...
  async run(ctx) {
    const start = daysAgoStartOfDay(13, ctx.tz);
    ...
    // Pre-fill 14 buckets so the line chart shows zero-days too.
    const buckets = new Map<string, number>();
    for (let i = 13; i >= 0; i--) {
      const d = new Date(start.getTime() + (13 - i) * 24 * 60 * 60 * 1000);
      buckets.set(dayKeyInTz(d, ctx.tz), 0);
    }
...
const callsByUser: CannedReport = {
  ...
  category: "Telephony",
  title: "Calls by user",
  blurb: "Call count and total talk time per agent over the last 7 days.",
  helpText:
    "Counts call logs per `agentUserId` over the last 7 days, also summing " +
    "`durationSec` for total talk time. Sorted by call count descending. " +
    "Use to compare dialing volume across the team.",
  defaultDateRange: "7d",
  buildDrillUrl: (row) => {
...
const talkTimeByUser: CannedReport = {
  ...
  blurb: "Total talk-time per agent over the last 7 days.",
  helpText:
    "Sums `durationSec` per `agentUserId` over the last 7 days. Sorted by total " +
    "talk time descending. Displayed as HH:MM:SS in the table; CSV export keeps " +
    "the same format.",
  defaultDateRange: "7d",
  buildDrillUrl: (row) => {
    const agentUserId = row.agentUserId ? String(row.agentUserId) : "";
