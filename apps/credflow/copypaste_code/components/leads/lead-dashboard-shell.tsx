...
import { CommandWhatsAppModal } from "@/components/leads/dashboard/command-whatsapp-modal";
import type { CommTab } from "@/components/leads/dashboard/communication-center";
...
export function LeadDashboardShell(props: LeadDashboardShellProps) {
  ...
  const [commsTab, setCommsTab] = useState<CommTab | undefined>();
  ...
  const quickHandlers = {
    ...
    onWhatsApp: () => {
      if (!phone) {
        toast.error("No phone for WhatsApp");
        return;
      }
  ...
  const commandActions = useMemo(
    () =>
      buildLeadCommandActions({
        ...
        onWhatsApp: quickHandlers.onWhatsApp,
        onComposeEmail: () => {
          setCommsTab("emails");
          setActiveTab("communications");
          quickHandlers.onEmail();
        },
  ...
  return (
    <div className="pb-20 xl:pb-0">
      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start">
        <main className="min-w-0 flex-1 xl:w-[70%]">
          <LeadDetailTabs
            ...
            communicationsInitialTab={commsTab}
          />
        </main>
      ...
      {phone ? (
        <CommandWhatsAppModal
          open={whatsappOpen}
          onClose={() => setWhatsappOpen(false)}
          leadName={lead.name}
          phone={phone}
          onOpenCommunications={() => {
            setCommsTab("whatsapp");
            setActiveTab("communications");
          }}
        />
      ) : null}
