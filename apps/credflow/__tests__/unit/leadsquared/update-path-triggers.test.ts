...
describe("workflow-engine · distribute_lead (owner update)", () => {
  it("triggers outbound sync after the owner update commits", async () => {
    ...
    db.crmLead.findFirst.mockResolvedValue({ id: "lead-1", tenantId: "t1", ownerId: null } as unknown as CrmLead);
...
describe("convert route (transaction path)", () => {
  ...
  it("triggers outbound sync AFTER the transaction commits (successful convert)", async () => {
    db.crmLead.findFirst.mockResolvedValue(lead as unknown as CrmLead);
...
describe("form-rule-apply · applyFormRules (set_stage → status/substatus)", () => {
  it("triggers outbound sync after the status update", async () => {
    db.crmFormRule.findMany.mockResolvedValue([] as never); // rules unused (evaluator mocked)
    db.crmLead.findUnique.mockResolvedValue({ stage: "New", status: "New", substatus: null } as never);
    db.crmLead.update.mockResolvedValue({} as never);
...
describe("bulk-assign route (updateMany fan-out)", () => {
  it("triggers outbound sync ONCE PER reassigned lead", async () => {
    ...
    db.crmAuditLog.create.mockResolvedValue({} as never); // recordLeadChange
...
describe("bulk import (upsertImportedLeadRow · dedupe update)", () => {
  it("triggers outbound sync after an externalId-match UPDATE", async () => {
    db.crmLead.findUnique.mockResolvedValue({ id: "imp-1", dynamicFields: null } as never);
    db.crmLead.update.mockResolvedValue({ id: "imp-1", tenantId: "t1" } as never);
  ...
  it("triggers outbound sync after an email/phone dedupe-match UPDATE", async () => {
    ...
    db.crmLead.findUnique.mockResolvedValue({ dynamicFields: null } as never); // dupe.id fetch
    db.crmLead.update.mockResolvedValue({ id: "imp-2", tenantId: "t1" } as never);
