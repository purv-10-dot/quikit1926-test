...
/** Build a CrmLead-shaped stub from a partial (narrowed through unknown, no `as any`). */
function leadRow(partial: Partial<CrmLead>): CrmLead {
  return { id: LEAD_ID, tenantId: TENANT, name: "Ada", deletedAt: null, ...partial } as unknown as CrmLead;
}
...
describe("processLeadSquaredSyncJob", () => {
  it("re-reads the lead (tenant-scoped) and forwards args to syncLeadOutbound", async () => {
    ...
    db.crmLead.findFirst.mockResolvedValue(lead);
  ...
  it("serializes work under a per-lead lock keyed by tenant+lead", async () => {
    ...
    db.crmLead.findFirst.mockResolvedValue(leadRow({ id: LEAD_ID }));
  ...
  it("skips a soft-deleted lead", async () => {
    const { db, sync, deps } = setup();
    db.crmLead.findFirst.mockResolvedValue(leadRow({ deletedAt: new Date() }));
  ...
  it("propagates the job's origin (always 'crm' from this queue)", async () => {
    const { db, sync, deps } = setup();
    db.crmLead.findFirst.mockResolvedValue(leadRow({}));
