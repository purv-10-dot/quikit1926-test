...
import type { CrmWorkflowDefinition, CrmWorkflowStatus } from "@quikit/database";
import {
  publish,
  unpublish,
  softDelete,
...
function def(status: CrmWorkflowStatus): CrmWorkflowDefinition {
  return {
    id: "wf1",
    tenantId: "t1",
    name: "Rule",
...
beforeEach(() => {
  ...
  db.crmWorkflowDefinition.findMany.mockReset();
  db.crmAutomationPendingStep.count.mockReset();
  // Return value is unused (tests assert on the call args, not the result).
  db.crmWorkflowDefinition.update.mockResolvedValue(def("Active"));
});
...
describe("lifecycle · firing gates (single source of truth for the engine)", () => {
  it("only Active admits NEW leads", () => {
    ...
    for (const s of ["Draft", "Draining", "Stopped", "Deleted", "Paused", "Archived"] as CrmWorkflowStatus[]) {
  ...
  it("Active AND Draining resume in-flight leads; nothing else does", () => {
    ...
    for (const s of ["Draft", "Stopped", "Deleted", "Paused", "Archived"] as CrmWorkflowStatus[]) {
  ...
  it("structure is editable only in Draft (canvas-lock / immutability)", () => {
    ...
    for (const s of ["Active", "Draining", "Stopped", "Deleted"] as CrmWorkflowStatus[]) {
...
describe("lifecycle · publish", () => {
  it("Draft → Active and stamps lastPublishedOn, tenant-scoped", async () => {
    db.crmWorkflowDefinition.findFirst.mockResolvedValue(def("Draft"));
    await publish("t1", "wf1");
    expect(db.crmWorkflowDefinition.findFirst).toHaveBeenCalledWith({
      where: { id: "wf1", tenantId: "t1", deletedAt: null },
    });
    const arg = db.crmWorkflowDefinition.update.mock.calls[0][0];
  ...
  it("rejects publish from a non-Draft status", async () => {
    db.crmWorkflowDefinition.findFirst.mockResolvedValue(def("Active"));
...
describe("lifecycle · unpublish", () => {
  it("Immediate → Stopped", async () => {
    ...
    expect(db.crmWorkflowDefinition.update.mock.calls[0][0].data).toEqual({ status: "Stopped" });
  });

  it("Delayed → Draining", async () => {
    db.crmWorkflowDefinition.findFirst.mockResolvedValue(def("Active"));
    await unpublish("t1", "wf1", "delayed");
    expect(db.crmWorkflowDefinition.update.mock.calls[0][0].data).toEqual({ status: "Draining" });
  });
...
describe("lifecycle · softDelete / restore", () => {
  it("soft-deletes (Deleted + deletedAt) and never hard-deletes", async () => {
    db.crmWorkflowDefinition.findFirst.mockResolvedValue(def("Stopped"));
  ...
  it("refuses to delete a Draining automation (non-deletable until drained)", async () => {
    db.crmWorkflowDefinition.findFirst.mockResolvedValue(def("Draining"));
  ...
  it("restore returns a Deleted automation to Draft and clears deletedAt", async () => {
    db.crmWorkflowDefinition.findFirst.mockResolvedValue({ ...def("Deleted"), deletedAt: new Date() });
    await restore("t1", "wf1");
    const arg = db.crmWorkflowDefinition.update.mock.calls[0][0];
    expect(arg.data).toEqual({ status: "Draft", deletedAt: null });
    // restore must be able to see soft-deleted rows
    expect(db.crmWorkflowDefinition.findFirst).toHaveBeenCalledWith({ where: { id: "wf1", tenantId: "t1" } });
  });
...
describe("lifecycle · reads", () => {
  it("listAutomations hides soft-deleted rows by default, tenant-scoped", async () => {
    ...
    expect(db.crmWorkflowDefinition.findMany).toHaveBeenCalledWith({
  ...
  it("isDrained is true only when no pending/processing steps remain", async () => {
    db.crmAutomationPendingStep.count.mockResolvedValue(0);
    expect(await isDrained("t1", "wf1")).toBe(true);
    db.crmAutomationPendingStep.count.mockResolvedValue(2);
    expect(await isDrained("t1", "wf1")).toBe(false);
    expect(db.crmAutomationPendingStep.count).toHaveBeenCalledWith({
