 ...
 *   FR-D5-1   Audit log           → CrmAuditLog entry with rule as actor
...
// Minimal CrmLead shape — only fields the engine selects (status, name, ownerId).
// Cast as any to avoid requiring all 40+ CrmLead columns in the mock fixture.
...
describe("FR-D3 — Disposition Automation Rule Engine", () => {
  ...
  it("FR-D3-1: sets lead status when disposition matches trigger", async () => {
    db.crmAutomationRule.findMany.mockResolvedValueOnce([makeRule()]);
    db.crmLead.findUnique.mockResolvedValueOnce(baseLead);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    db.crmLead.update.mockResolvedValueOnce({ ...baseLead, status: "Disqualified" } as any);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    db.crmAuditLog.create.mockResolvedValueOnce({} as any);
  ...
  it("FR-D3-3: only the first matching set_lead_status fires (sortOrder 1 wins over 2)", async () => {
    db.crmAutomationRule.findMany.mockResolvedValueOnce([
    ...
    db.crmLead.findUnique.mockResolvedValueOnce(baseLead);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    db.crmLead.update.mockResolvedValueOnce({ ...baseLead, status: "Contacted" } as any);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    db.crmAuditLog.create.mockResolvedValueOnce({} as any);

    await runAfterActivityLogged({ ...baseCtx, dispositionCode: "not_interested" });

    expect(db.crmLead.update).toHaveBeenCalledTimes(1);
    expect(db.crmLead.update).toHaveBeenCalledWith(
  ...
  it("FR-D3-4: engine queries rules exactly once — rule-made change does not re-trigger", async () => {
    ...
    expect(db.crmAutomationRule.findMany).toHaveBeenCalledTimes(1);
  ...
  it("FR-D3-5: creates task due at activityDatetime with expanded title (AC-5)", async () => {
    ...
    db.crmLead.findUnique.mockResolvedValueOnce(baseLead);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    db.crmTask.create.mockResolvedValueOnce({} as any);
    ...
    expect(db.crmTask.create).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("FR-D3-7: inactive rules are not evaluated (DB filters isActive=false)", async () => {
    ...
    expect(db.crmLead.findUnique).not.toHaveBeenCalled();
    expect(db.crmLead.update).not.toHaveBeenCalled();
  });
  ...
  it("FR-D5-1: writes CrmAuditLog with ruleId in metadata for every rule-made status change", async () => {
    ...
    db.crmAutomationRule.findMany.mockResolvedValueOnce([rule]);
    db.crmLead.findUnique.mockResolvedValueOnce(baseLead);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    db.crmLead.update.mockResolvedValueOnce({ ...baseLead, status: "Disqualified" } as any);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    expect(db.crmAuditLog.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          tenantId: TENANT,
          userId: null,
