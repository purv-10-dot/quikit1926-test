...
describe("transitionQuote â€” Opportunity probability sync (audit W-1)", () => {
  beforeEach(() => {
    ...
    db.qcfOpportunity.updateMany.mockReset();
    db.qcfQuoteStatusTransition.create.mockReset();
  ...
  it("Active â†’ Won: bumps the linked Opportunity to probability=100", async () => {
    ...
    db.qcfQuote.findFirst.mockResolvedValueOnce({
      id: "q1",
    ...
    db.qcfOpportunity.updateMany.mockResolvedValue({ count: 1 } as never);
  ...
  it("skips Opportunity sync when the quote has no opportunityId", async () => {
    ...
    expect(db.qcfOpportunity.updateMany).not.toHaveBeenCalled();
  });
  ...
  it("doesn't emit the OpportunityProbabilityFromQuote activity when the opp was already closed (updateMany count=0)", async () => {
    ...
    db.qcfOpportunity.updateMany.mockResolvedValue({ count: 0 } as never);
    db.qcfQuote.findFirst.mockResolvedValueOnce({ id: "q1", lines: [] } as never);
    ...
    const probActivities = db.qcfActivity.create.mock.calls.filter(
      (c) =>
