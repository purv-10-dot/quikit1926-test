 ...
 * first_db_crm_autotest. Builds real CrmWorkflowDefinition rows, drives them via
...
async function main() {
  ...
  const stageChanges = (leadId: string) =>
    prisma.crmActivity.count({ where: { tenantId: TENANT, leadId, type: "LeadStageChange" } });

  // ---- reset fixture baselines (idempotent, direct writes) ----
  await prisma.crmLead.update({ where: { id: "autotest-r19-match" }, data: { stage: "New Lead", substatus: "Negotiation" } });
  await prisma.crmLead.update({ where: { id: "autotest-r1-substatus-only" }, data: { stage: "Onboarding Done", substatus: "Student Lead" } });
  await prisma.crmLead.update({ where: { id: "autotest-r1-stage-only" }, data: { stage: "New Lead", substatus: "Support Issue" } });
  for (const id of R1_MATCH) await prisma.crmLead.update({ where: { id }, data: { stage: "New Lead" } });

  // ---- definitions ----
  const r19 = await prisma.crmWorkflowDefinition.create({
    data: {
  ...
  const r1 = await prisma.crmWorkflowDefinition.create({
    data: {
  ...
  const l19 = await prisma.crmLead.findUnique({ where: { id: "autotest-r19-match" } });
  ...
  for (const id of R1_MATCH) {
    ...
    const l = await prisma.crmLead.findUnique({ where: { id } });
  ...
  const lNeg1 = await prisma.crmLead.findUnique({ where: { id: "autotest-r1-substatus-only" } });
  ...
  const lNeg2 = await prisma.crmLead.findUnique({ where: { id: "autotest-r1-stage-only" } });
  ...
  // ---- cleanup defs ----
  await prisma.crmWorkflowDefinition.deleteMany({ where: { id: { in: [r19.id, r1.id] } } });
