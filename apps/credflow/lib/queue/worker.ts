...
async function main() {
  ...
  // production LSQ. Set WORKER_AUTOMATION_ONLY=1 to run ONLY automation+imports.
  ...
  const automationOnly = process.env.WORKER_AUTOMATION_ONLY === "1";
  console.log("========================================================");
  ...
  console.log(
    "  leadsquared sync/inbound    : " +
      (automationOnly
        ? "DISABLED (WORKER_AUTOMATION_ONLY=1)"
        : "ENABLED  -> pushes to REAL LeadSquared"),
  );
  console.log("  (set WORKER_AUTOMATION_ONLY=1 to disable external LSQ sync for testing)");
  console.log("========================================================");
  if (!automationOnly) {
    console.warn(
      "[worker] WARNING: LeadSquared sync is ENABLED - CRM changes WILL be pushed to the real LSQ account. " +
        "For safe automation testing on the autotest DB, stop and re-run: WORKER_AUTOMATION_ONLY=1 npm run worker",
    );
  }
