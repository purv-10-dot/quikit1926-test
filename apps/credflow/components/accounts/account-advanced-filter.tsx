...
} from "@/lib/account-filter-fields";
...
/** Operators the accounts API supports — anything else is hidden in the picker. */
...
function makeBlankCondition(): ConditionRow {
  const first = ACCOUNT_FILTER_FIELDS[0]!;
...
export function AccountAdvancedFilterModal({
  open,
  initial,
  onClose,
  onApply,
  ...
  function build(): FilterPayload {
    const cleaned = rows.filter((r) => {
      const def = getAccountFilterField(r.field);
      if (!def) return false;
...
function AccountConditionRowEditor({
  row,
  onChange,
  onRemove,
  dynamicOptions = {},
...
export function AccountAppliedFilterSummary({
  filter,
  onClear,
}: {
  ...
  return (
    <div className="mt-3 flex flex-wrap items-center gap-2 rounded-lg bg-crm-blue-soft px-3 py-2 text-xs text-crm-blue-dark">
      ...
      {filter.conditions.map((c, i) => {
        const def = getAccountFilterField(c.field);
 ...
 * accounts API DTO (`{ conditions, combinator }`). Operator names and the
 * value/values split differ between the two contracts — keep this in sync
 * with `lib/services/accounts/index.ts::buildSingleCondition`.
 */
export function buildAccountFilterRequest(filter: FilterPayload): {
  conditions: Array<{
    field: string;
    operator: string;
    value?: unknown;
  ...
  const out: ReturnType<typeof buildAccountFilterRequest>["conditions"] = [];
