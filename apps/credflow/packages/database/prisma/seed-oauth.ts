...
/** Use the same Postgres as apps/quikit (launcher reads quikit_dev). */
function loadDatabaseUrlFromQuikitEnv(): void {
  ...
  for (const file of ["apps/quikit/.env.local", "apps/quikit/.env"]) {
    ...
    for (const line of text.split("\n")) {
      ...
      console.log(`[seed-oauth] DATABASE_URL from ${file}\n`);
      return;
    }
  }
  throw new Error(
    "[seed-oauth] DATABASE_URL is unset. Set it or add DATABASE_URL to apps/quikit/.env.local",
  );
}
...
const QUIKCRM_BASE = resolveAppUrl("QUIKCRM_URL", "http://localhost:3009"); // prod-safety-allow: dev fallback, prod throws via resolveAppUrl
...
Then re-run this seed with the env vars set:
  ...
  QUIKSOCIAL_OAUTH_CLIENT_SECRET=... QUIKCRM_OAUTH_CLIENT_SECRET=... \\
