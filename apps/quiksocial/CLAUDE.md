...
4. **Bypass `withOrgAuth`** on any DB-touching API route. There are no exceptions outside `/api/health` and `/api/auth/*`.
5. **Write Prisma queries without an `orgId` filter**. The only legitimate cross-org query is on the super-admin app — and you are not building that.
