#!/usr/bin/env node
/**
 * Seeds a sample P2P flow into the "QuikConstruction Demo" tenant:
 ...
 *     and  `node scripts/seed-quikconstruction.mjs`:
 *
 *   DATABASE_URL=... node scripts/seed-quikconstruction-procurement.mjs
 */
...
async function main() {
  const tenant = await prisma.tenant.findUnique({ where: { slug: "quikconstruction-demo" } });
  if (!tenant) throw new Error("Run scripts/seed-quikconstruction.mjs first.");
  ...
  if (!vendor) throw new Error("Seed vendors first via scripts/seed-quikconstruction.mjs");
  ...
  console.log(`
╔═══════════════════════════════════════════════════╗
║  QuikConstruction P2P seed — complete             ║
╠═══════════════════════════════════════════════════╣
