#!/usr/bin/env node
/**
 * End-to-end seed for QuikConstruction — Finance, HRMS, Safety, Quality,
 * Approvals, Documents (stub), Expenses, Notes. Runs AFTER:
 *   1. seed-oauth.ts
 *   2. seed-quikconstruction.mjs (base tenant + users + companies + vendors)
 *   3. seed-quikconstruction-procurement.mjs (PR/PO/GRN/Stock)
 *
...
const TENANT_SLUG = "quikconstruction-demo";

async function main() {
  console.log("→ Full seed for QuikConstruction (Phases 5–14)…");

  const tenant = await prisma.tenant.findUnique({ where: { slug: TENANT_SLUG }, select: { id: true } });
  if (!tenant) throw new Error("Tenant not found. Run seed-quikconstruction.mjs first.");
  ...
  if (!adminUser || !memberUser) throw new Error("Demo users missing. Run seed-quikconstruction.mjs first.");
