...
async function main() {
  console.log("→ Seeding QuikConstruction Demo tenant…");

  // 1. App lookup (quikconstruction must already be registered via seed-oauth)
  const app = await prisma.app.findUnique({ where: { slug: "quikconstruction" } });
  if (!app) {
    throw new Error(
      "App 'quikconstruction' not found. Run `npx tsx packages/database/prisma/seed-oauth.ts` first.",
    );
  }

  // 2. Tenant upsert
  const tenant = await prisma.tenant.upsert({
    where: { slug: "quikconstruction-demo" },
    update: {},
    create: { name: "QuikConstruction Demo", slug: "quikconstruction-demo" },
  });
  ...
  console.log(`
╔══════════════════════════════════════════════════╗
║  QuikConstruction Demo tenant seeded             ║
╠══════════════════════════════════════════════════╣
